// trust-cg-lower/target_analysis.rs - Proof-guided target analysis pass
//
...
// architecture. For each computation subgraph, this pass examines trust_ir proof
// annotations to determine which compute targets (CPU scalar, SIMD, GPU,
// Neural Engine) are legal. The key insight: trust_ir proofs PRUNE the search
...
//! Proof-guided target analysis pass for Trust Codegen.
//!
...
//! trust_ir proof annotations, and estimate per-target cost.
//!
...
//! ```
//!
//! # Integration with trust_ir proofs
//!
...
//! Deterministic) that trust_ir attaches to computation subgraphs rather than
//! individual values.
//!
//! These annotations are producer-owned observations. Production keeps GPU/ANE placement and
//! parallel reassociation disabled until an exact validator-issued replay capability is wired; the
//! historical proof-pruning algorithm remains as a structural model and reporting surface.
...
use crate::compute_graph::AcceleratorSemanticRecipe;
...
pub enum ComputeTarget {
    ...
    /// Production status: unavailable until both validator-issued target replay
    /// authority and an exact compiler-derived CoreML tensor recipe are wired.
    /// Structural test modeling still applies the historical proof requirements.
    NeuralEngine,
}
...
pub struct TargetLegality {
    ...
    /// Exact compiler-derived accelerator semantics, when the graph builder
    /// could prove that a supported backend recipe denotes this subgraph.
    ///
    /// Private and non-serializable by construction: public target judgments
    /// and caller-authored `legal_targets` are not emission authority.
    accelerator_recipe: Option<AcceleratorSemanticRecipe>,
}
...
impl TargetLegality {
    ...
    pub fn is_legal(&self, target: ComputeTarget) -> bool {
        self.judgments.get(&target).is_some_and(|j| j.legal)
    }
    ...
    pub(crate) fn bind_accelerator_recipe(&mut self, recipe: AcceleratorSemanticRecipe) {
        self.accelerator_recipe = Some(recipe);
    }

    pub(crate) fn accelerator_recipe(&self) -> Option<&AcceleratorSemanticRecipe> {
        self.accelerator_recipe.as_ref()
    }
}
...
/// Subgraph-level proof properties that trust_ir attaches to computation
/// subgraphs (as opposed to individual values).
///
/// These extend the per-value [`Proof`] annotations from the adapter layer
/// with properties that apply to entire subgraphs. In the real trust_ir crate,
...
impl SubgraphDescriptor {
    ...
    ///
    /// A subgraph operates on arrays/vectors if any value has an aggregate
    /// array type or the fixed-width V128 lane carrier.
    pub fn operates_on_arrays(&self) -> bool {
        self.value_types
            .values()
            .any(|ty| matches!(ty, Type::Array(_, _) | Type::V128))
    }
}
...
/// design doc. For each computation subgraph, examines trust_ir proof
...
impl ProofAnalyzer {
    ...
    pub fn analyze(
        ...
        // SubgraphProof and ProofContext entries are producer-owned labels.  They remain visible in
        // descriptors and reports, but cannot authorize moving work to a different device or
        // reassociating a reduction.  Until the same exact validator-replay capability used by the
        // guard boundary is wired, production is CPU-only at this authority seam.  Unit tests keep
        // exercising the historical structural model under `cfg(test)`; integration tests cover the
        // real non-test policy.
        if !crate::guard_evidence::validator_guard_replay_authority_available() && !cfg!(test) {
            judgments.insert(
                ComputeTarget::CpuSimd,
                TargetJudgment {
                    legal: true,
                    reason: "SIMD remains an on-CPU structural candidate; proof labels grant no extra authority"
                        .to_string(),
                },
            );
            judgments.insert(
                ...
                TargetJudgment {
                    legal: false,
                    reason:
                        "GPU illegal: exact validator-issued target replay authority is not wired"
                            .to_string(),
                },
            );
            judgments.insert(
                ...
                TargetJudgment {
                    legal: false,
                    reason:
                        "ANE illegal: exact validator-issued target replay authority is not wired"
                            .to_string(),
                },
            );
        ...
        let has_valid_borrow = subgraph.values.iter().any(|v| {
            ctx.proof_ctx.value_proofs.get(v).is_some_and(|proofs| {
                proofs
                    .iter()
...
mod tests {
    ...
    fn test_display_subgraph_proof() {
        ...
        assert_eq!(format!("{}", SubgraphProof::Deterministic), "Deterministic");
    }
    ...
    fn test_valid_borrow_only_no_gpu() {
        ...
        proof_ctx
            .value_proofs
            .insert(Value(0), vec![Proof::ValidBorrow { borrow: ValueId(0) }]);
        let ctx = TargetProofContext::new(proof_ctx);
