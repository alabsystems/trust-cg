// trust-cg-lower/smulh_idiom.rs - AArch64 SMULH idiom detection
//
...
//! idiom.
//!
//! ay's simplex JIT emits signed `mulhi(i64, i64)` in widened form:
//!
//! ```text
...
/// the defs of its intermediates, which is only sound if no OTHER block
/// consumes those values (branch-arg edge copies); the single-use guards
/// below then consult the totals. Without them the guards see block-local
/// counts only (exact for single-block functions).
pub fn detect_smulh_idioms(
    ...
    // "May I skip this def?" guards must see cross-block uses when known.
...
mod tests {
    ...
    fn detects_smulh_idiom_with_i128_shift_iconst() {
        ...
        let analysis = detect_smulh_idioms(&insts, None);
