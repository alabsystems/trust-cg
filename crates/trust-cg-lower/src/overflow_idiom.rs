// trust-cg-lower/overflow_idiom.rs - i128-widened signed-overflow idiom detection
//
...
//!
//! ay (and other front-ends targeting SMT-style overflow checking) emits
...
///
/// `cross_block_use_counts`, when provided, are WHOLE-FUNCTION use counts
/// (see `InstructionSelector::value_use_counts`). Registering an idiom SKIPS
/// the defs of its i128 intermediates (and the icmp), which is only sound if
/// no OTHER block consumes those values (branch-arg edge copies); every
/// use-count guard below then consults the totals. Without them the guards
/// see block-local counts only (exact for single-block functions).
pub fn detect_overflow_idioms(
    instructions: &[Instruction],
    cross_block_use_counts: Option<&HashMap<Value, usize>>,
) -> OverflowAnalysis {
    ...
    // Count uses per Value across the block (inside LIR instruction
    // operands), then swap in the whole-function totals when the caller
    // provided them: every guard below is a "may I skip/elide this def?"
    // question, so it must see cross-block uses too.
    // BORROW the caller's whole-function totals instead of cloning them.
    // The clone ran once per block over an O(function-values) map — one of
    // three Theta(blocks x values) clone sites in the ISel pre-scan (the
    // type-map overlay in isel.rs removed the first; an adversarial review
    // of that fix located these two). Every downstream access is a read
    // (`get`), so a shared borrow is exactly equivalent.
    let local_use_count: HashMap<Value, usize>;
    let use_count: &HashMap<Value, usize> = match cross_block_use_counts {
        Some(total) => total,
        None => {
            let mut local: HashMap<Value, usize> = HashMap::new();
            for inst in instructions {
                for a in &inst.args {
                    *local.entry(*a).or_insert(0) += 1;
                }
            }
            local_use_count = local;
            &local_use_count
        }
    };
    ...
    for (icmp_idx, icmp) in instructions.iter().enumerate() {
        ...
        let Some((sext_sum_idx, wide_idx)) = pick_sext_and_wide(&def_site, lhs, rhs, instructions)
        else {
            continue;
        };
        ...
        if sext_a_idx == sext_b_idx || sext_a_idx == sext_sum_idx || sext_b_idx == sext_sum_idx {
            continue;
        }
        ...
        let Opcode::Sextend {
            ...
            if let Some((sa, sb)) = swap_sext_pair(&def_site, wide_rhs, wide_lhs, instructions) {
        ...
        let pair_matches = match wide_kind {
            ...
            OverflowKind::SignedAdd => {
                (sext_a_src == a && sext_b_src == b) || (sext_a_src == b && sext_b_src == a)
            }
        };
        if !pair_matches {
            continue;
...
mod tests {
    ...
    fn detects_signed_add_overflow_idiom() {
        ...
        let a = detect_overflow_idioms(&insts, None);
