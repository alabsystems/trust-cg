// trust-cg-lower/dispatch.rs - Dispatch codegen for heterogeneous compute
//
...
use crate::compute_graph::{
    ComputeGraph, ComputeNodeId, TargetRecommendation, TransferCost, estimate_transfer_cost,
};
...
impl DispatchPlan {
    ...
    pub fn count_transfers(&self) -> usize {
        self.ops
            .iter()
            .filter(|op| matches!(op, DispatchOp::DataTransfer { .. }))
            .count()
    }
    ...
    pub fn count_launches(&self) -> usize {
        self.ops
            .iter()
            .filter(|op| matches!(op, DispatchOp::KernelLaunch { .. }))
            .count()
    }
    ...
    pub fn count_syncs(&self) -> usize {
        self.ops
            .iter()
            .filter(|op| matches!(op, DispatchOp::Synchronize { .. }))
            .count()
    }
    ...
    pub fn count_fallbacks(&self) -> usize {
        self.ops
            .iter()
            .filter(|op| matches!(op, DispatchOp::CpuFallback { .. }))
            .count()
    }
}
...
impl fmt::Display for DispatchPlan {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        writeln!(
            f,
            "DispatchPlan ({} ops, ~{} cycles):",
            self.ops.len(),
            self.estimated_total_cycles
        )?;
...
pub enum DispatchError {
    ...
    MissingAssignment { node_id: ComputeNodeId },
    ...
    UnknownNode { node_id: ComputeNodeId },
...
impl fmt::Display for DispatchError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            DispatchError::MissingSyncBeforeRead {
                target,
                producer,
                consumer,
            } => {
            ...
            DispatchError::ProfitabilityMismatch {
                node_id,
                dispatched_target,
                recommended_target,
            } => {
...
pub fn generate_dispatch_plan(
    ...
    for node_id in &order {
        ...
        if target == ComputeTarget::CpuScalar && !assignment.contains_key(node_id) {
    ...
    for node_id in remaining_async {
        let target = assignment
            ...
            .copied()
            .unwrap_or(ComputeTarget::CpuScalar);
        ops.push(DispatchOp::Synchronize { target, node_id });
    }
...
pub fn generate_profitability_aware_dispatch_plan(graph: &ComputeGraph) -> DispatchPlan {
        ...
        graph
            ...
            .map(|node| {
                ...
                TargetRecommendation {
                    ...
                    reason: format!(
                        "{} selected as cheapest legal target (no profitability data)",
                        recommended
                    ),
...
pub fn validate_dispatch_plan(
    ...
    let graph_node_ids: HashSet<ComputeNodeId> = graph.nodes.iter().map(|n| n.id).collect();
    ...
    for op in &plan.ops {
        match op {
            DispatchOp::KernelLaunch { node_id, .. } | DispatchOp::CpuFallback { node_id, .. }
            ...
            DispatchOp::KernelLaunch {
                ...
                for edge in graph.incoming_edges(*node_id) {
                    ...
                    let src_target = plan
                        ...
                        .copied()
                        .unwrap_or(ComputeTarget::CpuScalar);
                    if src_target != *target
                        && edge.transfer_bytes > 0
...
impl fmt::Display for VerificationReport {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        writeln!(
            f,
            "  CPU fallback: {}",
            if self.cpu_fallback_ok { "PASS" } else { "FAIL" }
        )?;
        ...
        for mm in &self.profitability_mismatches {
            writeln!(
                f,
                "    - {} dispatched to {} but recommended {}",
                mm.node_id, mm.dispatched_target, mm.recommended_target
            )?;
        }
        Ok(())
    }
...
pub fn verify_cpu_fallback(graph: &ComputeGraph, _plan: &DispatchPlan) -> bool {
    graph
        .nodes
        .iter()
        .all(|node| node.legal_targets.contains(&ComputeTarget::CpuScalar))
}
...
pub fn verify_data_transfers(graph: &ComputeGraph, plan: &DispatchPlan) -> Result<(), Vec<String>> {
    ...
    let covered: HashSet<(ComputeNodeId, ComputeNodeId)> = plan
        ...
        .iter()
        .filter_map(|op| {
            if let DispatchOp::DataTransfer {
                edge_from, edge_to, ..
            } = op
            {
    ...
    for edge in &graph.edges {
        ...
        if src != dst && edge.transfer_bytes > 0 && !covered.contains(&(edge.from, edge.to)) {
            errors.push(format!(
...
pub fn verify_dispatch_plan_properties(
    ...
    let (synchronization_ok, synchronization_errors) = match verify_synchronization(graph, plan) {
        Ok(()) => (true, Vec::new()),
        Err(errors) => (false, errors),
    };
...
mod tests {
    ...
    fn test_scalar_dispatch_no_transfers() {
        ...
        assert_eq!(
            ...
            0,
            "scalar dispatch needs no transfers"
        );
    ...
    fn test_neon_dispatch_no_heavy_transfer() {
        ...
        assert_eq!(
            plan.count_transfers(),
            0,
            "NEON has no cross-domain transfer for standalone"
        );
    ...
    fn test_gpu_dispatch_full_sequence() {
        ...
        let transfer = plan
            .ops
            .iter()
            .find(|op| matches!(op, DispatchOp::DataTransfer { .. }));
        assert!(transfer.is_some(), "must have a data transfer");
        if let Some(DispatchOp::DataTransfer {
            src,
            dst,
            size_bytes,
            ..
    ...
    fn test_gpu_sync_before_readback() {
        ...
        assert!(
            cpu_readback_idx.is_some(),
            "CPU readback launch must be present"
        );
        assert!(
    ...
    fn test_dispatch_error_display() {
        let err = DispatchError::MissingAssignment {
        ...
        let err = DispatchError::UnknownNode {
    ...
    fn test_verify_gpu_plan_all_pass() {
        ...
        assert!(
            report.all_ok(),
            "GPU plan should pass all checks:\n{}",
            report
        );
    }
    ...
    fn test_verify_ane_plan_all_pass() {
        ...
        assert!(
            report.all_ok(),
            "ANE plan should pass all checks:\n{}",
            report
        );
    }
    ...
    fn test_verify_complex_mixed_target_graph() {
        ...
        assert!(
            report.all_ok(),
            "Diamond graph plan should pass:\n{}",
            report
        );
    ...
    fn test_validate_profitability_compliance_detects_mismatch() {
        use trust_cg_ir::cost_model::{CostModelGen, ProfitabilityAnalyzer};
    ...
    fn test_verification_report_display_profitability() {
        ...
        let display = format!("{}", report);
        assert!(
            display.contains("Profitability: PASS"),
            "Report should contain Profitability line"
        );
    }
