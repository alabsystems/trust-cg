// trust-cg-lower/abi.rs - Apple AArch64 calling convention
//
...
// Import physical register from trust-cg-ir (canonical source of truth).
pub use trust_cg_ir::regs::PReg;

// ---------------------------------------------------------------------------
// ABI register constants — re-exported from trust-cg-ir (single source of truth)
// ---------------------------------------------------------------------------

/// GPR constants for ABI use.
///
/// These re-export the canonical constants from `trust_cg_ir::regs` so that
/// downstream code can write `gpr::X0` without a long import path.
pub mod gpr {
    pub use trust_cg_ir::regs::{
...
pub enum ArgLocation {
    ...
    /// Register pair for values that occupy two 64-bit GPR slots: low 64-bit
    /// half in first register, high 64-bit half in second register.
    ///
    /// Per AAPCS64 / Apple AArch64 ABI, `i128` values and 9-16 byte non-HFA
    /// aggregates are passed and returned as two consecutive 64-bit GPRs. For
    /// example:
    /// - First i128 argument: X0 (low), X1 (high)
    /// - First 16-byte non-HFA aggregate: X0 (low eightbyte), X1 (high eightbyte)
...
/// Aggregate). Per AAPCS64, F16/F32/F64 are valid HFA element types.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum HfaBaseType {
    /// 16-bit float (half / f16)
    F16,
...
/// H-register argument sequence: H0-H7 (typed 16-bit view of V0-V7 for F16 args).
const H_ARG_REGS: [PReg; 8] = [
    gpr::H0,
    gpr::H1,
    gpr::H2,
    gpr::H3,
...
impl AppleAArch64ABI {
    ...
    fn detect_hfa(ty: &Type) -> Option<(Type, usize)> {
        match ty {
            Type::Struct(fields) => {
                ...
                if !matches!(base, Type::F16 | Type::F32 | Type::F64) {
                    return None;
                }
    ...
    fn flatten_hfa_fields(ty: &Type, out: &mut Vec<Type>) {
        match ty {
            Type::F16 | Type::F32 | Type::F64 => out.push(ty.clone()),
            Type::Struct(fields) => {
                for field in fields {
            ...
            match ty {
                ...
                Type::I128 => {
                    ...
                    if gpr_idx + 1 < GPR_ARG_REGS.len() {
                        ...
                        // i128 has 16-byte natural alignment on the stack.
                ...
                // Float types -> typed FPR aliases.
                Type::F16 | Type::F32 | Type::F64 => {
                    if fpr_idx < FPR_ARG_REGS.len() {
                        let typed_regs: &[PReg] = match ty {
                            Type::F16 => &H_ARG_REGS,
                            Type::F32 => &S_ARG_REGS,
                            Type::F64 => &D_ARG_REGS,
                            _ => unreachable!(),
                ...
                // 64-bit NEON/SIMD vector -> D register (V0-V7 low half), same
                // FPR sequence as scalar FP; 8-byte stack slot on spill.
                Type::V64 => {
                    if fpr_idx < FPR_ARG_REGS.len() {
                        result.push(ArgLocation::Reg(D_ARG_REGS[fpr_idx]));
                        fpr_idx += 1;
                    } else {
                ...
                Type::Struct(_) | Type::Array(_, _) | Type::Enum { .. } => {
                    ...
                    if let Some((base_ty, count)) = Self::detect_hfa(ty) {
                        if fpr_idx + count <= FPR_ARG_REGS.len() {
                            ...
                            let regs: Vec<PReg> =
                                (fpr_idx..fpr_idx + count).map(|i| typed_regs[i]).collect();
                            ...
                            // AAPCS64 C.5: once an HFA/HVA is passed on the stack,
                            // the NSRN is set to 8 — no later SIMD/FP argument may
                            // BACKFILL an FPR the HFA skipped. Without this clamp a
                            // following scalar float wrongly takes a still-"free"
                            // Vn that clang leaves unused, so caller and callee
                            // disagree on the register. (The i128 GPR path clamps
                            // symmetrically; mirror it for FPRs here.)
                            fpr_idx = FPR_ARG_REGS.len();
                        }
                    } else {
                        ...
                        if size <= 16 && gpr_idx < GPR_ARG_REGS.len() {
                            ...
                            if size <= 8 {
                                ...
                                // A 9-16B composite that no longer fits a register
                                // PAIR spills whole to the stack. AAPCS64 C.12: the
                                // NGRN is then set to 8 — a following integer/pointer
                                // scalar must NOT backfill the single GPR still free
                                // (X7 here). Without this clamp caller and callee
                                // disagree (clang leaves X7 unused and puts the next
                                // scalar on the stack).
                                result.push(ArgLocation::Stack {
                                    offset: stack_offset,
                                    size,
                            ...
                            if gpr_idx < GPR_ARG_REGS.len() {
                                ...
                                stack_offset += 8;
                                // NGRN already exhausted here; clamp defensively so
                                // the no-backfill invariant holds regardless.
                                gpr_idx = GPR_ARG_REGS.len();
                            }
                        } else {
                            // size <= 16 with the GPRs already exhausted: spill
                            // whole to the stack and keep NGRN pinned at 8.
                            result.push(ArgLocation::Stack {
                                offset: stack_offset,
                                size,
                            });
    ...
    pub fn classify_returns(returns: &[Type]) -> Vec<ArgLocation> {
        ...
        for ty in returns {
            match ty {
                ...
                Type::F16 => {
                    if fpr_idx < H_ARG_REGS.len() {
                        result.push(ArgLocation::Reg(H_ARG_REGS[fpr_idx]));
                        fpr_idx += 1;
                    } else {
                        result.push(ArgLocation::Indirect { ptr_reg: gpr::X8 });
                    }
                ...
                // 64-bit NEON/SIMD vector return -> D0 (V0 low half)
                Type::V64 => {
                    if fpr_idx < FPR_ARG_REGS.len() {
                        result.push(ArgLocation::Reg(D_ARG_REGS[fpr_idx]));
                        fpr_idx += 1;
    ...
    /// Typed FPR argument registers for F16: H0-H7.
    ///
    /// Returns the 16-bit view of the argument FPR registers. H0-H7 alias
    /// the lower 16 bits of V0-V7 and share the same FPR allocation sequence.
    pub fn h_arg_regs() -> &'static [PReg] {
        &H_ARG_REGS
    }
    ...
    pub fn classify_fp_arg(ty: &Type, fpr_idx: usize) -> Option<(ArgLocation, usize)> {
        ...
        match ty {
            Type::F16 => Some((ArgLocation::Reg(H_ARG_REGS[fpr_idx]), fpr_idx + 1)),
    ...
    pub fn classify_hfa(ty: &Type) -> Option<(HfaBaseType, usize)> {
        Self::detect_hfa(ty).and_then(|(base_ty, count)| match base_ty {
            Type::F16 => Some((HfaBaseType::F16, count)),
...
pub struct UnwindInfo {
    ...
    /// Rust panics (`rust_eh_personality`), etc.
...
// These mirror the constants in trust-cg-codegen/frame.rs but are defined
...
pub struct CompactUnwindEntry {
    ...
    /// Compact-mode personality function pointer.
    ///
    /// DWARF fallback entries leave this zero; `__eh_frame` owns EH metadata.
    pub personality: u64,
    /// Compact-mode Language-Specific Data Area pointer.
    ///
    /// DWARF fallback entries leave this zero; the FDE carries the LSDA.
    pub lsda: u64,
}
...
/// cannot be described by compact unwind (frameless or dynamic-marked frames).
///
/// # Arguments
///
...
pub fn generate_compact_unwind(
    ...
    // Runtime-sized metadata or no frame pointer => DWARF fallback.
    ...
    let gprs: Vec<&SavedRegister> = info.saved_registers.iter().filter(|r| !r.is_fpr).collect();
    ...
    if has_reg_pair(19, 20) {
        encoding |= COMPACT_X19_X20;
    }
    if has_reg_pair(21, 22) {
        encoding |= COMPACT_X21_X22;
    }
    if has_reg_pair(23, 24) {
        encoding |= COMPACT_X23_X24;
    }
    if has_reg_pair(25, 26) {
        encoding |= COMPACT_X25_X26;
    }
    if has_reg_pair(27, 28) {
        encoding |= COMPACT_X27_X28;
    }

    // FPR pairs: V8-V15 (encoding 72-79 in our unified PReg scheme).
    let fprs: Vec<&SavedRegister> = info.saved_registers.iter().filter(|r| r.is_fpr).collect();
    ...
    if has_fpr_pair(72, 73) {
        encoding |= COMPACT_D8_D9;
    } // V8/V9
    if has_fpr_pair(74, 75) {
        encoding |= COMPACT_D10_D11;
    } // V10/V11
    if has_fpr_pair(76, 77) {
        encoding |= COMPACT_D12_D13;
    } // V12/V13
    if has_fpr_pair(78, 79) {
        encoding |= COMPACT_D14_D15;
    } // V14/V15
...
// ===========================================================================
// Exception-handling metadata has one canonical authority:
// `crate::function::EhFunctionInfo`, derived from executable
// Invoke/LandingPad/Resume opcodes and authenticated by
// `Function::validate_eh_structure`. Serialized LSDA offsets/actions are
// target-layout data and therefore live in the checked codegen bridge, not
// in a second ABI-level public model.
...
mod tests {
    ...
    fn classify_mixed_int_float_params() {
        // fn bar(i64, f64, i32, f32) -> X0, D0, X1, S1
        let locs = AppleAArch64ABI::classify_params(&[Type::I64, Type::F64, Type::I32, Type::F32]);
        assert_eq!(locs.len(), 4);
        assert_eq!(locs[0], ArgLocation::Reg(gpr::X0));
        assert_eq!(locs[1], ArgLocation::Reg(gpr::D0));
        assert_eq!(locs[2], ArgLocation::Reg(gpr::X1));
        assert_eq!(locs[3], ArgLocation::Reg(gpr::S1));
    }
    ...
    fn classify_float_return() {
        ...
        assert_eq!(locs.len(), 1);
        assert_eq!(locs[0], ArgLocation::Reg(gpr::D0));
    }
    ...
    #[test]
    fn detect_hfa_two_f16_fields() {
        let ty = Type::Struct(vec![Type::F16, Type::F16]);
        let hfa = AppleAArch64ABI::detect_hfa(&ty);
        assert_eq!(hfa, Some((Type::F16, 2)));
    }
    ...
    #[test]
    fn classify_hfa_param_f16_uses_h_registers() {
        let hfa = Type::Struct(vec![Type::F16, Type::F16]);
        let locs = AppleAArch64ABI::classify_params(&[hfa]);
        assert_eq!(locs.len(), 1);
        assert_eq!(locs[0], ArgLocation::RegSequence(vec![gpr::H0, gpr::H1]));
    }
    ...
    fn classify_hfa_param_four_f32_in_v_regs() {
        ...
        assert_eq!(
            locs[0],
            ArgLocation::RegSequence(vec![gpr::S0, gpr::S1, gpr::S2, gpr::S3])
        );
    }

    #[test]
    fn classify_hfa_param_with_preceding_floats() {
        // fn(f64, HFA{f32, f32}) -> D0 for f64, then HFA in S1+S2
    ...
    fn classify_hfa_param_overflow_to_stack() {
        // 7 f32 scalars use S0-S6, then HFA(2x F32) can't fit -> stack
        ...
        // S0-S6 for the 7 scalars
        for i in 0..7 {
            assert_eq!(locs[i], ArgLocation::Reg(S_ARG_REGS[i]));
        }
    ...
    fn return_small_struct_16_bytes_in_x0_x1() {
        // struct { i64, i64 } = 16 bytes -> return in X0+X1.
    ...
    #[test]
    fn return_non_hfa_9_bytes_in_x0_x1() {
        let ty = Type::Array(Box::new(Type::I8), 9);
        assert_eq!(ty.bytes(), 9);
        let locs = AppleAArch64ABI::classify_returns(&[ty]);
        assert_eq!(locs.len(), 1);
        assert_eq!(locs[0], ArgLocation::RegPair(gpr::X0, gpr::X1));
    }
    ...
    #[test]
    fn classify_hfa_return_two_f16_uses_h_registers() {
        let hfa = Type::Struct(vec![Type::F16, Type::F16]);
        let locs = AppleAArch64ABI::classify_returns(&[hfa]);
        assert_eq!(locs.len(), 1);
        assert_eq!(locs[0], ArgLocation::RegSequence(vec![gpr::H0, gpr::H1]));
    ...
    fn return_hfa_four_f64_in_v0() {
        ...
        assert_eq!(
            locs[0],
            ArgLocation::RegSequence(vec![gpr::D0, gpr::D1, gpr::D2, gpr::D3])
        );
    }
    ...
    fn return_hfa_array_three_f32() {
        ...
        assert_eq!(
            locs[0],
            ArgLocation::RegSequence(vec![gpr::S0, gpr::S1, gpr::S2])
        );
    }
    ...
    fn classify_8th_arg_on_stack_boundary() {
        ...
        // 9th arg: on the stack at offset 0. Apple packs fixed stack args at
        // their natural size (I32 -> 4 bytes), NOT an 8-byte slot. Verified
        // against clang -O2: `str w8, [sp]`.
    ...
    fn classify_return_f32() {
        ...
        assert_eq!(locs.len(), 1);
        assert_eq!(locs[0], ArgLocation::Reg(gpr::S0));
    }
    ...
    fn classify_medium_non_hfa_struct_in_two_gprs() {
        // struct { i64, i32 } = 16 bytes -> 2 GPRs.
    ...
    #[test]
    fn classify_non_hfa_9_byte_array_in_two_gprs() {
        let medium = Type::Array(Box::new(Type::I8), 9);
        assert_eq!(medium.bytes(), 9);
        let locs = AppleAArch64ABI::classify_params(&[medium]);
        assert_eq!(locs.len(), 1);
        assert_eq!(locs[0], ArgLocation::RegPair(gpr::X0, gpr::X1));
    }
    ...
    fn classify_mixed_8_gpr_8_fpr_all_in_regs() {
        ...
        for loc in &locs {
            assert!(
                matches!(loc, ArgLocation::Reg(_)),
                "Expected all 16 args in registers, got {:?}",
                loc
            );
        }
    }
    ...
    fn classify_float_overflow_9_fpr_args() {
        // 9 f64 args: first 8 in D0-D7, 9th on stack
        let params: Vec<Type> = (0..9).map(|_| Type::F64).collect();
        let locs = AppleAArch64ABI::classify_params(&params);
        assert_eq!(locs.len(), 9);
        for i in 0..8 {
            assert_eq!(locs[i], ArgLocation::Reg(D_ARG_REGS[i]));
        }
        assert!(matches!(locs[8], ArgLocation::Stack { offset: 0, size: 8 }));
    }
    ...
    fn unwind_standard_frame_with_gpr_pair() {
        ...
        let info = UnwindInfo::standard_frame(&[(gpr::X19, gpr::X20, false)], 32, false);
    ...
    fn unwind_standard_frame_register_offsets() {
        ...
        let info = UnwindInfo::standard_frame(&[(gpr::X19, gpr::X20, false)], 32, true);
        // FP at offset 0
        assert_eq!(info.saved_registers[0].fp_offset, 0);
        // LR at offset 8
        assert_eq!(info.saved_registers[1].fp_offset, 8);
    ...
    fn unwind_info_saved_gprs_filter() {
        let info = UnwindInfo::standard_frame(
            &[(gpr::X19, gpr::X20, false), (gpr::V8, gpr::V9, true)],
            48,
            false,
        );
    ...
    fn compact_unwind_with_fpr_pairs() {
        let info = UnwindInfo::standard_frame(
            &[(gpr::V8, gpr::V9, true), (gpr::V10, gpr::V11, true)],
            48,
            false,
        );
    ...
    fn compact_unwind_mixed_gpr_fpr() {
        ...
        let expected = COMPACT_MODE_FRAME | COMPACT_X19_X20 | COMPACT_D8_D9 | COMPACT_D14_D15;
        assert_eq!(entry.encoding, expected);
    }
    ...
    fn dwarf_cfi_with_fpr_saves() {
        let info = UnwindInfo::standard_frame(&[(gpr::V8, gpr::V9, true)], 32, false);
        let ops = generate_dwarf_cfi(&info);
    ...
    fn dwarf_cfi_op_variants_constructible() {
        ...
        let def_cfa = DwarfCfiOp::DefCfa {
            register: 31,
            offset: 16,
        };
        let offset = DwarfCfiOp::Offset {
            register: 29,
            factored_offset: 2,
        };
        let reg = DwarfCfiOp::Register {
            register: 19,
            value_register: 0,
        };
    ...
    fn classify_pure_f32_function() {
        ...
        // Both args in S0, S1; return in S0
        let locs = AppleAArch64ABI::classify_params(&[Type::F32, Type::F32]);
        assert_eq!(locs.len(), 2);
        assert_eq!(locs[0], ArgLocation::Reg(gpr::S0));
        assert_eq!(locs[1], ArgLocation::Reg(gpr::S1));

        let ret = AppleAArch64ABI::classify_returns(&[Type::F32]);
        assert_eq!(ret.len(), 1);
        assert_eq!(ret[0], ArgLocation::Reg(gpr::S0));
    }
    ...
    fn classify_mixed_int_fp_with_return() {
        ...
        // i64 -> X0, f64 -> D0, i32 -> X1, f32 -> V1
        let locs = AppleAArch64ABI::classify_params(&[Type::I64, Type::F64, Type::I32, Type::F32]);
        assert_eq!(locs.len(), 4);
        assert_eq!(locs[0], ArgLocation::Reg(gpr::X0)); // i64
        assert_eq!(locs[1], ArgLocation::Reg(gpr::D0)); // f64
        assert_eq!(locs[2], ArgLocation::Reg(gpr::X1)); // i32
        assert_eq!(locs[3], ArgLocation::Reg(gpr::S1)); // f32

        // Return f64 -> D0
        let ret = AppleAArch64ABI::classify_returns(&[Type::F64]);
        assert_eq!(ret.len(), 1);
        assert_eq!(ret[0], ArgLocation::Reg(gpr::D0));
    }
    ...
    fn classify_fp_overflow_10_f32_args() {
        ...
        for i in 0..8 {
            assert_eq!(
                locs[i],
                ArgLocation::Reg(S_ARG_REGS[i]),
                "F32 arg {} should be in S{}",
                i,
                i
            );
        }
        // 9th and 10th on the stack. Apple packs F32 stack args at their
        // natural 4-byte size/alignment: offsets 0 and 4, NOT 0 and 8. Verified
        // against clang -O2 (the two F32s fuse into one `str x8, [sp]`).
        assert!(matches!(locs[8], ArgLocation::Stack { offset: 0, size: 4 }));
        assert!(matches!(locs[9], ArgLocation::Stack { offset: 4, size: 4 }));
    }
    ...
    fn classify_mixed_v128_and_f64() {
        ...
        // fn(v128, f64, v128) -> V0, D1, V2
        let locs = AppleAArch64ABI::classify_params(&[Type::V128, Type::F64, Type::V128]);
        assert_eq!(locs.len(), 3);
        assert_eq!(locs[0], ArgLocation::Reg(gpr::V0)); // v128
        assert_eq!(locs[1], ArgLocation::Reg(gpr::D1)); // f64
    ...
    fn classify_mixed_int_v128() {
        ...
        let locs =
            AppleAArch64ABI::classify_params(&[Type::I64, Type::V128, Type::I64, Type::V128]);
        assert_eq!(locs.len(), 4);
        assert_eq!(locs[0], ArgLocation::Reg(gpr::X0));
    ...
    #[test]
    fn classify_f16_return_uses_h0_alias() {
        let ret = AppleAArch64ABI::classify_returns(&[Type::F16]);
        assert_eq!(ret.len(), 1);
        assert_eq!(ret[0], ArgLocation::Reg(gpr::H0));
    }

    #[test]
    fn classify_multiple_fp_returns() {
        // Returning (f64, f32) -> D0, S1
        let ret = AppleAArch64ABI::classify_returns(&[Type::F64, Type::F32]);
        assert_eq!(ret.len(), 2);
        assert_eq!(ret[0], ArgLocation::Reg(gpr::D0));
        assert_eq!(ret[1], ArgLocation::Reg(gpr::S1));
    }
    ...
    fn classify_hfa_return_three_f64() {
        ...
        assert_eq!(
            ret[0],
            ArgLocation::RegSequence(vec![gpr::D0, gpr::D1, gpr::D2])
        );
    }
    ...
    #[test]
    fn classify_fp_arg_f16_returns_h_register() {
        let (loc, next) = AppleAArch64ABI::classify_fp_arg(&Type::F16, 0).unwrap();
        assert_eq!(loc, ArgLocation::Reg(gpr::H0));
        assert_eq!(next, 1);
    }
    ...
    fn classify_variadic_v128_goes_on_stack() {
        assert_variadic_stack(
            AppleAArch64ABI::classify_variadic(&Type::V128, 0),
            0,
            16,
            16,
        );
    ...
    #[test]
    fn classify_hfa_param_single_f16() {
        let hfa = Type::Struct(vec![Type::F16]);
        let locs = AppleAArch64ABI::classify_params(&[hfa]);
        assert_eq!(locs.len(), 1);
        assert_eq!(locs[0], ArgLocation::RegSequence(vec![gpr::H0]));
    }
    ...
    fn classify_hfa_param_three_f64_after_five_scalars() {
        ...
        assert_eq!(
            locs[5],
            ArgLocation::RegSequence(vec![gpr::D5, gpr::D6, gpr::D7])
        );
    }
    ...
    #[test]
    fn classify_hfa_return_single_f16() {
        let hfa = Type::Struct(vec![Type::F16]);
        let ret = AppleAArch64ABI::classify_returns(&[hfa]);
        assert_eq!(ret.len(), 1);
        assert_eq!(ret[0], ArgLocation::RegSequence(vec![gpr::H0]));
    }
    ...
    fn classify_hfa_and_large_struct_interleaved() {
        ...
        let locs =
            AppleAArch64ABI::classify_params(&[hfa_f64, Type::I32, large, Type::F32, hfa_f32]);
        assert_eq!(locs.len(), 5);
        assert_eq!(locs[0], ArgLocation::RegSequence(vec![gpr::D0, gpr::D1]));
        assert_eq!(locs[1], ArgLocation::Reg(gpr::X0));
        assert_eq!(locs[2], ArgLocation::Indirect { ptr_reg: gpr::X1 });
        assert_eq!(locs[3], ArgLocation::Reg(gpr::S2));
        assert_eq!(
            locs[4],
            ArgLocation::RegSequence(vec![gpr::S3, gpr::S4, gpr::S5])
        );
    }
    ...
    fn compact_unwind_propagates_personality_flag() {
        let mut info = UnwindInfo::standard_frame(&[(gpr::X19, gpr::X20, false)], 32, false);
        info.personality = Some("__gxx_personality_v0".to_string());
        info.has_lsda = false;
        let entry = generate_compact_unwind(&info, 0, 128);
    ...
    #[test]
    fn compact_unwind_dwarf_fallback_leaves_eh_to_eh_frame() {
        // Runtime-sized stack metadata forces DWARF fallback; personality/LSDA references
        // are carried by CIE/FDE augmentation in __eh_frame instead.
        let mut info = UnwindInfo::leaf_frame();
        info.has_dynamic_alloc = true;
        info.personality = Some("rust_eh_personality".to_string());
        info.has_lsda = true;
        let entry = generate_compact_unwind(&info, 0, 32);
    ...
    #[test]
    fn compact_unwind_frameless_dwarf_fallback_leaves_eh_to_eh_frame() {
        let info = UnwindInfo {
            saved_registers: vec![],
            frame_size: 32,
            has_frame_pointer: false,
