// trust-cg-lower/tests/panic_fuzz_lower.rs
// Property-based panic-fuzz harness for `translate_function` (trust_ir adapter).
//
...
//
// Contract under test: for *any* trust_ir function, `translate_function` must
...
// random-malformed trust_ir inputs. The malformed stream is the more
// interesting of the two: it feeds trust_ir whose block IDs, value IDs, type
// IDs, and function signatures are deliberately inconsistent, because
// this is exactly what a buggy frontend would produce.
//
// Run:
//   cargo test -p trust-cg-lower --test panic_fuzz_lower
// Increase case count via env:
//   PROPTEST_CASES=100000 cargo test -p trust-cg-lower --test panic_fuzz_lower

use std::panic;

use proptest::prelude::*;
use trust_cg_lower::{AdapterError, translate_function};
use trust_ir::{
    BinOp, Block as TrustIrBlock, BlockId, Constant, FuncId, FuncTy, FuncTyId,
    Function as TrustIrFunction, ICmpOp, Inst, InstrNode, Module as TrustIrModule, SwitchCase, Ty,
    ValueId,
};
...
/// Full-coverage ICmpOp strategy: all 10 variants of `trust_ir::ICmpOp`.
///
...
fn func_spec_strategy(malformed: bool) -> impl Strategy<Value = FuncSpec> {
    ...
    (1usize..=4usize).prop_flat_map(move |nb| {
        let bc = nb as u32;
        (
            0u8..=2,
            ...
            .prop_map(
                move |(num_params, blocks, bad_entry, bad_func_ty)| FuncSpec {
                    num_params,
                    blocks,
...
// ---------------------------------------------------------------------------
// Spec -> trust_ir::Function materialisation (hand-rolled, no builder)
//
// We bypass `trust_ir_build::FunctionBuilder` because we want the freedom to
...
fn materialise(spec: &FuncSpec) -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("_panic_fuzz");
    ...
    let used_ft_id = if spec.bad_func_ty {
        FuncTyId::new(9999)
    } else {
        ft_id
    };
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "_fuzz_fn", used_ft_id, entry_block_id);
        ...
        for inst in &bspec.insts {
            match inst {
                ...
                InstKind::ICmp(op) => {
                    ...
                    // Bool in trust_ir (via ICmp) — stash as-is; downstream consumers
                ...
                InstKind::Gep => {
                    ...
                    // panic. Note trust_ir's variant is named `GEP` (not
                ...
                InstKind::InsertField => {
                    let (agg, val) = pick_two_or_zero(&mut defined_i64, &mut body, &mut alloc_vid);
                    let vid = alloc_vid();
                    body.push(
...
fn assert_no_panic(func: &TrustIrFunction, module: &TrustIrModule) {
    // trust_ir::Function/Module are Clone + UnwindSafe-in-practice; wrap
...
/// Opt-in evidence that the panic-fuzz lane fails loudly when the catch
/// wrapper observes a real panic. Normal test runs leave this disabled.
#[test]
fn panic_fuzz_negative_control_fails_when_enabled() {
    if std::env::var_os("TRUST_CG_PANIC_FUZZ_NEGATIVE_CONTROL").is_none() {
        return;
    }

    fn deliberate_panic() {
        panic!("deliberate panic-fuzz negative control");
    }

    let result = panic::catch_unwind(deliberate_panic);
    let payload = result.expect_err("deliberate panic-fuzz negative control must panic");
    let msg = if let Some(s) = payload.downcast_ref::<&'static str>() {
        (*s).to_string()
    } else if let Some(s) = payload.downcast_ref::<String>() {
        s.clone()
    ...
    panic!("panic-fuzz negative control observed deliberate panic: {msg}");
}
...
proptest! {
    ...
    /// Random-but-valid trust_ir (no dangling refs, entry block = 0, registered
    ...
    /// Deliberately malformed trust_ir (dangling ValueIds, out-of-range block
...
/// a `TrustIrFunction` whose `ty` referenced an unregistered `FuncTyId`
...
fn regression_unregistered_func_ty_panics() {
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "_bad_ft", bad_ft, BlockId::new(0));
...
fn regression_out_of_range_entry_returns_err() {
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "_bad_entry", ft_id, bad_entry);
    func.blocks = vec![TrustIrBlock {
        id: BlockId::new(0),
        params: vec![],
        body: vec![InstrNode::new(Inst::Return { values: vec![] })],
