...
fn workdir(stem: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("rcl2_m68_{stem}_{}", std::process::id()));
...
// MISCOMPILE #68: two pre-existing float lowering bugs found by float fuzzing
// (the earlier sweep wrongly skipped floats).
//   (a) `select_fneg` computed `-x` as `0.0 - x` (SUBSD), which yields `+0.0`
//       for `-(0.0)` instead of `-0.0` (and can alter a NaN's sign). Float neg
//       must flip the IEEE sign bit (XOR sign mask). Observable via division by
//       the negated zero.
//   (b) `select_fcvt_from_int` converted a 32-bit signed source with a 64-bit
//       CVTSI2SS over the ZERO-extended carrier, so `-7i32 as f32` produced
//       4_294_967_289.0 instead of -7.0. The source must be sign-extended first.

/// `-(0.0f64)` must be `-0.0`, so `-5.0 / -(0.0) == +inf` (> 0). With the old
/// `0.0 - x` neg it was `+0.0`, giving `-inf` (< 0).
#[test]
fn m68_neg_zero_sign_via_division_matches_llvm() {
    differential_case(
        "neg_zero_div",
        "    let z = -core::hint::black_box(0.0f64);\n\
         \x20   (core::hint::black_box(-5.0f64) / z > 0.0) as i32",
        1,
    );
}

/// `-(-3.5f64) == 3.5`. Basic float negation.
#[test]
fn m68_double_neg_value_matches_llvm() {
    differential_case(
        "double_neg",
        "    (-(-core::hint::black_box(3.5f64))) as i32",
        3,
    );
}

/// Negative `i32 as f32` must be exact: `-7i32 as f32 == -7.0f32` (before the fix
/// it was 4_294_967_289.0).
#[test]
fn m68_neg_i32_to_f32_exact_matches_llvm() {
    differential_case(
        "i32_f32",
        "    ((core::hint::black_box(-7i32) as f32) == -7.0f32) as i32",
        1,
    );
}

/// Larger negative `i32 as f32` round-trip: `-539906i32 as f32 as i64`, low 7
/// bits == 126.
#[test]
fn m68_neg_i32_to_f32_roundtrip_matches_llvm() {
    differential_case(
        "i32_f32_rt",
        "    (((core::hint::black_box(-539906i32) as f32) as i64) as i32) & 0x7F",
        126,
    );
}

/// f32 negation value: `-(2.5f32) == -2.5`.
#[test]
fn m68_f32_neg_value_matches_llvm() {
    differential_case(
        "f32_neg",
        "    ((-core::hint::black_box(2.5f32)) * -1.0f32) as i32",
        2,
    );
}
