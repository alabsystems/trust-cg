...
fn workdir(stem: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("rcl2_m67_{stem}_{}", std::process::id()));
...
// MISCOMPILE #67: signed `overflowing_mul`/`checked_mul` on i8/i16/i32 where an
// operand is 0 CRASHED with SIGFPE. The overflow flag was computed via the
// AArch64 SDIV identity (`(a*b) / rhs != lhs`), assuming SDIV-by-zero returns 0
// (true on AArch64). On x86-64 IDIV by zero TRAPS (#DE), so `x.overflowing_mul(0)`
// divided by zero and crashed. Fixed by the division-free signed wide-multiply
// check. These programs are gated at -Copt-level 3 only: the out-of-line overflow
// intrinsic hits a separate x87/sse2 target-feature fail-closed gap at O0 (#65).
//
// NOTE: the harness differential_case here is patched to opt level "3" only.

/// `5i32.overflowing_mul(0)` = (0, false). Before the fix this SIGFPE'd; the
/// product is 0.
#[test]
fn m67_i32_overflowing_mul_by_zero_matches_llvm() {
    differential_case(
        "i32_omul_zero",
        "    let (p, o): (i32, bool) = core::hint::black_box(5i32)\n\
         \x20       .overflowing_mul(core::hint::black_box(0i32));\n\
         \x20   (p.wrapping_mul(7) + (o as i32)) & 0xff",
        0,
    );
}

/// `123i16.overflowing_mul(0)` = (0, false) — i16 by-zero, no SIGFPE.
#[test]
fn m67_i16_overflowing_mul_by_zero_matches_llvm() {
    differential_case(
        "i16_omul_zero",
        "    let (p, o): (i16, bool) = core::hint::black_box(123i16)\n\
         \x20       .overflowing_mul(core::hint::black_box(0i16));\n\
         \x20   ((p as i32) + (o as i32)) & 0xff",
        0,
    );
}

/// i8 overflowing_mul by zero (exercises the i8 -> i16 sign-extension that also
/// had to be enabled in `select_extend`).
#[test]
fn m67_i8_overflowing_mul_by_zero_matches_llvm() {
    differential_case(
        "i8_omul_zero",
        "    let (p, o): (i8, bool) = core::hint::black_box(50i8)\n\
         \x20       .overflowing_mul(core::hint::black_box(0i8));\n\
         \x20   ((p as i32) + (o as i32)) & 0xff",
        0,
    );
}

/// A genuine signed overflow still reports overflow correctly: `2_000_000_000i32
/// * 2` overflows. flag=true, wrapped product is the low 32 bits.
#[test]
fn m67_i32_overflowing_mul_overflow_flag_matches_llvm() {
    differential_case(
        "i32_omul_ovf",
        "    let (_p, o): (i32, bool) = core::hint::black_box(2_000_000_000i32)\n\
         \x20       .overflowing_mul(core::hint::black_box(2i32));\n\
         \x20   o as i32",
        1,
    );
}

/// `i8 as i16` / `u8 as u16` widening casts now lower (previously fail-closed
/// `Sextend/Uextend {from: I8, to: I16}`). `(-100i8 as i16) + (200u8 as u16) == 100`.
#[test]
fn m67_i8_to_i16_extend_matches_llvm() {
    differential_case(
        "i8_i16_ext",
        "    let a = core::hint::black_box(-100i8) as i16;\n\
         \x20   let b = core::hint::black_box(200u8) as u16;\n\
         \x20   ((a as i32) + (b as i32)) & 0xff",
        100,
    );
}
