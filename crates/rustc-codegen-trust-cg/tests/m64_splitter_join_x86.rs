...
fn workdir(stem: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("rcl2_m64_{stem}_{}", std::process::id()));
...
// MISCOMPILE #64: the greedy live-range SPLITTER's join-block split-copy
// placement is unsound under register pressure for CALL-FREE joins (the guard
// only rejected the call+join `?`-diamond, m52). An ordinary scalar program
// with ~13 live values flowing across a `match` join returned the WRONG result
// at -Copt-level 2/3 (correct at 0/1, since splitting is O2+). Fixed by
// rejecting splitting for ANY join block (pipeline.rs JoinBlockUnderSplit);
// straight-line / linear code still splits. Delta-debugged minimal repro: at
// O3 the buggy backend returned 27 instead of 167.
#[test]
fn m64_match_join_under_register_pressure_matches_llvm() {
    differential_case(
        "match_join_pressure",
        r#"    let v0: i8 = core::hint::black_box(-118i8);
    let v1: i32 = core::hint::black_box(1610898894i32);
    let v2: i8 = core::hint::black_box(-38i8);
    let v3: u16 = core::hint::black_box(35312u16);
    let v4: i64 = core::hint::black_box(1108431103889294839i64);
    let v5: i64 = v4.wrapping_sub(v4);
    let v6: i8 = if v2 <= v2 { v2 } else { v2 };
    let v8: u16 = if v3 <= v3 { v3 } else { v3 };
    let v9: i64 = if v5 < v4 { v5 } else { v4 };
    let v10: i8 = match v6 { 125i8 => v6.wrapping_add(1), -58i8 => v6.wrapping_sub(1), _ => v6 };
    let v12: i64 = if v5 >= v4 { v5 } else { v4 };
    let v13: i64 = v12 ^ v9;
    let v15: i8 = if v10 >= v6 { v10 } else { v6 };
    let mut acc: u64 = 0;
    acc = acc.wrapping_mul(31).wrapping_add(v0 as i64 as u64);
    acc = acc.wrapping_mul(31).wrapping_add(v1 as i64 as u64);
    acc = acc.wrapping_mul(31).wrapping_add(v2 as i64 as u64);
    acc = acc.wrapping_mul(31).wrapping_add(v3 as i64 as u64);
    acc = acc.wrapping_mul(31).wrapping_add(v4 as i64 as u64);
    acc = acc.wrapping_mul(31).wrapping_add(v5 as i64 as u64);
    acc = acc.wrapping_mul(31).wrapping_add(v6 as i64 as u64);
    acc = acc.wrapping_mul(31).wrapping_add(v8 as i64 as u64);
    acc = acc.wrapping_mul(31).wrapping_add(v9 as i64 as u64);
    acc = acc.wrapping_mul(31).wrapping_add(v10 as i64 as u64);
    acc = acc.wrapping_mul(31).wrapping_add(v12 as i64 as u64);
    acc = acc.wrapping_mul(31).wrapping_add(v13 as i64 as u64);
    acc = acc.wrapping_mul(31).wrapping_add(v15 as i64 as u64);
    (acc & 0xFF) as i32"#,
        167,
    );
}
