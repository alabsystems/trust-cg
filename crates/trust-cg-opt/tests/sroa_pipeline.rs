...
//! `crates/trust-cg-opt/tests/aggregate_pipeline.rs`.
...
fn sroa_o0_no_change() {
    ...
    assert!(count(&after, AArch64Opcode::LdrRI) >= 1, "O0: LDR kept");
}
...
fn sroa_respects_escape_at_all_levels() {
    fn make_escape() -> MachFunction {
        let mut func = MachFunction::new("sroa_escape".to_string(), Signature::new(vec![], vec![]));
        let entry = func.entry;
        func.next_vreg = 20;
        ...
        // Keep one call in non-tail position so later tail-call optimization
        // cannot erase the only synthetic vreg use and turn this into a dead
        // address-materialization fixture.
        push(
            &mut func,
            entry,
            MachInst::new(
        ...
        push(
            ...
            MachInst::new(
                AArch64Opcode::Bl,
                vec![MachOperand::Symbol("callee_tail".to_string()), vreg64(10)],
            ),
        );
        push(&mut func, entry, MachInst::new(AArch64Opcode::Ret, vec![]));
        func
    ...
    // address escapes (i.e., is passed to a non-tail call). Other passes may
    // transform the tail-position call at higher levels, but the non-tail call
    // keeps the address materialization live.
...
// `crates/trust-cg-lower/tests/aggregate_constants.rs` (1 StackAddr, per-leaf
// Store shape). That proves the trust_ir -> MachIR shape, but NOT that the SROA
...
// nested test fires before any real trust_ir input hits the pipeline.
// ---------------------------------------------------------------------------
