// trust-cg-opt/tests/regression_convergence.rs
//
...
// dependencies) and CFG simplification (oscillating sub-passes). The fixes are:
//   1. Preserving original order when schedule_list / schedule_list_pressure_aware
//      cannot legally break cyclic dependencies
...
use trust_cg_opt::{
    ...
    scheduler::{
        ExecutionPort, ScheduleDAG, ScheduleNode, schedule_list, schedule_list_pressure_aware,
    },
};
...
fn add_ri(dst: u32, src: u32, value: i64) -> MachInst {
    MachInst::new(AArch64Opcode::AddRI, vec![vreg(dst), vreg(src), imm(value)])
}
...
/// dependency cycle (A depends on B, B depends on A). The scheduler cannot
/// prove a reordered schedule is legal, so it must preserve original order.
#[test]
...
/// (0 depends on 2, 1 depends on 0, 2 depends on 1). Preserve original order
/// rather than dropping any dependency edge.
#[test]
fn test_scheduler_three_node_cycle_terminates() {
    ...
    assert_under_five_seconds(start);
    assert_eq!(order, vec![InstId(0), InstId(1), InstId(2)]);
}

/// Regression: when free (no-dep) nodes coexist with cyclic nodes, the
/// scheduler must terminate without breaking dependencies. Once it discovers
/// the remaining cycle, it falls back to the original region order.
#[test]
fn test_scheduler_mixed_cycle_and_free() {
    ...
    assert_under_five_seconds(start);
    assert_eq!(order, vec![InstId(0), InstId(1), InstId(2), InstId(3)]);
}

/// Regression: schedule_list_pressure_aware must also terminate on cyclic
/// DAGs. It has its own copy of the original-order fallback. Test with a
...
fn test_scheduler_pressure_aware_cyclic_terminates() {
    ...
    assert_under_five_seconds(start);
    assert_eq!(order, vec![inst0, inst1]);
}
...
fn test_cfg_simplify_chain_of_empty_blocks() {
    ...
    assert_eq!(func.inst(entry_block.insts[0]).opcode, AArch64Opcode::Ret);
}
...
/// a bounded iteration count) must terminate on a multi-block diamond CFG. This
...
///              <---B--- bb2
#[test]
fn regression_convergence_pipeline_o3_multiblock_terminates() {
    ...
    assert_eq!(stats.runs.len(), pass_count);
    let unskipped_run_bound = pass_count * stats.iterations as usize;
    assert!(
        stats.total_pass_runs() < unskipped_run_bound,
        "the fixed-point driver should skip at least one deterministic no-op"
    );
    for (pass_name, runs) in &stats.runs {
        assert!(
            (1..=stats.iterations).contains(runs),
            "pass {pass_name} ran {runs} times across {} O3 iterations",
            stats.iterations
        );
    }
    assert!(has_return(&func));
