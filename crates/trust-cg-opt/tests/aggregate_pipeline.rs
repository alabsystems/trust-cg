...
//! array element addressing, enum tag/payload stores, aggregate loads/stores) survive every
//! optimisation level without being miscompiled.
//!
//! The machine IR patterns below mimic what the ISel produces for
//! `Opcode::StructGep`, `Opcode::ArrayGep`, enum `Constant::Aggregate`
//! materialisation, and bulk memory copies: `AddRI` + `LdrRI`/`StrRI`
//! for struct fields, `LslRI` + `AddRR` + `LdrRI`/`StrRI` for array element
//! addressing, and `StrbRI` tag plus `StrRI` payload writes for enums.
//!
//! See
...
use trust_cg_ir::{
    AArch64Opcode, BlockId, InstId, MachBlock, MachFunction, MachInst, MachOperand, RegClass,
    Signature, VReg,
};
use trust_cg_opt::{OptLevel, OptimizationPipeline};
...
/// Build a function modelling tagged-union enum materialisation after #443:
///
/// ```text
///   base         = v0
///   tag          = 1                         ; selected variant index
///   payload      = 42                        ; selected variant field
///   payload_addr = base + 8                  ; aligned payload offset
///   STRB tag, [base, #0]                     ; tag/discriminator
///   STR  payload, [payload_addr, #0]         ; variant payload field
///   RET
/// ```
fn make_enum_tagged_union_write_function(name: &str) -> MachFunction {
    let mut func = MachFunction::new(name.to_string(), Signature::new(vec![], vec![]));
    let entry = func.entry;

    let tag = func.push_inst(MachInst::new(AArch64Opcode::Movz, vec![vreg32(1), imm(1)]));
    func.append_inst(entry, tag);

    let payload = func.push_inst(MachInst::new(AArch64Opcode::Movz, vec![vreg64(2), imm(42)]));
    func.append_inst(entry, payload);

    let payload_addr = func.push_inst(MachInst::new(
        AArch64Opcode::AddRI,
        vec![vreg64(3), vreg64(0), imm(8)],
    ));
    func.append_inst(entry, payload_addr);

    let tag_store = func.push_inst(MachInst::new(
        AArch64Opcode::StrbRI,
        vec![vreg32(1), vreg64(0), imm(0)],
    ));
    func.append_inst(entry, tag_store);

    let payload_store = func.push_inst(MachInst::new(
        AArch64Opcode::StrRI,
        vec![vreg64(2), vreg64(3), imm(0)],
    ));
    func.append_inst(entry, payload_store);
...
fn assert_enum_tagged_union_pattern_preserved(func: &MachFunction, ctx: &str) {
    let entry = func.entry;
    let tag_stores = count_opcode(func, entry, AArch64Opcode::StrbRI);
    let payload_stores = count_opcode(func, entry, AArch64Opcode::StrRI);
    let ret = count_opcode(func, entry, AArch64Opcode::Ret);

    assert!(
        tag_stores >= 1,
        "[{}] enum tag store was removed; variant identity must remain observable",
        ctx
    );
    assert!(
        payload_stores >= 1,
        "[{}] enum payload store was removed; selected variant payload must survive",
        ctx
    );
    assert!(ret >= 1, "[{}] RET disappeared", ctx);
}
...
// ---------------------------------------------------------------------------
// Enum tagged-union materialisation — every opt level preserves both the tag
// store and the selected-variant payload store.
// ---------------------------------------------------------------------------

#[test]
fn enum_tagged_union_write_o0_identity() {
    let mut func = make_enum_tagged_union_write_function("enum_tagged_o0");
    let before_insts: Vec<InstId> = func.block(func.entry).insts.clone();
    OptimizationPipeline::new(OptLevel::O0).run(&mut func);
    let after_insts = &func.block(func.entry).insts;
    assert_eq!(&before_insts, after_insts, "O0 must be identity");
    assert_enum_tagged_union_pattern_preserved(&func, "O0");
}

#[test]
fn enum_tagged_union_write_o1_preserves_tag_and_payload() {
    let mut func = make_enum_tagged_union_write_function("enum_tagged_o1");
    OptimizationPipeline::new(OptLevel::O1).run(&mut func);
    assert_enum_tagged_union_pattern_preserved(&func, "O1");
}

#[test]
fn enum_tagged_union_write_o2_preserves_tag_and_payload() {
    let mut func = make_enum_tagged_union_write_function("enum_tagged_o2");
    OptimizationPipeline::new(OptLevel::O2).run(&mut func);
    assert_enum_tagged_union_pattern_preserved(&func, "O2");
}

#[test]
fn enum_tagged_union_write_o3_preserves_tag_and_payload() {
    let mut func = make_enum_tagged_union_write_function("enum_tagged_o3");
    OptimizationPipeline::new(OptLevel::O3).run(&mut func);
    assert_enum_tagged_union_pattern_preserved(&func, "O3");
}
