...
//
// ty supremacy blocker 7 (issue #395): the compilation cache must be
// deterministic across runs. If `stable_hash` on the same input returns
// different bytes twice, every repeat compilation is a cache miss — ty's
...
use trust_cg_opt::CacheBackend;
use trust_cg_opt::{CacheKey, InMemoryCache, StableHasher, stable_hash};

#[test]
fn stable_hash_is_deterministic() {
    let input = b"trust-cg cache determinism smoke input 0123456789";
