...
pub struct LoopAnalysis {
    ...
    ///
    /// DETERMINISM: a `BTreeMap`, NOT a `HashMap` — `all_loops()` iteration order
    /// decides which loop a pass transforms first, which decides vreg numbering
    /// and ultimately the emitted BYTES. `std::collections::HashMap`'s
    /// random-seeded hasher made the same binary emit different (all valid)
    /// objects run-to-run on multi-loop functions; a verified compiler must be
    /// reproducible. Header-ordered iteration is total and stable.
...
impl LoopAnalysis {
    ...
    pub fn compute(func: &MachFunction, dom: &DomTree) -> Self {
        ...
        let mut loops: BTreeMap<BlockId, NaturalLoop> = BTreeMap::new();
        ...
        // A header may have multiple back-edges.  The first one can describe a
        // strict subset of the final merged loop body (most sharply, a header
        // self-edge plus a second latch).  Computing the preheader while that
        // partial body is still being assembled can therefore leave a stale
        // "non-loop" predecessor that is actually inside the merged loop.
        // Recompute from the complete body so every consumer sees a genuine
        // outside-loop predecessor or `None`.
        for natural_loop in loops.values_mut() {
            natural_loop.preheader = find_preheader(func, natural_loop.header, &natural_loop.body);
        }
...
fn compute_nesting(loops: &mut BTreeMap<BlockId, NaturalLoop>) {
    ...
    for &h1 in &headers {
        ...
        for &h2 in &headers {
            ...
            if body1.is_subset(body2) && body1.len() < body2.len() && body2.len() < best_parent_size
            {
                best_parent = Some(h2);
...
mod tests {
    ...
    fn make_nested_loops() -> MachFunction {
        let mut func = MachFunction::new("nested".to_string(), Signature::new(vec![], vec![]));
        let bb0 = func.entry;
        let bb1 = func.create_block();
        let bb2 = func.create_block();
        let bb3 = func.create_block();
    ...
    #[test]
    fn test_merged_latches_recompute_preheader_from_complete_body() {
        // bb0 is both the entry/header and a self-latch.  bb1 is a second
        // latch.  When the self-edge is visited first, the partial body is
        // {bb0} and bb1 superficially looks like a preheader.  After merging
        // the bb1 -> bb0 back-edge, however, bb1 is in the loop and there is no
        // outside-loop predecessor at all.
        let mut func =
            MachFunction::new("merged_latches".to_string(), Signature::new(vec![], vec![]));
        let bb0 = func.entry;
        let bb1 = func.create_block();
        ...
        let dom = DomTree::compute(&func);
        let la = LoopAnalysis::compute(&func, &dom);
        let lp = la.get_loop(bb0).expect("merged loop at entry header");

        assert_eq!(lp.body, HashSet::from([bb0, bb1]));
        assert_eq!(
            lp.preheader, None,
            "a block in the final merged loop body cannot remain its preheader"
        );
    }
    ...
    fn test_loop_with_multiple_exits() {
        ...
        let mut func = MachFunction::new("multi_exit".to_string(), Signature::new(vec![], vec![]));
        let bb0 = func.entry;
        let bb1 = func.create_block();
        let bb2 = func.create_block();
        let bb3 = func.create_block();
        ...
        assert!(
            !lp.body.contains(&BlockId(4)),
            "exit block bb4 should not be in loop"
        );
    }
    ...
    fn test_triple_nested_loop_depths() {
        ...
        let mut func =
            MachFunction::new("triple_nested".to_string(), Signature::new(vec![], vec![]));
        let bb0 = func.entry;
        ...
        assert_eq!(
            la.num_loops(),
            3,
            "should detect 3 loops (outer, mid, inner)"
        );
    ...
    fn test_loop_with_multiple_latches() {
        ...
        let mut func = MachFunction::new("multi_latch".to_string(), Signature::new(vec![], vec![]));
        let bb0 = func.entry;
        let bb1 = func.create_block();
        let bb2 = func.create_block();
        let bb3 = func.create_block();
    ...
    fn test_no_preheader_with_multiple_entries() {
        ...
        let mut func =
            MachFunction::new("no_preheader".to_string(), Signature::new(vec![], vec![]));
        let bb0 = func.entry;
    ...
    fn test_linear_chain_no_loops() {
        ...
        let mut func = MachFunction::new("chain".to_string(), Signature::new(vec![], vec![]));
        let bb0 = func.entry;
        let bb1 = func.create_block();
        let bb2 = func.create_block();
        let bb3 = func.create_block();
    ...
    fn test_self_loop_preheader() {
        ...
        let mut func =
            MachFunction::new("self_loop_ph".to_string(), Signature::new(vec![], vec![]));
        let bb0 = func.entry;
        let bb1 = func.create_block();
        let bb2 = func.create_block();
        ...
        assert_eq!(
            lp.preheader,
            Some(BlockId(0)),
            "bb0 should be preheader of self-loop"
        );
    }
    ...
    fn test_single_block_function_no_loops() {
        let mut func =
            MachFunction::new("single_block".to_string(), Signature::new(vec![], vec![]));
        let bb0 = func.entry;
        let ret = func.push_inst(MachInst::new(AArch64Opcode::Ret, vec![]));
