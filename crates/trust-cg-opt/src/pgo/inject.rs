// trust-cg-opt/pgo/inject.rs - Basic-block counter-injection pass
//
...
// (`__trust_cg_profile_bb_enter$<func>$<bb>`) at the top of every basic
...
// `__trust_cg_profile_bb_enter(func_id, bb_id)` with integer args can
...
//! `crates/trust-cg-codegen/src/jit.rs:1477`).
...
///
/// ```text
///   BL  __trust_cg_profile_bb_enter$foo$3
/// ```
///
...
pub const COUNTER_SYMBOL_PREFIX: &str = "__trust_cg_profile_bb_enter";
...
/// [`trust_cg_ir::target_info`] (same gating as the per-function
/// trampoline).
pub fn inject_block_counters(func: &mut MachFunction) -> CounterMap {
    ...
    for (next_index, block_id) in (0_u32..).zip(blocks) {
        let symbol = counter_symbol(&func.name, block_id.0);

        let inst = MachInst::new(
            AArch64Opcode::Bl,
            vec![
                MachOperand::Symbol(symbol.clone()),
                MachOperand::Imm(block_id.0 as i64),
            ],
        );
...
mod tests {
    ...
    fn counter_symbol_format_is_stable() {
        assert_eq!(
            ...
            "__trust_cg_profile_bb_enter$foo$3"
        );
    }
