// trust-cg-opt/cache.rs - Compilation cache + stable hash
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
// xxHash-derived constants and finalization steps:
...
//! Compilation cache hooks and stable hashing for Trust Codegen.
//!
...
//!
//! ```
//! use trust_cg_opt::cache::{CacheBackend, CacheKey, InMemoryCache, stable_hash};
//!
...
impl StableHasher {
    /// Create a hasher seeded with the canonical Trust Codegen cache seeds.
    pub fn new() -> Self {
...
///
/// ```
/// use trust_cg_opt::cache::stable_hash;
...
impl CacheKey {
    ...
    pub fn new(
        ...
        let digest = Self::compute_digest(module_hash, opt_level, &target_triple, &cpu, &features);
...
static FILE_CACHE_TMP_COUNTER: AtomicU64 = AtomicU64::new(0);
...
impl FileCache {
    ...
    fn tmp_path_for(&self, key: &CacheKey, shard_dir: &Path) -> PathBuf {
        let nonce = FILE_CACHE_TMP_COUNTER.fetch_add(1, Ordering::Relaxed);
        shard_dir.join(format!(
            ".{}.{}.{}.tmp",
            std::process::id(),
            nonce,
            key.hex()
    ...
    fn write_atomic(&self, key: &CacheKey, value: &[u8]) -> io::Result<()> {
        ...
        // Temp file name includes a per-call nonce so same-process writers
        // racing on the same key never share a staging path.
        let tmp = self.tmp_path_for(key, shard_dir);
...
impl CacheBackend for FileCache {
    fn get(&self, key: &CacheKey) -> Option<Vec<u8>> {
        match std::fs::read(self.path_for(key)) {
            ...
            Err(e) => {
                eprintln!(
                    "trust-cg-opt::cache: warning: failed to read {}: {}",
    ...
    fn put(&self, key: &CacheKey, value: &[u8]) {
        if let Err(e) = self.write_atomic(key, value) {
            eprintln!(
                "trust-cg-opt::cache: warning: failed to write {}: {}",
                self.path_for(key).display(),
                e
            );
        }
...
mod tests {
    ...
    #[test]
    fn test_file_cache_temp_paths_are_per_call_unique() {
        let root = tmpdir("tmp_unique");
        let cache = FileCache::new(&root).unwrap();
        let k = key(0xbeef, 2);
        let target = cache.path_for(&k);
        let shard_dir = target.parent().unwrap();

        let first = cache.tmp_path_for(&k, shard_dir);
        let second = cache.tmp_path_for(&k, shard_dir);

        assert_ne!(
            first, second,
            "same-key writes must not reuse a temp path within one process",
        );
        let _ = std::fs::remove_dir_all(&root);
    }
