...
use trust_cg_ir::{AArch64Opcode, BlockId, InstId, MachFunction, MachInst, OpcodeCategory, VReg};
...
impl<'a> MatchCtx<'a> {
    ...
    pub fn operand_def(&self, idx: usize) -> Option<&MachInst> {
        ...
        let def_id = self.def_map.get(&vreg)?;
...
mod tests {
    use super::*;
    use trust_cg_ir::{MachOperand, RegClass, Signature};
    ...
    fn definer_of_sees_movi_before_add() {
        ...
        let (func, entry, ids) = make_func_with_block(vec![
            MachInst::new(AArch64Opcode::MovI, vec![vreg(1), MachOperand::Imm(5)]),
            MachInst::new(
                AArch64Opcode::AddRI,
    ...
    fn definer_of_intervening_redef_picks_newest() {
        ...
        let (func, entry, ids) = make_func_with_block(vec![
            MachInst::new(AArch64Opcode::MovI, vec![vreg(1), MachOperand::Imm(5)]),
            MachInst::new(AArch64Opcode::MovI, vec![vreg(1), MachOperand::Imm(9)]),
            MachInst::new(
                AArch64Opcode::AddRI,
        ...
        assert_eq!(
            *ctx.def_map.get(&VReg::new(1, RegClass::Gpr64)).unwrap(),
            ids[1],
    ...
    #[test]
    fn definer_of_same_id_different_class_is_class_exact() {
        // movi v1:Gpr32, #5
        // movi v1:Gpr64, #9
        // add  v2:Gpr64, v1:Gpr32, #0
        //
        // A raw-id def map would resolve the Gpr32 use to the newer Gpr64
        // writer. The rewrite engine must keep these identities distinct.
        let (func, entry, ids) = make_func_with_block(vec![
            MachInst::new(
                AArch64Opcode::MovI,
                vec![vreg_class(1, RegClass::Gpr32), MachOperand::Imm(5)],
            ),
            MachInst::new(
                AArch64Opcode::MovI,
                vec![vreg_class(1, RegClass::Gpr64), MachOperand::Imm(9)],
            ),
            MachInst::new(
                AArch64Opcode::AddRI,
                vec![vreg(2), vreg_class(1, RegClass::Gpr32), MachOperand::Imm(0)],
            ),
        ]);
        let def_map = def_map_after(&func, entry, 2);
        let ctx = MatchCtx {
        ...
        let def = ctx.definer_of(1).expect("class-exact definer should exist");
        assert_eq!(def.opcode, AArch64Opcode::MovI);
        assert!(matches!(def.operands[1], MachOperand::Imm(5)));
    }
