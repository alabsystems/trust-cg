// trust-cg-verify/strength_reduce_proofs.rs - SMT proofs for strength reduction pass correctness
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// Proves that each algebraic identity used by the strength reduction pass
// (crates/trust-cg-opt/src/strength_reduce.rs) is correct for all bitvector
...
//
// Each proof encodes a trust_ir-side (original) expression and an AArch64-side
...
/// Encode unsigned modulo `x % y` using native SMT-LIB `bvurem`.
...
fn proof_mul_to_shift_n(n: u32, width: u32) -> ProofObligation {
    ...
    // trust_ir side: x * (2^n)
    let trust_ir = x.clone().bvmul(power_const);
    // AArch64 side: x << n
...
fn proof_mul_to_add_width(width: u32) -> ProofObligation {
    ...
    let trust_ir = x.clone().bvmul(two);
...
fn proof_mul_to_sub_shift_n(n: u32, width: u32) -> ProofObligation {
    ...
    // trust_ir side: x * (2^n - 1)
    let trust_ir = x.clone().bvmul(constant);
    // AArch64 side: (x << n) - x
...
pub fn proof_mul_to_sub_shift_64() -> Vec<ProofObligation> {
    vec![2, 3, 4, 5]
        .into_iter()
        .map(|n| proof_mul_to_sub_shift_n(n, 64))
        .collect()
}
...
pub fn proof_mul_to_sub_shift_8() -> Vec<ProofObligation> {
    vec![2, 3, 4, 5]
        .into_iter()
        .map(|n| proof_mul_to_sub_shift_n(n, 8))
        .collect()
}
...
fn proof_div_to_shift_n(n: u32, width: u32) -> ProofObligation {
    ...
    // trust_ir side: x / (2^n) (unsigned)
    let trust_ir = x.clone().bvudiv(power_const);
...
///
/// We encode umod as native SMT-LIB `bvurem` so the solver reasons about the
/// exact operation instead of a load-sensitive `udiv`/`mul`/`sub` expansion.
fn proof_mod_to_mask_n(n: u32, width: u32) -> ProofObligation {
    ...
    // trust_ir side: x % (2^n) encoded as x - udiv(x, 2^n) * 2^n
    let trust_ir = encode_umod(x.clone(), power_const);
...
fn proof_iv_update_at_iter(iter: u64, width: u32) -> ProofObligation {
    ...
    // trust_ir side: base + iter * stride (closed form)
    let iter_const = SmtExpr::bv_const(iter, width);
    let trust_ir = base.clone().bvadd(iter_const.bvmul(stride.clone()));
...
fn proof_no_change_add(width: u32) -> ProofObligation {
    ...
    let trust_ir = a.clone().bvadd(b.clone());
...
fn proof_no_change_sub(width: u32) -> ProofObligation {
    ...
    let trust_ir = a.clone().bvsub(b.clone());
...
///
///   TOTAL = 66
/// Retracted degenerate X==X StrengthReduce obligations (idempotence double-apply, no-change safety, DCE-composition, iv+stride==iv+stride — X==X). Genuine x*2^k==x<<k, x*(2^n-1)==(x<<n)-x, x/2^k==x>>k, x%2^k==x&mask, base+k*stride==acc proofs remain.
const STRENGTH_RETRACTED_DEGENERATE: &[&str] = &[
    "StrengthReduce: DCE composition (dead mul removable)",
    "StrengthReduce: DCE composition (dead mul removable) (8-bit)",
    "StrengthReduce: idempotence (x << 1) after double-apply",
    "StrengthReduce: idempotence (x << 1) after double-apply (8-bit)",
    "StrengthReduce: idempotence (x << 2) after double-apply",
    "StrengthReduce: idempotence (x << 2) after double-apply (8-bit)",
    "StrengthReduce: idempotence (x << 3) after double-apply",
    "StrengthReduce: idempotence (x << 3) after double-apply (8-bit)",
    "StrengthReduce: iv + stride == iv + stride (IV update step)",
    "StrengthReduce: iv + stride == iv + stride (IV update step) (8-bit)",
    "StrengthReduce: no-change safety (a + b)",
    "StrengthReduce: no-change safety (a + b) (8-bit)",
    "StrengthReduce: no-change safety (a - b)",
    "StrengthReduce: no-change safety (a - b) (8-bit)",
];
...
pub fn all_strength_reduce_proofs() -> Vec<ProofObligation> {
    ...
    // #62 retraction: drop degenerate X==X self-equalities (see STRENGTH_RETRACTED_DEGENERATE).
    proofs.retain(|p| !STRENGTH_RETRACTED_DEGENERATE.contains(&p.name.as_str()));
    proofs
}
...
mod tests {
    ...
    fn test_all_strength_reduce_proofs_count() {
        ...
        assert_eq!(
            proofs.len(),
            54,
            "expected 54 strength reduction proofs (12 degenerate idempotence/no-change/\
             DCE-composition/iv+stride X==X retracted in #62), got {}",
            proofs.len()
        );
    }
