// trust-cg-verify/vectorization_proofs.rs - NEON auto-vectorization lowering proofs
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// Proof obligations verifying that the NEON auto-vectorization pass
// (`trust_cg_opt::vectorize::scalar_to_neon_op()`) correctly maps scalar
...
// trust_ir vector ops -> NEON instructions. These proofs verify the
...
//
// Reference: crates/trust-cg-opt/src/vectorize.rs (scalar_to_neon_op)
...
//! ```text
//! trust_ir side:  map_lanes_binary(vn, vm, arrangement, |a, b| scalar_op(a, b))
//! NEON side:  encode_neon_op(arrangement, vn, vm)
//! Proof:      forall vn, vm: trust_ir_side == NEON_side
//! ```
//!
...
use crate::smt::{
    SmtExpr, VectorArrangement, concat_lanes, lane_extract, map_lanes_binary, map_lanes_binary_imm,
    map_lanes_unary,
};
...
///
/// trust_ir semantics: per-lane `bvadd` (wrapping).
...
///
/// trust_ir BIC: `bvand(vn, bvxor(vm, all_ones))` where bvxor with all-ones = NOT.
...
fn proof_vectorize_shl(arrangement: VectorArrangement, imm: u32, label: &str) -> ProofObligation {
    let vn = symbolic_vector("vn", arrangement);
...
fn proof_vectorize_ushr(arrangement: VectorArrangement, imm: u32, label: &str) -> ProofObligation {
    let vn = symbolic_vector("vn", arrangement);
...
fn proof_vectorize_sshr(arrangement: VectorArrangement, imm: u32, label: &str) -> ProofObligation {
    let vn = symbolic_vector("vn", arrangement);
...
/// trust_ir semantics: per-lane `va[i] + vn[i] * vm[i]` (wrapping).
...
mod tests {
    use super::*;
    use crate::lowering_proof::{TransvalCheckKind, verify_by_evaluation};
    ...
    // =======================================================================
    // Aggregate inventory test: all 45 vectorization proofs
    // =======================================================================
    ...
    fn test_all_vectorization_proofs() {
        ...
        assert_eq!(
            proofs.len(),
            45,
            "expected 45 proofs across all arrangements"
        );

        let mut names = HashSet::with_capacity(proofs.len());
        for obligation in &proofs {
            assert_eq!(
                obligation.category,
                Some(TransvalCheckKind::Vectorization),
                "proof '{}' must be categorized as vectorization",
                obligation.name
            );
            assert!(
                names.insert(obligation.name.as_str()),
                "duplicate vectorization proof '{}'",
                obligation.name
            );
        }
    }
