// trust-cg-verify/macho_proofs.rs - SMT proofs for Mach-O emission correctness
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// Proves that the Mach-O object file emission pipeline in
// trust-cg-codegen/src/macho/ produces structurally valid output.
//
...
//
// Reference: crates/trust-cg-codegen/src/macho/
...
//
// Encoding:
//   trust_ir_expr  = offset (the intended byte displacement)
...
pub fn proof_branch26_range() -> ProofObligation {
    ...
    // trust_ir: the intended byte offset.
    let trust_ir = offset.clone();
    ...
    let in_range = offset.clone().extract(27, 0).sign_ext(36).eq_expr(offset);
...
pub fn proof_branch26_range_8bit() -> ProofObligation {
    ...
    let in_range = offset.clone().extract(5, 0).sign_ext(2).eq_expr(offset);
...
pub fn proof_page21_alignment() -> ProofObligation {
    ...
    // trust_ir: expected result is 0 (low bits are zero).
    ...
    ProofObligation {
        ...
        trust_ir_expr: trust_ir,
        aarch64_expr: aarch64,
        inputs: vec![("target".to_string(), width), ("pc".to_string(), width)],
        preconditions: vec![],
        fp_inputs: vec![],
        category: Some(crate::lowering_proof::TransvalCheckKind::InstructionLowering),
    }
...
pub fn proof_pageoff12_masking() -> ProofObligation {
    ...
    let trust_ir = addr.clone().bvand(mask12.clone());
...
pub fn proof_pageoff12_masking_8bit() -> ProofObligation {
    ...
    let trust_ir = addr.clone().bvand(mask4.clone());
...
pub fn proof_adrp_add_pair() -> ProofObligation {
    ...
    // trust_ir: the intended target address.
    let trust_ir = target.clone();
...
pub fn proof_symbol_table_ordering() -> ProofObligation {
    ...
    // trust_ir: expected start of external partition = 0 + nlocal = nlocal.
    let trust_ir = SmtExpr::bv_const(0, width).bvadd(nlocal.clone());
...
pub fn proof_string_table_offset() -> ProofObligation {
    ...
    // trust_ir: expected result when preconditions hold: 1 (in bounds).
    let trust_ir = one.clone();
...
pub fn proof_section_index_validity() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: trust_ir,
        aarch64_expr: aarch64,
        inputs: vec![("n_sect".to_string(), width), ("nsects".to_string(), width)],
...
pub fn proof_load_command_size() -> ProofObligation {
    ...
    // trust_ir: sizeofcmds (the declared total).
    let trust_ir = sizeofcmds.clone();
...
pub fn proof_section_offset_monotonicity() -> ProofObligation {
    ...
    // trust_ir: expected = 1 (non-overlapping).
    let trust_ir = one.clone();

    // AArch64: check offset_a + size_a <= offset_b.
    let end_a = offset_a.clone().bvadd(size_a.clone());
    let non_overlap = SmtExpr::ite(
        end_a.clone().bvule(offset_b.clone()),
        one.clone(),
        zero.clone(),
    );
    ...
    let pre_no_overflow = size_a.bvule(SmtExpr::bv_const(u64::MAX, width).bvsub(offset_a));
...
pub fn proof_section_offset_monotonicity_8bit() -> ProofObligation {
    ...
    let pre_order = offset_b.clone().bvuge(offset_a.clone());
    let pre_no_overlap = end_a.bvule(offset_b);
    let pre_no_overflow = size_a.bvule(SmtExpr::bv_const(0xFF, width).bvsub(offset_a));
...
pub fn proof_magic_number() -> ProofObligation {
    ...
    // trust_ir: the expected magic number from Mach-O specification.
    let trust_ir = SmtExpr::bv_const(0xFEED_FACF, width);
...
pub fn proof_magic_number_8bit() -> ProofObligation {
    ...
    let trust_ir = SmtExpr::bv_const(0xCF, width);
...
/// Collect the Mach-O relocation-addressing proofs for ay smoke lanes.
///
/// Returns 10 proof obligations covering the current ARM64 linker relocation
/// arithmetic surface: BRANCH26, PAGE21, PAGEOFF12, ADRP+ADD reconstruction,
/// and signed field-fit detection, each at native and reduced width.
pub fn macho_relocation_proofs() -> Vec<ProofObligation> {
    vec![
...
///   Retracted degenerate const==const MachO tautologies (Pageoff12 masking, magic number — restated a constant). Genuine relocation/ADRP+ADD/range/structural MachO proofs remain.
const MACHO_RETRACTED_DEGENERATE: &[&str] = &[
    "MachO: Pageoff12 masking (64-bit)",
    "MachO: Pageoff12 masking (8-bit)",
    "MachO: magic number MH_MAGIC_64 (32-bit)",
    "MachO: magic number low byte (8-bit)",
];

pub fn all_macho_proofs() -> Vec<ProofObligation> {
    let mut proofs = macho_relocation_proofs();
    proofs.extend([
        // Symbol binding proofs
    ...
    // #62 retraction: drop degenerate X==X self-equalities (see MACHO_RETRACTED_DEGENERATE).
    proofs.retain(|p| !MACHO_RETRACTED_DEGENERATE.contains(&p.name.as_str()));
    proofs
}
...
mod tests {
    ...
    #[test]
    fn test_macho_relocation_proofs_count() {
        let proofs = macho_relocation_proofs();
        assert_eq!(
            proofs.len(),
            10,
            "expected 10 Mach-O relocation proofs, got {}",
            proofs.len()
        );
    }
    ...
    fn test_all_macho_proofs_count() {
        ...
        assert_eq!(
            proofs.len(),
            20,
            "expected 20 Mach-O proofs (4 degenerate Pageoff12-masking/magic-number \
             const tautologies retracted in #62), got {}",
            proofs.len()
        );
    }
