// trust-cg-verify/scheduler_proofs.rs - SMT proofs for instruction scheduling correctness
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// Proves that instruction scheduling (reordering within a basic block) in
// trust-cg-opt preserves program semantics. The scheduler builds a dependency
...
//     + proof-reorderable store-load routing
...
//
// Reference: crates/trust-cg-opt/src/scheduler.rs
...
//! | [`proof_sched_proof_reorderable_store_load_independence`] | Proven disjoint store-load route uses non-aliased model (64-bit) |
//! | [`proof_sched_proof_reorderable_store_load_independence_8bit`] | Same, exhaustive at 8-bit |
//!
...
//
//   trust_ir_expr = ite(order_a < order_b, val_a, POISON)
//
...
pub fn proof_sched_raw_data_dependency_8bit() -> ProofObligation {
    ...
    let a_before_b = order_a.clone().bvult(order_b.clone());
    let read_result = SmtExpr::ite(a_before_b, val_a.clone(), SmtExpr::bv_const(0, width));
...
//
//   trust_ir_expr = ite(term_pos > inst_pos, 1, 0)  -- check order
...
pub fn proof_sched_independent_reorder() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: order_ab,
        aarch64_expr: order_ba,
        inputs: vec![("x".to_string(), width), ("y".to_string(), width)],
        preconditions: vec![],
        fp_inputs: vec![],
        category: Some(crate::lowering_proof::TransvalCheckKind::InstructionLowering),
    }
...
//
//   trust_ir_expr = ite(pos_pred < pos_succ, 1, 0)   -- ordering check
...
// evaluation order.
//
//   trust_ir_expr = concat(select(mem, addr1), select(mem, addr2))
...
pub fn proof_sched_load_load_independence() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: order_ab,
        aarch64_expr: order_ba,
        inputs: vec![("addr1".to_string(), width), ("addr2".to_string(), width)],
        preconditions: vec![],
        fp_inputs: vec![],
        category: Some(crate::lowering_proof::TransvalCheckKind::InstructionLowering),
    }
...
//
//   trust_ir_expr = ite(order_a < order_b, original_val, new_val)
...
pub fn proof_sched_war_anti_dependency_8bit() -> ProofObligation {
    ...
    let a_before_b = order_a.clone().bvult(order_b.clone());
    let read_result = SmtExpr::ite(a_before_b, original_val.clone(), new_val);
...
//
//   trust_ir_expr = ite(pressure <= limit, 1, 0)   -- pressure check
...
//
//   trust_ir_expr = ite(pos_b >= pos_a + latency, 1, 0)  -- latency check
...
fn non_aliased_store_load_independence_obligation(width: u32, name: &str) -> ProofObligation {
...
pub fn proof_sched_non_aliased_load_independence() -> ProofObligation {
    non_aliased_store_load_independence_obligation(
        64,
        "Sched: non-aliased load independent of store",
    )
}
...
pub fn proof_sched_non_aliased_load_independence_8bit() -> ProofObligation {
    non_aliased_store_load_independence_obligation(
        8,
        "Sched: non-aliased load independent of store (8-bit)",
    )
}

/// Proof route: proof-reorderable disjoint StrRI/LdrRI store-load independence.
///
/// The scheduler only omits a StrRI -> LdrRI memory edge after reducing the
/// pair to statically disjoint byte ranges. Each loaded byte is therefore at a
/// non-equal address from each stored byte, which is exactly the existing
/// non-aliased store-load independence model above.
pub fn proof_sched_proof_reorderable_store_load_independence() -> ProofObligation {
    non_aliased_store_load_independence_obligation(
        64,
        "Sched: proof-reorderable disjoint store-load independence",
    )
}

/// Proof route: proof-reorderable disjoint store-load independence (8-bit).
pub fn proof_sched_proof_reorderable_store_load_independence_8bit() -> ProofObligation {
    non_aliased_store_load_independence_obligation(
        8,
        "Sched: proof-reorderable disjoint store-load independence (8-bit)",
    )
}
...
/// Return all instruction scheduling correctness proofs (30 total).
///
...
/// - Memory ordering: store-load RAW + non-aliased independence (6 proofs)
/// - Critical path optimality: makespan equals critical path (2 proofs)
///   Retracted degenerate X==X scheduler obligations (critical-path/reorder/load-load — restated a scheduling property as a value identity; no independent encoder). Genuine RAW/WAW/WAR/topo/pressure/latency/store-load proofs remain.
const SCHED_RETRACTED_DEGENERATE: &[&str] = &[
    "Sched: critical path optimality (8-bit)",
    "Sched: critical path optimality (makespan = critical path length)",
    "Sched: independent instructions reorder freely",
    "Sched: independent instructions reorder freely (8-bit)",
    "Sched: load-load independence (8-bit)",
    "Sched: load-load independence (reads can reorder)",
];
...
pub fn all_scheduler_proofs() -> Vec<ProofObligation> {
    vec![
        ...
        // Memory ordering (store-load + non-aliased independence + proof route)
        ...
        proof_sched_proof_reorderable_store_load_independence(),
        proof_sched_proof_reorderable_store_load_independence_8bit(),
        // Critical path optimality
        proof_sched_critical_path_optimality(),
        proof_sched_critical_path_optimality_8bit(),
    ]
    .into_iter()
    // #62 retraction: drop degenerate X==X self-equalities (see SCHED_RETRACTED_DEGENERATE).
    .filter(|p| !SCHED_RETRACTED_DEGENERATE.contains(&p.name.as_str()))
    .collect()
}
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // Memory ordering tests (store-load + non-aliased + proof route)
    // -----------------------------------------------------------------------
    ...
    #[test]
    fn test_sched_proof_reorderable_store_load_independence() {
        assert_valid(&proof_sched_proof_reorderable_store_load_independence());
    }

    #[test]
    fn test_sched_proof_reorderable_store_load_independence_8bit() {
        assert_valid(&proof_sched_proof_reorderable_store_load_independence_8bit());
    }
    ...
    fn test_all_scheduler_proofs() {
        ...
        assert_eq!(
            proofs.len(),
            24,
            "expected 24 scheduler proofs (6 degenerate retracted in #62)"
        );
    ...
    fn test_wrong_terminator_order_detected() {
        ...
        match result {
            VerificationResult::Invalid { .. } => {} // expected
            other => panic!(
                "Expected Invalid for wrong terminator order, got {:?}",
                other
            ),
        }
    }
    ...
    fn test_smt2_output_raw_dependency() {
        ...
        assert!(smt2.contains("(set-logic"), "should contain set-logic");
        assert!(
            smt2.contains("(declare-const val_a"),
            "should declare val_a"
        );
        assert!(smt2.contains("(check-sat)"), "should contain check-sat");
    }
    ...
    fn test_smt2_output_latency_ordering() {
        ...
        assert!(smt2.contains("(set-logic"), "should contain set-logic");
        assert!(
            smt2.contains("(declare-const pos_a"),
            "should declare pos_a"
        );
        assert!(
            smt2.contains("(declare-const latency"),
            "should declare latency"
        );
        assert!(
            smt2.contains("(declare-const pos_b"),
            "should declare pos_b"
        );
        assert!(smt2.contains("(check-sat)"), "should contain check-sat");
    }
    ...
    fn test_wrong_register_pressure_detected() {
        ...
        match result {
            VerificationResult::Invalid { .. } => {} // expected
            other => panic!(
                "Expected Invalid for wrong register pressure, got {:?}",
                other
            ),
        }
    }
    ...
    fn test_proof_count() {
        ...
        assert_eq!(
            proofs.len(),
            24,
            "expected 24 scheduler proofs (6 degenerate critical-path/reorder/load-load \
             X==X retracted in #62), got {}",
            proofs.len()
        );
    }
    ...
    fn test_proof_names_unique() {
        ...
        names.dedup();
        assert_eq!(names.len(), original_len, "proof names should be unique");
    }
}
