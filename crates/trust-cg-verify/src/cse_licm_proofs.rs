// trust-cg-verify/cse_licm_proofs.rs - SMT proofs for CSE and LICM optimization correctness
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// Proves that Common Subexpression Elimination (CSE) and Loop-Invariant Code
// Motion (LICM) transforms in trust-cg-opt preserve semantics. Each proof encodes
...
//
// Reference: crates/trust-cg-opt/src/cse.rs
// Reference: crates/trust-cg-opt/src/licm.rs
    ...
    for &size in sizes {
        ...
        for &addr in edge_addrs {
            ...
            for &value in edge_values {
                ...
                trust_ir_store(&mut mem, aligned, masked_value, size);
                let loaded = trust_ir_load(&mem, aligned, size);
        ...
        for _ in 0..10_000 {
            ...
            let loaded = trust_ir_load(&mem, addr, size);
            if loaded != value {
                return VerificationResult::Invalid {
                    counterexample: format!(
                        "store_load_ordering: addr=0x{:x}, size={}, stored=0x{:x}, loaded=0x{:x}",
...
pub fn verify_licm_negative_load_over_store() -> VerificationResult {
    ...
    let before = trust_ir_load(&mem, addr, 4);
    trust_ir_store(&mut mem, addr, 0xDEAD_BEEF, 4);
    let after = trust_ir_load(&mem, addr, 4);
    ...
    for &(test_addr, test_val, seed) in test_cases {
        ...
        let b = trust_ir_load(&m, aligned, 4);
        trust_ir_store(&mut m, aligned, test_val & 0xFFFF_FFFF, 4);
        let a = trust_ir_load(&m, aligned, 4);
...
pub fn all_pure_determinism_proofs() -> Vec<ProofObligation> {
    vec![proof_pure_determinism_add(), proof_pure_determinism_mul()]
}
...
pub fn all_cse_licm_proofs() -> Vec<ProofObligation> {
    ...
    // #62 retraction: the CSE "op(a,b) == op(a,b)", LICM "op outside == inside",
    // and PureDeterminism "op is side-effect free" obligations were all degenerate
    // X==X self-equalities (no independent encoder; restated the analysis as the
    // identity) and have been RETRACTED. Only the GENUINE commutativity proofs
    // (op(a,b) == op(b,a), where bvadd(a,b) vs bvadd(b,a) is a real swap that a
    // non-commutative wiring would refute) remain.
...
mod tests {
    ...
    fn test_licm_negative_non_invariant() {
        ...
        let obligation = ProofObligation {
            ...
            trust_ir_expr: encode_add(a.clone(), loop_var.clone()),
        ...
        match result {
            VerificationResult::Invalid { .. } => {} // expected
            other => panic!(
                "Expected Invalid for non-invariant operand, got {:?}",
                other
            ),
        }
    }
    ...
    fn test_licm_negative_non_invariant_mul() {
        ...
        let obligation = ProofObligation {
            ...
            trust_ir_expr: encode_mul(a.clone(), loop_var.clone()),
    ...
    fn test_licm_negative_load_not_hoistable_over_store() {
        ...
        trust_ir_store(&mut mem, addr, 0xCAFE_BABE, 4);
