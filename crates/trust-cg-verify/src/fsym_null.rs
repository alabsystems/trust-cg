...
use crate::smt::{EvalResult, SmtExpr, SmtSort};
...
pub struct PathContext {
    /// Path guards; their conjunction is the path condition. Guards may
    /// evaluate to `Bool(true)` or trust_ir-style i1 bitvector truth (`bv1(1)`).
...
/// Branch edge to materialize when extending a path condition.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum BranchArm {
    /// Branch-taken edge where the branch condition is true.
    Then,
    /// Fallthrough/else edge where the branch condition is false.
    Else,
}

/// Pair of path contexts produced by a conditional branch.
#[derive(Debug, Clone)]
pub struct BranchFork {
    /// Path context for the true/taken edge.
    pub then_ctx: PathContext,
    /// Path context for the false/fallthrough edge.
    pub else_ctx: PathContext,
}

impl PathContext {
    /// Return a copy of this context with one additional guard.
    pub fn with_guard(&self, guard: SmtExpr) -> Self {
        let mut next = self.clone();
        next.guards.push(guard);
        next
    }

    /// Return a copy of this context extended with the guard for one branch arm.
    ///
    /// `fsym` accepts Bool branch conditions and trust_ir-style i1 bitvectors. Other
    /// sorts become an unsatisfiable guard so malformed branch conditions do not
    /// produce UB witnesses in the evaluator.
    pub fn with_branch_guard(&self, condition: SmtExpr, arm: BranchArm) -> Self {
        self.with_guard(branch_arm_guard(condition, arm))
    }

    /// Fork this path context into true and false branch arms.
    pub fn fork_branch(&self, condition: SmtExpr) -> BranchFork {
        BranchFork {
            then_ctx: self.with_branch_guard(condition.clone(), BranchArm::Then),
            else_ctx: self.with_branch_guard(condition, BranchArm::Else),
        }
    }
}
...
fn branch_arm_guard(condition: SmtExpr, arm: BranchArm) -> SmtExpr {
    match condition.sort() {
        SmtSort::Bool => match arm {
            BranchArm::Then => condition,
            BranchArm::Else => condition.not_expr(),
        },
        SmtSort::BitVec(1) => match arm {
            BranchArm::Then => condition,
            BranchArm::Else => condition.eq_expr(SmtExpr::bv_const(0, 1)),
        },
        SmtSort::BitVec(_) | SmtSort::Array(_, _) | SmtSort::FloatingPoint(_, _) => {
            SmtExpr::bool_const(false)
        }
    }
}

fn guard_holds(guard: &SmtExpr, env: &HashMap<String, u64>) -> bool {
    match guard.try_eval(env) {
        Ok(EvalResult::Bool(true)) => true,
        Ok(EvalResult::Bv(1) | EvalResult::Bv128(1)) => {
            matches!(guard.try_bv_width(), Ok(1))
        }
        _ => false,
    }
}

pub(crate) fn guards_hold(guards: &[SmtExpr], env: &HashMap<String, u64>) -> bool {
    guards.iter().all(|guard| guard_holds(guard, env))
}
...
/// 2. If `op.ptr` is a concrete non-null `BvConst`, return `Safe`.
/// 3. Otherwise, try each candidate environment. If no candidates are
///    supplied, try a single empty environment:
///    - Evaluate each path guard; if any guard is not `Bool(true)` or i1
///      bitvector truth, skip this candidate.
///    - If all guards hold and `op.ptr` evaluates to `Bv(0)` or `Bv128(0)`,
///      return `Ub` with that witness.
...
#[cfg(test)]
mod tests {
    use super::{FsymVerdict, MemOp, PathContext, check_null_deref};
    ...
    fn not_null_annotation_short_circuits() {
        ...
        assert_eq!(
            check_null_deref(&mem_op, &ctx(vec![], vec![])),
            FsymVerdict::Safe
        );
    }
    ...
    #[test]
    fn concrete_null_under_false_guard_is_not_ub() {
        assert_eq!(
            check_null_deref(
                &op(SmtExpr::bv_const(0, 64)),
    ...
    #[test]
    fn i1_bitvector_guard_allows_concrete_null() {
    ...
    #[test]
    fn wide_bitvector_guard_one_is_not_i1_truth() {
    ...
    #[test]
    fn fork_branch_i1_then_path_allows_null_witness() {
        let witness = HashMap::from([(String::from("p"), 0_u64), (String::from("g"), 1_u64)]);
        let fork = ctx(vec![], vec![witness.clone()]).fork_branch(SmtExpr::var("g", 1));

        assert_eq!(
            check_null_deref(&op(SmtExpr::var("p", 64)), &fork.then_ctx),
            FsymVerdict::Ub { witness }
        );
        assert_eq!(
            check_null_deref(&op(SmtExpr::var("p", 64)), &fork.else_ctx),
            unknown()
        );
    }

    #[test]
    fn fork_branch_i1_else_path_allows_null_witness() {
        let witness = HashMap::from([(String::from("p"), 0_u64), (String::from("g"), 0_u64)]);
        let fork = ctx(vec![], vec![witness.clone()]).fork_branch(SmtExpr::var("g", 1));
    ...
    #[test]
    fn fork_branch_bool_else_path_uses_negated_guard() {
        let condition = SmtExpr::var("g", 1).eq_expr(SmtExpr::bv_const(1, 1));
        let witness = HashMap::from([(String::from("p"), 0_u64), (String::from("g"), 0_u64)]);
        let fork = ctx(vec![], vec![witness.clone()]).fork_branch(condition);
    ...
    #[test]
    fn fork_branch_rejects_wide_bitvector_condition() {
        let witness = HashMap::from([(String::from("p"), 0_u64), (String::from("g"), 1_u64)]);
        let fork = ctx(vec![], vec![witness]).fork_branch(SmtExpr::var("g", 8));
