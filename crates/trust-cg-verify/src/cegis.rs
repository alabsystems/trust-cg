// trust-cg-verify/cegis.rs - Counter-Example Guided Inductive Synthesis (CEGIS)
//
...
//! use trust_cg_verify::cegis::{CegisLoop, CegisResult};
//!
...
impl CegisLoop {
    ...
    pub fn new(max_iterations: usize, timeout_ms: u64) -> Self {
        Self {
            ...
            solver_config: AYConfig::default().with_timeout(timeout_ms),
    ...
    pub fn with_solver_config(mut self, config: AYConfig) -> Self {
        ...
        if vars.len() == 1 {
            ...
            for edge_a in &edge_values {
                for edge_b in &edge_values {
                    let mut input = ConcreteInput::new();
                    input.insert(name_a.clone(), crate::smt::mask(edge_a(*width_a), *width_a));
                    input.insert(name_b.clone(), crate::smt::mask(edge_b(*width_b), *width_b));
    ...
    pub fn verify(
        ...
        for iteration in 0..self.max_iterations {
            ...
            let obligation = ProofObligation {
                ...
                trust_ir_expr: source_smt.clone(),
            ...
            let result = verify_with_ay(&obligation, &self.solver_config);
            ...
            match result {
                ...
                AYResult::SolverUnsat => {
                    return CegisResult::Error(
                        "solver returned UNSAT without an independently accepted proof".to_string(),
                    );
                }
                AYResult::CounterExample(assignments) => {
                ...
                AYResult::Unknown(msg) => {
                    return CegisResult::Error(format!("solver returned unknown: {}", msg));
                }
                AYResult::Error(msg) => {
    ...
    fn check_concrete(&self, source_smt: &SmtExpr, target_smt: &SmtExpr) -> Option<ConcreteInput> {
...
mod tests {
    ...
    fn test_cegis_verify_identical_expressions() {
        // Use the ay bridge (CLI fallback) to verify x + 0 == x
        let mut cegis = CegisLoop::new(10, 5000);
        let vars = vec![("x".to_string(), 32)];
    ...
    fn test_cegis_with_existing_proof_obligation() {
        ...
        match &result {
            ...
            other => {
                panic!(
                    "Expected Equivalent or Error for iadd proof, got {:?}",
                    other
                );
            }
        }
