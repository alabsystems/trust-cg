// trust-cg-verify/gvn_proofs.rs - SMT proofs for Global Value Numbering correctness
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// Proves that Global Value Numbering (GVN) in trust-cg-opt/gvn.rs preserves
...
//
// Reference: crates/trust-cg-opt/src/gvn.rs
...
use crate::memory_model::{SmtMemory, trust_ir_load, trust_ir_store};
...
pub fn proof_gvn_reflexivity_add() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_add(a.clone(), b.clone()),
...
pub fn proof_gvn_reflexivity_mul() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_mul(a.clone(), b.clone()),
...
pub fn proof_gvn_reflexivity_sub() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_sub(a.clone(), b.clone()),
...
pub fn proof_gvn_reflexivity_and() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_and(a.clone(), b.clone()),
...
pub fn proof_gvn_reflexivity_or() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_or(a.clone(), b.clone()),
...
pub fn proof_gvn_reflexivity_xor() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_xor(a.clone(), b.clone()),
...
pub fn verify_gvn_load_after_load() -> VerificationResult {
    ...
    for &size in sizes {
        ...
        for &addr in addrs {
            ...
            for seed in [0x00u8, 0xAA, 0xFF] {
                ...
                let load1 = trust_ir_load(&mem, aligned, size);
                let load2 = trust_ir_load(&mem, aligned, size);
                if load1 != load2 {
            ...
            for &stored_val in &[0u64, 1, 0xDEAD_BEEF, 0xCAFE_BABE_1234_5678] {
                ...
                trust_ir_store(&mut mem, aligned, masked, size);
                let load1 = trust_ir_load(&mem, aligned, size);
                let load2 = trust_ir_load(&mem, aligned, size);
                if load1 != load2 {
                    return VerificationResult::Invalid {
    ...
    for _ in 0..10_000 {
        ...
        trust_ir_store(&mut mem, addr, value, size);
        let l1 = trust_ir_load(&mem, addr, size);
        let l2 = trust_ir_load(&mem, addr, size);
        if l1 != l2 {
            return VerificationResult::Invalid {
                counterexample: format!(
...
pub fn verify_gvn_store_kills_load_vn() -> VerificationResult {
    ...
    for &(addr, initial, stored, seed) in test_cases {
        ...
        let mut mem = SmtMemory::new(seed);
        // Store initial value
        trust_ir_store(&mut mem, aligned, initial & 0xFFFF_FFFF, 4);
        let load_before = trust_ir_load(&mem, aligned, 4);
        // Store new value — this should invalidate load VN
        trust_ir_store(&mut mem, aligned, stored & 0xFFFF_FFFF, 4);
        let load_after = trust_ir_load(&mem, aligned, 4);
...
pub fn verify_gvn_call_kills_all_loads() -> VerificationResult {
    ...
    for &addr in addrs {
        let mut mem = SmtMemory::new(0x00);
        let load_before = trust_ir_load(&mem, addr, 4);
        // Model a call's side effect as storing an arbitrary value
        trust_ir_store(&mut mem, addr, 0xCA11_BEEF, 4);
        let load_after = trust_ir_load(&mem, addr, 4);
...
pub fn verify_gvn_non_aliasing_loads() -> VerificationResult {
    ...
    for &(addr_a, addr_b) in test_cases {
        ...
        trust_ir_store(&mut mem, addr_a, 0xAAAA_AAAA, 4);
        trust_ir_store(&mut mem, addr_b, 0xBBBB_BBBB, 4);

        let load_b_before = trust_ir_load(&mem, addr_b, 4);

        // Store different value at addr_a
        trust_ir_store(&mut mem, addr_a, 0xCCCC_CCCC, 4);

        let load_b_after = trust_ir_load(&mem, addr_b, 4);
    ...
    for _ in 0..10_000 {
        ...
        let mut mem = SmtMemory::new(0x00);
        trust_ir_store(&mut mem, addr_b, val_b & 0xFFFF_FFFF, 4);
        let load_b = trust_ir_load(&mem, addr_b, 4);
        trust_ir_store(&mut mem, addr_a, val_a & 0xFFFF_FFFF, 4);
        let load_b2 = trust_ir_load(&mem, addr_b, 4);
...
pub fn verify_gvn_side_effect_preservation() -> VerificationResult {
    ...
    for &(addr, value, seed) in test_cases {
        ...
        trust_ir_store(&mut mem, aligned, masked, 4);
        let loaded = trust_ir_load(&mem, aligned, 4);
...
pub fn all_gvn_idempotence_proofs() -> Vec<ProofObligation> {
    vec![proof_gvn_idempotence_add(), proof_gvn_idempotence_mul()]
}
...
pub fn all_gvn_proofs() -> Vec<ProofObligation> {
    ...
    // #62 retraction: GVN reflexivity (op(a,b)==op(a,b)), consistency, dominance,
    // value-preservation and idempotence obligations were all degenerate X==X
    // self-equalities (they restated the value-numbering/dominance side-analysis
    // as the identity, with no independent encoder) and have been RETRACTED. Only
    // the GENUINE commutativity proofs (op(a,b) == op(b,a)) remain.
