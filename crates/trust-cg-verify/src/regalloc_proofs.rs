// trust-cg-verify/regalloc_proofs.rs - SMT proofs for Register Allocation correctness
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// Proves that register allocation in trust-cg-regalloc preserves program
...
//
// Reference: crates/trust-cg-regalloc/src/
...
//
// trust_ir_expr: the constraint check -- ite(overlap == 1 AND reg_a == reg_b, 0, 1)
...
///   trust_ir_expr = ite(reg_a == reg_b, 0, 1)  (0 if collision, 1 if safe)
...
/// guarantees), the trust_ir_expr evaluates to 1 == aarch64_expr.
pub fn proof_regalloc_non_interference() -> ProofObligation {
    ...
    ProofObligation {
        ...
        inputs: vec![("reg_a".to_string(), width), ("reg_b".to_string(), width)],
...
//   trust_ir_expr = bvor(has_preg, has_spill)   (at least one must be 1)
...
pub fn proof_regalloc_parallel_copy_two() -> ProofObligation {
    ...
    ProofObligation {
        ...
        aarch64_expr: after,
        inputs: vec![("src1".to_string(), width), ("src2".to_string(), width)],
        preconditions: vec![],
        fp_inputs: vec![],
        category: Some(crate::lowering_proof::TransvalCheckKind::RegisterAllocation),
    }
...
//   trust_ir_expr = ite(pred_sel == 0, v1, v2)  (phi semantics)
...
pub fn proof_regalloc_caller_saved_spill_restore() -> ProofObligation {
    ...
    ProofObligation {
        ...
        inputs: vec![("val".to_string(), width), ("slot".to_string(), width)],
        preconditions: vec![],
        fp_inputs: vec![],
        category: Some(crate::lowering_proof::TransvalCheckKind::RegisterAllocation),
    }
...
pub fn proof_regalloc_phi_elimination_3pred() -> ProofObligation {
    ...
    let copy_result = SmtExpr::ite(is_pred0, v1, SmtExpr::ite(is_pred1, v2, v3));
...
pub fn proof_regalloc_copy_coalescing_soundness() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: src_val,
        aarch64_expr: dst_val,
        inputs: vec![("src_val".to_string(), width), ("preg".to_string(), width)],
        preconditions: vec![],
        fp_inputs: vec![],
        category: Some(crate::lowering_proof::TransvalCheckKind::RegisterAllocation),
    }
...
/// trust_ir_expr = intended result = concat(c, a, b)  (new_a=c, new_b=a, new_c=b)
...
pub fn proof_regalloc_rematerialization_add_imm() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: original,
        aarch64_expr: rematerialized,
        inputs: vec![("base".to_string(), width), ("imm".to_string(), width)],
        preconditions: vec![],
        fp_inputs: vec![],
        category: Some(crate::lowering_proof::TransvalCheckKind::RegisterAllocation),
    }
...
pub fn proof_regalloc_regfile_write_read_identity() -> ProofObligation {
    ...
    ProofObligation {
        ...
        inputs: vec![("val".to_string(), width), ("reg".to_string(), width)],
        preconditions: vec![],
        fp_inputs: vec![],
        category: Some(crate::lowering_proof::TransvalCheckKind::RegisterAllocation),
    }
...
// allocator (crates/trust-cg-regalloc/src/greedy.rs) must maintain. They
...
/// (crates/trust-cg-regalloc/src/spill.rs) must ensure that two concurrently-
...
///   Retracted degenerate X==X RegAlloc obligations (phi-elim/parallel-copy/swap/remat/class-constraint/callee-saved/live-through — restated the allocator invariant as a value identity). Genuine non-interference/completeness/spill-roundtrip/non-aliasing/coalesce proofs remain.
const REGALLOC_RETRACTED_DEGENERATE: &[&str] = &[
    "Greedy RA: register class constraint (PReg in required class)",
    "RegAlloc Phase2: parallel copy 3-way cycle resolution",
    "RegAlloc Phase2: phi elimination 3-predecessor",
    "RegAlloc Phase2: phi elimination 3-predecessor (8-bit)",
    "RegAlloc Phase2: rematerialization add-immediate soundness",
    "RegAlloc Phase2: rematerialization constant soundness",
    "RegAlloc: callee-saved register preserved across call",
    "RegAlloc: live-through in callee-saved reg preserved",
    "RegAlloc: parallel copy (independent) preserves values",
    "RegAlloc: parallel copy swap via temp preserves values",
    "RegAlloc: phi elimination preserves predecessor value",
    "RegAlloc: phi elimination preserves predecessor value (8-bit)",
];
...
pub fn all_regalloc_proofs() -> Vec<ProofObligation> {
    ...
    // #62 retraction: drop degenerate X==X self-equalities (see REGALLOC_RETRACTED_DEGENERATE).
    proofs.retain(|p| !REGALLOC_RETRACTED_DEGENERATE.contains(&p.name.as_str()));
    proofs
}
...
mod tests {
    ...
    fn test_all_regalloc_phase2_proofs() {
        ...
        assert_eq!(
            proofs.len(),
            15,
            "expected 15 Phase 2 proofs, got {}",
            proofs.len()
        );
    ...
    fn test_all_greedy_regalloc_proofs() {
        ...
        assert_eq!(
            proofs.len(),
            12,
            "expected 12 Phase 3 (greedy RA) proofs, got {}",
            proofs.len()
        );
        for obligation in &proofs {
            let result = verify_by_evaluation(obligation);
    ...
    fn test_all_regalloc_proofs_combined() {
        ...
        assert_eq!(
            proofs.len(),
            31,
            "expected 31 genuine regalloc proofs (12 degenerate phi-elim/parallel-copy/\
             remat/class-constraint/callee-saved/live-through X==X retracted in #62), got {}",
            proofs.len()
        );
    }
}
