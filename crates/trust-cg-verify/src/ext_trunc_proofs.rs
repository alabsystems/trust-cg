// trust-cg-verify/ext_trunc_proofs.rs - SMT proofs for extension and truncation
//
...
// the trust_ir semantics and the AArch64 instruction semantics as SMT bitvector
// expressions and prove equivalence for all inputs.
//
// Reference: ARM ARM C6.2 (SBFM, UBFM aliases)
//            crates/trust-cg-lower/src/isel.rs: select_extend, select_trunc
//            crates/trust-cg-ir/src/inst.rs: Sxtb/Sxth/Sxtw/Uxtb/Uxth/Uxtw
...
///
/// trust_ir semantics: sign_extend(extract(x, from_bits-1, 0), to_bits - from_bits)
...
///
/// trust_ir semantics: zero_extend(extract(x, from_bits-1, 0), to_bits - from_bits)
...
///
/// trust_ir semantics: extract(x, to_bits-1, 0)
...
///   sign_extend(x[7:0], 56) at trust_ir level
///   == SXTB Xd, Wn (alias SBFM Xd, Xn, #0, #7)
pub fn proof_sxtb_8_to_64() -> ProofObligation {
    let width = 64;
    let x = SmtExpr::var("x", width);

    let trust_ir = sext_semantics(&x, 8, 64);
...
///
/// Theorem: forall x : BV32 .
///   sign_extend(x[15:0], 16) at trust_ir level
///   == SXTH Wd, Wn (alias SBFM Wd, Wn, #0, #15)
pub fn proof_sxth_16_to_32() -> ProofObligation {
    let width = 32;
    let x = SmtExpr::var("x", width);

    let trust_ir = sext_semantics(&x, 16, 32);
...
///
/// Theorem: forall x : BV64 .
///   sign_extend(x[15:0], 48) at trust_ir level
///   == SXTH Xd, Xn (alias SBFM Xd, Xn, #0, #15)
pub fn proof_sxth_16_to_64() -> ProofObligation {
    let width = 64;
    let x = SmtExpr::var("x", width);

    let trust_ir = sext_semantics(&x, 16, 64);
...
///
/// Theorem: forall x : BV64 .
///   sign_extend(x[31:0], 32) at trust_ir level
///   == SXTW Xd, Wn (alias SBFM Xd, Xn, #0, #31)
pub fn proof_sxtw_32_to_64() -> ProofObligation {
    let width = 64;
    let x = SmtExpr::var("x", width);

    let trust_ir = sext_semantics(&x, 32, 64);
...
///
/// This proves both:
/// 1. The trust_ir Uextend semantics match the UXTB instruction
/// 2. The UBFM encoding (UXTB) is equivalent to AND masking
pub fn proof_uxtb_8_to_32() -> ProofObligation {
    let width = 32;
    let x = SmtExpr::var("x", width);

    // trust_ir: zero_extend(x[7:0], 24)
    let trust_ir = zext_semantics(&x, 8, 32);
...
pub fn proof_uxtb_8_to_64() -> ProofObligation {
    ...
    let trust_ir = zext_semantics(&x, 8, 64);
...
pub fn proof_uxth_16_to_32() -> ProofObligation {
    ...
    let trust_ir = zext_semantics(&x, 16, 32);
...
pub fn proof_uxth_16_to_64() -> ProofObligation {
    ...
    let trust_ir = zext_semantics(&x, 16, 64);
...
pub fn proof_uxtw_32_to_64() -> ProofObligation {
    ...
    let trust_ir = zext_semantics(&x, 32, 64);
...
pub fn proof_trunc_to_i8() -> ProofObligation {
    ...
    // trust_ir: truncate to 8 bits = extract low 8 bits
    let trust_ir = trunc_semantics(&x, 8);
...
pub fn proof_trunc_to_i16() -> ProofObligation {
    ...
    let trust_ir = trunc_semantics(&x, 16);
...
/// Trunc i64->i32 (see `select_trunc` default arm). This proves the trust_ir
...
pub fn proof_trunc_to_i32_from_64() -> ProofObligation {
    ...
    let trust_ir = trunc_semantics(&x, 32);
...
mod tests {
    ...
    fn verify_roundtrip_zext_trunc_16() {
        ...
        assert!(
            matches!(result, VerificationResult::Valid),
            "Roundtrip zext/trunc 16 failed: {:?}",
            result
        );
    }
    ...
    fn verify_roundtrip_sext_trunc_16() {
        ...
        assert!(
            matches!(result, VerificationResult::Valid),
            "Roundtrip sext/trunc 16 failed: {:?}",
            result
        );
    }
    ...
    fn all_ext_trunc_proofs_count() {
        ...
        assert_eq!(
            proofs.len(),
            23,
            "Expected 23 ext/trunc proofs, got {}",
            proofs.len()
        );
    }
