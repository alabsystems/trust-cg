// trust-cg-verify/opt_proofs.rs - SMT proofs for optimization pass transforms
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// Proves that optimization transforms in trust-cg-opt preserve semantics.
...
// Reference: crates/trust-cg-opt/src/dce.rs
// Reference: crates/trust-cg-opt/src/copy_prop.rs

//! SMT proofs for optimization pass transforms.
//!
//! Each proof corresponds to a transform in the trust-cg-opt optimization passes:
//!
//! | Rule | LHS | RHS | Proof |
//! |------|-----|-----|-------|
...
pub fn proof_const_fold_add() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_add_rr(k1.clone(), k2.clone()),
...
pub fn proof_const_fold_sub() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_sub_rr(k1.clone(), k2.clone()),
...
pub fn proof_and_absorb() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_and_ri(x, 0, width),
...
pub fn proof_or_absorb() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_orr_ri(x, all_ones, width),
...
/// the `has_side_effects` predicate in `trust-cg-opt::dce`), which ensures
...
/// analysis in `trust-cg-opt::copy_prop`.
pub fn proof_copy_prop_identity() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_identity(x.clone()),
...
pub fn all_opt_proofs() -> Vec<ProofObligation> {
    // #62 retraction: the degenerate X==X self-equalities (ConstFold ADD/SUB
    // const tautology, DCE "y==y", CopyProp "x==x") were RETRACTED — they proved
    // nothing about a lowering. Only the genuine algebraic-absorption identities
    // (AND(x,0)==0, OR(x,-1)==-1; trust_ir bvand/bvor vs the folded MOVI) remain.
    vec![proof_and_absorb(), proof_or_absorb()]
}

/// Return all optimization proofs including 32-bit variants.
...
mod tests {
    ...
    fn test_wrong_and_nonzero_detected() {
        ...
        let obligation = ProofObligation {
            ...
            trust_ir_expr: encode_and_ri(x, 1, width),
            aarch64_expr: encode_movi(0, width),
            inputs: vec![("x".to_string(), width)],
            preconditions: vec![],
            fp_inputs: vec![],
    ...
    fn test_wrong_or_zero_detected() {
        ...
        let obligation = ProofObligation {
            ...
            trust_ir_expr: encode_orr_ri(x, 0, width),
