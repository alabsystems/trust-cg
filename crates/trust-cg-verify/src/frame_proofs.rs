// trust-cg-verify/frame_proofs.rs - SMT proofs for Frame Index Elimination correctness
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// Proves that frame index elimination in trust-cg-codegen/src/frame.rs preserves
...
//
// Reference: crates/trust-cg-codegen/src/frame.rs
...
//! | [`proof_frame_large_negative_offset_materialization`] | SUB X16, FP, #abs_offset produces correct address (64-bit) |
//! | [`proof_frame_large_negative_offset_materialization_8bit`] | Same, exhaustive at 8-bit |
//!
...
//!
//! ## STP Spill Scalarization Proofs
//!
//! | Proof | Property |
//! |-------|----------|
//! | [`proof_frame_stp_spill_scalarization`] | Scalarized STP spill stores preserve pair addresses (64-bit) |
//! | [`proof_frame_stp_spill_scalarization_8bit`] | Same, exhaustive at 8-bit |
...
//
// Encoding:
//   trust_ir_expr = base + offset  (intended effective address)
...
pub fn proof_frame_offset_computation() -> ProofObligation {
    ...
    // trust_ir semantics: the intended effective address.
    let trust_ir = base.clone().bvadd(slot_offset.clone());
    ...
    ProofObligation {
        ...
        preconditions: vec![],
        fp_inputs: vec![],
        category: Some(crate::lowering_proof::TransvalCheckKind::RegisterAllocation),
    }
}
...
pub fn proof_frame_large_offset_materialization() -> ProofObligation {
    ...
    ProofObligation {
        ...
        inputs: vec![("base".to_string(), width), ("offset".to_string(), width)],
        preconditions: vec![],
        fp_inputs: vec![],
        category: Some(crate::lowering_proof::TransvalCheckKind::RegisterAllocation),
    }
...
/// Proof: Large negative offset materialization -- SUB X16, base, abs_offset.
///
/// Theorem: forall base, abs_offset : BV64 .
///   base + (-abs_offset) == base - abs_offset
///
/// The large-offset frame path materializes the absolute value of a negative
/// slot offset, then emits SUB scratch, base, scratch. The final memory operand
/// uses [scratch, #0], so this proves the materialized address is the intended
/// FP/SP-relative slot address.
pub fn proof_frame_large_negative_offset_materialization() -> ProofObligation {
    let width = 64;
    let base = SmtExpr::var("base", width);
    let abs_offset = SmtExpr::var("abs_offset", width);

    let intended = base.clone().bvadd(abs_offset.clone().bvneg());
    let materialized = base.bvsub(abs_offset);

    ProofObligation {
        machine_side_provenance: crate::lowering_proof::MachineSideProvenance::StaticDb,
        name: "FrameLayout: large negative offset materialization (SUB base, abs_offset)"
            .to_string(),
        trust_ir_expr: intended,
        aarch64_expr: materialized,
        inputs: vec![
...
/// Proof: Large negative offset materialization (8-bit, exhaustive).
pub fn proof_frame_large_negative_offset_materialization_8bit() -> ProofObligation {
    let width = 8;
    let base = SmtExpr::var("base", width);
    let abs_offset = SmtExpr::var("abs_offset", width);
    ...
    ProofObligation {
        machine_side_provenance: crate::lowering_proof::MachineSideProvenance::StaticDb,
        name: "FrameLayout: large negative offset materialization (8-bit)".to_string(),
        trust_ir_expr: intended,
        aarch64_expr: materialized,
        inputs: vec![
            ("base".to_string(), width),
...
//
// Encoding:
//   trust_ir_expr = 0 (aligned, the expected result)
...
pub fn proof_frame_outgoing_arg_offset() -> ProofObligation {
    ...
    let trust_ir = sp_val.clone().bvadd(arg_offset.clone());
...
pub fn proof_frame_red_zone_offset() -> ProofObligation {
    ...
    let trust_ir = sp_val.clone().bvsub(abs_offset.clone());
...
pub fn proof_frame_emergency_spill_slot() -> ProofObligation {
    ...
    let trust_ir = base.clone().bvadd(offset.clone());
...
// ===========================================================================
// 14. STP Spill Scalarization Proofs
// ===========================================================================

fn proof_frame_stp_spill_scalarization_width(width: u32, suffix: &str) -> ProofObligation {
    let value0 = SmtExpr::var("value0", width);
    let value1 = SmtExpr::var("value1", width);
    let base = SmtExpr::var("base", width);
    let pair_offset = SmtExpr::var("pair_offset", width);

    let pair_addr0 = base.clone().bvadd(pair_offset.clone());
    let pair_addr1 = pair_addr0.clone().bvadd(SmtExpr::bv_const(8, width));

    let scalar_base = base.bvadd(pair_offset);
    let scalar_addr0 = scalar_base.clone();
    let scalar_addr1 = scalar_base.bvadd(SmtExpr::bv_const(8, width));

    let mem = SmtExpr::const_array(SmtSort::BitVec(width), SmtExpr::bv_const(0, width));
    let trust_ir_mem = SmtExpr::store(mem.clone(), pair_addr0.clone(), value0.clone());
    let trust_ir_mem = SmtExpr::store(trust_ir_mem, pair_addr1.clone(), value1.clone());

    let scalar_mem = SmtExpr::store(mem, scalar_addr0, value0);
    let scalar_mem = SmtExpr::store(scalar_mem, scalar_addr1, value1);

    let trust_ir = SmtExpr::select(trust_ir_mem.clone(), pair_addr1.clone())
        .concat(SmtExpr::select(trust_ir_mem, pair_addr0.clone()));
    let aarch64 = SmtExpr::select(scalar_mem.clone(), pair_addr1)
        .concat(SmtExpr::select(scalar_mem, pair_addr0));

    ProofObligation {
        machine_side_provenance: crate::lowering_proof::MachineSideProvenance::StaticDb,
        name: format!("FrameLayout: stp spill scalarization preserves two stores{suffix}"),
        trust_ir_expr: trust_ir,
        aarch64_expr: aarch64,
        inputs: vec![
            ("value0".to_string(), width),
            ("value1".to_string(), width),
            ("base".to_string(), width),
            ("pair_offset".to_string(), width),
        ],
        preconditions: vec![],
        fp_inputs: vec![],
        category: Some(crate::lowering_proof::TransvalCheckKind::RegisterAllocation),
...
/// Proof: scalarizing `STP v0, v1, [base, #off]` into two stores writes the
/// same values at `base + off` and `base + off + 8`.
pub fn proof_frame_stp_spill_scalarization() -> ProofObligation {
    proof_frame_stp_spill_scalarization_width(64, "")
}

/// Proof: STP spill scalarization (8-bit, exhaustive).
pub fn proof_frame_stp_spill_scalarization_8bit() -> ProofObligation {
    proof_frame_stp_spill_scalarization_width(8, " (8-bit)")
}
...
/// Return all frame index elimination correctness proofs (30 total).
///
/// Covers fourteen categories of frame layout invariants:
...
/// - Large offset materialization: ADD/SUB base, offset is correct (4 proofs)
...
/// - STP spill scalarization: two scalar STRs preserve pair-store addresses (2 proofs)
pub fn all_frame_proofs() -> Vec<ProofObligation> {
    vec![
        ...
        proof_frame_large_negative_offset_materialization(),
        proof_frame_large_negative_offset_materialization_8bit(),
        // Stack alignment
        ...
        // STP spill scalarization
        proof_frame_stp_spill_scalarization(),
        proof_frame_stp_spill_scalarization_8bit(),
    ]
    .into_iter()
    // #62 retraction: degenerate X==X FrameLayout obligations (offset/red-zone/
    // outgoing-arg/large-positive-offset/emergency-spill/stp-scalarization) — the
    // computed offset is restated identically on both sides with no independent
    // emitter. The GENUINE proofs (12-bit range check, large-NEGATIVE SUB,
    // SP-alignment mod 16/32, callee-save non-overlap, distinct-slot non-aliasing,
    // callee-save restore identity, FP/SP addressing, triple callee-save) remain.
    .filter(|p| !FRAME_RETRACTED_DEGENERATE.contains(&p.name.as_str()))
    .collect()
}

const FRAME_RETRACTED_DEGENERATE: &[&str] = &[
    "FrameLayout: emergency spill slot address via X16",
    "FrameLayout: emergency spill slot address via X16 (8-bit)",
    "FrameLayout: large offset materialization (8-bit)",
    "FrameLayout: large offset materialization (ADD base, offset)",
    "FrameLayout: offset computation (8-bit)",
    "FrameLayout: offset computation (FP + slot_offset)",
    "FrameLayout: outgoing arg offset (8-bit)",
    "FrameLayout: outgoing arg offset (SP + arg_offset)",
    "FrameLayout: red zone offset (8-bit)",
    "FrameLayout: red zone offset (SP - abs_offset)",
    "FrameLayout: stp spill scalarization preserves two stores",
    "FrameLayout: stp spill scalarization preserves two stores (8-bit)",
];
...
mod tests {
    ...
    #[test]
    fn test_frame_large_negative_offset_materialization() {
        let obligation = proof_frame_large_negative_offset_materialization();
        let result = verify_by_evaluation(&obligation);
        assert!(
            matches!(result, VerificationResult::Valid),
            "large negative offset materialization proof failed: {:?}",
            result
        );
    }

    #[test]
    fn test_frame_large_negative_offset_materialization_8bit() {
        let obligation = proof_frame_large_negative_offset_materialization_8bit();
        let result = verify_by_evaluation(&obligation);
        assert!(
            matches!(result, VerificationResult::Valid),
            "large negative offset materialization proof (8-bit) failed: {:?}",
            result
        );
    }
    ...
    fn test_all_frame_proofs_count() {
        ...
        assert_eq!(
            proofs.len(),
            18,
            "expected 18 frame proofs (12 degenerate offset/red-zone/outgoing-arg/\
             large-offset/emergency-spill/stp-scalarization X==X retracted in #62), got {}",
            proofs.len()
        );
    }
    ...
    #[test]
    fn test_frame_stp_spill_scalarization() {
        let obligation = proof_frame_stp_spill_scalarization();
        let result = verify_by_evaluation(&obligation);
        assert!(
            matches!(result, VerificationResult::Valid),
            "STP spill scalarization proof failed: {:?}",
            result
        );
    }

    #[test]
    fn test_frame_stp_spill_scalarization_8bit() {
        let obligation = proof_frame_stp_spill_scalarization_8bit();
        let result = verify_by_evaluation(&obligation);
        assert!(
            matches!(result, VerificationResult::Valid),
            "STP spill scalarization proof (8-bit) failed: {:?}",
            result
        );
    }
