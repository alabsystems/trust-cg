// trust-cg-verify/const_fold_proofs.rs - SMT proofs for constant folding optimizations
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// Proves that constant folding rules in trust-cg-opt/const_fold.rs preserve
...
//
// Reference: crates/trust-cg-opt/src/const_fold.rs

//! SMT proofs for constant folding optimization rules.
//!
//! Each proof corresponds to a constant folding rule in `trust-cg-opt::const_fold`:
//!
...
pub fn proof_const_fold_add() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_add(c1.clone(), c2.clone()),
...
pub fn proof_const_fold_sub() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_sub(c1.clone(), c2.clone()),
...
pub fn proof_const_fold_mul() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_mul(c1.clone(), c2.clone()),
...
pub fn proof_const_fold_and() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_and(c1.clone(), c2.clone()),
...
pub fn proof_const_fold_or() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_or(c1.clone(), c2.clone()),
...
pub fn proof_const_fold_xor() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_xor(c1.clone(), c2.clone()),
...
pub fn proof_const_fold_shl() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_shl(c1.clone(), c2.clone()),
...
pub fn proof_const_fold_sdiv() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_sdiv(c1.clone(), c2.clone()),
...
pub fn proof_const_fold_neg() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_neg(c.clone()),
...
pub fn proof_const_fold_not() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_not(c.clone(), width),
...
pub fn proof_identity_add_zero() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_add(x.clone(), SmtExpr::bv_const(0, width)),
...
pub fn proof_identity_mul_one() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_mul(x.clone(), SmtExpr::bv_const(1, width)),
        aarch64_expr: encode_identity(x),
        inputs: vec![("x".to_string(), width)],
        preconditions: vec![],
        fp_inputs: vec![],
...
pub fn proof_identity_mul_zero() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_mul(x, SmtExpr::bv_const(0, width)),
...
pub fn proof_identity_and_zero() -> ProofObligation {
    ...
    ProofObligation {
        machine_side_provenance: crate::lowering_proof::MachineSideProvenance::StaticDb,
        name: "Identity: x & 0 == 0".to_string(),
        trust_ir_expr: encode_and(x, SmtExpr::bv_const(0, width)),
        aarch64_expr: encode_movi(0, width),
        inputs: vec![("x".to_string(), width)],
        preconditions: vec![],
        fp_inputs: vec![],
...
pub fn proof_identity_or_zero() -> ProofObligation {
    ...
    ProofObligation {
        machine_side_provenance: crate::lowering_proof::MachineSideProvenance::StaticDb,
        name: "Identity: x | 0 == x".to_string(),
        trust_ir_expr: encode_or(x.clone(), SmtExpr::bv_const(0, width)),
        aarch64_expr: encode_identity(x),
        inputs: vec![("x".to_string(), width)],
        preconditions: vec![],
        fp_inputs: vec![],
...
pub fn proof_identity_xor_zero() -> ProofObligation {
    ...
    ProofObligation {
        machine_side_provenance: crate::lowering_proof::MachineSideProvenance::StaticDb,
        name: "Identity: x ^ 0 == x".to_string(),
        trust_ir_expr: encode_xor(x.clone(), SmtExpr::bv_const(0, width)),
        aarch64_expr: encode_identity(x),
        inputs: vec![("x".to_string(), width)],
        preconditions: vec![],
        fp_inputs: vec![],
...
pub fn proof_identity_sub_self() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: encode_sub(x.clone(), x),
        aarch64_expr: encode_movi(0, width),
        inputs: vec![("x".to_string(), width)],
        preconditions: vec![],
        fp_inputs: vec![],
...
pub fn all_const_fold_proofs_with_variants() -> Vec<ProofObligation> {
    ...
    // #62 retraction: the "ConstFold: OP(C1, C2) == MOVI(eval)" obligations are
    // degenerate const==const tautologies (both sides are the SAME compile-time
    // constant — no independent encoder, a wrong fold could not refute). They are
    // NOT instruction lowerings and have been RETRACTED. The GENUINE algebraic
    // identities (x+0==x, x*1==x, x*0==0, x&0==0, x|0==x, x^0==x, x-x==0; trust_ir
    // bvadd/bvmul/... vs the folded constant, which a wrong identity refutes)
    // remain — those are "Identity:"-prefixed.
    proofs.retain(|p| !p.name.starts_with("ConstFold:"));
...
mod tests {
    ...
    fn test_wrong_add_nonzero_identity_detected() {
        ...
        let obligation = ProofObligation {
            ...
            trust_ir_expr: encode_add(x.clone(), SmtExpr::bv_const(1, width)),
            aarch64_expr: encode_identity(x),
            inputs: vec![("x".to_string(), width)],
            preconditions: vec![],
            fp_inputs: vec![],
    ...
    fn test_wrong_mul_two_identity_detected() {
        ...
        let obligation = ProofObligation {
            ...
            trust_ir_expr: encode_mul(x.clone(), SmtExpr::bv_const(2, width)),
            aarch64_expr: encode_identity(x),
            inputs: vec![("x".to_string(), width)],
            preconditions: vec![],
            fp_inputs: vec![],
    ...
    fn test_wrong_or_one_identity_detected() {
        ...
        let obligation = ProofObligation {
            machine_side_provenance: crate::lowering_proof::MachineSideProvenance::StaticDb,
            name: "WRONG: x | 1 == x".to_string(),
            trust_ir_expr: encode_or(x.clone(), SmtExpr::bv_const(1, width)),
            aarch64_expr: encode_identity(x),
            inputs: vec![("x".to_string(), width)],
            preconditions: vec![],
            fp_inputs: vec![],
    ...
    fn test_wrong_neg_identity_detected() {
        ...
        let obligation = ProofObligation {
            ...
            trust_ir_expr: encode_neg(x.clone()),
            aarch64_expr: encode_identity(x),
            inputs: vec![("x".to_string(), width)],
            preconditions: vec![],
            fp_inputs: vec![],
