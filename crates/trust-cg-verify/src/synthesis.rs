// trust-cg-verify/synthesis.rs - Offline peephole synthesis via SMT
//
...
//!
//! ```rust
//! use trust_cg_verify::synthesis::{SynthesisEngine, SearchSpace, SearchConfig};
//!
...
impl ProvenRuleDb {
    ...
    /// Seed the database with the Layer C bit-reverse mask fusion marker.
    ///
    /// Pattern: `Movz/Movk* mask ; AndRR dst, src, mask` where the materialized
    /// mask is an AArch64 logical immediate and the mask vreg has one read.
    /// Replacement: `AndRI dst, src, mask`.
    ///
    /// The proof obligation is the same bitvector expression on both sides
    /// (`src & mask`), while the matcher/profitability gate accounts for the
    /// removed move-wide materialization chain. This keeps #854 focused on the
    /// `revertBits.c` shift/mask/or hot path without pretending to synthesize
    /// a full bit-reverse replacement.
    pub fn seed_layer_c() -> Self {
        let mut db = Self::new();
        db.add(ProvenRule {
            name: "LayerC: Movz/Movk mask; AndRR x, y, mask => AndRI x, y, mask".to_string(),
            candidate: RuleCandidate {
                pattern: vec![
                    InstrPattern {
                        opcode: SynthOpcode::MovImm,
                        operands: vec![OperandPattern::AnyImmediate],
                    },
                    InstrPattern {
                ...
                replacement: vec![InstrPattern {
                    opcode: SynthOpcode::And,
                    operands: vec![OperandPattern::InputReg, OperandPattern::AnyImmediate],
                }],
            },
            proof_hash: 0,
            cost_delta: 1,
...
pub struct SynthesisEngine {
    ...
    /// Optional fuel-only failed-proof counterexamples. Empty by default.
    failed_proof_counterexamples: FailedProofCounterexampleCorpus,
    /// Exact source/target scope required before failed-proof seeds are consumed.
...
impl SynthesisEngine {
    ...
    /// consumption.
    ///
    /// The corpus remains available for introspection, but seeds are ignored
    /// until [`Self::with_failed_proof_counterexamples_for_scope`] provides an
    /// exact source/target scope.
    ...
    /// source/target scope.
    ///
    /// The corpus never registers rules. It only contributes concrete inputs
    /// to the CEGIS fast path when seed source, target, and variable names
    /// exactly match the synthesis obligation.
    ...
    /// Number of configured failed-proof counterexample seeds.
    pub fn failed_proof_counterexample_seed_count(&self) -> usize {
        self.failed_proof_counterexamples.len()
    }

    #[cfg(test)]
    fn initialized_cegis_counterexample_count_for_test(&mut self) -> usize {
        self.get_or_init_cegis().counterexample_count()
    }
    ...
    fn get_or_init_cegis(&mut self) -> &mut CegisLoop {
        if self.cegis.is_none() {
            ...
            if let Some((source_region, target)) = self.failed_proof_counterexample_scope.as_ref() {
                let variable_names = vars
                    .iter()
                    .map(|(name, _)| name.clone())
                    .collect::<Vec<_>>();
                for input in self.failed_proof_counterexamples.concrete_inputs_for_scope(
                    source_region,
                    target,
                    &variable_names,
                ) {
    ...
    fn build_obligation(&self, candidate: &RuleCandidate) -> (SmtExpr, SmtExpr, ProofObligation) {
        ...
        let obligation = ProofObligation {
            ...
            trust_ir_expr: pattern_expr.clone(),
...
mod tests {
    ...
    fn test_cost_model() {
        ...
        assert_eq!(
            SynthesisEngine::estimate_cost(std::slice::from_ref(&add)),
            1
        );
    ...
    #[test]
    fn test_failed_proof_counterexample_fuel_does_not_register_rule() {
        use crate::cegis::ConcreteInput;
        use crate::failed_proof_reducer::{
            FailedProofCounterexampleCorpus, FailedProofCounterexampleSeedFilter,
            FailedProofReducerArtifact,
        };
        use crate::rewrite_admission::{
            CostContext, ProofFailureKind, ReducerMetadata, SourceRegionIdentity,
            TargetAbiLayoutIdentity, TransformIdentity,
        };

        let source_region = SourceRegionIdentity::new(
            "trust-cg-stable128:synthesis-fuel",
            "trust-cg-stable128-v1",
            "synthesis_test",
        )
        .with_kernel_name("synthesis_test")
        .with_function_symbol("synthesis_test_fn")
        .with_region_label("candidate");
        ...
        let record = crate::rewrite_admission::RewriteAdmissionRecord::new(
            source_region,
            vec![],
            target,
            CostContext::aarch64("trust-cg-aarch64", "2026.04", 2, 1),
            TransformIdentity::new("synthesis.bad-fuel", "v1"),
        )
        .with_cegis_result(
            &CegisResult::NotEquivalent {
                counterexample: ConcreteInput::from_pairs(&[("x", 7)]),
                found_by_concrete: true,
            },
            Some(ReducerMetadata::new(
                ProofFailureKind::BadCandidate,
                "synthesis-test",
            )),
        );
        let artifact = FailedProofReducerArtifact::from_admission_record(&record)
            .expect("bad candidate artifact should classify");
        ...
        let mut mismatched_source = record.source_region.clone();
        mismatched_source.function_symbol = Some("other_synthesis_fn".to_string());
        let mut mismatched_engine = SynthesisEngine::with_mode(8, VerifyMode::Cegis)
        ...
        assert_eq!(
            mismatched_engine.failed_proof_counterexample_seed_count(),
            1
        );
        assert_eq!(
            mismatched_engine.initialized_cegis_counterexample_count_for_test(),
            6,
            "mismatched source scope must not inject failed-proof fuel"
        );

        let candidate = RuleCandidate {
            pattern: vec![InstrPattern {
                opcode: SynthOpcode::Add,
                operands: vec![OperandPattern::InputReg, OperandPattern::Immediate(7)],
            }],
            replacement: vec![],
        };
        let mut engine = SynthesisEngine::with_mode(8, VerifyMode::Cegis)
        ...
        assert_eq!(engine.failed_proof_counterexample_seed_count(), 1);
        assert_eq!(
            engine.initialized_cegis_counterexample_count_for_test(),
            7,
            "matching source/target scope should inject exactly one failed-proof seed"
        );

        let mut db = ProvenRuleDb::new();
        if let Some(rule) = engine.verify_candidate_cegis(&candidate) {
            db.add(rule);
        }
