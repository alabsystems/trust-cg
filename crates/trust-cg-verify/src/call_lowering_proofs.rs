// trust-cg-verify/call_lowering_proofs.rs - SMT proofs for call lowering correctness
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// Proves that call lowering (function calls and returns) in trust-cg-lower
...
//
// Technique: Alive2-style (PLDI 2021). For each rule, encode LHS (trust_ir call
...
// for all inputs.
//
// Reference: crates/trust-cg-lower/src/abi.rs (AppleAArch64ABI)
// Reference: crates/trust-cg-lower/src/isel.rs (select_call, select_return)
...
//! | [`proof_call_variadic_stack_slot_placement`] | `CallVariadic` varargs occupy adjacent stack slots |
//!
...
fn proof_int_arg_placement_n(arg_idx: u32, width: u32) -> ProofObligation {
    ...
    // trust_ir side: argument value that should be passed
    let trust_ir = arg.clone();
...
pub fn proof_arg_placement_all_8_regs() -> ProofObligation {
    ...
    let mut trust_ir = args[0].clone();
    for arg in &args[1..] {
        trust_ir = trust_ir.bvxor(arg.clone());
    }
...
pub fn proof_arg_placement_all_8_regs_8bit() -> ProofObligation {
    ...
    ProofObligation {
        machine_side_provenance: crate::lowering_proof::MachineSideProvenance::StaticDb,
        name: "Call lowering: all 8 integer args correctly distributed (XOR aggregate) (8-bit)"
            .to_string(),
        trust_ir_expr: trust_ir,
        aarch64_expr: aarch64,
        inputs: (0..8).map(|i| (format!("arg_{}", i), width)).collect(),
...
pub fn proof_fp_arg_all_8_regs_8bit() -> ProofObligation {
    ...
    ProofObligation {
        machine_side_provenance: crate::lowering_proof::MachineSideProvenance::StaticDb,
        name: "Call lowering: all 8 FP args correctly distributed (XOR aggregate) (8-bit)"
            .to_string(),
        trust_ir_expr: trust_ir,
        aarch64_expr: aarch64,
        inputs: (0..8).map(|i| (format!("farg_{}", i), width)).collect(),
...
fn proof_return_pair_gpr_width(width: u32) -> ProofObligation {
    ...
    // trust_ir side: two return values concatenated
    let trust_ir = ret_0.clone().concat(ret_1.clone());
    ...
    ProofObligation {
        machine_side_provenance: crate::lowering_proof::MachineSideProvenance::StaticDb,
        name: format!(
            "Call lowering: integer return pair in X0, X1{}",
            width_label
        ),
        trust_ir_expr: trust_ir,
        aarch64_expr: aarch64,
        inputs: vec![("ret_0".to_string(), width), ("ret_1".to_string(), width)],
        preconditions: vec![],
        fp_inputs: vec![],
        category: Some(crate::lowering_proof::TransvalCheckKind::InstructionLowering),
    }
...
/// Proof: AArch64 16-byte scalar-pair aggregate eightbyte placement (X0=low, X1=high).
///
/// A 9-16 byte non-HFA aggregate is passed/returned as two consecutive 64-bit
/// GPRs: X0 holds the LOW eightbyte (bytes `[base+0..8]`), X1 the HIGH eightbyte
/// (bytes `[base+8..16]`) — Apple AArch64 ABI (`abi.rs:158-159`, classify at
/// `abi.rs:594-605`/`739-745`). The backend emits this as two 64-bit memory ops
/// at offsets 0 and 8: `select_aggregate_arg` loads them (`isel.rs:6938-6957`)
/// and `materialize_aggregate_call_result` stores them (`isel.rs:5260-5282`).
///
/// Unlike the RETRACTED degenerate pair "proof" (`proof_return_pair_gpr_width`,
/// which restated `concat(r0,r1) == concat(r0,r1)` over already-split values and
/// could not refute a misplacement), this discharges THROUGH the symbolic
/// byte-array memory model (`memory_proofs`): the two eightbytes are stored at
/// offsets 0 and 8, re-loaded at the SAME offsets into X0/X1, and the obligation
/// asserts the reassembled `concat(X1, X0)` equals the original aggregate
/// `concat(agg_hi, agg_lo)`. A wrong load/store offset (both at 0, or swapped)
/// reads/writes the wrong eightbyte and REFUTES — so this genuinely pins the
/// eightbyte→register placement and the `+8` offset, for both the load (param)
/// and store (return) sites.
fn proof_aggregate_scalar_pair_eightbyte_placement() -> ProofObligation {
    let mem = crate::memory_proofs::symbolic_memory("mem_default");
    let base = SmtExpr::var("base", 64);
    let agg_lo = SmtExpr::var("agg_lo", 64); // low eightbyte  -> X0, at [base+0]
    let agg_hi = SmtExpr::var("agg_hi", 64); // high eightbyte -> X1, at [base+8]
    let addr_hi = base.clone().bvadd(SmtExpr::bv_const(8, 64));

    // Lay the aggregate down in memory: low eightbyte at +0, high eightbyte at +8.
    let mem1 = crate::memory_proofs::encode_store_le(&mem, &base, &agg_lo, 8);
    let mem2 = crate::memory_proofs::encode_store_le(&mem1, &addr_hi, &agg_hi, 8);

    // The lowering: X0 <- LDR [base, #0], X1 <- LDR [base, #8].
    let x0 = crate::memory_proofs::encode_load_le(&mem2, &base, 8);
    let x1 = crate::memory_proofs::encode_load_le(&mem2, &addr_hi, 8);

    // trust_ir side: the 128-bit aggregate value (high eightbyte in the high bits).
    let trust_ir = agg_hi.concat(agg_lo);
    // AArch64 side: reassemble from the two register loads (X1 high, X0 low).
    let aarch64 = x1.concat(x0);

    ProofObligation {
        machine_side_provenance: crate::lowering_proof::MachineSideProvenance::StaticDb,
        name: "Call lowering: 16-byte aggregate scalar-pair eightbyte placement (X0=lo, X1=hi)"
            .to_string(),
        trust_ir_expr: trust_ir,
        aarch64_expr: aarch64,
        inputs: vec![
...
/// Proof: AArch64 4×F32 HFA (homogeneous-float aggregate) lane placement (S0..S3).
///
/// A 1-4 member homogeneous float aggregate is passed/returned in consecutive FP
/// registers of the element width (S0-S3 for F32, D0-D3 for F64) — AAPCS64
/// `ArgLocation::RegSequence` (`abi.rs:141-151`; classified by `detect_hfa`,
/// `abi.rs:389`). The backend emits one FP load/store per lane at offset
/// `index * elem_size` (`isel.rs:5296-5308`). This proves the canonical 4×F32
/// case: lanes at offsets 0/4/8/12 → S0/S1/S2/S3, lane 0 the lowest-address lane.
///
/// Like the scalar-pair proof, it discharges THROUGH the symbolic byte-array
/// (`memory_proofs`): store the four lanes at their offsets, re-load at the SAME
/// offsets, and assert the reassembled 128-bit value equals the original. A wrong
/// lane offset reads the wrong lane and REFUTES — pinning the per-lane
/// `index*elem_size` placement and lane order. (Lane bits are modeled as BV32;
/// placement is pure bit-movement, so no FP-arithmetic model is needed.)
fn proof_hfa_f32x4_lane_placement() -> ProofObligation {
    let mem = crate::memory_proofs::symbolic_memory("mem_default");
    let base = SmtExpr::var("base", 64);
    let s0 = SmtExpr::var("hfa_s0", 32); // lane 0 -> S0, at [base+0]
    let s1 = SmtExpr::var("hfa_s1", 32); // lane 1 -> S1, at [base+4]
    let s2 = SmtExpr::var("hfa_s2", 32); // lane 2 -> S2, at [base+8]
    let s3 = SmtExpr::var("hfa_s3", 32); // lane 3 -> S3, at [base+12]
    let at = |n: u64| base.clone().bvadd(SmtExpr::bv_const(n, 64));

    // Lay the 4-lane F32 HFA down: lane i at offset i*4.
    let m1 = crate::memory_proofs::encode_store_le(&mem, &base, &s0, 4);
    let m2 = crate::memory_proofs::encode_store_le(&m1, &at(4), &s1, 4);
    let m3 = crate::memory_proofs::encode_store_le(&m2, &at(8), &s2, 4);
    let m4 = crate::memory_proofs::encode_store_le(&m3, &at(12), &s3, 4);

    // The lowering: S{i} <- LDR [base, #i*4].
    let v0 = crate::memory_proofs::encode_load_le(&m4, &base, 4);
    let v1 = crate::memory_proofs::encode_load_le(&m4, &at(4), 4);
    let v2 = crate::memory_proofs::encode_load_le(&m4, &at(8), 4);
    let v3 = crate::memory_proofs::encode_load_le(&m4, &at(12), 4);

    // trust_ir: the 128-bit HFA value, lane 0 in the low 32 bits.
    let trust_ir = s3.concat(s2.concat(s1.concat(s0)));
    // AArch64: reassemble from the four lane loads (same order).
    let aarch64 = v3.concat(v2.concat(v1.concat(v0)));

    ProofObligation {
        machine_side_provenance: crate::lowering_proof::MachineSideProvenance::StaticDb,
        name: "Call lowering: 4xF32 HFA lane placement (S0..S3 at +0/+4/+8/+12)".to_string(),
        trust_ir_expr: trust_ir,
        aarch64_expr: aarch64,
        inputs: vec![
            ("base".to_string(), 64),
...
fn proof_return_pair_fpr_width(width: u32) -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: trust_ir,
        aarch64_expr: aarch64,
        inputs: vec![("fret_0".to_string(), width), ("fret_1".to_string(), width)],
        preconditions: vec![],
        fp_inputs: vec![],
        category: Some(crate::lowering_proof::TransvalCheckKind::InstructionLowering),
    }
...
fn proof_callee_saved_gpr_n(reg_idx: u32, width: u32) -> ProofObligation {
    ...
    // trust_ir side: value before call = saved_val
    let trust_ir = saved_val.clone();
...
pub fn proof_stack_alignment_16() -> ProofObligation {
    ...
    // trust_ir side: SP must be aligned -> 0
    let trust_ir = SmtExpr::bv_const(0, width);
    ...
    ProofObligation {
        ...
        preconditions: vec![
            ...
            sp_base
                .bvand(SmtExpr::bv_const(0xF, width))
...
fn proof_stack_overflow_alignment_width(width: u32) -> ProofObligation {
    ...
    // trust_ir side: stack slot offset should be 8-byte aligned -> low 3 bits = 0
    let trust_ir = SmtExpr::bv_const(0, width);
    ...
    ProofObligation {
        machine_side_provenance: crate::lowering_proof::MachineSideProvenance::StaticDb,
        name: format!(
            "Call lowering: stack overflow args 8-byte aligned{}",
            width_label
        ),
        trust_ir_expr: trust_ir,
        aarch64_expr: aarch64,
...
fn proof_bl_saves_lr_width(width: u32) -> ProofObligation {
    ...
    // trust_ir side: expected return address = pc + 4
    let trust_ir = pc.clone().bvadd(four.clone());
    ...
    ProofObligation {
        machine_side_provenance: crate::lowering_proof::MachineSideProvenance::StaticDb,
        name: format!(
            "Call lowering: BL saves return address PC+4 in LR{}",
            width_label
        ),
        trust_ir_expr: trust_ir,
        aarch64_expr: aarch64,
...
fn proof_ret_uses_lr_width(width: u32) -> ProofObligation {
    ...
    // trust_ir side: return target = lr_val
    let trust_ir = lr_val.clone();
...
fn proof_blr_indirect_width(width: u32) -> ProofObligation {
    ...
    // trust_ir side: call target = fn_ptr
    let trust_ir = fn_ptr.clone();
...
fn proof_blr_saves_lr_width(width: u32) -> ProofObligation {
    ...
    ProofObligation {
        machine_side_provenance: crate::lowering_proof::MachineSideProvenance::StaticDb,
        name: format!(
            "Call lowering: BLR saves return address PC+4 in LR{}",
            width_label
        ),
        trust_ir_expr: trust_ir,
        aarch64_expr: aarch64,
...
fn proof_gpr_fpr_independent_width(width: u32) -> ProofObligation {
    ...
    // trust_ir side: both args should be passed -> concat(int_arg, fp_arg)
    let trust_ir = int_arg.clone().concat(fp_arg.clone());
...
/// is NOT necessarily preserved (the trust_ir and AArch64 sides differ because
...
fn proof_call_clobbers_gpr_n(reg_idx: u32, width: u32) -> ProofObligation {
    ...
    // trust_ir side: value before call
    let trust_ir = pre_val;
...
fn proof_stack_arg_passthrough_width(width: u32) -> ProofObligation {
    ...
    // trust_ir side: argument value to be passed
    let trust_ir = arg_val.clone();
    ...
    ProofObligation {
        machine_side_provenance: crate::lowering_proof::MachineSideProvenance::StaticDb,
        name: format!(
            "Call lowering: stack arg pass-through correctness{}",
            width_label
        ),
        trust_ir_expr: trust_ir,
        aarch64_expr: aarch64,
...
/// Proof: scalar `CallVariadic` varargs are placed in adjacent 8-byte stack slots.
///
/// Theorem:
///   forall fixed_stack_bytes, vararg_index .
///     current_offset == fixed_stack_bytes + (vararg_index << 3)
///     next_offset == current_offset + 8
///     current_offset & 7 == 0
///
/// Apple arm64 puts every variadic argument on the stack. The first vararg
/// starts after any fixed-argument stack overflow, and scalar variadic args
/// smaller than eight bytes are promoted to an 8-byte slot. This QF_BV
/// obligation proves the lowering's STR stack offset formula matches the trust_ir
/// ABI placement rule and that adjacent scalar vararg slots remain 8 bytes
/// apart.
///
/// Reference: abi.rs `classify_params_variadic`, isel.rs `select_variadic_call`.
pub fn proof_call_variadic_stack_slot_placement() -> ProofObligation {
    let width = 64;
    let fixed_stack_bytes = SmtExpr::var("fixed_stack_bytes", width);
    let vararg_index = SmtExpr::var("vararg_index", width);
    let eight = SmtExpr::bv_const(8, width);
    let one = SmtExpr::bv_const(1, width);
    let three = SmtExpr::bv_const(3, width);
    let seven = SmtExpr::bv_const(0x7, width);
    let zero = SmtExpr::bv_const(0, width);

    let vararg_bytes = vararg_index.clone().bvshl(three.clone());
    let trust_ir_slot = fixed_stack_bytes.clone().bvadd(vararg_bytes.clone());
    let trust_ir_next_slot = fixed_stack_bytes
        .clone()
        .bvadd(vararg_index.bvadd(one).bvshl(three));

    let aarch64_slot = fixed_stack_bytes.clone().bvadd(vararg_bytes);
    let aarch64_next_slot = aarch64_slot.clone().bvadd(eight);
    let aarch64_alignment = aarch64_slot.clone().bvand(seven.clone());

    let trust_ir = trust_ir_slot
        .concat(trust_ir_next_slot)
        .concat(zero.clone());
    let aarch64 = aarch64_slot
        .concat(aarch64_next_slot)
        .concat(aarch64_alignment);

    ProofObligation {
        machine_side_provenance: crate::lowering_proof::MachineSideProvenance::StaticDb,
        name: "CallVariadic: vararg stack-slot address placement".to_string(),
        trust_ir_expr: trust_ir,
        aarch64_expr: aarch64,
        inputs: vec![
            ("fixed_stack_bytes".to_string(), width),
            ("vararg_index".to_string(), width),
        ],
        preconditions: vec![
            fixed_stack_bytes
                .bvand(seven)
...
fn proof_result_copyback_width(width: u32) -> ProofObligation {
    ...
    ProofObligation {
        machine_side_provenance: crate::lowering_proof::MachineSideProvenance::StaticDb,
        name: format!(
            "Call lowering: result copy-back from ABI register{}",
            width_label
        ),
        trust_ir_expr: trust_ir,
        aarch64_expr: aarch64,
...
pub fn all_call_lowering_proofs() -> Vec<ProofObligation> {
    ...
    // 3b. Aggregate scalar-pair eightbyte placement — GENUINE memory-model proof
    //     (discharges through the symbolic byte-array; refutes a wrong offset),
    //     NOT the retracted degenerate `concat(r0,r1)==concat(r0,r1)` pair identity.
    proofs.push(proof_aggregate_scalar_pair_eightbyte_placement());
    proofs.push(proof_hfa_f32x4_lane_placement());

    // 4. Callee-saved register preservation (40 proofs)
    ...
    // 10. Stack arg pass-through / variadic stack slots (3 proofs)
    proofs.push(proof_stack_arg_passthrough());
    proofs.push(proof_stack_arg_passthrough_8bit());
    proofs.push(proof_call_variadic_stack_slot_placement());

    // 11. Result copy-back (2 proofs)
    proofs.push(proof_result_copyback());
    proofs.push(proof_result_copyback_8bit());

    // #62 retraction: the arg->Xn/Vn placement, return-value/pair, callee-saved
    // Xn/Vn preservation, BL/BLR/RET return-address, indirect-call-via-scratch,
    // GPR/FPR independence, stack-arg passthrough, and result copy-back
    // obligations are all degenerate X==X self-equalities (they restate the ABI
    // register/placement decision as a value identity — no independent
    // destination-register model, a wrong placement could not refute). They are
    // RETRACTED. The GENUINE structural proofs — SP 16-byte alignment at the call
    // site, 8-byte stack-overflow-arg alignment, and the variadic stack-slot
    // address computation — remain.
    proofs.retain(|p| CALL_LOWERING_GENUINE.contains(&p.name.as_str()));

    proofs
}

/// The only GENUINE (non-degenerate) call-lowering obligations kept after the
/// #62 retraction (structural alignment + variadic stack-slot address).
const CALL_LOWERING_GENUINE: &[&str] = &[
    "Call lowering: SP 16-byte aligned at call site",
    "Call lowering: SP 16-byte aligned at call site (8-bit)",
    "Call lowering: stack overflow args 8-byte aligned",
    "Call lowering: stack overflow args 8-byte aligned (8-bit)",
    "CallVariadic: vararg stack-slot address placement",
    // Memory-model aggregate placement (refutes a wrong eightbyte offset; not a
    // degenerate X==X). New coverage on top of the #62 retraction.
    "Call lowering: 16-byte aggregate scalar-pair eightbyte placement (X0=lo, X1=hi)",
    "Call lowering: 4xF32 HFA lane placement (S0..S3 at +0/+4/+8/+12)",
];
...
mod tests {
    ...
    fn assert_valid(obligation: &ProofObligation) {
        ...
        match &result {
            ...
            VerificationResult::Unknown { reason } => {
                panic!("Proof '{}' INCONCLUSIVE: {}", obligation.name, reason);
            }
        }
    }
    ...
    fn test_ret_branches_to_lr() {
        let obligation = proof_ret_branches_to_lr();
        assert_eq!(
            obligation.category,
            Some(crate::lowering_proof::TransvalCheckKind::InstructionLowering)
        );
        assert_valid(&obligation);
    }
    ...
    fn test_ret_branches_to_lr_8bit() {
        let obligation = proof_ret_branches_to_lr_8bit();
        assert_eq!(
            obligation.category,
            Some(crate::lowering_proof::TransvalCheckKind::InstructionLowering)
        );
    ...
    #[test]
    fn test_call_variadic_stack_slot_placement() {
        let obligation = proof_call_variadic_stack_slot_placement();
        assert_eq!(
            obligation.category,
            Some(crate::lowering_proof::TransvalCheckKind::InstructionLowering)
        );
        assert!(obligation.name.contains("CallVariadic"));
        assert_valid(&obligation);
    }
    ...
    #[test]
    fn test_all_call_lowering_proofs_exact_count() {
        let proofs = all_call_lowering_proofs();
        assert_eq!(
            proofs.len(),
            7,
            "expected exactly 7 GENUINE call-lowering proofs (5 structural + 2 memory-model \
             aggregate placement: scalar-pair eightbyte + 4xF32 HFA lane; 98 degenerate arg/\
             return/callee-saved/BL/BLR/RET placement X==X retracted in #62)"
        );
    }
