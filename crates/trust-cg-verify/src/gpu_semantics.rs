// trust-cg-verify/gpu_semantics.rs - Metal GPU parallel operation SMT encoding
//
...
// specifications. For reductions, equivalence of a parallel tree schedule to
// the sequential fold is justified ONLY for `ExactAC`-class operator/element
// combinations (see `ReduceOrderClass`); floating-point add/mul round after
// every step and are NOT associative, so FP reductions are `StrictTree`:
// the source-order fold IS the semantics.
//
// The public release boundary is summarized in the repository-level
// LIMITATIONS.md.
...
//! algebraic properties (associativity, commutativity) hold **exactly** for
//! the element type being reduced. trust_ir proofs guarantee these properties
//! for bitvector (integer/bitwise) operations.
//!
//! **Reduction order is semantics.** IEEE 754 floating-point addition and
//! multiplication round after every step and are NOT associative, so no
//! Associative proof exists for them and a parallel tree reduction computes a
//! genuinely different value than the source-order fold. [`ReduceOrderClass`]
//! classifies every operator/element combination:
//!
//! - [`ReduceOrderClass::ExactAC`]: reordering (tree schedules) is sound.
//! - [`ReduceOrderClass::StrictTree`]: the source-order sequential fold IS the
//!   semantics; tree schedules are refused (fail-closed) by
//!   [`encode_parallel_reduce_checked`].
//!
//! For `ExactAC` operations we encode GPU kernels as sequential specifications
//! and rely on trust_ir proofs to justify that the parallel implementation is
//! equivalent. `StrictTree` reductions must execute in source order unless a
//! future release defines and validates an all-orders numerical witness.
//!
...
// ---------------------------------------------------------------------------
// Reduction order classification
// ---------------------------------------------------------------------------

/// Element kind of a reduction, distinguishing exact bitvector arithmetic
/// from rounding IEEE 754 floating-point arithmetic.
///
/// The kind (together with the operator) determines the
/// [`ReduceOrderClass`]: bitvector operations are exactly associative +
/// commutative, floating-point add/mul are not.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ElemKind {
    /// Fixed-width bitvector elements `(_ BitVec w)` with the given width.
    BitVec(u32),
    /// IEEE 754 floating-point elements. FP addition and multiplication
    /// round after every step, so they are NOT associative: the reduction
    /// order is part of the semantics. (The concrete SMT encodings in this
    /// module use binary32; the order classification applies to every FP
    /// format.)
    Fp,
}

/// How a reduction's evaluation order relates to its semantics.
///
/// Reduction order is semantics. A parallel tree schedule may only replace
/// the source-order fold when the operator is *exactly* associative +
/// commutative on the element type.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ReduceOrderClass {
    /// Exactly associative + commutative: every evaluation order (including
    /// any parallel tree schedule) produces the bit-identical result, so
    /// trust_ir's Associative + Commutative proofs justify reordering.
    /// Integer/bitwise (bitvector) reductions are in this class.
    ExactAC,
    /// Order-sensitive: only the source-order sequential fold is the
    /// semantics; a tree schedule computes a different value. ALL
    /// floating-point reductions are in this class -- fp add/mul because
    /// rounding breaks associativity, and (deliberately fail-closed) fp
    /// min/max because SMT-LIB `fp.min`/`fp.max` leave the ±0 case
    /// unspecified. Such reductions must execute in source order unless a
    /// future release defines and validates an all-orders numerical witness.
    StrictTree,
}

/// Error returned by [`encode_parallel_reduce_checked`] when the requested
/// encoding would be unsound for the operator × element-kind combination.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ReduceEncodingError {
    /// A parallel tree schedule was claimed for a
    /// [`ReduceOrderClass::StrictTree`] reduction. No Associative proof
    /// exists (fp add/mul are not associative), so tree-execution
    /// equivalence is unsatisfiable: refuse, fail-closed.
    TreeScheduleUnsound { op: GpuReduceOp, elem: ElemKind },
    /// The operation is not meaningful for the element kind (bitwise ops on
    /// floating-point elements).
    OpElemMismatch { op: GpuReduceOp, elem: ElemKind },
}

impl fmt::Display for ReduceEncodingError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ReduceEncodingError::TreeScheduleUnsound { op, elem } => write!(
                f,
                "tree-schedule reduction with {op:?} over {elem:?} elements is \
                 order-sensitive (StrictTree): no Associative+Commutative proof \
                 exists, so tree execution is not equivalent to the source-order \
                 fold; execute in source order instead (see LIMITATIONS.md)"
            ),
            ReduceEncodingError::OpElemMismatch { op, elem } => write!(
                f,
                "reduce op {op:?} is not meaningful for {elem:?} elements"
            ),
        }
    }
}

impl std::error::Error for ReduceEncodingError {}
...
/// accumulator. Whether a parallel (tree) execution of the reduction is legal
/// is NOT a property of the operation alone: it depends on the element kind.
/// [`GpuReduceOp::order_class`] classifies each op × [`ElemKind`] combination;
/// trust_ir must prove Associative + Commutative for the operation *on that
/// element type* (possible only for the `ExactAC` class) before the solver
/// considers a tree schedule legal for GPU dispatch.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl GpuReduceOp {
    /// Classify the reduction order semantics of this operation over the
    /// given element kind.
    ///
    /// - [`ElemKind::BitVec`]: every op is [`ReduceOrderClass::ExactAC`] --
    ///   `bvadd`/`bvmul` are modular-arithmetic AC, min/max are lattice AC,
    ///   and the bitwise ops are AC. Tree schedules are sound.
    /// - [`ElemKind::Fp`]: every op is [`ReduceOrderClass::StrictTree`].
    ///   `fp.add`/`fp.mul` round after each step and are not associative.
    ///   Min/Max are ALSO classified `StrictTree` deliberately (fail-closed):
    ///   SMT-LIB `fp.min`/`fp.max` have unspecified ±0 behavior, so a tree
    ///   reordering is not provably result-identical. Bitwise ops on FP are
    ///   not meaningful and stay fail-closed here as well (and are rejected
    ///   outright by [`encode_parallel_reduce_checked`]).
    pub fn order_class(&self, elem: ElemKind) -> ReduceOrderClass {
        match elem {
            ElemKind::BitVec(_) => ReduceOrderClass::ExactAC,
            ElemKind::Fp => ReduceOrderClass::StrictTree,
        }
    }
    ...
    ///
    /// NOTE: every FP reduce op is [`ReduceOrderClass::StrictTree`] (see
    /// [`GpuReduceOp::order_class`]): `fp_add`/`fp_mul` under RNE are not
    /// associative, so a fold built from this function is a SOURCE-ORDER
    /// semantics, never a justification for a parallel tree schedule.
...
///
/// NOTE: the reducing intrinsics (Sum, Min, Max, And, Or, Xor) combine lanes
/// in a hardware-defined order. Encoding them as a sequential fold (see
/// [`encode_simdgroup_reduce`]) is faithful only for
/// [`ReduceOrderClass::ExactAC`] op × element combinations; on FP data they
/// are `StrictTree` and must not be claimed equivalent to a source-order
/// fold.
///
...
///
/// trust_ir proof requirements: Pure, InBounds(0..N), ValidBorrow(input, output)
///
/// ay theory: QF_ABV (arrays + bitvectors)
///
/// # Arguments
...
pub fn encode_parallel_map(
    ...
    let mut result = SmtExpr::const_array(SmtSort::BitVec(64), default_value_for_sort(elem_sort));
    for i in 0..n {
...
/// SMT encoding of a Metal parallel reduce kernel, in SOURCE ORDER.
///
/// Semantics: `result = fold(op, identity, input[0..N])` -- the sequential
/// left-to-right fold. This is the SOURCE-ORDER
/// ([`ReduceOrderClass::StrictTree`]) semantics of the reduction; it is exact
/// for every operator.
///
/// # Equivalence to a parallel tree schedule
///
/// The GPU executes a tree reduction (log2(N) parallel steps). That tree
/// schedule is equivalent to this fold ONLY for
/// [`ReduceOrderClass::ExactAC`]-class op × element combinations (see
/// [`GpuReduceOp::order_class`]), where trust_ir proves Associative +
/// Commutative so all evaluation orders produce the bit-identical result.
///
/// Floating-point add/mul round after every step and are NOT associative --
/// the Associative side condition is unsatisfiable for FP, so an FP tree
/// reduction computes a genuinely different value than this fold. FP
/// reductions must be executed in source order unless a future release defines
/// and validates an all-orders numerical witness. Use
/// [`encode_parallel_reduce_checked`] to enforce that gate instead of relying
/// on caller discipline.
///
/// trust_ir proof requirements (for tree-schedule equivalence; satisfiable
/// only for the `ExactAC` class):
///   - Associative(op): (a op b) op c = a op (b op c)
///   - Commutative(op): a op b = b op a
///   - Pure, InBounds(0..N)
///
/// ay theory: QF_ABV or QF_FPBV (if FP elements)
///
/// # Arguments
/// * `input` - Input array expression: `(Array BV64 BV_elem)`
/// * `op` - Binary operator (folded in source order)
...
/// Order-checked SMT encoding of a Metal parallel reduce kernel.
///
/// Like [`encode_parallel_reduce`], but takes the op/element kind explicitly
/// and refuses (fail-closed) encodings whose justification would be unsound:
///
/// - `tree_schedule = false`: the caller executes the reduction in source
///   order. Always sound; returns the sequential fold, which IS that
///   semantics.
/// - `tree_schedule = true`: the caller claims the GPU may execute a parallel
///   tree schedule and that this fold still describes the result. That claim
///   holds only for [`ReduceOrderClass::ExactAC`] combinations; for
///   [`ReduceOrderClass::StrictTree`] (all FP reductions) it returns
///   [`ReduceEncodingError::TreeScheduleUnsound`].
///
/// Bitwise ops over FP elements return
/// [`ReduceEncodingError::OpElemMismatch`] regardless of schedule.
///
/// FP encodings use binary32 elements (the identity comes from
/// [`GpuReduceOp::identity_fp32`]), matching the rest of this module.
pub fn encode_parallel_reduce_checked(
    input: &SmtExpr,
    op: &GpuReduceOp,
    elem: ElemKind,
    n: u64,
    tree_schedule: bool,
) -> Result<SmtExpr, ReduceEncodingError> {
    let bitwise = matches!(
        op,
        GpuReduceOp::BitwiseAnd | GpuReduceOp::BitwiseOr | GpuReduceOp::BitwiseXor
    );
    if elem == ElemKind::Fp && bitwise {
        return Err(ReduceEncodingError::OpElemMismatch { op: *op, elem });
    }
    if tree_schedule && op.order_class(elem) == ReduceOrderClass::StrictTree {
        return Err(ReduceEncodingError::TreeScheduleUnsound { op: *op, elem });
    }
    Ok(match elem {
        ElemKind::BitVec(width) => encode_parallel_reduce_bv(input, *op, width, n),
        ElemKind::Fp => {
            let identity = op.identity_fp32();
            encode_parallel_reduce(
                input,
                &|a, b| op.apply_fp(a.clone(), b.clone()),
                &identity,
                n,
            )
        }
...
/// Bitvector reductions are [`ReduceOrderClass::ExactAC`], so this fold is
/// also a sound description of any parallel tree schedule.
pub fn encode_parallel_reduce_bv(
    ...
    encode_parallel_reduce(
        input,
        &|a, b| op.apply_bv(a.clone(), b.clone()),
        &identity,
        n,
    )
}
...
/// with the fold in SOURCE ORDER.
///
/// Common pattern: dot product = `sum(a[i] * b[i] for i in 0..N)`.
///
/// The fused encoding avoids materializing the intermediate mapped array.
///
/// The same ordering caveat as [`encode_parallel_reduce`] applies: this
/// source-order fold describes a parallel tree schedule ONLY for
/// [`ReduceOrderClass::ExactAC`] reduce op × element combinations. FP
/// map-reduce (e.g. an f32 dot product) is `StrictTree`: the accumulation
...
/// 2. The trust_ir proof of Pure guarantees no data races
...
pub fn encode_simdgroup_shuffle(simd_data: &SmtExpr, source_lane: &SmtExpr) -> SmtExpr {
...
pub fn encode_simdgroup_broadcast(
    ...
    let mut result = SmtExpr::const_array(SmtSort::BitVec(32), default_value_for_sort(elem_sort));
...
/// SMT encoding of a SIMD group reduction operation, as a SOURCE-ORDER fold.
///
/// `simd_sum(val)`, `simd_min(val)`, etc.: reduce all lanes in the SIMD
/// group. The hardware combines lanes in an unspecified tree order, so this
/// sequential-fold encoding is faithful ONLY for
/// [`ReduceOrderClass::ExactAC`] op × element combinations (see
/// [`GpuReduceOp::order_class`]) -- integer/bitwise lane data. For FP lane
/// data the intrinsics are `StrictTree`-class: the fold below is NOT a sound
/// model of `simd_sum` over floats, and callers must not claim tree
/// equivalence for it.
///
...
mod tests {
    ...
    fn build_array_bv32(values: &[u64]) -> SmtExpr {
        let mut arr = SmtExpr::const_array(SmtSort::BitVec(64), SmtExpr::bv_const(0, 32));
    ...
    fn build_simd_array(values: &[u64], elem_width: u32) -> SmtExpr {
        let mut arr = SmtExpr::const_array(SmtSort::BitVec(32), SmtExpr::bv_const(0, elem_width));
        for (i, &v) in values.iter().enumerate() {
            let idx = SmtExpr::bv_const(i as u64, 32);
    ...
    fn test_parallel_map_identity() {
        ...
        let result = encode_parallel_map(&input, &|elem| elem.clone(), 3, &SmtSort::BitVec(32));
    ...
    fn test_reduce_min() {
        ...
        let result = encode_parallel_reduce_bv(&input, GpuReduceOp::Min, 32, 4);
    ...
    fn test_reduce_max() {
        ...
        let result = encode_parallel_reduce_bv(&input, GpuReduceOp::Max, 32, 4);
    ...
    fn test_reduce_bitwise_and() {
        ...
        let result = encode_parallel_reduce_bv(&input, GpuReduceOp::BitwiseAnd, 32, 3);
    ...
    fn test_reduce_bitwise_or() {
        ...
        let result = encode_parallel_reduce_bv(&input, GpuReduceOp::BitwiseOr, 32, 3);
    ...
    fn test_reduce_bitwise_xor() {
        ...
        let result = encode_parallel_reduce_bv(&input, GpuReduceOp::BitwiseXor, 32, 3);
    ...
    fn test_reduce_empty() {
        ...
        let result = encode_parallel_reduce_bv(&input, GpuReduceOp::Add, 32, 0);
    ...
    fn test_reduce_single_element() {
        ...
        let result = encode_parallel_reduce_bv(&input, GpuReduceOp::Add, 32, 1);
    ...
    // =======================================================================
    // ReduceOrderClass / encode_parallel_reduce_checked tests
    // =======================================================================

    const ALL_REDUCE_OPS: [GpuReduceOp; 7] = [
        GpuReduceOp::Add,
        GpuReduceOp::Mul,
        GpuReduceOp::Min,
        GpuReduceOp::Max,
    ...
    const FP_REDUCE_OPS: [GpuReduceOp; 4] = [
        GpuReduceOp::Add,
        GpuReduceOp::Mul,
        GpuReduceOp::Min,
        GpuReduceOp::Max,
    ...
    const BITWISE_REDUCE_OPS: [GpuReduceOp; 3] = [
        GpuReduceOp::BitwiseAnd,
        GpuReduceOp::BitwiseOr,
        GpuReduceOp::BitwiseXor,
    ];

    /// Helper to build a concrete fp32 array indexed by BV64.
    fn build_array_fp32(values: &[f32]) -> SmtExpr {
        let mut arr = SmtExpr::const_array(SmtSort::BitVec(64), SmtExpr::fp32_const(0.0));
        for (i, &v) in values.iter().enumerate() {
            let idx = SmtExpr::bv_const(i as u64, 64);
            arr = SmtExpr::store(arr, idx, SmtExpr::fp32_const(v));
        }
        arr
    }

    #[test]
    fn test_order_class_bitvec_is_exact_ac_for_every_op() {
        for op in ALL_REDUCE_OPS {
            for width in [8u32, 16, 32, 64] {
                assert_eq!(
                    op.order_class(ElemKind::BitVec(width)),
                    ReduceOrderClass::ExactAC,
                    "{op:?} over BitVec({width}) must be ExactAC"
                );
            }
        }
    }
    ...
    #[test]
    fn test_order_class_fp_is_strict_tree_for_every_op() {
        // ALL FP ops are StrictTree: Add/Mul because rounding breaks
        // associativity; Min/Max deliberately fail-closed (SMT fp.min/fp.max
        // have unspecified ±0 behavior); bitwise fail-closed (not meaningful).
        for op in ALL_REDUCE_OPS {
            assert_eq!(
                op.order_class(ElemKind::Fp),
                ReduceOrderClass::StrictTree,
                "{op:?} over Fp must be StrictTree"
            );
        }
    }

    #[test]
    fn test_checked_rejects_fp_tree_schedule() {
        let input = build_array_fp32(&[1.0, 2.0, 3.0, 4.0]);
        for op in FP_REDUCE_OPS {
            let result = encode_parallel_reduce_checked(&input, &op, ElemKind::Fp, 4, true);
            assert_eq!(
                result,
                Err(ReduceEncodingError::TreeScheduleUnsound {
                    op,
                    elem: ElemKind::Fp
                }),
                "{op:?}: FP tree-schedule claim must be refused"
            );
        }
    }

    #[test]
    fn test_checked_rejects_bitwise_on_fp() {
        let input = build_array_fp32(&[1.0, 2.0]);
        for op in BITWISE_REDUCE_OPS {
            for tree_schedule in [false, true] {
                let result =
                    encode_parallel_reduce_checked(&input, &op, ElemKind::Fp, 2, tree_schedule);
                assert_eq!(
                    result,
                    Err(ReduceEncodingError::OpElemMismatch {
                        op,
                        elem: ElemKind::Fp
                    }),
                    "{op:?} over Fp (tree_schedule={tree_schedule}) must be refused"
                );
            }
        }
    }
    ...
    #[test]
    fn test_checked_allows_fp_source_order() {
        // tree_schedule=false: source-order fold IS the StrictTree semantics.
        let input = build_array_fp32(&[1.0, 2.0, 3.0, 4.0]);
        let result =
            encode_parallel_reduce_checked(&input, &GpuReduceOp::Add, ElemKind::Fp, 4, false)
                .expect("source-order FP reduce must be encodable");
        assert_eq!(eval_empty(&result), EvalResult::Float(10.0));
    }

    #[test]
    fn test_checked_integer_tree_schedule_matches_unchecked() {
        // Integer (BitVec) behavior is unchanged: the checked entry point
        // accepts a tree-schedule claim and produces the exact expression
        // encode_parallel_reduce_bv produced before.
        let values: [u64; 4] = [100, 5, 42, 7];
        let input = build_array_bv32(&values);
        for op in ALL_REDUCE_OPS {
            for tree_schedule in [false, true] {
                let checked = encode_parallel_reduce_checked(
                    &input,
                    &op,
                    ElemKind::BitVec(32),
                    4,
                ...
                .unwrap_or_else(|e| {
                    panic!(
                        "{op:?} over BitVec(32) (tree_schedule={tree_schedule}) must encode: {e}"
                    )
                });
                let unchecked = encode_parallel_reduce_bv(&input, op, 32, 4);
                assert_eq!(
                    eval_empty(&checked),
                    eval_empty(&unchecked),
                    "{op:?}: checked and unchecked BV encodings must agree"
                );
            }
        }
    }
    ...
    #[test]
    fn test_fp_add_reduction_is_order_sensitive() {
        // The concrete witness for the StrictTree classification: the same
        // four f32 values reduced in source order vs. in a balanced tree
        // produce DIFFERENT results, so no AC proof can exist for fp_add.
        //
        // e = 2^-24: 1.0f32 + e rounds back to 1.0 (RNE, tie-to-even), but
        // e + e = 2^-23 is exact and 1.0 + 2^-23 is representable.
        let e = 2.0f32.powi(-24);
        let values = [1.0f32, 0.0, e, e];
        let input = build_array_fp32(&values);

        // Source order: ((((0 + 1.0) + 0.0) + e) + e) = 1.0
        let source_order =
            encode_parallel_reduce_checked(&input, &GpuReduceOp::Add, ElemKind::Fp, 4, false)
                .expect("source-order FP reduce must be encodable");

        // Tree order: (1.0 + 0.0) + (e + e) = 1.0 + 2^-23 != 1.0
        let op = GpuReduceOp::Add;
        let leaf = |i: u64| SmtExpr::select(input.clone(), SmtExpr::bv_const(i, 64));
        let tree = op.apply_fp(op.apply_fp(leaf(0), leaf(1)), op.apply_fp(leaf(2), leaf(3)));

        let source_val = eval_empty(&source_order);
        let tree_val = eval_empty(&tree);
        assert_eq!(source_val, EvalResult::Float(1.0));
        assert_eq!(
            tree_val,
            EvalResult::Float((1.0f32 + 2.0f32.powi(-23)) as f64)
        );
        assert_ne!(
            source_val, tree_val,
            "fp_add tree reduction must differ from the source-order fold"
        );
    }
    ...
    fn test_simdgroup_broadcast() {
        ...
        let data = build_simd_array(&[10, 20, 30, 40], 32);
        let result =
            encode_simdgroup_broadcast(&data, &SmtExpr::bv_const(1, 32), 4, &SmtSort::BitVec(32));
    ...
    fn test_simdgroup_prefix_sum() {
        ...
        let result = encode_simdgroup_prefix_sum(&data, 4, &SmtSort::BitVec(32));
