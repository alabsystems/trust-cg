// trust-cg-verify/cfg_proofs.rs - SMT proofs for CFG Simplification correctness
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// Proves that CFG simplification transforms in trust-cg-opt/cfg_simplify.rs
...
//
// Reference: crates/trust-cg-opt/src/cfg_simplify.rs
...
pub fn proof_cbz_deterministic() -> ProofObligation {
    ...
    let branch_taken_after =
        SmtExpr::ite(condition2, SmtExpr::bv_const(1, 1), SmtExpr::bv_const(0, 1));

    ProofObligation {
        machine_side_provenance: crate::lowering_proof::MachineSideProvenance::StaticDb,
        name: "CFG: CBZ evaluation is deterministic for all v".to_string(),
        trust_ir_expr: branch_taken_before,
...
/// #62 retraction: degenerate X==X "branch fold / threading / edge split /
/// empty-block redirect / unreachable removal / CBZ deterministic" CFG
/// obligations. These restated a control-flow structural property as a value
/// self-equality (no independent encoder), so they proved nothing and are
/// removed from the database. The GENUINE CFG proofs (CBZ/CBNZ/TBZ/TBNZ
/// take/fall-through, dup-branch ite-elim, phi true/false selection) remain.
const CFG_RETRACTED_DEGENERATE: &[&str] = &[
    "CFG: CBZ deterministic (8-bit)",
    "CFG: CBZ evaluation is deterministic for all v",
    "CFG: branch fold preserves computation z = a * b",
    "CFG: branch fold preserves computation z = a - b",
    "CFG: branch fold preserves z = a + b (8-bit)",
    "CFG: branch fold preserves z = a - b (8-bit)",
    "CFG: branch threading through jump chain preserves final target",
    "CFG: critical edge split preserves value y",
    "CFG: critical edge split preserves value y (8-bit)",
    "CFG: empty block redirect preserves target",
    "CFG: empty block redirect preserves target (8-bit)",
    "CFG: unconditional branch fold preserves computation z = a + b",
    "CFG: unconditional branch fold preserves value y",
    "CFG: unconditional branch fold preserves value y (8-bit)",
    "CFG: unreachable block removal preserves live values",
];
...
pub fn all_cfg_proofs_with_variants() -> Vec<ProofObligation> {
    ...
    proofs.retain(|p| !CFG_RETRACTED_DEGENERATE.contains(&p.name.as_str()));
    proofs
}
...
mod tests {
    ...
    fn test_cfg_negative_different_targets() {
        ...
        let obligation = ProofObligation {
            ...
            trust_ir_expr: conditional,
            aarch64_expr: unconditional,
            inputs: vec![("v".to_string(), width), ("x".to_string(), width)],
            preconditions: vec![],
            fp_inputs: vec![],
            category: Some(crate::lowering_proof::TransvalCheckKind::ControlFlow),
        };
