// trust-cg-verify/rule_discovery.rs - AI-native automatic rule discovery
//
...
//! use trust_cg_verify::rule_discovery::{RuleDiscovery, RuleProposal, RuleResult};
//!
...
impl RuleProposal {
    ...
    fn find_var_width_in_expr(expr: &SmtExpr, target: &str) -> Option<u32> {
        match expr {
            ...
            SmtExpr::Var { .. } | SmtExpr::BvConst { .. } | SmtExpr::BoolConst(_) => None,
            ...
            | SmtExpr::FPLe { lhs, rhs } => Self::find_var_width_in_expr(lhs, target)
                .or_else(|| Self::find_var_width_in_expr(rhs, target)),
            SmtExpr::FPAdd { lhs, rhs, .. }
            | SmtExpr::FPSub { lhs, rhs, .. }
            | SmtExpr::FPMul { lhs, rhs, .. }
            | SmtExpr::FPDiv { lhs, rhs, .. } => Self::find_var_width_in_expr(lhs, target)
                .or_else(|| Self::find_var_width_in_expr(rhs, target)),
            SmtExpr::FPFma { a, b, c, .. } => Self::find_var_width_in_expr(a, target)
            ...
            | SmtExpr::BvBitsToFP { operand, .. } => Self::find_var_width_in_expr(operand, target),
            SmtExpr::Concat { hi, lo, .. } => Self::find_var_width_in_expr(hi, target)
                .or_else(|| Self::find_var_width_in_expr(lo, target)),
            SmtExpr::TrapIfZero { guard, value, .. } => Self::find_var_width_in_expr(guard, target)
            ...
            SmtExpr::Select { array, index } => Self::find_var_width_in_expr(array, target)
            ...
            SmtExpr::Store {
                ...
                .or_else(|| Self::find_var_width_in_expr(index, target))
                .or_else(|| Self::find_var_width_in_expr(value, target)),
            SmtExpr::ConstArray { value, .. } => Self::find_var_width_in_expr(value, target),
            SmtExpr::MemLoad { addr, .. } => Self::find_var_width_in_expr(addr, target),
            SmtExpr::UF { args, .. } => {
                for arg in args {
...
impl RuleDiscovery {
    ...
    pub fn propose_rule(&mut self, proposal: RuleProposal) -> RuleResult {
        ...
        let result = self
            .cegis
            .verify(&proposal.pattern, &proposal.replacement, &vars);
    ...
    fn verify_by_evaluation_fallback(
        ...
        let obligation = ProofObligation {
            ...
            trust_ir_expr: proposal.pattern.clone(),
...
fn estimate_expr_cost(expr: &SmtExpr) -> i32 {
    match expr {
        ...
        SmtExpr::BvSDiv { lhs, rhs, .. }
        | SmtExpr::BvUDiv { lhs, rhs, .. }
        | SmtExpr::BvURem { lhs, rhs, .. } => 5 + estimate_expr_cost(lhs) + estimate_expr_cost(rhs),
        SmtExpr::BvAdd { lhs, rhs, .. }
        | SmtExpr::BvSub { lhs, rhs, .. }
        | SmtExpr::BvAnd { lhs, rhs, .. }
        | SmtExpr::BvOr { lhs, rhs, .. }
        ...
        | SmtExpr::BvShl { lhs, rhs, .. }
        | SmtExpr::BvLshr { lhs, rhs, .. }
        | SmtExpr::BvAshr { lhs, rhs, .. } => 1 + estimate_expr_cost(lhs) + estimate_expr_cost(rhs),
        SmtExpr::Eq { lhs, rhs }
        | SmtExpr::BvSlt { lhs, rhs, .. }
        | SmtExpr::BvSge { lhs, rhs, .. }
        | SmtExpr::BvSgt { lhs, rhs, .. }
        ...
        | SmtExpr::FPSub { lhs, rhs, .. }
        | SmtExpr::FPMul { lhs, rhs, .. }
        | SmtExpr::FPDiv { lhs, rhs, .. } => 3 + estimate_expr_cost(lhs) + estimate_expr_cost(rhs),
        SmtExpr::FPFma { a, b, c, .. } => {
        ...
        | SmtExpr::FPIsInf { operand }
        | SmtExpr::FPIsZero { operand }
        | SmtExpr::FPIsNormal { operand } => 1 + estimate_expr_cost(operand),
        SmtExpr::FPSqrt { operand, .. } | SmtExpr::FPRoundToIntegral { operand, .. } => {
        ...
        | SmtExpr::BvToFP { operand, .. }
        | SmtExpr::FPToFP { operand, .. }
        | SmtExpr::BvBitsToFP { operand, .. } => 2 + estimate_expr_cost(operand),
        SmtExpr::Extract { operand, .. }
        | SmtExpr::ZeroExtend { operand, .. }
        | SmtExpr::SignExtend { operand, .. } => 1 + estimate_expr_cost(operand),
        SmtExpr::Concat { hi, lo, .. } => 1 + estimate_expr_cost(hi) + estimate_expr_cost(lo),
        SmtExpr::TrapIfZero { guard, value, .. } => {
            1 + estimate_expr_cost(guard) + estimate_expr_cost(value)
        }
        SmtExpr::Ite {
            cond,
            then_expr,
        ...
        SmtExpr::Store {
            ...
            index,
            value,
        } => 2 + estimate_expr_cost(array) + estimate_expr_cost(index) + estimate_expr_cost(value),
        SmtExpr::ConstArray { value, .. } => estimate_expr_cost(value),
        // A memory load is a memory-access op (a UF read) plus its address cost.
        SmtExpr::MemLoad { addr, .. } => 2 + estimate_expr_cost(addr),
        SmtExpr::UF { args, .. } => 2 + args.iter().map(estimate_expr_cost).sum::<i32>(),
        SmtExpr::FPConst { .. } => 0,
        SmtExpr::UFDecl { .. } => 0,
        SmtExpr::ForAll {
            lower, upper, body, ..
...
mod tests {
    ...
    fn test_proposal_with_precondition() {
        ...
        .with_precondition(SmtExpr::var("y", 8).eq_expr(SmtExpr::bv_const(2, 8)));
    ...
    fn test_discover_double_negation() {
        ...
        let proposal =
            RuleProposal::new(SmtExpr::var("x", 8).bvneg().bvneg(), SmtExpr::var("x", 8))
    ...
    fn test_reject_neg_identity() {
        ...
        let proposal = RuleProposal::new(SmtExpr::var("x", 8).bvneg(), SmtExpr::var("x", 8))
