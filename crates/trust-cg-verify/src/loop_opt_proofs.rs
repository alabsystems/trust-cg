// trust-cg-verify/loop_opt_proofs.rs - SMT proofs for loop optimization correctness
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// Proves that loop unrolling and strength reduction transforms in trust-cg-opt
...
//
// Reference: crates/trust-cg-opt/src/loop_unroll.rs
// Reference: crates/trust-cg-opt/src/strength_reduce.rs
...
pub fn proof_unroll_preserves_iteration_count() -> ProofObligation {
    ...
    let trust_ir = encode_unrolled_sum(base.clone(), stride.clone(), 4, width);
...
pub fn proof_unroll_preserves_body_semantics() -> ProofObligation {
    ...
    let trust_ir = encode_body(iv_at_2.clone(), constant.clone());
...
pub fn proof_unroll_iv_final_value() -> ProofObligation {
    ...
    let trust_ir = encode_final_iv(init.clone(), 4, step.clone(), width);
...
pub fn proof_unroll_boundary_count_1() -> ProofObligation {
    ...
    let trust_ir = encode_unrolled_sum(base.clone(), stride.clone(), 1, width);
...
///
/// We prove this for concrete iteration counts 0..4 (matching TCG_MAX_UNROLL_TRIP_COUNT)
...
pub fn proof_strength_reduce_mul_to_add() -> ProofObligation {
    ...
    let trust_ir = three.bvmul(stride.clone());
...
pub fn proof_strength_reduce_iv_update() -> ProofObligation {
    ...
    let trust_ir = iv.clone().bvadd(stride.clone());
...
pub fn proof_strength_reduce_overflow_safety() -> ProofObligation {
    ...
    let trust_ir = four.bvmul(stride.clone());
    // Repeated addition: stride + stride + stride + stride
    let aarch64 = encode_add_chain(SmtExpr::bv_const(0, width), stride, 4);

    ProofObligation {
        machine_side_provenance: crate::lowering_proof::MachineSideProvenance::StaticDb,
        name: "StrengthReduce: 4 * stride == stride + stride + stride + stride (overflow safety)"
            .to_string(),
        trust_ir_expr: trust_ir,
        aarch64_expr: aarch64,
        inputs: vec![("stride".to_string(), width)],
...
pub fn proof_strength_reduce_base_case() -> ProofObligation {
    ...
    let trust_ir = base.clone().bvadd(zero.bvmul(stride));
...
/// Retracted degenerate X==X LoopUnroll/LoopOpt obligations (count=0/1/identity/unrolled-body/sum, dead-IV-elim — X==X). Genuine StrengthReduce k*stride==repeated-add and final-IV proofs remain.
const LOOP_RETRACTED_DEGENERATE: &[&str] = &[
    "LoopOpt: dead IV elimination preserves live values",
    "LoopOpt: dead IV elimination preserves live values (8-bit)",
    "LoopUnroll: count=0 eliminates loop (result = base)",
    "LoopUnroll: count=0 eliminates loop (result = base) (8-bit)",
    "LoopUnroll: count=1 is identity",
    "LoopUnroll: count=1 is identity (8-bit)",
    "LoopUnroll: unrolled body(iv, c) == original body(iv, c)",
    "LoopUnroll: unrolled body(iv, c) == original body(iv, c) (8-bit)",
    "LoopUnroll: unrolled sum (N=4) == loop sum (N=4)",
    "LoopUnroll: unrolled sum (N=4) == loop sum (N=4) (8-bit)",
    // The misnamed "StrengthReduce: iv + stride" IV-update-step proof lives in
    // this LoopOptimization module (not the StrengthReduction category); it is a
    // degenerate X==X (iv+stride == iv+stride). The GENUINE strength-reduction
    // value proofs (k*stride == repeated-add, base+0*stride == base) remain.
    "StrengthReduce: iv + stride == iv + stride (IV update step)",
    "StrengthReduce: iv + stride == iv + stride (IV update step) (8-bit)",
];
...
pub fn all_loop_opt_proofs() -> Vec<ProofObligation> {
    ...
    .into_iter()
    // #62 retraction: drop degenerate X==X self-equalities (see LOOP_RETRACTED_DEGENERATE).
    .filter(|p| !LOOP_RETRACTED_DEGENERATE.contains(&p.name.as_str()))
    .collect()
}
...
mod tests {
    ...
    fn test_all_loop_opt_proofs_count() {
        ...
        assert_eq!(
            proofs.len(),
            10,
            "expected 10 loop opt proofs (12 degenerate LoopUnroll/LoopOpt/iv+stride \
             X==X retracted in #62; genuine StrengthReduce k*stride + final-IV remain), got {}",
            proofs.len()
        );
    }
