// trust-cg-verify/addr_mode_proofs.rs - SMT proofs for address mode formation
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// Proves that each address mode formation transformation in
// trust-cg-opt/addr_mode.rs preserves the effective address computation.
//
...
//
// Reference: crates/trust-cg-opt/src/addr_mode.rs
// Reference: ARM Architecture Reference Manual (DDI 0487), C6.2

//! SMT proofs for AArch64 address mode formation correctness.
//!
//! Each proof corresponds to a transformation in `trust-cg-opt::addr_mode`:
//!
...
pub fn proof_base_plus_reg_effective_addr() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: before_addr,
        aarch64_expr: after_addr,
        inputs: vec![("base".to_string(), width), ("index".to_string(), width)],
        preconditions: vec![],
        fp_inputs: vec![],
        category: Some(crate::lowering_proof::TransvalCheckKind::InstructionLowering),
    }
...
pub fn proof_base_plus_imm_effective_addr_w32() -> ProofObligation {
    ...
    let before_addr = base
        .clone()
        .bvadd(imm.clone())
        .bvadd(SmtExpr::bv_const(0, width));
    let after_addr = base.bvadd(imm);
...
pub fn proof_scaled_offset_range_1b() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: SmtExpr::ite(
            hw_encodable.clone(),
            SmtExpr::bv_const(1, 1),
            SmtExpr::bv_const(0, 1),
        ),
        aarch64_expr: SmtExpr::ite(
            sw_encodable,
            SmtExpr::bv_const(1, 1),
            SmtExpr::bv_const(0, 1),
        ),
...
pub fn proof_scaled_offset_range_2b() -> ProofObligation {
    ...
    let in_range = offset
        .clone()
        .bvlshr(SmtExpr::bv_const(1, width))
        .bvule(max_imm12);
...
pub fn proof_scaled_offset_range_4b() -> ProofObligation {
    ...
    let in_range = offset
        .clone()
        .bvlshr(SmtExpr::bv_const(2, width))
        .bvule(max_imm12);
    let hw_encodable = aligned.and_expr(in_range);
...
pub fn proof_scaled_offset_range_8b() -> ProofObligation {
    ...
    let in_range = offset
        .clone()
        .bvlshr(SmtExpr::bv_const(3, width))
        .bvule(max_imm12);
    let hw_encodable = aligned.and_expr(in_range);
...
pub fn proof_unscaled_signed_9bit_range() -> ProofObligation {
    ...
    ProofObligation {
        ...
        trust_ir_expr: SmtExpr::ite(hw_check, SmtExpr::bv_const(1, 1), SmtExpr::bv_const(0, 1)),
...
pub fn all_addr_mode_proofs() -> Vec<ProofObligation> {
    ...
    .into_iter()
    // #62 retraction: the pre/post-index writeback and scaled/unscaled
    // offset-RANGE obligations are degenerate X==X (the writeback/range value is
    // restated identically on both sides; no independent address-mode encoder).
    // The GENUINE effective-address fold proofs (base+imm, base+reg, base+imm
    // combined, scaled 1-byte) remain.
    .filter(|p| !ADDRMODE_RETRACTED_DEGENERATE.contains(&p.name.as_str()))
    .collect()
}

const ADDRMODE_RETRACTED_DEGENERATE: &[&str] = &[
    "AddrMode: post-index effective addr is original base",
    "AddrMode: post-index writeback correctness",
    "AddrMode: pre-index writeback correctness",
    "AddrMode: pre/post-index same writeback value",
    "AddrMode: scaled offset range 2-byte access",
    "AddrMode: scaled offset range 4-byte access",
    "AddrMode: scaled offset range 8-byte access",
    "AddrMode: unscaled signed 9-bit offset range",
];
...
mod tests {
    ...
    fn test_all_addr_mode_proofs() {
        ...
        assert_eq!(
            proofs.len(),
            10,
            "Expected 10 address mode proofs (8 degenerate writeback/range proofs \
             retracted in #62), got {}",
            proofs.len()
        );
    ...
    fn test_wrong_base_plus_reg_nonzero_offset_detected() {
        ...
        let before = base
            .clone()
            .bvadd(index.clone())
            .bvadd(SmtExpr::bv_const(8, width));
    ...
    fn test_smt2_output_base_plus_imm() {
        ...
        assert!(
            smt2.contains("(declare-const imm (_ BitVec 64))"),
            "should declare imm"
        );
    ...
    fn test_smt2_output_base_plus_reg() {
        ...
        assert!(
            smt2.contains("(declare-const index (_ BitVec 64))"),
            "should declare index"
        );
        assert!(smt2.contains("(check-sat)"), "should contain check-sat");
    }
    ...
    fn test_smt2_output_scaled_offset() {
        ...
        assert!(smt2.contains("(set-logic"), "should contain set-logic");
        assert!(
            smt2.contains("(declare-const offset (_ BitVec 16))"),
            "should declare offset"
        );
        assert!(smt2.contains("(check-sat)"), "should contain check-sat");
    }
