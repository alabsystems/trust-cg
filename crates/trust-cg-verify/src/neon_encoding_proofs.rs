// trust-cg-verify/neon_encoding_proofs.rs - NEON encoding correctness proofs
//
...
// trust_ir vector ops -> NEON) by verifying NEON instruction semantics at the
...
//! proving "trust_ir op == NEON op", they prove "NEON encoding semantics match the
//! expected lane-decomposed behavior". This validates the encoding layer.
//!
//! ## Coverage
//!
//! 13 proof categories, 29 total proof obligations:
...
//! - Horizontal reduction: CMEQ.4S + UMAXV any/nonzero (1 proof)
...
use crate::neon_semantics::{
    ...
    encode_neon_sub, encode_neon_umaxv_4s,
};
use crate::smt::{SmtExpr, VectorArrangement, concat_lanes, lane_extract, map_lanes_binary};
...
fn unary_bitwise_inputs(width: u32) -> Vec<(String, u32)> {
    if width <= 64 {
        ...
        vec![("vn_lo".to_string(), 64), ("vn_hi".to_string(), 64)]
    }
}
...
fn proof_add_lane_decomposition(arrangement: VectorArrangement, label: &str) -> ProofObligation {
...
fn proof_sub_lane_decomposition(arrangement: VectorArrangement, label: &str) -> ProofObligation {
    let vn = symbolic_vector("vn", arrangement);
    let vm = symbolic_vector("vm", arrangement);
...
fn proof_mul_lane_decomposition(arrangement: VectorArrangement, label: &str) -> ProofObligation {
    let vn = symbolic_vector("vn", arrangement);
    let vm = symbolic_vector("vm", arrangement);
...
///
/// trust_ir side: `bvxor(vn, all_ones)` (bitwise inversion).
...
///
/// trust_ir side: manually construct vector with all lanes equal to the scalar.
...
///
/// trust_ir side: decompose vector into lanes, replace lane `idx`, reassemble.
...
fn proof_ins_lane(arrangement: VectorArrangement, idx: u32, label: &str) -> ProofObligation {
...
///
/// trust_ir side: for each lane, `if vn[i] == vm[i] then all_ones else 0`.
/// NEON side: `encode_neon_cmeq(arrangement, vn, vm)`.
fn proof_cmeq_comparison(arrangement: VectorArrangement, label: &str) -> ProofObligation {
    let vn = symbolic_vector("vn", arrangement);
    let vm = symbolic_vector("vm", arrangement);
...
// ---------------------------------------------------------------------------
// 11. CMEQ.4S + UMAXV horizontal-any proofs
// ---------------------------------------------------------------------------

/// Proof: `UMAXV` over a `CMEQ.4S` mask implements i32 horizontal any.
///
/// Scalar side: `0xFFFF_FFFF` iff any corresponding i32 lane is equal,
/// otherwise `0`.
/// NEON side: `CMEQ.4S` builds all-ones/zero lane masks, then `UMAXV.4S`
/// reduces the mask by unsigned maximum.
pub fn proof_cmeq_4s_umaxv_horizontal_any() -> ProofObligation {
    let vn = symbolic_vector("vn", VectorArrangement::S4);
    let vm = symbolic_vector("vm", VectorArrangement::S4);

    let any_lane_equal = (0..VectorArrangement::S4.lane_count())
        .map(|idx| {
            lane_extract(&vn, VectorArrangement::S4, idx).eq_expr(lane_extract(
                &vm,
                VectorArrangement::S4,
                idx,
            ))
        })
        .reduce(|acc, lane_eq| acc.or_expr(lane_eq))
        .expect("S4 has at least one lane");
    let expected = SmtExpr::ite(
        any_lane_equal,
        SmtExpr::bv_const(u32::MAX as u64, 32),
        SmtExpr::bv_const(0, 32),
    );

    let cmp_mask = encode_neon_cmeq(VectorArrangement::S4, &vn, &vm);
    let actual = encode_neon_umaxv_4s(&cmp_mask);

    let mut inputs = vector_inputs("vn", VectorArrangement::S4);
    inputs.extend(vector_inputs("vm", VectorArrangement::S4));

    ProofObligation {
        machine_side_provenance: crate::lowering_proof::MachineSideProvenance::StaticDb,
        name: "NeonEncoding: CMEQ.4S + UMAXV horizontal any".to_string(),
        trust_ir_expr: expected,
        aarch64_expr: actual,
        inputs,
        preconditions: vec![],
...
// ---------------------------------------------------------------------------
// 12. MOVI immediate proofs
// ---------------------------------------------------------------------------

/// Proof: MOVI sets all bytes to the immediate value.
///
/// trust_ir side: construct a bitvector with all bytes equal to `imm`.
...
fn proof_dup_lane_consistency(arrangement: VectorArrangement, label: &str) -> ProofObligation {
    let lane_bits = arrangement.lane_bits();
    let scalar = SmtExpr::var("scalar", lane_bits);
    let dup_result = encode_neon_dup(arrangement, &scalar);

    let lane0 = lane_extract(&dup_result, arrangement, 0);
    let last_lane = lane_extract(&dup_result, arrangement, arrangement.lane_count() - 1);
...
/// Return all 29 NEON encoding proof obligations.
///
/// 13 categories:
...
///   - Comparison: CMEQ (2) = 2
///   - Horizontal reduction: CMEQ.4S + UMAXV any/nonzero (1) = 1
///   - Memory: LD1/ST1 (2) = 2
...
pub fn all_neon_encoding_proofs() -> Vec<ProofObligation> {
    vec![
        ...
        proof_cmeq_4s(),
        proof_cmeq_16b(),
        // Horizontal reduction (1 proof)
        proof_cmeq_4s_umaxv_horizontal_any(),
...
mod tests {
    ...
    // =======================================================================
    // CMEQ.4S + UMAXV horizontal any
    // =======================================================================

    #[test]
    fn test_cmeq_4s_umaxv_horizontal_any() {
        assert_valid(&proof_cmeq_4s_umaxv_horizontal_any());
    }
    ...
    // =======================================================================
    // Aggregate: all 29 proofs
    // =======================================================================

    #[test]
    fn test_all_neon_encoding_proofs() {
        let proofs = all_neon_encoding_proofs();
        assert_eq!(proofs.len(), 29, "expected 29 NEON encoding proofs");
    ...
    fn test_wrong_encoding_detected() {
        ...
        let obligation = ProofObligation {
            ...
            trust_ir_expr: map_lanes_binary(&vn, &vm, arrangement, |a, b| a.bvsub(b)),
        ...
        match result {
            ...
            other => panic!("Expected Invalid for wrong encoding, got {:?}", other),
        }
    }
}
