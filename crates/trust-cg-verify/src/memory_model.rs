// trust-cg-verify/memory_model.rs - SMT memory model for load/store verification
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// Models byte-addressable memory as Array(BitVec64, BitVec8) for verification
// of trust_ir load/store lowering to AArch64 LDR/STR instructions.
//
...
//!
//! This module verifies that trust_ir memory operations lower correctly to
//! AArch64 load/store instructions. The key properties verified:
//!
//! 1. **Load equivalence**: `trust_ir::Load(ty, addr)` == `LDR Rt, [Xn, #imm]`
//! 2. **Store equivalence**: `trust_ir::Store(val, addr)` == `STR Rt, [Xn, #imm]`
...
/// operations (store followed by load) between trust_ir and AArch64 semantics.
///
...
pub enum MemoryProofKind {
    /// Verify that trust_ir load semantics match AArch64 LDR semantics.
    ///
    /// Both sides: load `access_bytes` from `base + offset` in memory.
    /// The trust_ir side uses unscaled byte offset.
    ...
    /// Verify that trust_ir store semantics match AArch64 STR semantics.
    ///
...
///
/// This models trust_ir's address computation where the offset is in bytes.
...
// ---------------------------------------------------------------------------
// trust_ir memory operation semantics
// ---------------------------------------------------------------------------

/// Encode trust_ir Load semantics: read `size_bytes` from `addr` in memory.
///
/// `trust_ir::Load(ty, addr)` reads `ty.bytes()` bytes starting at `addr`
/// in little-endian order and returns the assembled value.
pub fn trust_ir_load(mem: &SmtMemory, addr: u64, size_bytes: u32) -> u64 {
    mem.load_le(addr, size_bytes)
}

/// Encode trust_ir Store semantics: write `value` at `addr` in memory.
///
/// `trust_ir::Store(value, addr)` writes `size_bytes` bytes of `value`
/// starting at `addr` in little-endian order.
pub fn trust_ir_store(mem: &mut SmtMemory, addr: u64, value: u64, size_bytes: u32) {
...
pub fn aarch64_ldr_imm(mem: &SmtMemory, base: u64, scaled_imm: u64, size_bytes: u32) -> u64 {
...
pub fn aarch64_ldp(mem: &SmtMemory, base: u64, scaled_imm: u64, size_bytes: u32) -> (u64, u64) {
    let addr = effective_address_scaled(base, scaled_imm, size_bytes);
...
fn check_memory_point(
    ...
    match &obligation.kind {
        MemoryProofKind::LoadEquivalence { scaled_offset } => {
            ...
            // trust_ir: load from base + byte_offset
            let trust_ir_result = trust_ir_load(&mem, addr, size);

            // AArch64: LDR [base, #scaled_offset] (auto-scales)
            let aarch64_result = aarch64_ldr_imm(&mem, base, *scaled_offset, size);

            if mask(trust_ir_result, size * 8) != mask(aarch64_result, size * 8) {
                return Some(format!(
                    "LoadEq: base=0x{:x}, offset={}, value=0x{:x}, trust_ir=0x{:x}, aarch64=0x{:x}",
                    base, scaled_offset, value, trust_ir_result, aarch64_result
                ));
            }
        }
        ...
        MemoryProofKind::StoreEquivalence { scaled_offset } => {
            ...
            // trust_ir: store value at base + byte_offset
            let mut trust_ir_mem = SmtMemory::new(mem_seed);
            let addr = effective_address(base, byte_offset);
            trust_ir_store(&mut trust_ir_mem, addr, value, size);
            ...
            for i in 0..size {
                let t = trust_ir_mem.read_byte(addr.wrapping_add(i as u64));
                let a = aarch64_mem.read_byte(addr.wrapping_add(i as u64));
                if t != a {
                    return Some(format!(
                        "StoreEq: base=0x{:x}, offset={}, value=0x{:x}, byte[{}]: trust_ir=0x{:02x}, aarch64=0x{:02x}",
        ...
        MemoryProofKind::StoreLoadRoundtrip => {
            ...
            trust_ir_store(&mut mem, base, masked_value, size);
            let loaded = trust_ir_load(&mem, base, size);
        ...
        MemoryProofKind::StoreLoadDifferentAddr => {
            ...
            // Store at addr_a
            trust_ir_store(&mut mem, addr_a, value, size);
        ...
        MemoryProofKind::LoadPair { scaled_offset } => {
            ...
            // trust_ir: two separate loads
            let trust_ir_r1 = trust_ir_load(&mem, addr, size);
            let trust_ir_r2 = trust_ir_load(&mem, addr.wrapping_add(size as u64), size);

            // AArch64: LDP
            let (aarch64_r1, aarch64_r2) = aarch64_ldp(&mem, base, *scaled_offset, size);

            if mask(trust_ir_r1, size * 8) != mask(aarch64_r1, size * 8)
                || mask(trust_ir_r2, size * 8) != mask(aarch64_r2, size * 8)
            {
                return Some(format!(
                    "LDP: base=0x{:x}, offset={}, trust_ir=({:x},{:x}), aarch64=({:x},{:x})",
                    base, scaled_offset, trust_ir_r1, trust_ir_r2, aarch64_r1, aarch64_r2
                ));
            }
        }
...
/// Proof: `trust_ir::Load(I32, addr)` == `LDRWui [Xn, #0]` (zero offset).
///
...
/// Proof: `trust_ir::Load(I32, addr+16)` == `LDRWui [Xn, #4]` (offset=4, scale=4).
///
...
/// Proof: `trust_ir::Load(I64, addr)` == `LDRXui [Xn, #0]`.
...
/// Proof: `trust_ir::Load(I64, addr+24)` == `LDRXui [Xn, #3]` (offset=3, scale=8).
...
/// Proof: `trust_ir::Store(I32, val, addr+8)` == `STRWui [Xn, #2]` (offset=2, scale=4).
...
/// Proof: two adjacent trust_ir loads == AArch64 LDP (load pair, 64-bit).
///
...
/// Proof: two adjacent trust_ir loads == AArch64 LDP (load pair, 32-bit).
...
mod tests {
    ...
    /// Verify that trust_ir and AArch64 store produce identical byte patterns.
    #[test]
    fn test_trust_ir_aarch64_store_byte_pattern() {
        let value: u64 = 0xCAFE_BABE;
        let base: u64 = 0x2000;

        let mut trust_ir_mem = SmtMemory::new(0);
        trust_ir_store(&mut trust_ir_mem, base, value, 4);
