// trust-cg-verify/tests/panic_fuzz_verify.rs
...
//
// Run:
//   cargo test -p trust-cg-verify --test panic_fuzz_verify
// Increase case count via env:
//   PROPTEST_CASES=100000 cargo test -p trust-cg-verify --test panic_fuzz_verify

use proptest::prelude::*;
use trust_cg_ir::function::{MachBlock, MachFunction, Signature, Type};
use trust_cg_ir::inst::{AArch64Opcode, MachInst};
use trust_cg_ir::operand::MachOperand;
use trust_cg_ir::regs::{
    D0, D1, D2, D3, PReg, RegClass, S0, S1, S2, S3, SP, SpecialReg, VReg, W0, W1, W2, W3, W4, W5,
    W6, W7, X0, X1, X2, X3, X4, X5, X6, X7,
};
use trust_cg_ir::types::{BlockId, InstId};
use trust_cg_verify::function_verifier::{FunctionVerifier, verify_function};
...
fn opcode_strategy() -> impl Strategy<Value = AArch64Opcode> {
    ...
    prop_oneof![
        ...
        Just(FmulRR),
        Just(FnegRR),
        // Logical / shifts (bound to the general BitwiseShift proofs)
        Just(AndRR),
        Just(AndRI),
        Just(OrrRR),
        Just(OrrRI),
...
fn func_spec_strategy() -> impl Strategy<Value = FuncSpec> {
    (
        ...
        .prop_flat_map(|(insts, num_blocks, name, num_params, num_returns)| {
            ...
            prop::collection::vec(per_block, num_blocks..=num_blocks).prop_map(move |blocks| {
                FuncSpec {
                    insts: insts.clone(),
                    blocks,
                    name: name.clone(),
