#!/usr/bin/env bash
# gen_x86_rosetta_truth.sh — REGENERATOR for x86_64_rosetta_truth.json.
#
# Author: Andrew Yates <andrewyates.name@gmail.com>
# Copyright 2026 Andrew Yates | License: Apache-2.0
#
# Builds the Rosetta x86 oracle harness (gen_x86_rosetta_truth.c) as an x86-64
# Mach-O with `clang -arch x86_64`, runs it under `arch -x86_64` (Rosetta 2 — an
# INDEPENDENT x86 implementation, NOT a second in-house model), and writes the
# committed fixture x86_64_rosetta_truth.json. The harness forks a child for each
# trapping IDIV/DIV so a #DE (SIGFPE) is recorded as result="trap" without dying.
#
...
#
# Usage:
#   crates/trust-cg-verify/tests/fixtures/gen_x86_rosetta_truth.sh
# (run from anywhere; paths are resolved relative to this script).
set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SRC="$HERE/gen_x86_rosetta_truth.c"
BIN="$HERE/gen_x86_rosetta_truth.bin"
OUT="$HERE/x86_64_rosetta_truth.json"
...
clang -arch x86_64 -O0 -Wall -Wextra -o "$BIN" "$SRC"
file "$BIN" >&2
