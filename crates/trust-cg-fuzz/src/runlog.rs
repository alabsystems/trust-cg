// trust-cg-fuzz/src/runlog.rs - JSON run log schema
//
...
pub struct Repro {
    ...
    pub seed: u64,
    /// Path to the minimized input (trust_ir json for trust-ir-gen, .c file for
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RunLog {
    /// Driver name: "trust-ir-gen" | "csmith-driver" | "yarpgen-driver".
    pub driver: String,
    /// "ok" | "unavailable".
    pub status: String,
    ...
    pub crashes: u64,
    /// Runs where the oracle and Trust Codegen disagreed on the result — actual
    /// miscompiles.
    pub miscompiles: u64,
