// trust-cg-fuzz/src/bin/csmith_driver.rs - csmith differential driver
//
...
// and pipes the `.ll` through `trust-cg-ws2-import` into a Mach-O object.
...
// (csmith, clang, or trust-cg-ws2-import). Once those exist we actually run.
//
// CLI:
//   csmith-driver --duration SECS --out DIR [--seed-start N]
//                 [--trust-cg-ws2-import PATH] [--clang PATH]
...
fn resolve_importer(candidate: &str) -> Option<String> {
    ...
    if let Some(file_name) = Path::new(candidate).file_name().and_then(|s| s.to_str())
        && command_works(file_name, "--help", false)
    {
...
fn resolve_clang(candidate: &str) -> Option<String> {
    ...
    if let Some(file_name) = Path::new(candidate).file_name().and_then(|s| s.to_str())
        && file_name != candidate
        && command_works(file_name, "--version", true)
    {
        return Some(file_name.to_string());
    }
    None
...
fn save_repro_source(out_dir: &Path, driver: &str, seed: u64, source: &Path) -> Option<String> {
    if !source.exists() {
        return None;
    }
...
fn push_repro(
    ...
    let minimized_input_path =
        source.and_then(|path| save_repro_source(out_dir, driver, seed, path));
...
fn main() -> Result<(), String> {
    ...
    let importer_arg: String = parse_arg(
        &args,
        "--trust-cg-ws2-import",
        "./target/release/trust-cg-ws2-import".to_string(),
    )?;
    ...
    let importer = match resolve_importer(&importer_arg) {
        ...
        None => {
            log.reason = Some(
                "trust-cg-ws2-import not found. Build it with: `cargo build --release -p trust-cg-llvm-import --bin trust-cg-ws2-import --features driver`."
                    .to_string(),
            );
            log.finished_at = iso_now();
            write_log(&out_path, "csmith-driver.json", &log);
            eprintln!("[csmith-driver] unavailable: trust-cg-ws2-import missing");
            return Ok(());
        }
    };
    ...
    let clang = match resolve_clang(&clang_arg) {
        ...
        None => {
            log.reason =
                Some("clang --version failed. Install clang and ensure it is on PATH.".to_string());
            log.finished_at = iso_now();
            write_log(&out_path, "csmith-driver.json", &log);
    ...
    let temp_root = env::temp_dir().join(format!("trust-cg-fuzz-csmith-{}", std::process::id()));
    ...
    while Instant::now() < deadline {
        ...
        let import_output = match Command::new(&importer).arg(&prog_ll).arg(&prog_o).output() {
            Ok(output) => output,
            Err(err) => {
                log.crashes += 1;
                push_repro(
