// trust-cg-fuzz/src/bin/yarpgen_driver.rs - YARPGen differential driver
//
...
// `.ll` through `trust-cg-ws2-import` into a Mach-O object.
...
fn main() -> Result<(), String> {
    ...
    let importer = match resolve_importer(&importer_arg) {
        ...
        None => {
            ...
            log.finished_at = iso_now();
            write_log(&out_path, "yarpgen-driver.json", &log);
            eprintln!("[yarpgen-driver] unavailable: trust-cg-ws2-import missing");
            return Ok(());
        }
    };
    ...
    let temp_root = env::temp_dir().join(format!("trust-cg-fuzz-yarpgen-{}", std::process::id()));
    let mut seed = 1_u64;
    let mut iter = 0_u64;
