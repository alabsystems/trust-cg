// trust-cg-jit-matrix/tests/bcp_watched_literal_chunked_trust_ir.rs
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// Integration tests for the CHUNKED-layout watched-literal BCP kernel
// authored in `bcp_module_builder::build_bcp_propagate_watched_literal_chunked_module`.
//
// The chunked kernel runs the same two-watched-literal algorithm as
// `build_bcp_propagate_watched_literal_module`, but uses a linked-list
// watch table (`watch_heads[lit_index] -> watch_nodes pool`) sized
// `O(num_vars + num_clauses)` instead of the fixed-capacity row-major
// `(2*num_vars + 2) * num_clauses` table the NN-era kernel uses. This
// test file mirrors `bcp_watched_literal_trust_ir.rs` so the two
// kernels are observably equivalent on `result` + final `values`.
...
use trust_cg_jit_matrix::jit_bcp_kernel::JitBcpWatchedLiteralChunkedKernelProvider;
...
/// Run the chunked-layout JIT'd watched-literal kernel and return the
/// result plus the final values table (1..=num_vars). Mirrors the
/// helper of the same name in `bcp_watched_literal_trust_ir.rs`.
fn run_jit(num_vars: usize, clauses: &[Vec<i32>], input: &[u32]) -> (u32, Vec<(usize, i8)>) {
    let provider =
        JitBcpWatchedLiteralChunkedKernelProvider::compile(num_vars, clauses.to_vec(), input.len())
            .expect("JIT compile of chunked watched-literal BCP kernel");
...
/// Native reference: same algorithm via `BcpState`.
...
fn binary_conflict_full_assignment() {
    // Same shape as the fixed-cap kernel's `binary_conflict_full_assignment`:
...
/// Run the chunked JIT with a caller-supplied implied-literals buffer
/// and return the resulting `KernelStatus`.
...
/// (and optionally a clause-id translation table).
...
/// Memory delta: the chunked layout's watch infrastructure should be
/// O(num_vars + num_clauses), not O(num_vars * num_clauses).
#[test]
fn chunked_watch_memory_is_proportional_to_num_clauses() {
    use trust_cg_jit_matrix::bcp_baseline::random_3sat;
    let num_vars = 100;
    let num_clauses = 430;
    let clauses = random_3sat(num_vars, num_clauses, 0xDEC1A);
    let provider =
        JitBcpWatchedLiteralChunkedKernelProvider::compile(num_vars, clauses, 8).unwrap();
    let bytes = provider.watch_memory_bytes();
    // heads = (2*num_vars + 2) * 4 = 808
    // nodes = (2*num_clauses + 1) * 8 = 6888
    // total = 7696. The fixed-cap layout would be ~345 KB.
    let expected_heads = (2 * num_vars + 2) * 4;
    let expected_nodes = (2 * num_clauses + 1) * 8;
    let expected = expected_heads + expected_nodes;
    assert_eq!(bytes, expected);
    // Sanity bound: fixed-cap would be (2*num_vars + 2) * num_clauses * 4
    let fixed_cap_bytes = (2 * num_vars + 2) * num_clauses * 4;
    assert!(
        bytes < fixed_cap_bytes / 40,
        "chunked should be >>40x smaller than fixed-cap on this shape; got chunked={} fixed_cap={}",
        bytes,
        fixed_cap_bytes,
    );
}
