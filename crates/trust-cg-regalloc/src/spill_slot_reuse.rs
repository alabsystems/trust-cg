// trust-cg-regalloc/spill_slot_reuse.rs - Spill slot reuse optimization
//
...
pub struct SpillSlotReuseResult {
    ...
    pub slot_rewrites: BTreeMap<StackSlotId, StackSlotId>,
}
...
pub fn compute_spill_slot_reuse(
    ...
    let mut result = SpillSlotReuseResult {
        slots_eliminated: 0,
        slot_rewrites: BTreeMap::new(),
    };

    // Group spills by register class (same class = same slot size).
    let mut groups: BTreeMap<RegClass, Vec<(VReg, StackSlotId)>> = BTreeMap::new();
    ...
    for spills in groups.values() {
        ...
        let mut colors: Vec<(StackSlotId, Vec<VReg>)> = Vec::new();

        for &(vreg, original_slot) in spills {
            let vreg_interval = match exact_interval_for_vreg(intervals, vreg) {
                Some(iv) => iv,
                None => {
                    // No interval data — can't share.
                    colors.push((original_slot, vec![vreg]));
                    continue;
                }
            };
            ...
            for (color_slot, color_vregs) in &mut colors {
                let conflicts = color_vregs.iter().any(|&existing_vreg| {
                    if let Some(existing_interval) =
                        exact_interval_for_vreg(intervals, existing_vreg)
                    {
...
fn exact_interval_for_vreg(
    intervals: &BTreeMap<u32, LiveInterval>,
    vreg: VReg,
) -> Option<&LiveInterval> {
    intervals
        .get(&vreg.id)
        .filter(|interval| interval.vreg == vreg)
}
...
pub fn apply_spill_slot_reuse(
    ...
    rewrites: &BTreeMap<StackSlotId, StackSlotId>,
) {
    ...
    prune_rewritten_stack_slots(func, rewrites);
}

fn prune_rewritten_stack_slots(
    func: &mut MachFunction,
    rewrites: &BTreeMap<StackSlotId, StackSlotId>,
) {
    let referenced_slots = referenced_stack_slots(func);
    for slot in rewrites.keys() {
...
fn referenced_stack_slots(func: &MachFunction) -> BTreeSet<StackSlotId> {
    func.insts
        .iter()
        .flat_map(|inst| inst.defs.iter().chain(inst.uses.iter()))
        .filter_map(|op| match op {
            crate::machine_types::MachOperand::StackSlot(slot) => Some(*slot),
            _ => None,
        })
        .collect()
}
...
mod tests {
    ...
    fn test_non_overlapping_share_slot() {
        ...
        let intervals =
            BTreeMap::from([(0, interval(0, &[(0, 5)])), (1, interval(1, &[(10, 15)]))]);

        let result = compute_spill_slot_reuse(&spills, &intervals);
        assert_eq!(result.slots_eliminated, 1);
        assert_eq!(
            result.slot_rewrites.get(&StackSlotId(1)),
            Some(&StackSlotId(0))
        );
    }
    ...
    fn test_overlapping_need_separate_slots() {
        ...
        let intervals =
            BTreeMap::from([(0, interval(0, &[(0, 10)])), (1, interval(1, &[(5, 15)]))]);
    ...
    fn test_empty_spills() {
        let result = compute_spill_slot_reuse(&[], &BTreeMap::new());
        assert_eq!(result.slots_eliminated, 0);
        assert!(result.slot_rewrites.is_empty());
    }
    ...
    fn test_single_spill_no_reuse() {
        ...
        let intervals = BTreeMap::from([(0, interval(0, &[(0, 10)]))]);
        let result = compute_spill_slot_reuse(&spills, &intervals);
        assert_eq!(result.slots_eliminated, 0);
    }
    ...
    fn test_adjacent_intervals_share_slot() {
        ...
        let intervals = BTreeMap::from([(0, interval(0, &[(0, 5)])), (1, interval(1, &[(5, 10)]))]);
        let result = compute_spill_slot_reuse(&spills, &intervals);
        assert_eq!(result.slots_eliminated, 1);
    }
    ...
    fn test_missing_interval_conservative() {
        ...
        let intervals = BTreeMap::from([(0, interval(0, &[(0, 5)]))]);
        let result = compute_spill_slot_reuse(&spills, &intervals);
    ...
    #[test]
    fn test_same_id_different_class_interval_is_conservative() {
        // A numeric-id match from another register class is not interval data
        // for this spilled vreg. Treat it as missing and avoid slot sharing.
        let gpr_v0 = VReg {
            id: 0,
            class: RegClass::Gpr64,
        };
        ...
        let mut fpr_interval = LiveInterval::new(fpr_v0);
        fpr_interval.add_range(0, 5);
        let mut gpr_interval = LiveInterval::new(gpr_v1);
        gpr_interval.add_range(10, 15);
        let intervals = BTreeMap::from([(0, fpr_interval), (1, gpr_interval)]);
    ...
    fn test_apply_spill_slot_reuse_rewrites_operands() {
        ...
        let mut func = MachFunction {
            ...
            stack_slots: BTreeMap::from([
                (StackSlotId(0), RegAllocStackSlot::new(8, 8)),
                (StackSlotId(1), RegAllocStackSlot::new(8, 8)),
            ]),
        };

        let rewrites = BTreeMap::from([(StackSlotId(1), StackSlotId(0))]);
            ...
            for op in inst.defs.iter().chain(inst.uses.iter()) {
                if let MachOperand::StackSlot(slot) = op {
                    assert_ne!(
                        *slot,
                        StackSlotId(1),
                        "slot 1 should have been rewritten to slot 0"
                    );
                }
            }
        }
        assert!(
            ...
            "shared destination slot metadata should remain"
        );
        assert!(
            !func.stack_slots.contains_key(&StackSlotId(1)),
            "rewritten-away source slot metadata should be pruned"
        );
        assert_eq!(func.stack_slots.len(), 1);
    }

    #[test]
    fn test_apply_spill_slot_reuse_keeps_referenced_source_slot_metadata() {
        use crate::machine_types::*;
        let mut func = MachFunction {
            name: "rewrite_chain_keeps_referenced_source".into(),
            insts: vec![MachInst {
                opcode: crate::spill::PSEUDO_SPILL_LOAD,
                defs: vec![MachOperand::VReg(vreg(0))],
                uses: vec![MachOperand::StackSlot(StackSlotId(2))],
                implicit_defs: Vec::new(),
                implicit_uses: Vec::new(),
                flags: InstFlags::READS_MEMORY,
                tied_operands: vec![],
            ...
            stack_slots: BTreeMap::from([
                (StackSlotId(0), RegAllocStackSlot::new(8, 8)),
                (StackSlotId(1), RegAllocStackSlot::new(8, 8)),
                (StackSlotId(2), RegAllocStackSlot::new(8, 8)),
            ]),
        };

        let rewrites = BTreeMap::from([
            (StackSlotId(1), StackSlotId(0)),
            (StackSlotId(2), StackSlotId(1)),
        ]);
        apply_spill_slot_reuse(&mut func, &rewrites);
        ...
        assert!(
            func.stack_slots.contains_key(&StackSlotId(0)),
            "non-source destination slot metadata should remain"
        );
        assert!(
            func.stack_slots.contains_key(&StackSlotId(1)),
            "source slot metadata should remain when a rewritten operand still references it"
        );
        assert!(
            !func.stack_slots.contains_key(&StackSlotId(2)),
            "unreferenced rewritten-away source slot metadata should be pruned"
        );
    }
    ...
    fn test_apply_spill_slot_reuse_empty_rewrites() {
        ...
        apply_spill_slot_reuse(&mut func, &BTreeMap::new());
    ...
    fn test_many_non_overlapping_all_share_one_slot() {
        ...
        assert_eq!(
            result.slot_rewrites.get(&StackSlotId(2)),
            Some(&StackSlotId(0))
        );
        assert_eq!(
            result.slot_rewrites.get(&StackSlotId(3)),
            Some(&StackSlotId(0))
        );
        assert_eq!(
            result.slot_rewrites.get(&StackSlotId(4)),
            Some(&StackSlotId(0))
        );
    }
    ...
    fn test_mixed_classes_separate_groups() {
        ...
        assert_eq!(
            result.slot_rewrites.get(&StackSlotId(11)),
            Some(&StackSlotId(10))
        );
    }
}
