// trust-cg-regalloc/phi_elim.rs - Phi elimination and critical edge splitting
//
...
//!
//! The algorithm:
//! 1. **Phi edge splitting:** Insert jump blocks only on multi-successor source
//!    edges that need edge-specific Phi copies. This ensures copies can be
//!    placed without affecting other paths while avoiding branch-only split
//!    blocks for ordinary no-Phi critical edges.
...
/// Synthetic unconditional branch inserted when critical-edge splitting needs
/// an explicit jump block. Codegen maps this back to AArch64 `B`.
///
/// Keep this outside the real AArch64 opcode discriminant range: regalloc also
/// carries frontend `AArch64Opcode::Copy` instructions as raw u16 opcodes.
pub const PSEUDO_JUMP: u16 = 0xFFE2;
...
pub const IR_COPY_OPCODE: u16 = trust_cg_ir::inst::AArch64Opcode::Copy as u16;
...
/// Split CFG edges that need edge-specific Phi copies.
///
/// A Phi copy can be inserted directly in a predecessor only when that
/// predecessor has a single successor. For a multi-successor predecessor, the
/// copy must live on the specific outgoing edge; this pass creates a jump block
/// for only those edges. Ordinary no-Phi critical edges are left alone.
///
    ...
    for &block_id in &func.block_order {
        ...
        if block.succs.len() > 1 {
            for &succ_id in &block.succs {
                if edge_needs_phi_copy(func, block_id, succ_id) {
...
fn edge_needs_phi_copy(func: &MachFunction, src_id: BlockId, dst_id: BlockId) -> bool {
    let Some(dst_block) = func.blocks.get(dst_id.0 as usize) else {
        return false;
    };
    let Some(pred_index) = dst_block.preds.iter().position(|&pred| pred == src_id) else {
        return false;
    };

    dst_block.insts.iter().any(|&inst_id| {
        ...
        let Some(dest_vreg) = inst.defs.first().and_then(|op| op.as_vreg()) else {
            return false;
        };
        let Some(src_vreg) = inst.uses.get(pred_index).and_then(|op| op.as_vreg()) else {
            return false;
        };
...
pub fn eliminate_phis(func: &mut MachFunction) {
    ...
    let mut copies_by_pred: BTreeMap<BlockId, Vec<(VReg, VReg)>> = BTreeMap::new();
    ...
    // Insert one resolved parallel-copy sequence per predecessor edge. Block
    // arguments are simultaneous assignments; emitting them one-by-one can
    // clobber a later source on multi-parameter edges.
    for pred_id in func.block_order.clone() {
        let Some(copies) = copies_by_pred.remove(&pred_id) else {
            continue;
        };

        let resolved = resolve_parallel_copies(&copies, func);
        ...
        for (offset, (dest, src)) in resolved.into_iter().enumerate() {
            ...
            pred_block.insts.insert(insert_pos + offset, copy_id);
        }
    }

    for (pred_id, copies) in copies_by_pred {
        let resolved = resolve_parallel_copies(&copies, func);
        let pred_block = &mut func.blocks[pred_id.0 as usize];
        let insert_pos = find_terminator_pos(pred_block, &func.insts);
        for (offset, (dest, src)) in resolved.into_iter().enumerate() {
...
pub fn resolve_parallel_copies(
    ...
    while !remaining.is_empty() {
        ...
        // The remaining copies form at least one cycle — break one edge with a
        // temporary, then resume the acyclic emission pass above.
...
mod tests {
    ...
    #[test]
    fn pseudo_jump_opcode_does_not_alias_ir_copy() {
        assert_ne!(
            PSEUDO_JUMP, IR_COPY_OPCODE,
            "PSEUDO_JUMP must not alias frontend Copy opcodes"
        );
        assert!(PSEUDO_JUMP > trust_cg_ir::inst::AArch64Opcode::Nop as u16);
    }
    ...
    fn test_parallel_copy_with_cycle() {
        ...
        assert!(
            result.len() >= 3,
            "cycle should introduce a temp: got {:?}",
            result
        );
    }
    ...
    fn test_parallel_copy_three_way_cycle() {
        ...
        assert!(
            func.next_vreg > 10,
            "should have allocated a temporary VReg"
        );
    }
    ...
    #[test]
    fn test_split_multisuccessor_edges_to_phi_blocks() {
        use crate::machine_types::*;
        let v0 = VReg {
            id: 0,
            class: RegClass::Gpr64,
        ...
        assert_eq!(func.blocks[0].succs.len(), 2);
        assert_ne!(func.blocks[0].succs[0], BlockId(1));
        assert_ne!(func.blocks[0].succs[1], BlockId(2));
        assert_eq!(func.blocks[1].preds.len(), 1);
        assert_ne!(func.blocks[1].preds[0], BlockId(0));
        assert_eq!(func.blocks[2].preds.len(), 1);
        assert_ne!(func.blocks[2].preds[0], BlockId(0));
    }
    ...
    fn make_critical_edge_cfg() -> MachFunction {
        ...
        MachFunction {
            ...
            blocks: vec![
                ...
                MachBlock {
                    // Block 2: 2 preds (from block 0 and block 1) — target of critical edges
                    insts: vec![i2],
    ...
    fn make_critical_edge_with_phi_copy_cfg() -> MachFunction {
        let mut func = make_critical_edge_cfg();
        func.name = "critical_edge_phi".into();
        func.next_vreg = 3;

        let phi = InstId(func.insts.len() as u32);
        func.insts.push(MachInst {
            opcode: 0x00,
            defs: vec![MachOperand::VReg(VReg {
                id: 2,
        ...
        func.blocks[2].insts.insert(0, phi);

        func
    }

    fn make_critical_edge_with_identity_phi_cfg() -> MachFunction {
        let mut func = make_critical_edge_cfg();
        func.name = "critical_edge_identity_phi".into();
        func.next_vreg = 1;
    ...
    #[test]
    fn test_critical_edge_splitting_skips_no_phi_critical_edges() {
        let mut func = make_critical_edge_cfg();
        let initial_block_count = func.blocks.len();
        let initial_inst_count = func.insts.len();

        let edges_split = split_critical_edges(&mut func);

        assert_eq!(
            edges_split, 0,
            "no-Phi critical edges must not get branch-only split blocks"
        );
        assert_eq!(func.blocks.len(), initial_block_count);
        assert_eq!(func.insts.len(), initial_inst_count);
        assert!(
            func.insts.iter().all(|inst| inst.opcode != PSEUDO_JUMP),
            "no-Phi CFG must not acquire PSEUDO_JUMP trampolines"
        );
    }

    #[test]
    fn test_critical_edge_splitting_with_phi_edge_copies() {
        let mut func = make_critical_edge_with_phi_copy_cfg();
        let initial_block_count = func.blocks.len();

        let edges_split = split_critical_edges(&mut func);

        assert_eq!(edges_split, 2, "should split 2 Phi copy edges");
        assert_eq!(func.blocks.len(), initial_block_count + 2);
    }

    #[test]
    fn test_critical_edge_splitting_skips_identity_phi_edges() {
        let mut func = make_critical_edge_with_identity_phi_cfg();
        let initial_block_count = func.blocks.len();
        let initial_inst_count = func.insts.len();
        ...
        assert_eq!(
            edges_split, 0,
            "identity Phi inputs do not need edge-copy split blocks"
        );
        assert_eq!(func.blocks.len(), initial_block_count);
        assert_eq!(func.insts.len(), initial_inst_count);
        assert!(
            func.insts.iter().all(|inst| inst.opcode != PSEUDO_JUMP),
            "identity Phi CFG must not acquire PSEUDO_JUMP trampolines"
        );
    }
    ...
    fn test_eliminate_phis_removes_phi_instructions() {
        ...
        for &block_id in &func.block_order {
            ...
            for &inst_id in &block.insts {
                ...
                assert!(
                    !inst.flags.is_phi(),
                    "phi should have been eliminated: {:?}",
                    inst
                );
            }
        }
    ...
    fn test_eliminate_phis_inserts_copies() {
        ...
        assert!(
            copy_count >= 2,
            "should insert at least 2 copies, got {}",
            copy_count
        );
        assert!(
            func.insts.len() > insts_before,
            "should have more instructions after phi elim"
        );
    }
    ...
    fn test_parallel_copy_correctness_swap() {
        ...
        let mut regs: std::collections::BTreeMap<u32, i64> = std::collections::BTreeMap::new();
    ...
    fn test_eliminate_phis_copies_go_to_correct_predecessor() {
        ...
        let block1_has_copy = func.blocks[1]
            .insts
            .iter()
            .any(|&id| func.insts[id.0 as usize].opcode == PSEUDO_COPY);
        assert!(block1_has_copy, "block 1 should have a copy for the phi");

        // Block 2 (else) should now have a copy instruction (v2 <- v1).
        let block2_has_copy = func.blocks[2]
            .insts
            .iter()
            .any(|&id| func.insts[id.0 as usize].opcode == PSEUDO_COPY);
    ...
    fn test_critical_edge_split_new_block_in_block_order() {
        ...
        assert!(
            func.block_order.len() >= 6,
            "should have original 4 + 2 new blocks"
        );
    ...
    #[test]
    fn test_eliminate_phis_resolves_parallel_copy_cycle_per_predecessor() {
        use crate::machine_types::*;
        ...
        let copy_ids: Vec<_> = block0
            .insts
            .iter()
            .copied()
            .filter(|id| func.insts[id.0 as usize].opcode == PSEUDO_COPY)
            .collect();
        assert_eq!(copy_ids.len(), 3, "swap needs a temp copy sequence");
        assert!(func.next_vreg > 2, "resolver should allocate a temp vreg");
        assert_eq!(block0.insts.last(), Some(&br), "copies stay before branch");
    }
    ...
    fn test_phi_self_copy_eliminated() {
        ...
        let block0_copy_count = func.blocks[0]
            .insts
            .iter()
            .filter(|&&id| func.insts[id.0 as usize].opcode == PSEUDO_COPY)
            .count();
        ...
        let block1_copy_count = func.blocks[1]
            .insts
            .iter()
            .filter(|&&id| func.insts[id.0 as usize].opcode == PSEUDO_COPY)
            .count();
    ...
    fn test_critical_edge_split_then_phi_elim_combined() {
        ...
        assert!(
            total_copies >= 2,
            "should have at least 2 copies (one per phi input)"
        );
    }
    ...
    fn test_eliminate_phis_diamond_correct_source_per_predecessor() {
        ...
        let block1_copies: Vec<_> = func.blocks[1]
            .insts
            .iter()
            .filter_map(|&id| {
        ...
        assert_eq!(
            block1_copies[0],
            (Some(v2), Some(v0)),
            "block 1 copy should be v2 <- v0 (from then-branch), got {:?}",
            block1_copies[0]
        );

        // Block 2 (else-branch, pred index 1): should copy v1 -> v2.
        let block2_copies: Vec<_> = func.blocks[2]
            .insts
            .iter()
            .filter_map(|&id| {
                let inst = &func.insts[id.0 as usize];
        ...
        assert_eq!(
            block2_copies[0],
            (Some(v2), Some(v1)),
            "block 2 copy should be v2 <- v1 (from else-branch), got {:?}",
            block2_copies[0]
        );
    }
    ...
    fn test_eliminate_phis_three_predecessors() {
        ...
        assert_eq!(
            copies3.len(),
            0,
            "block 3 should have no copies (not a phi predecessor)"
        );
    }
    ...
    fn test_critical_edge_split_blocks_have_explicit_jump() {
        ...
        // Each new split block should have exactly one instruction: a
        // pseudo jump with IS_BRANCH | IS_TERMINATOR.
        let new_block_ids: Vec<BlockId> = func
            .block_order
            .iter()
            .copied()
        ...
        for &block_id in &new_block_ids {
            ...
            assert_eq!(
                jump_inst.opcode, PSEUDO_JUMP,
                "split block {:?} instruction should be PSEUDO_JUMP",
                block_id
            );
            assert!(
            ...
            let has_block_operand = jump_inst
                .uses
                .iter()
                .any(|op| matches!(op, MachOperand::Block(b) if *b == target));
            assert!(
                has_block_operand,
    ...
    fn test_critical_edge_split_rewrites_source_branch_operands() {
        ...
        let block0_branch_targets: Vec<BlockId> = block0_branch
            .uses
            .iter()
            .filter_map(|op| {
        ...
        let block1_branch_targets: Vec<BlockId> = block1_branch
            .uses
            .iter()
            .filter_map(|op| {
                if let MachOperand::Block(b) = op {
    ...
    fn test_critical_edge_split_non_adjacent_layout_correctness() {
        ...
        let mut func = MachFunction {
            ...
            blocks: vec![
                ...
                MachBlock {
                    // Block 2: 2 preds — critical edge target
        ...
        let split_blocks: Vec<BlockId> = func
            .block_order
            .iter()
            .copied()
            .filter(|b| b.0 >= 4)
