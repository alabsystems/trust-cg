// trust-cg-regalloc/x86_adapter.rs - x86-64 register allocator adapter
//
...
//!
//! This module bridges the x86-64 register model ([`trust_cg_ir::x86_64_regs`])
...
use trust_cg_ir::x86_64_regs::{self, X86PReg};
...
/// Convert a fixed x86 operand to the regalloc register it must reserve.
///
/// This is intentionally broader than [`x86_to_preg`]: byte/word GPR operands
/// are not allocatable classes in the current regalloc model, but fixed
/// operands like `AL` and `AX` still alias their containing parent register and
/// must be visible to liveness/interference.
pub fn x86_fixed_operand_to_preg(x86: X86PReg) -> Option<PReg> {
    x86_to_preg(x86).or_else(|| {
        if x86.is_gpr8() || x86.is_gpr16() {
            x86_64_regs::x86_containing_gpr64(x86).and_then(x86_to_preg)
        } else {
            None
        }
    })
...
pub fn is_x86_preg(preg: PReg) -> bool {
    let enc = preg.encoding();
    (512..=527).contains(&enc) || (528..=543).contains(&enc) || (544..=559).contains(&enc)
}
...
pub fn x86_64_allocatable_regs() -> BTreeMap<RegClass, Vec<PReg>> {
...
pub fn x86_64_caller_saved_regs() -> BTreeSet<PReg> {
...
pub fn x86_64_callee_saved_regs() -> BTreeSet<PReg> {
    let mut regs = BTreeSet::new();
...
/// x86-64 copy-coalescing tuning, honoring the `TCG_NO_X86_COALESCE_TUNING`
/// kill switch (set it to any value to fall back to the empty default tuning,
/// disabling the kill-at-def loop-carried latch merges for A/B measurement).
fn x86_64_coalesce_tuning() -> crate::coalesce::CoalesceTuning {
    if std::env::var_os("TCG_NO_X86_COALESCE_TUNING").is_some() {
        crate::coalesce::CoalesceTuning::default()
    } else {
        crate::coalesce::CoalesceTuning::x86_64()
    }
}
...
pub fn x86_64_alloc_config() -> crate::AllocConfig {
    crate::AllocConfig {
        ...
        hints: BTreeMap::new(),
        coalesce_tuning: x86_64_coalesce_tuning(),
    }
}
...
pub fn translate_allocation(
    allocation: &BTreeMap<crate::machine_types::VReg, PReg>,
) -> BTreeMap<crate::machine_types::VReg, X86PReg> {
    allocation
        .iter()
        .filter_map(|(&vreg, &preg)| preg_to_x86(preg).map(|x86| (vreg, x86)))
        .collect()
}
...
mod tests {
    ...
    fn test_gpr64_roundtrip() {
        ...
        for &gpr in &X86_ALL_GPRS {
            ...
            let back =
                preg_to_x86(preg).unwrap_or_else(|| panic!("failed to convert back {:?}", preg));
    ...
    #[test]
    fn test_fixed_gpr16_canonicalizes_to_parent_preg() {
        assert_eq!(
            x86_fixed_operand_to_preg(AX),
            x86_to_preg(RAX),
            "AX should reserve parent RAX"
        );
        assert_eq!(
            x86_fixed_operand_to_preg(R15W),
            x86_to_preg(R15),
            "R15W should reserve parent R15"
        );
    }

    #[test]
    fn test_fixed_gpr8_canonicalizes_to_parent_preg() {
        assert_eq!(
            x86_fixed_operand_to_preg(AL),
            x86_to_preg(RAX),
            "AL should reserve parent RAX"
        );
        assert_eq!(
            x86_fixed_operand_to_preg(R15B),
            x86_to_preg(R15),
            "R15B should reserve parent R15"
        );
    }

    #[test]
    fn test_fixed_operand_mapping_keeps_allocatable_views() {
        assert_eq!(x86_fixed_operand_to_preg(RAX), x86_to_preg(RAX));
        assert_eq!(x86_fixed_operand_to_preg(EAX), x86_to_preg(EAX));
        assert_eq!(x86_fixed_operand_to_preg(XMM0), x86_to_preg(XMM0));
        assert_eq!(x86_fixed_operand_to_preg(RFLAGS), None);
    }
    ...
    fn test_caller_saved_count() {
        ...
        assert_eq!(
            cs.len(),
            25,
            "expected 25 caller-saved regs (9 GPR + 16 XMM)"
        );
    }
    ...
    fn test_call_crossing_with_x86_regs() {
        ...
        let intervals = BTreeMap::from([(0u32, interval)]);
        ...
        let hints =
            call_clobber::compute_call_crossing_hints(&crossings, &callee_saved, &allocatable);
