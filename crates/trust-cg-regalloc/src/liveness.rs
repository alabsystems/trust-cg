// trust-cg-regalloc/liveness.rs - Live interval computation
//
...
impl LiveInterval {
    ...
    pub fn overlaps(&self, other: &LiveInterval) -> bool {
        ...
        let (Some(a_first), Some(a_last)) = (self.ranges.first(), self.ranges.last()) else {
            return false;
        };
        let (Some(b_first), Some(b_last)) = (other.ranges.first(), other.ranges.last()) else {
            return false;
        };
    ...
    pub fn add_range(&mut self, start: u32, end: u32) {
        ...
        if let (Some(rs), Some(re)) = (remove_start, remove_end) {
            ...
            self.ranges
                .insert(rs, LiveRange::new(merged_start, merged_end));
        } else {
...
pub struct LivenessResult {
    ...
    pub intervals: BTreeMap<u32, LiveInterval>,
    /// Instruction numbering: maps (BlockId, instr_index_in_block) to global index.
    pub inst_numbering: BTreeMap<InstId, u32>,
}
...
/// A dense bitset over vreg ids for the liveness dataflow. Vreg ids are a
/// contiguous `0..next_vreg` range, so a bitset makes the fixpoint's union /
/// insert / remove / equality operations word-parallel (O(words)) instead of
/// the O(n log n) `BTreeSet` operations the fixpoint previously paid on every
/// block on every iteration — profiled as the dominant core-pipeline cost.
/// Semantics are identical to the `BTreeSet<u32>` it replaces.
...
impl LiveSet {
    #[inline]
    fn new(n_vregs: usize) -> Self {
        Self {
            words: vec![0u64; n_vregs.div_ceil(64)],
        }
    }
    #[inline]
    ...
    /// `self |= other` (both must be the same width).
    #[inline]
    ...
    /// Copy `other` into `self` without reallocating (same width).
    #[inline]
    fn copy_from(&mut self, other: &LiveSet) {
        self.words.copy_from_slice(&other.words);
    }
    /// Iterate the ids of set bits in ascending order.
...
pub fn compute_live_intervals(func: &MachFunction) -> LivenessResult {
    ...
    // Bitset width: cover every vreg id that appears. `next_vreg` is the id
    // upper bound (ids are allocated 0..next_vreg); scan defensively so a vreg
    // id >= next_vreg (malformed input) can never index out of bounds.
    let mut n_vregs = func.next_vreg as usize;
    for inst in &func.insts {
        for v in inst.vreg_defs().chain(inst.vreg_uses()) {
            n_vregs = n_vregs.max(v.id as usize + 1);
        }
    }

    // Interval storage is built into a DENSE id-indexed `Vec<Option<..>>` for
    // O(1) access on the hot per-use/def path, then converted to the public
    // `BTreeMap` output ONCE at the end — the previous `BTreeMap::entry` on
    // every use/def paid O(log n) + btree-node churn per access, a top
    // core-pipeline cost. Output is identical (same keys, same sorted order).
    let mut interval_vec: Vec<Option<LiveInterval>> = (0..n_vregs).map(|_| None).collect();

    // Step 1: Number instructions linearly across all blocks. `inst_num_vec`
    // (dense, indexed by `InstId.0` which already indexes `func.insts`) backs
    // the many hot lookups below in O(1); the public `BTreeMap` is assembled
    // from it once at the end.
    let mut inst_num_vec: Vec<u32> = vec![0; func.insts.len()];
    let mut inst_numbering: BTreeMap<InstId, u32> = BTreeMap::new();
    let mut idx: u32 = 0;
    for &block_id in &func.block_order {
        let block = &func.blocks[block_id.0 as usize];
        for &inst_id in &block.insts {
            inst_num_vec[inst_id.0 as usize] = idx;
            inst_numbering.insert(inst_id, idx);
            inst_loop_depths.push(block.loop_depth);
            idx += 1;
        }
    }
    ...
    let mut live_in: Vec<LiveSet> = vec![LiveSet::new(n_vregs); num_blocks];
    let mut live_out: Vec<LiveSet> = vec![LiveSet::new(n_vregs); num_blocks];
    // Reused scratch buffers so the fixpoint does not allocate a fresh set per
    // block per iteration (the old BTreeSet path did).
    let mut new_live_out = LiveSet::new(n_vregs);
    let mut new_live_in = LiveSet::new(n_vregs);
    ...
    while changed {
        if iteration >= max_iterations {
            eprintln!(
                "trust-cg-regalloc: liveness fixpoint did not converge after {} iterations ({} blocks)",
        ...
        for &block_id in func.block_order.iter().rev() {
            ...
            for &succ_id in &block.succs {
                new_live_out.union_from(&live_in[succ_id.0 as usize]);
            }

            // live_in[b] = use[b] union (live_out[b] - def[b])
            new_live_in.copy_from(&new_live_out);
            ...
            if new_live_in != live_in[bi] || new_live_out != live_out[bi] {
                changed = true;
                live_in[bi].copy_from(&new_live_in);
                live_out[bi].copy_from(&new_live_out);
            }
        }
    }
    ...
    for &block_id in &func.block_order {
        ...
        let block_start = inst_num_vec[block.insts[0].0 as usize];
        let block_end = inst_num_vec[block.insts.last().unwrap().0 as usize] + 1;
        ...
        // to block_end.
        let mut last_defs_in_block: BTreeMap<u32, u32> = BTreeMap::new(); // vreg_id -> def position
        for &inst_id in &block.insts {
            let inst = &func.insts[inst_id.0 as usize];
            let idx = inst_num_vec[inst_id.0 as usize];
            for vreg in inst.vreg_defs() {
        ...
        for vreg_id in live_out[bi].iter() {
            let interval = interval_vec[vreg_id as usize].get_or_insert_with(|| {
                LiveInterval::new(VReg {
                    id: vreg_id,
                    class: vreg_class_for_id(vreg_id, &vreg_classes),
                })
            });
        ...
        //
        // Precompute each vreg's last-use end position in ONE pass over the
        // block, so the per-live-in-vreg handling below is O(live_in) lookups
        // instead of O(live_in * insts) — the latter is quadratic on large
        // loop-body blocks and was the dominant super-linear cost in interval
        // building (the `end`-position is `idx + 1`; instructions are visited
        // in order so the last write for a vreg is its final use).
        // BTreeMap (not HashMap): the rustc-workspace tool build denies
        // rustc::default-hash-types, and deterministic iteration is a
        // reproducibility bonus; u32-keyed lookups here are not hot enough
        // for the constant-factor difference to matter.
        let mut last_use_end_in_block: BTreeMap<u32, u32> = BTreeMap::new();
        for &inst_id in &block.insts {
            let idx = inst_num_vec[inst_id.0 as usize];
            for vreg in func.insts[inst_id.0 as usize].vreg_uses() {
                last_use_end_in_block.insert(vreg.id, idx + 1);
            }
        }
        for vreg_id in live_in[bi].iter() {
            if live_out[bi].contains(vreg_id) {
                continue; // Already handled above.
            }
            if let Some(&last_use_end) = last_use_end_in_block.get(&vreg_id)
                && last_use_end > block_start
            {
                let interval = interval_vec[vreg_id as usize].get_or_insert_with(|| {
                    LiveInterval::new(VReg {
                        id: vreg_id,
        ...
        let mut reaching_defs: BTreeMap<u32, u32> = BTreeMap::new();
        for &inst_id in &block.insts {
            let inst = &func.insts[inst_id.0 as usize];
            let idx = inst_num_vec[inst_id.0 as usize];
            ...
            for vreg in inst.vreg_uses() {
                let interval =
                    interval_vec[vreg.id as usize].get_or_insert_with(|| LiveInterval::new(vreg));
    ...
    for interval in interval_vec.iter_mut().flatten() {
        compute_spill_weight(interval, &inst_loop_depths);
    }

    // Assemble the public interval map once: dense id-indexed storage -> the
    // BTreeMap the allocator consumes (identical keys / sorted order; only
    // vregs that received an interval are emitted, matching the old
    // `entry`-built map). `inst_numbering` was built directly in Step 1.
    let intervals: BTreeMap<u32, LiveInterval> = interval_vec
        .into_iter()
        .enumerate()
        .filter_map(|(id, iv)| iv.map(|iv| (id as u32, iv)))
        .collect();
...
fn collect_vreg_classes(func: &MachFunction) -> BTreeMap<u32, RegClass> {
    let mut classes = BTreeMap::new();

    for inst in &func.insts {
        for vreg in inst.vreg_defs().chain(inst.vreg_uses()) {
            classes
                .entry(vreg.id)
                .and_modify(|class| *class = merge_vreg_class(*class, vreg.class))
                .or_insert(vreg.class);
        }
    }
...
fn vreg_class_for_id(vreg_id: u32, classes: &BTreeMap<u32, RegClass>) -> RegClass {
    classes.get(&vreg_id).copied().unwrap_or(RegClass::Gpr64)
}
...
fn compute_spill_weight(interval: &mut LiveInterval, inst_loop_depths: &[u32]) {
    ...
    {
        let loop_depth = inst_loop_depths.get(*pos as usize).copied().unwrap_or(0);
        weight += base.powi(loop_depth as i32);
    }
...
/// Per-depth-level spill-weight base. Default 10 (the historical value, and
/// what the aarch64 greedy flip was tuned against). `TCG_RA_WEIGHT_BASE`
/// overrides it for A/B measurement only: the x86 loop-depth experiment
/// showed `10^depth` OVER-protects hot-loop intervals (depth-2 = 100x) and
/// triggers eviction cascades in register-shuffle kernels (b06 -9.6%,
/// b16 -6%, b07 -5.5% against b05 +7%); a softer base lets greedy's Split
/// stage absorb the pressure instead. Weights only choose WHICH interval
/// spills — a bad choice is a perf outcome, never a correctness one
/// (regalloc_validator + TV-5 gate every allocation).
fn spill_weight_base() -> f64 {
    use std::sync::OnceLock;
    static BASE: OnceLock<f64> = OnceLock::new();
    *BASE.get_or_init(|| {
        std::env::var("TCG_RA_WEIGHT_BASE")
            .ok()
...
mod tests {
    ...
    #[test]
    fn liveset_matches_btreeset_reference() {
        use std::collections::BTreeSet;
        // Deterministic pseudo-random op sequence; the bitset must stay
        // byte-for-byte equivalent to a BTreeSet<u32> reference across
        // insert/remove/contains/union/copy/clear/iter.
        let n = 200usize;
        ...
        let mut bs = LiveSet::new(n);
        let mut rf: BTreeSet<u32> = BTreeSet::new();
        let mut bs2 = LiveSet::new(n);
        let mut rf2: BTreeSet<u32> = BTreeSet::new();
        for _ in 0..4000 {
            let id = (next() % n as u64) as u32;
            match next() % 5 {
                0 | 1 => {
                    bs.insert(id);
                    rf.insert(id);
                ...
                3 => {
                    ...
                    rf2.insert(id);
                    bs.union_from(&bs2);
                    rf.extend(rf2.iter().copied());
                }
                _ => {
                    assert_eq!(bs.contains(id), rf.contains(&id));
                }
            }
            // Full-state equivalence: set-bit iteration == sorted BTreeSet.
            let got: Vec<u32> = bs.iter().collect();
            let want: Vec<u32> = rf.iter().copied().collect();
            assert_eq!(got, want);
        }
        // copy_from + clear
        let mut c = LiveSet::new(n);
        c.copy_from(&bs);
        assert_eq!(c, bs);
        c.clear();
    ...
    fn test_live_interval_overlaps_with_holes() {
        let mut a = LiveInterval::new(VReg {
            id: 0,
            class: RegClass::Gpr64,
        });
        a.add_range(0, 5);
        ...
        let mut b = LiveInterval::new(VReg {
            id: 1,
            class: RegClass::Gpr64,
        });
        ...
        let mut c = LiveInterval::new(VReg {
            id: 2,
            class: RegClass::Gpr64,
        });
    ...
    fn test_compute_live_intervals_cross_block() {
        ...
        let interval = result
            .intervals
            .get(&0)
            .expect("v0 should have an interval");
        ...
        assert!(
            interval.is_live_at(use_idx),
            "v0 should be live at its use in block 1"
        );
    }
    ...
    fn test_compute_live_intervals_loop_carried() {
        ...
        let v1 = result
            .intervals
            .get(&1)
            .expect("v1 should have an interval");
        ...
        assert!(
            v1.is_live_at(exit_use_idx),
            "v1 should be live at its use in exit block"
        );
    }
    ...
    fn test_compute_live_intervals_dead_def() {
        ...
        let v0 = result
            .intervals
            .get(&0)
            .expect("dead def should still get an interval");
    ...
    fn make_custom_function(
        ...
        for (bi, block_instrs) in block_specs.iter().enumerate() {
            ...
            for (defs, uses) in block_instrs {
                ...
                let def_ops: Vec<MachOperand> = defs
                    .iter()
                    .map(|&id| {
                    ...
                    .collect();
                let use_ops: Vec<MachOperand> = uses
                    .iter()
                    .map(|&id| {
                        max_vreg = max_vreg.max(id + 1);
                        MachOperand::VReg(VReg {
            ...
            let succs: Vec<BlockId> = succs_map
                .get(bi)
                .map(|s| s.iter().map(|&id| BlockId(id)).collect())
                .unwrap_or_default();
            let preds: Vec<BlockId> = preds_map
                .get(bi)
                .map(|p| p.iter().map(|&id| BlockId(id)).collect())
                .unwrap_or_default();
    ...
    fn test_live_interval_overlaps_empty() {
        let empty = LiveInterval::new(VReg {
            id: 0,
            class: RegClass::Gpr64,
        });
        let mut non_empty = LiveInterval::new(VReg {
            id: 1,
            class: RegClass::Gpr64,
        });
    ...
    fn test_diamond_cfg_liveness() {
        ...
        assert!(
            v0.is_live_at(idx_i2),
            "v0 should be live at its use in block 1"
        );
        assert!(
            v0.is_live_at(idx_i4),
            "v0 should be live at its use in block 2"
        );
        ...
        assert!(
            v1.is_live_at(idx_i6),
            "v1 should be live at its use in merge block"
        );
    }
    ...
    fn test_spill_weight_increases_with_loop_depth() {
        ...
        assert!(
            w2 > w0,
            "weight at depth 2 ({w2}) should be greater than depth 0 ({w0})"
        );
    }
    ...
    fn test_multiple_defs_same_vreg() {
        ...
        assert!(
            v0.def_positions.len() >= 2,
            "v0 should have at least 2 def positions"
        );
    }
    ...
    fn test_many_blocks_linear_chain() {
        ...
        for bi in 0..num_blocks {
            ...
            let preds = if bi == 0 {
                ...
                vec![BlockId((bi - 1) as u32)]
            };
            let succs = if bi == num_blocks - 1 {
                Vec::new()
            } else {
                vec![BlockId((bi + 1) as u32)]
            };
    ...
    fn test_issue_306_cset_does_not_clobber_loop_counter() {
        ...
        let v3_def_pos = *v3_interval
            .def_positions
            .first()
            .expect("v3 should have a def position");
        let v3_last_use = *v3_interval
            .use_positions
            .iter()
            .max()
    ...
    fn test_issue_306_continuous_range_def_to_use_same_block() {
        ...
        assert!(
            v0.is_live_at(1),
            "v0 must be live at pos 1 (between def and use)"
        );
        assert!(
            v0.is_live_at(2),
            "v0 must be live at pos 2 (between def and use)"
        );
    ...
    fn test_issue_300_reaching_def_handles_multiple_inplace_updates() {
        ...
        let v3 = result
            .intervals
            .get(&3)
            .expect("v3 should have an interval");
    ...
    #[test]
    fn test_cross_block_live_in_preserves_operand_reg_class() {
        use crate::machine_types::*;
        ...
        let func = MachFunction {
            name: "cross_block_gpr32_livein".into(),
            insts: vec![
                MachInst {
                    opcode: 1,
                    defs: vec![],
        ...
        assert_eq!(
            interval.vreg.class,
            RegClass::Gpr32,
            "cross-block live-in/out intervals must not default to Gpr64"
        );
        assert!(
            interval.is_live_at(1),
            "v0 should remain live through the branch into the successor: {:?}",
            interval.ranges
        );
    }
}
