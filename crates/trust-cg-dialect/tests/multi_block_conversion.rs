// trust-cg-dialect - Multi-block conversion regression tests (F2)
//
...
use trust_cg_dialect::conversion::{
...
use trust_cg_dialect::id::{BlockId, DialectOpId};
use trust_cg_dialect::module::{DialectFunction, DialectModule};
use trust_cg_dialect::op::{Attribute, DialectOp};
...
/// ```text
/// bb0(p: i64):
///   %t0 = trust_ir.const {value = 1}
///   trust_ir.add %p, %t0  // lands in bb0
...
///   verif.fingerprint_batch(%p, %p) {goto = bb1}
/// bb1(q: i64):
///   %t1 = trust_ir.const {value = 2}
///   verif.fingerprint_batch(%q, %q) {goto = bb2}
/// bb2:
///   trust_ir.ret
/// ```
///
...
fn build_three_block_source() -> (
    ...
    f.append_op(
        bb0,
        DialectOpId::new(trust_ir_id, trust_ir::TRUST_IR_CONST),
        vec![(t0, Type::I64)],
        vec![],
...
fn empty_driver_preserves_multi_block_structure() {
    let (mut module, _verif_id, _trust_ir_id, _machir_id, src_blocks) = build_three_block_source();

    // Record expected per-block op counts up front.
    let src_fn = &module.functions[0];
    let src_op_counts: Vec<usize> = src_fn.blocks.iter().map(|b| b.ops.len()).collect();
    ...
    assert_eq!(
        src_edges,
        vec![vec![src_blocks[1]], vec![src_blocks[2]], vec![]]
    );
    ...
    let trust_ir_ret_name = module
        .resolve(dst.ops[dst.blocks[2].ops[0].0 as usize].op)
        .map(|d| d.name)
        .unwrap();
    assert_eq!(trust_ir_ret_name, "trust_ir.ret");
}

// ---------------------------------------------------------------------------
// Test 2: pattern-based conversion preserves block structure too.
//
// Register a pattern for `verif.fingerprint_batch` that replaces it with
// a single `trust_ir.xor` op and verify the resulting function still has 3
// blocks, terminator still trails, and block params re-map onto the new
// ValueIds so block 1's ops consume the destination %q.
// ---------------------------------------------------------------------------

struct VerifStubToTrustIrXor {
    verif_id: trust_cg_dialect::id::DialectId,
    trust_ir_id: trust_cg_dialect::id::DialectId,
}

impl ConversionPattern for VerifStubToTrustIrXor {
...
fn pattern_conversion_preserves_multi_block_structure() {
    let (mut module, verif_id, trust_ir_id, _machir_id, _src_blocks) = build_three_block_source();

    let mut driver = ConversionDriver::new();
    driver.register(Box::new(VerifStubToTrustIrXor {
        verif_id,
        trust_ir_id,
    }));
    driver
        .run(&mut module)
        .expect("verif stub conversion succeeds");
    ...
    let dst_counts: Vec<usize> = dst.blocks.iter().map(|b| b.ops.len()).collect();
    // bb0 still has {trust_ir.const, trust_ir.add, trust_ir.xor(new)} = 3 ops.
    // bb1 still has {trust_ir.const, trust_ir.xor(new)} = 2 ops.
    // bb2 still has {trust_ir.ret} = 1 op.
    ...
    // The bb1 trust_ir.xor should consume the destination-space ValueId of %q
    ...
    let xor_op = bb1_ops
        ...
        .find(|o| {
            module
                .resolve(o.op)
                .map(|d| d.name == "trust_ir.xor")
                .unwrap_or(false)
        })
        .expect("bb1 has a trust_ir.xor");
    assert_eq!(
        xor_op.operands,
    ...
    // fixed, this assertion would find trust_ir.ret in bb0, alongside non-
    // terminator ops — exactly the "terminators mid-function" bug.
    let bb2_ret_name = module
        .resolve(dst.ops[dst.blocks[2].ops[0].0 as usize].op)
        .map(|d| d.name)
        .unwrap();
    assert_eq!(bb2_ret_name, "trust_ir.ret");
...
// trust_ir.ret. This assertion directly pins that failure mode.
// ---------------------------------------------------------------------------
...
fn terminator_in_last_block_does_not_bubble_up() {
    ...
    // produced by bb0's trust_ir.add (not left as the source ValueId).
...
fn block_map_is_one_to_one() {
    let (mut module, _verif_id, _trust_ir_id, _machir_id, _src_blocks) = build_three_block_source();
    ...
    let mut ids: Vec<u32> = module.functions[0].blocks.iter().map(|b| b.id.0).collect();
    ids.sort_unstable();
    ids.dedup();
