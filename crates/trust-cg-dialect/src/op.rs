// trust-cg-dialect - DialectOp + Attribute
//
...
impl Attribute {
        ...
        F: FnMut(BlockId) -> BlockId,
    {
        match self {
            ...
            Attribute::I64(_) | Attribute::U64(_) | Attribute::Bool(_) | Attribute::Str(_) => {}
        }
    }
...
pub struct DialectOp {
    ...
    /// Optional source-level provenance. Uses the existing trust_ir span type via
    /// `trust_cg_ir`'s public surface when available; v1 scaffolding leaves this
    /// as an `Option<(u32, u32)>` byte-range placeholder so `trust-cg-dialect`
    /// does not need to depend on the full `trust_ir` crate.
...
///
/// Kept decoupled from `trust_ir::SourceSpan` so this crate does not need to depend
/// on the trust_ir crate. A conversion helper can be added later when dialect ops
/// are created directly from trust_ir instructions.
