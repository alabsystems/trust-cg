// trust-cg-dialect - Dialect trait + OpDef + Capabilities
//
...
//! §5: it references `trust_cg_ir::function::Type` directly rather than defining
...
pub enum TypeConstraint {
    ...
    Aggregate,
    /// Exact match against a specific `trust_cg_ir::Type`.
    ///
    ...
    /// polymorphic ops like `trust_ir.add` whose operand types must agree.
    SameAs(usize),
}
...
pub trait Dialect: Send + Sync {
    /// Stable string namespace (e.g. `"verif"`, `"trust_ir"`, `"machir"`).
