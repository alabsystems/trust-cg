// trust-cg-dialect - machir.* -> trust_cg_ir::MachFunction adapter
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0

//! Translate a fully-lowered `machir.*` [`DialectFunction`] into a
//! `trust_cg_ir::MachFunction`. This is the v1 bridge that lets the dialect
//! framework reach the existing Trust Codegen pipeline without modifying the
//! pipeline itself.
//!
...
use trust_cg_ir::{
    AArch64Opcode, BlockId as MachBlockId, InstId, MachFunction, MachInst, MachOperand, Signature,
    VReg,
};
...
pub fn emit_mach_function(
    ...
    let mut value_to_vreg: BTreeMap<ValueId, VReg> = BTreeMap::new();
...
fn emit_movz(
    ...
    mf: &mut MachFunction,
    value_to_vreg: &mut BTreeMap<ValueId, VReg>,
    block: MachBlockId,
) -> Result<(), EmitError> {
    ...
    let imm = op
        ...
        .ok_or_else(|| {
            EmitError::Malformed("machir.movz.i64 missing i64/u64 `value` attribute".to_string())
        })?;
...
fn emit_ret(
    op: &crate::op::DialectOp,
    mf: &mut MachFunction,
    value_to_vreg: &BTreeMap<ValueId, VReg>,
    block: MachBlockId,
) -> Result<(), EmitError> {
