// trust-cg-dialect - Pass trait + Legality
//
...
pub fn validate_type_constraints_with_env(
    op: &DialectOp,
    def: &OpDef,
    value_types: &std::collections::BTreeMap<crate::id::ValueId, Type>,
) -> Result<(), PassError> {
...
pub fn validate_legality(module: &DialectModule, legality: &Legality) -> Result<(), PassError> {
    use std::collections::BTreeMap;

    for func in &module.functions {
        // Per-function value->type environment from parameters + op results.
        let mut value_types: BTreeMap<crate::id::ValueId, Type> = BTreeMap::new();
        ...
        for op in func.iter_ops() {
            if !legality.accepts.is_empty() && !legality.accepts.contains(&op.op.dialect) {
                let name = module
                    .resolve(op.op)
                    .map(|d| d.name)
            ...
            if !legality.produces.is_empty() && !legality.produces.contains(&op.op.dialect) {
                let name = module
                    .resolve(op.op)
                    .map(|d| d.name)
                    .unwrap_or("<unregistered>");
