// `trust-cg-test doctor` — environment check.
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0

//! Environment self-check for `trust-cg-test`.
//!
...
/// Arguments for `trust-cg-test doctor`.
...
fn required_tools(target: TargetCmd) -> Vec<&'static str> {
    match target {
        ...
        TargetCmd::Prove => vec!["ay"],
        TargetCmd::Pipeline => vec!["ay"],
...
fn all_tools() -> Vec<&'static str> {
    vec![
        "cargo", "rustc", "cc", "clang", "csmith", "yarpgen", "creduce", "ay", "gh",
    ]
}
...
fn print_human(r: &DoctorReport) {
    println!("trust-cg-test doctor  (target: {})", r.target);
    println!();
    println!(
