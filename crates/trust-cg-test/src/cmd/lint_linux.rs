// `trust-cg-test lint-linux` — cross-compile check for Linux targets.
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0

//! Cross-compilation lint for the `#[cfg(target_os = "linux")]` paths in
//! `trust-cg-codegen` (especially `src/jit.rs`).
//!
//! ## Why this exists
//!
...
//! box and must not break `trust-cg-test doctor` or CI.
//!
//! # Examples
//!
//! ```text
...
/// `#[cfg(target_os = "linux")]` blocks. Today only `trust-cg-codegen` has
...
const DEFAULT_PACKAGE: &str = "trust-cg-codegen";

/// Arguments for `trust-cg-test lint-linux`.
...
pub struct LintLinuxArgs {
    ...
    /// Package(s) to check. Defaults to `trust-cg-codegen`. Repeatable.
...
fn print_human(r: &LintReport, any_ran: bool) {
    println!("trust-cg-test lint-linux");
    println!();
    if !r.rustup_present {
    ...
    println!();
    match r.exit {
        ResultStatus::Ok if any_ran => {
            println!("  status: OK (all installed Linux targets compiled)");
        }
        ResultStatus::Ok => println!(
            "  status: OK (no Linux targets installed; install one with `rustup target add <triple>`)"
        ),
        ResultStatus::Failed => println!(
            "  status: FAILED (see failing row above; re-run `cargo check --target <triple> -p <package>` for full output)"
        ),
        ResultStatus::EnvBroken => println!("  status: ENV_BROKEN (cargo missing)"),
        ResultStatus::NotImplemented | ResultStatus::UsageError | ResultStatus::Errored => {
            println!("  status: {:?}", r.exit);
        }
    }
}
