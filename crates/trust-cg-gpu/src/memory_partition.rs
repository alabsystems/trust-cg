// trust-cg-gpu/memory_partition.rs - Split host/device allocations
//
...
use trust_cg_codegen::metal_emitter::MtlStorageMode;
...
impl MemoryPartition {
    ...
    pub fn run(&self, regions: &mut [KernelRegion]) -> Vec<BufferPlan> {
        ...
        // from trust_ir type info and shape proofs.
        ...
        for region in regions.iter_mut() {
            ...
            let collect = |buf: BufferId, role: BufferRole| {
                ...
                let storage = match space {
                    AddressSpace::Device | AddressSpace::Constant => MtlStorageMode::Shared,
                ...
                BufferPlan {
                    ...
                    consumer_regions: consumers.get(&buf).cloned().unwrap_or_default(),
                    role,
                }
            };
