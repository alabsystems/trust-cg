// trust-cg-gpu/hetero_partition.rs - CPU vs GPU region dispatch
//
...
//! analyzer from `trust-cg-ir::cost_model`.

use trust_cg_lower::compute_graph::{ComputeGraph, TargetRecommendation};
...
impl HeteroPartition {
    ...
    pub fn run(&self, graph: &ComputeGraph, regions: &[KernelRegion]) -> Vec<TargetRecommendation> {
        ...
        for region in regions {
            ...
            let node_id_for_rec = region.nodes.first().copied().unwrap_or(
                ...
                trust_cg_lower::compute_graph::ComputeNodeId(0),
            );
