// trust-cg-gpu/region.rs - Shared kernel-region data structures
//
...
use trust_cg_lower::compute_graph::{AcceleratorSemanticRecipe, ComputeNodeId, TrustIrValueId};
...
///
/// Buffers may correspond to real trust_ir `ValueId`s (when a region reads
...
pub struct KernelRegion {
    ...
    /// Exact TrustIR values bound to `input_buffers`, in binding order.
    pub input_values: Vec<TrustIrValueId>,
    /// Exact TrustIR values bound to `output_buffers`, in binding order.
    pub output_values: Vec<TrustIrValueId>,
    ...
    /// Sealed compiler recipe from GraphBuilder.  Private so manual regions
    /// cannot mint accelerator launch authority.
    semantic_recipe: Option<AcceleratorSemanticRecipe>,
}
...
impl KernelRegion {
    ...
    #[allow(clippy::too_many_arguments)] // Inputs are the complete extraction boundary.
    pub fn new(
        id: RegionId,
        name: String,
    ...
    pub(crate) fn bind_semantic_recipe(&mut self, recipe: AcceleratorSemanticRecipe) {
        self.semantic_recipe = Some(recipe);
    }

    pub(crate) fn semantic_recipe(&self) -> Option<&AcceleratorSemanticRecipe> {
        self.semantic_recipe.as_ref()
    }
