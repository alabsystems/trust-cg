// trust-cg-ir integration tests for core types
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// Integration tests that exercise the public API of trust-cg-ir across modules.

use trust_cg_ir::aarch64_regs;
use trust_cg_ir::cc::{AArch64CC, FloatSize, OperandSize};
use trust_cg_ir::function::{
    EnumTagWidth, MachBlock, MachFunction, Signature, StackSlot, StackSlotAllocationKind,
    StackSlotSizeSource, Type,
};
use trust_cg_ir::inst::{AArch64Opcode, InstFlags, MachInst};
use trust_cg_ir::operand::MachOperand;
use trust_cg_ir::regs::{PReg, RegClass, SpecialReg, VReg, X0, X1, X29};
use trust_cg_ir::types::{BlockId, FrameIdx, InstId, StackSlotId, VRegId};
...
fn crate_root_reexports_types() {
    ...
    let _cc: trust_cg_ir::AArch64CC = AArch64CC::EQ;
    let _os: trust_cg_ir::OperandSize = OperandSize::S64;
    let _fs: trust_cg_ir::FloatSize = FloatSize::F32;
    let _block: trust_cg_ir::MachBlock = MachBlock::new();
    let _ty: trust_cg_ir::Type = Type::I64;
    let _opcode: trust_cg_ir::AArch64Opcode = AArch64Opcode::AddRR;
    let _flags: trust_cg_ir::InstFlags = InstFlags::EMPTY;
    let _preg: trust_cg_ir::PReg = X0;
    let _rc: trust_cg_ir::RegClass = RegClass::Gpr64;
    let _vreg: trust_cg_ir::VReg = VReg::new(0, RegClass::Gpr64);
    let _sp: trust_cg_ir::SpecialReg = SpecialReg::SP;
    let _bid: trust_cg_ir::BlockId = BlockId(0);
    let _iid: trust_cg_ir::InstId = InstId(0);
    let _vid: trust_cg_ir::VRegId = VRegId(0);
    let _sid: trust_cg_ir::StackSlotId = StackSlotId(0);
    let _fi: trust_cg_ir::FrameIdx = FrameIdx(0);
}
...
fn build_function_with_stack_slots() {
    ...
    assert_eq!(
        func.stack_slots[0].allocation,
        StackSlotAllocationKind::Fixed
    );
    assert_eq!(
        func.stack_slots[1].allocation,
        StackSlotAllocationKind::Fixed
    );
}

#[test]
fn stack_slot_api_fixed_and_runtime() {
    let fixed = StackSlot::new(8, 8);
    assert!(fixed.is_fixed_size());
    assert!(!fixed.is_runtime_sized());
    assert_eq!(fixed.size_source(), None);

    let dynamic = StackSlot::new_dynamic(StackSlotSizeSource::Value(3), 16);
    assert!(dynamic.is_runtime_sized());
    assert!(!dynamic.is_fixed_size());
    assert_eq!(dynamic.size_source(), Some(StackSlotSizeSource::Value(3)));
    assert_eq!(
        dynamic.allocation,
        StackSlotAllocationKind::RuntimeSized {
            size_source: StackSlotSizeSource::Value(3),
        }
    );
}

#[test]
fn build_function_with_dynamic_stack_slots_keeps_fixed_slot_fields() {
    let sig = Signature::new(vec![Type::I64], vec![]);
    let mut func = MachFunction::new("spill_runtime".to_string(), sig);

    let _ss_fixed = func.alloc_stack_slot(StackSlot::new(8, 8));
    let _ss_dyn = func.alloc_stack_slot(StackSlot::new_dynamic(StackSlotSizeSource::Unknown, 8));
    ...
    assert_eq!(
        func.stack_slots[1].allocation,
        StackSlotAllocationKind::RuntimeSized {
            size_source: StackSlotSizeSource::Unknown,
        }
    );
    assert_eq!(func.stack_slots[1].size, 0);
    assert_eq!(func.stack_slots[1].align, 8);
}
...
fn regclass_for_type_consistent_with_type_properties() {
    ...
    // V128 must retain vector-register semantics despite sharing I128's width.
    assert_eq!(RegClass::for_type(&Type::V128), RegClass::Fpr128);
    assert_ne!(
        RegClass::for_type(&Type::V128),
        RegClass::for_type(&Type::I128)
    );
    ...
    assert_eq!(RegClass::for_type(&enum_ty), RegClass::Gpr64);
}

#[test]
fn enum_tagged_union_layout_is_tag_plus_aligned_payload() {
    let enum_ty = Type::Enum {
        tag_width: EnumTagWidth::U8,
        variants: vec![vec![], vec![Type::I64], vec![Type::I32, Type::I16]],
    };

    assert!(enum_ty.is_aggregate());
    assert_eq!(enum_ty.enum_tag_type(), Some(Type::I8));
    assert_eq!(enum_ty.enum_payload_offset(), Some(8));
    assert_eq!(enum_ty.align(), 8);
    assert_eq!(enum_ty.bytes(), 16);
}
...
fn aarch64_regs_overlap() {
    // X0 and W0 overlap
    assert!(aarch64_regs::regs_overlap(
        aarch64_regs::X0,
        aarch64_regs::W0
    ));
...
fn aarch64_regs_condcode_integration() {
    ...
    assert_eq!(
        aarch64_regs::CondCode::EQ.invert(),
        aarch64_regs::CondCode::NE
    );
    ...
    // Parse
    assert_eq!("eq".parse(), Ok(aarch64_regs::CondCode::EQ));
    assert_eq!("cs".parse(), Ok(aarch64_regs::CondCode::HS));
}
