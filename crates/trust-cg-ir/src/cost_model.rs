// trust-cg-ir/cost_model.rs - AArch64 Apple Silicon instruction cost model
//
...
//!
//! ```text
//! use trust_cg_ir::cost_model::{MultiTargetCostModel, ComputeTarget, CostModelGen};
//!
...
impl AppleSiliconCostModel {
    ...
    fn cost_pair(&self, opcode: AArch64Opcode) -> (u32, f64) {
        ...
        match opcode {
            ...
            // EOR with ROR-shifted second source: the shifted-register logical
            // form issues on the same integer ALU units at 1-cycle latency on
            // Apple Silicon (the in-lane immediate ROR is free in the ALU).
            // Shifted-register logical forms cost what the unshifted ones do:
            // the shifter sits in the same pipe on the modeled core.
            EorRRShift | EorRRLsl | EorRRLsr => (1, m4_int_tp),
            // ADD/SUB with an LSL-shifted second source: the shifted-register
            // arithmetic form issues on the same integer ALU units at 1-cycle
            // latency on Apple Silicon (the in-lane immediate shift is free).
            AddRRShift | SubRRShift | AddRRShiftLsr => (1, m4_int_tp),
            OrnRR => (1, m4_int_tp),
            BicRR => (1, m4_int_tp),

            // ===== Shifts (1-cycle, uses ALU units) =====
            LslRR | LsrRR | AsrRR => (1, m4_int_tp),
            LslRI | LsrRI | AsrRI | RorRI | Rbit => (1, m4_int_tp),
            ...
            // ELF local-exec TLS TPREL adds: plain ADD-immediate ALU ops.
            AddTprelHi12 | AddTprelLo12 => (1, m4_int_tp),
            ...
            LdrRI => (4, 2.0),
            // Volatile loads have the same machine cost as plain loads.
            VolatileLdrRI | VolatileLdrbRI | VolatileLdrhRI => (4, 2.0),
            LdrPreIndex => (4, 2.0),
            LdrPostIndex => (4, 2.0),
            LdrbRI => (4, 2.0),
            LdrhRI => (4, 2.0),
            ...
            StrRI => (1, 2.0),
            // Volatile stores have the same machine cost as plain stores.
            VolatileStrRI | VolatileStrbRI | VolatileStrhRI => (1, 2.0),
            StrPreIndex => (1, 2.0),
            StrPostIndex => (1, 2.0),
            StrbRI => (1, 2.0),
            StrhRI => (1, 2.0),
            ...
            FsubRR => (3, 2.0),
            FmulRR => (4, 2.0),
            // FMADD (fused multiply-add): 4-cycle latency on the FP mul/FMA unit,
            // 2-way issue — same class as FMUL (Firestorm/M-series).
            FmaddRR => (4, 2.0),
            FdivRR => (10, 0.5),             // Single: ~10, Double: ~15
            FminnmRR | FmaxnmRR => (3, 2.0), // FMINNM/FMAXNM: ~3-cycle FP op
            FnegRR => (1, 2.0),              // Simple bit flip, uses FP unit
            FabsRR => (1, 2.0),              // Simple bit clear, uses FP unit
            // FRINTM/FRINTP/FRINTZ round-to-integral: ~3-cycle FP op (not a
            // trivial bit-op like FNEG/FABS), one of the FP units.
            FrintmRR | FrintpRR | FrintzRR => (3, 2.0),
            ...
            // FCSEL: FPR-domain conditional select (reads NZCV, no FP arith).
            // On Apple Silicon this issues on the FP/vector pipes at ~2-cycle
            // latency — the whole point is that it stays in the FP bank instead
            // of the old FMOV->CSEL->FMOV cross-bank round trip (~6cy each way).
            FcselRR => (2, 2.0),
            ...
            NeonFsubV => (3, 2.0),
            NeonFmulV => (4, 2.0),
            // FP vector fused multiply-accumulate: multiply-class latency.
            // NeonFmlaLaneV (FMLA by element) has the same multiply-class cost.
            NeonFmlaV | NeonFmlsV | NeonFmlaLaneV => (4, 2.0),
            // Vector integer->FP conversion: FP-convert latency.
            NeonUcvtfV | NeonScvtfV => (3, 2.0),
            // Vector f32->f64 widening convert (FCVTL/FCVTL2): FP-convert latency,
            // two-register-misc SIMD-pipe op like the int->FP converts.
            NeonFcvtlV | NeonFcvtl2V => (3, 2.0),
            // FP lane extract to scalar D register (MOV Dd, Vn.D[lane]).
            NeonDupScalarD => (3, 1.0),
            NeonFdivV => (10, 0.5),
            NeonFcmgtV => (2, 2.0),
            // Logic ops: 1-2 cycle latency
            NeonAndV | NeonOrrV | NeonEorV | NeonBicV | NeonNotV | NeonRbitV | NeonRev32V
            | NeonRev64V => (1, 2.0),
            // Compare: 2 cycles (signed: gt/ge; unsigned: hi/hs)
            NeonCmeqV | NeonCmgtV | NeonCmgeV | NeonCmhiV | NeonCmhsV => (2, 2.0),
            // Integer min/max (per lane): 2-cycle latency, runs on the SIMD pipes.
            NeonSmaxV | NeonSminV | NeonUmaxV | NeonUminV => (2, 2.0),
            // Horizontal reduction across four i32 lanes.
            NeonUmaxv => (3, 1.0),
            // Horizontal add across two i64 lanes.
            NeonAddpScalar => (3, 1.0),
            // DUP/INS/MOVI: 2-3 cycle latency
            NeonDupElem | NeonDupGen => (2, 2.0),
            NeonInsGen | NeonUmovGen => (3, 1.0),
            NeonMovi => (2, 2.0),
            // SIMD loads: 4-cycle L1 hit, stores: 1-cycle dispatch
            NeonLd1Post => (4, 2.0),
            // LDP Q-pair post-index: one load-unit op delivering 32 bytes
            // (two Q registers); same L1-hit latency as a single Q load.
            NeonLdpQPost => (4, 2.0),
            NeonSt1Post => (1, 2.0),
            // STP Q-pair post-index: one store-unit op writing 32 bytes (two Q
            // registers); same 1-cycle dispatch as a single Q store.
            NeonStpQPost => (1, 2.0),
            // SIMD shift by immediate: 2-cycle latency, runs on the SIMD pipes.
            NeonShlVImm | NeonUshrVImm | NeonSshrVImm => (2, 2.0),
            // Population count (per byte) and unsigned add-long-pairwise (widening
            // pairwise reduce): 2-3 cycle SIMD-pipe latency, like the other
            // two-register-misc NEON ops.
            NeonCntV => (2, 2.0),
            NeonUaddlpV => (3, 2.0),
            NeonSaddlpV => (3, 2.0),
            // Per-lane signed absolute value: 2-cycle SIMD-pipe latency, like the
            // other same-shape two-register-misc NEON ops.
            NeonAbsV => (2, 2.0),
            // Unsigned dot-product accumulate (FEAT_DotProd): multiply-class
            // SIMD-pipe latency, like the vector multiply.
            NeonUdotV => (3, 2.0),
            // Bitwise insert-if-true: plain SIMD logical, like AND/ORR.
            NeonBitV => (2, 2.0),
            // Byte-wise extract/concatenate (EXT sliding window): permute-class
            // 2-cycle SIMD-pipe latency, like DUP and the immediate shifts.
            NeonExtV => (2, 2.0),
            // Widening multiply-accumulate-long (SMLAL/SMLAL2/UMLAL/UMLAL2):
            // multiply-class SIMD-pipe latency, like the vector multiply and UDOT.
            NeonSmlalV | NeonSmlal2V | NeonUmlalV | NeonUmlal2V => (3, 2.0),
            // Widening add-wide (UADDW/UADDW2): add-class SIMD-pipe latency,
            // like the plain vector add (no multiply in the path).
            // (SADDW/SADDW2 are the signed siblings — same add-class pipe.)
            NeonUaddwV | NeonUaddw2V | NeonSaddwV | NeonSaddw2V => (2, 2.0),
            // Vector multiply-accumulate (MLA.4S): multiply-class SIMD-pipe
            // latency, like MUL/UDOT/the xMLAL family (measured ~3cy on M4).
            NeonMlaV => (3, 2.0),
            // Pairwise widening accumulate (UADALP .4S -> .2D): accumulate-class
            // latency (measured ~3cy serial-chain on M4, same as UDOT).
            NeonUadalpV => (3, 2.0),

            // ===== Pseudo-instructions (no execution cost) =====
            Phi | StackAlloc | Copy | Nop => (0, f64::INFINITY),

            // ===== Emission-time alignment padding =====
            // A real encoded NOP: retires without executing (zero latency,
            // wide issue). Created only after all cost-model consumers ran,
            // so this entry exists for totality.
            AlignNop => (0, f64::INFINITY),

            // ===== Trap instructions / guard pseudos =====
            // BRK and fallback trap pseudos have one trap instruction's cost.
            // Exact proof-only guards use the same conservative unit cost until
            // proof opts remove them or lowering expands them.
            ...
            | TrapShiftRangeIfOOB | TrapOverflowExact => (1, 1.0),
            ...
            Ldadd | Ldadda | Ldaddal | Ldaddl => (6, 0.5),
            Ldclr | Ldclra | Ldclral | Ldclrl => (6, 0.5),
            Ldeor | Ldeora | Ldeoral | Ldeorl => (6, 0.5),
            Ldset | Ldseta | Ldsetal | Ldsetl => (6, 0.5),
            Ldsmax | Ldsmaxa | Ldsmaxal | Ldsmaxl => (6, 0.5),
            Ldsmin | Ldsmina | Ldsminal | Ldsminl => (6, 0.5),
            Ldumax | Ldumaxa | Ldumaxal | Ldumaxl => (6, 0.5),
            Ldumin | Ldumina | Lduminal | Lduminl => (6, 0.5),
            Swp | Swpa | Swpal | Swpl => (6, 0.5),
            // CAS: ~8 cycles (compare + conditional store).
            Cas | Casa | Casal | Casl => (8, 0.5),
...
pub enum NeonOp {
    ...
    Bic,
    /// Bit reverse within each byte (RBIT Vd.{8B,16B}, Vn.{8B,16B})
    Rbit,
    /// Reverse bytes within 32-bit lanes (REV32 Vd.{8B,16B}, Vn.{8B,16B})
    Rev32,
    /// Reverse bytes within 64-bit lanes (REV64 Vd.{8B,16B}, Vn.{8B,16B})
    Rev64,
...
impl NeonOp {
    ...
    pub fn from_name(name: &str) -> Option<Self> {
        match name.to_ascii_uppercase().as_str() {
            ...
            "BIC" => Some(Self::Bic),
            "RBIT" => Some(Self::Rbit),
            "REV32" => Some(Self::Rev32),
            "REV64" => Some(Self::Rev64),
            "SHL" => Some(Self::Shl),
            "USHR" => Some(Self::Ushr),
            "SSHR" => Some(Self::Sshr),
            "FADD" => Some(Self::Fadd),
...
impl MultiTargetCostModel {
    ...
    pub fn neon_cost(&self, op: NeonOp, arr: NeonArrangement) -> (u32, f64) {
        ...
        match op {
            ...
            // RBIT/REV are byte permute/bit-manipulate NEON ops. Use the same
            // conservative cost class as simple integer NEON operations until a
            // measured Apple-specific split is needed.
            NeonOp::Rbit | NeonOp::Rev32 | NeonOp::Rev64 => (2, fp_tp),
...
mod tests {
    ...
    fn load_latency_is_4() {
        ...
        assert_eq!(model.latency(AArch64Opcode::LdrPreIndex), 4);
        assert_eq!(model.latency(AArch64Opcode::LdrPostIndex), 4);
    ...
    fn store_latency_is_low() {
        ...
        assert_eq!(model.latency(AArch64Opcode::StrPreIndex), 1);
        assert_eq!(model.latency(AArch64Opcode::StrPostIndex), 1);
    ...
    #[test]
    fn neon_umov_extract_cost_is_modeled() {
        let model = m1();
        assert_eq!(model.latency(AArch64Opcode::NeonUmovGen), 3);
        assert_eq!(model.throughput(AArch64Opcode::NeonUmovGen), 1.0);
    }

    #[test]
    fn neon_addp_scalar_cost_is_modeled() {
        let model = m1();
        assert_eq!(model.latency(AArch64Opcode::NeonAddpScalar), 3);
        assert_eq!(model.throughput(AArch64Opcode::NeonAddpScalar), 1.0);
    }
    ...
    fn neon_op_from_name_known() {
        ...
        assert_eq!(NeonOp::from_name("rbit"), Some(NeonOp::Rbit));
        assert_eq!(NeonOp::from_name("rev32"), Some(NeonOp::Rev32));
        assert_eq!(NeonOp::from_name("rev64"), Some(NeonOp::Rev64));
