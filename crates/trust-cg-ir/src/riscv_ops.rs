// trust-cg-ir - RISC-V opcode definitions
//
...
pub enum RiscVOpcode {
    ...
    // =====================================================================
    /// EBREAK — raise a breakpoint exception (environment trap).
    ///
    /// The RISC-V analogue of x86's `UD2` / AArch64's `BRK`: a real,
    /// unconditional runtime trap instruction. It encodes as the fixed
    /// I-type word `0x00100073` (SYSTEM opcode, funct12 = 1, all register
    /// fields zero), so it takes no register/immediate operands. The
    /// bounds-check carrier expansion uses it to materialize a trap block
    /// for the failing edge of a surviving `index < bound` check.
    ///
    /// Appended after Nop to preserve older implicit opcode discriminants.
    Ebreak,
    ...
    ///
    /// This is the RISC-V analogue of AArch64's / x86's `TrapBoundsCheckExact`.
    /// The (future) RISC-V instruction selector emits it ONLY for a
    /// `getelementptr inbounds` access whose exact element count is statically
    /// known (i.e. a genuinely `InBounds`-proven access), recording the
    /// discharged-obligation binding by the operand fingerprint the
    /// Certified-Elimination Kernel computes.
    ///
    ...
    /// otherwise the RISC-V codegen pipeline EXPANDS it to a real unsigned
    /// `BGEU index, bound -> EBREAK` runtime check
    /// (`expand_riscv_bounds_check_carriers`). Direct encoder use intentionally
    /// fails closed (`RiscVEncodeError::UnsupportedOpcode`) so an un-expanded
...
impl RiscVOpcode {
    ...
    pub fn default_flags(self) -> InstFlags {
        ...
        match self {
            ...
            Div | Divu | Rem | Remu | Divw | Divuw | Remw | Remuw => InstFlags::HAS_SIDE_EFFECTS,

            // EBREAK is a real target trap instruction (breakpoint exception):
            // it terminates the current flow with a side effect, like x86 UD2.
            Ebreak => InstFlags::HAS_SIDE_EFFECTS.union(InstFlags::IS_TERMINATOR),
            ...
            // is NOT a terminator on RISC-V: the access falls through it). It must
            ...
            // real BGEU(index>=bound) -> EBREAK runtime check before encoding.
    ...
    ///
    /// `TrapBoundsCheckExact` is a pseudo (proof-only carrier) — it has no direct
    /// hardware encoding and is deleted or expanded before emission. `Ebreak` is
    /// NOT a pseudo: it is a real trap instruction with a fixed encoding.
    pub fn is_pseudo(self) -> bool {
        matches!(
            self,
            Self::Phi | Self::StackAlloc | Self::Nop | Self::TrapBoundsCheckExact
        )
    }
...
mod tests {
    ...
    fn opcode_count() {
        ...
        // Pseudo: 3 + 1 (TrapBoundsCheckExact carrier) = 4
        // System/trap: 1 (Ebreak)
        // Total: 30 + 11 + 8 + 13 + 16 + 4 + 1 = 83
        ...
        assert_eq!(all_opcodes.len(), 83);
    }

    #[test]
    fn ebreak_is_trap_terminator_with_side_effects() {
        let flags = RiscVOpcode::Ebreak.default_flags();
        assert!(flags.contains(InstFlags::HAS_SIDE_EFFECTS));
        assert!(flags.contains(InstFlags::IS_TERMINATOR));
        // EBREAK is a real instruction, not a pseudo.
        assert!(!RiscVOpcode::Ebreak.is_pseudo());
    }

    #[test]
    fn bounds_check_carrier_is_side_effecting_pseudo() {
        let flags = RiscVOpcode::TrapBoundsCheckExact.default_flags();
        // Side-effecting so DCE/CSE never drop/reorder it before the kernel gate.
        assert!(flags.contains(InstFlags::HAS_SIDE_EFFECTS));
        assert!(flags.contains(InstFlags::IS_PSEUDO));
        // Pseudo (proof-only carrier, no direct hardware encoding).
        assert!(RiscVOpcode::TrapBoundsCheckExact.is_pseudo());
        // Not a terminator: the proven access falls through it.
        assert!(!flags.contains(InstFlags::IS_TERMINATOR));
    }
}
