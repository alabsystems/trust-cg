// trust-cg-ir - RISC-V register model
//
...
///
/// The encoding is internal to Trust Codegen and organizes registers by class.
...
/// Allocatable FPRs.
///
/// FAIL-CLOSED ABI INVARIANT: only the caller-saved FPRs (ft0-ft11, fa0-fa7)
/// are allocatable. The callee-saved FPRs fs0-fs11 are intentionally EXCLUDED
/// because no Fsd/Fld callee-saved FPR save/restore path exists in the
/// prologue/epilogue yet (the GPR path uses Sd/Ld and compute_frame_size counts
/// only GPR callee-saved slots). Were fs0-fs11 allocatable, an FP value live
/// ACROSS a call could be coloured onto an fs-register and then silently
/// clobbered by the callee with no save/restore — a miscompile. Dropping them
/// from the pool makes that IMPOSSIBLE by construction: a call-spanning FP value
/// can only land in a caller-saved FPR (forced off by the call-clobber model,
/// hence spilled) — never in a never-preserved callee-saved FPR. This keeps the
/// assign() fail-closed pool check load-bearing (any allocation outside this
/// pool is rejected). Restore fs0-fs11 here only once an Fsd/Fld callee-saved
/// save path + frame slots exist (mirror the GPR path).
pub const RISCV_ALLOCATABLE_FPRS: [RiscVPReg; 20] = RISCV_CALLER_SAVED_FPRS;
...
mod tests {
    ...
    fn test_callee_saved_array_consistency() {
        ...
        for &reg in &RISCV_CALLEE_SAVED_GPRS {
            assert!(
                riscv_is_callee_saved(reg),
                "{:?} in array but not callee-saved",
                reg
            );
        }
    ...
    fn test_caller_saved_array_consistency() {
        ...
        for &reg in &RISCV_CALLER_SAVED_GPRS {
            assert!(
                riscv_is_caller_saved(reg),
                "{:?} in array but not caller-saved",
                reg
            );
        }
    ...
    fn test_register_counts() {
        ...
        // fs0-fs11 are intentionally NOT allocatable (no Fsd/Fld callee-saved
        // save path); only the 20 caller-saved FPRs are.
        assert_eq!(RISCV_ALLOCATABLE_FPRS.len(), 20);
        // The allocatable pool must contain NO callee-saved FPR — this is the
        // soundness invariant that makes an FP-across-call clobber impossible.
        for fs in RISCV_CALLEE_SAVED_FPRS {
            assert!(
                !RISCV_ALLOCATABLE_FPRS.contains(&fs),
                "callee-saved FPR {fs:?} must not be allocatable"
            );
        }
