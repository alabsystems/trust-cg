// trust-cg-codegen/elf/debug.rs - DWARF debug sections and ELF metadata stubs
//
...
impl DwarfDebugStubs {
    ...
    pub fn debug_line_bytes(&self) -> Vec<u8> {
        ...
        let header_body = vec![
            ...
            0, // line_base
            1, // line_range
            1, // opcode_base (no standard opcode lengths)
            0, // directory_entry_format_count (ULEB128 0)
            0, // directories_count (ULEB128 0)
            0, // file_name_entry_format_count (ULEB128 0)
            0, // file_names_count (ULEB128 0)
        ];
...
/// this stub supports generating them for when Trust Codegen produces
...
mod tests {
    ...
    fn test_note_gnu_stack_metadata() {
        ...
        assert_eq!(
            note.section_flags() & SHF_EXECINSTR,
            0,
            "must not be executable"
        );
        assert_eq!(note.section_align(), 1);
        assert!(
            note.section_bytes().is_empty(),
            "marker section has no data"
        );
    }
    ...
    fn test_program_header_null() {
        ...
        assert!(
            ...
            "null phdr should be zeros after type"
        );
    }
