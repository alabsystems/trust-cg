// trust-cg-codegen/riscv/encode.rs - RISC-V instruction binary encoder
//
...
use trust_cg_ir::riscv_regs::RiscVPReg;
...
pub enum RiscVEncodeError {
    ...
    /// Opcode has no direct hardware encoding and must be expanded or deleted
    /// before emission (Sentinel S5: the proof-only bounds-check carrier). Reaching
    /// the encoder is fail-closed: rather than emit a silent NOP that would drop a
    /// bounds check (a miscompile), the encoder rejects.
    UnsupportedOpcode(RiscVOpcode),
}
    ...
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::UnsupportedOpcode(op) => {
                write!(
                    f,
                    "{:?}: opcode has no direct hardware encoding (must be expanded or deleted before emission)",
                    op
                )
            }
        }
...
/// The fixed I-type encoding for EBREAK: SYSTEM opcode (0b1110011) with
/// funct12 = 1 (imm[11:0] = 0x001) and all register fields zero. The canonical
/// word is `0x00100073`.
...
pub fn encode_i_type(opcode: u32, rd: u32, funct3: u32, rs1: u32, imm12: i32) -> u32 {
    ...
    imm << 20 | (rs1 & 0x1F) << 15 | (funct3 & 0x7) << 12 | (rd & 0x1F) << 7 | (opcode & 0x7F)
}
...
pub fn encode_u_type(opcode: u32, rd: u32, imm20: i32) -> u32 {
    // INTENTIONAL MASK (by-design, not a reject-candidate): LUI/AUIPC carry a
    // 20-bit upper immediate already shifted into bits[31:12]; the assembler
    // convention is that the 20-bit field is the full encodable space, so the
    // `& 0xFFFFF` narrows to exactly the architectural field rather than
    // silently truncating an out-of-range value.
    let imm = (imm20 as u32) & 0xFFFFF;
    imm << 12 | (rd & 0x1F) << 7 | (opcode & 0x7F)
}
...
pub fn encode_instruction(
    ...
    match opcode {
        ...
        // =================================================================
        // System / trap (Sentinel S5)
        // =================================================================
        // EBREAK is a fixed-encoding I-type trap (no register/immediate operands).
        Ebreak => Ok(RISCV_EBREAK),

        // =================================================================
        // Proof-only bounds-check carrier (Sentinel S5): fail closed.
        // =================================================================
        // A surviving carrier MUST be expanded to a real BGEU+EBREAK check
        // (`expand_riscv_bounds_check_carriers`) or deleted under kernel
        ...
        // that would drop the bounds check (a miscompile).
        TrapBoundsCheckExact => Err(RiscVEncodeError::UnsupportedOpcode(opcode)),
    }
}
...
fn encode_r(
    ...
    let rs1 = ops
        .rs1
        .ok_or(RiscVEncodeError::MissingRs1(opcode))?
        .hw_enc() as u32;
    let rs2 = ops
        .rs2
        .ok_or(RiscVEncodeError::MissingRs2(opcode))?
        .hw_enc() as u32;
    Ok(encode_r_type(op, rd, funct3, rs1, rs2, funct7))
}

/// Signed 12-bit immediate bounds shared by the I- and S-type formats.
const IMM12_MIN: i32 = -2048;
const IMM12_MAX: i32 = 2047;

/// FAIL-CLOSED: reject an immediate that does not fit the signed 12-bit field
/// before the bit-packers (`encode_i_type`/`encode_s_type`) silently mask it
/// with `& 0xFFF`. A masked out-of-range immediate is a silent miscompile
/// (e.g. an over-large frame `ADDI SP, SP, -frame_size` that moves SP the wrong
/// way); a typed error is always preferable to wrong machine code.
#[inline]
fn check_imm12(opcode: RiscVOpcode, imm: i32) -> Result<(), RiscVEncodeError> {
    if !(IMM12_MIN..=IMM12_MAX).contains(&imm) {
        return Err(RiscVEncodeError::ImmediateOutOfRange {
            opcode,
            value: imm,
            bits: 12,
        });
...
fn encode_shift_imm(
    ...
    let rs1 = ops
        ...
        .ok_or(RiscVEncodeError::MissingRs1(opcode))?
        .hw_enc() as u32;
    // INTENTIONAL MASK (by-design, not a reject-candidate): SLLI/SRLI/SRAI on
    // RV64 take a 6-bit shift amount; the architecture defines shift counts mod
    // register width, so the low 6 bits ARE the architectural shift amount and
    // `& 0x3F` narrows to exactly the valid encodable space.
...
fn encode_shift_imm_w(
    ...
    let rs1 = ops
        ...
        .ok_or(RiscVEncodeError::MissingRs1(opcode))?
        .hw_enc() as u32;
    // INTENTIONAL MASK (by-design, not a reject-candidate): SLLIW/SRLIW/SRAIW
    // take a 5-bit shift amount defined mod 32 (shamt[5]=1 is reserved on the
    // W-forms), so `& 0x1F` narrows to exactly the valid 5-bit shift space.
...
/// FAIL-CLOSED: a B-type branch offset encodes bits [12:1] (a SIGNED 13-bit
/// value with an implicit bit-0 = 0). The bit-packer (`encode_b_type`) silently
/// MASKS the offset — an out-of-range or odd offset would be truncated into a
/// WRONG target (a silent miscompile). Reject anything that does not fit the
/// signed 13-bit even range BEFORE packing. Mirrors the I/S-type `check_imm12`
/// guard.
///
/// Signed 13-bit range: `[-4096, 4094]`; bit 0 must be 0 (offset is even).
const BRANCH_OFFSET_MIN: i32 = -4096;
const BRANCH_OFFSET_MAX: i32 = 4094;

#[inline]
fn check_branch_offset(opcode: RiscVOpcode, offset: i32) -> Result<(), RiscVEncodeError> {
    if !(BRANCH_OFFSET_MIN..=BRANCH_OFFSET_MAX).contains(&offset) || (offset & 1) != 0 {
        return Err(RiscVEncodeError::ImmediateOutOfRange {
            opcode,
            value: offset,
            bits: 13,
...
/// FAIL-CLOSED: a J-type (JAL) offset encodes bits [20:1] (a SIGNED 21-bit
/// value with an implicit bit-0 = 0). Same masking hazard as B-type — reject an
/// out-of-range or odd offset rather than silently truncate it.
///
/// Signed 21-bit range: `[-1048576, 1048574]`; bit 0 must be 0 (offset is even).
const JUMP_OFFSET_MIN: i32 = -1_048_576;
const JUMP_OFFSET_MAX: i32 = 1_048_574;

#[inline]
fn check_jump_offset(opcode: RiscVOpcode, offset: i32) -> Result<(), RiscVEncodeError> {
    if !(JUMP_OFFSET_MIN..=JUMP_OFFSET_MAX).contains(&offset) || (offset & 1) != 0 {
        return Err(RiscVEncodeError::ImmediateOutOfRange {
            opcode,
            value: offset,
            bits: 21,
...
fn encode_b(
    ...
    let rs2 = ops
        ...
        .ok_or(RiscVEncodeError::MissingRs2(opcode))?
        .hw_enc() as u32;
    check_branch_offset(opcode, ops.imm)?;
...
fn encode_j(ops: &RiscVInstOperands, opcode: RiscVOpcode) -> Result<u32, RiscVEncodeError> {
    let rd = ops.rd.ok_or(RiscVEncodeError::MissingRd(opcode))?.hw_enc() as u32;
    check_jump_offset(opcode, ops.imm)?;
...
mod tests {
    use super::*;
    use trust_cg_ir::riscv_regs::{F0, F1, F10, F11};
    use trust_cg_ir::riscv_regs::{X0, X1, X2, X5, X10, X11, X12, X15, X28, X31};
    ...
    fn test_nop_encoding() {
        ...
        assert_eq!(
            enc(RiscVOpcode::Nop, &RiscVInstOperands::none()),
            0x00000013
        );
        assert_eq!(
            enc(RiscVOpcode::Phi, &RiscVInstOperands::none()),
            0x00000013
        );
        assert_eq!(
            enc(RiscVOpcode::StackAlloc, &RiscVInstOperands::none()),
            0x00000013
        );
    }

    // -----------------------------------------------------------------------
    // System / trap + proof-only carrier (Sentinel S5)
    // -----------------------------------------------------------------------

    #[test]
    fn test_ebreak_encoding() {
        // EBREAK = 0x00100073 (SYSTEM opcode, funct12 = 1, all reg fields 0).
        assert_eq!(
            enc(RiscVOpcode::Ebreak, &RiscVInstOperands::none()),
            0x0010_0073
        );
    }

    #[test]
    fn test_bounds_carrier_fails_closed() {
        // Soundness: the encoder MUST reject an un-expanded proof-only carrier
        ...
        assert!(matches!(
            result,
            Err(RiscVEncodeError::UnsupportedOpcode(
                RiscVOpcode::TrapBoundsCheckExact
            ))
        ));
    }

    /// Exhaustive fuzz table: every RiscVOpcode either encodes to a 32-bit word
    /// or fails closed (no panic). Mirrors the x86 `panic_fuzz_encode` table —
    /// the new carrier returns the fail-closed `UnsupportedOpcode` error.
    #[test]
    fn panic_fuzz_encode_every_opcode() {
        use RiscVOpcode::*;
        // A maximally-populated operand bundle so format encoders never hit a
        // missing-register error; the point is "no panic", not exact encodings.
        let ops = RiscVInstOperands {
            rd: Some(X10),
            rs1: Some(X11),
            rs2: Some(X12),
            imm: 4,
        ...
        assert_eq!(all.len(), 83, "fuzz table must enumerate every opcode");
        for op in all {
            // Must not panic; Ok or fail-closed Err are both acceptable.
            let _ = encode_instruction(op, &ops);
        }
        // The carrier specifically must fail closed.
        assert!(matches!(
            encode_instruction(TrapBoundsCheckExact, &ops),
            Err(RiscVEncodeError::UnsupportedOpcode(_))
        ));
    }
    ...
    /// FAIL-CLOSED: the maximum in-range (signed 13-bit, even) B-type offsets
    /// encode without error; out-of-range or odd offsets are REJECTED (not
    /// silently masked into a wrong target).
    #[test]
    fn b_type_range_check_fail_closed() {
        // Max positive (4094) and max negative (-4096) in-range, even: encode Ok.
        assert!(
            encode_instruction(RiscVOpcode::Beq, &RiscVInstOperands::branch(X10, X11, 4094))
                .is_ok()
        );
        assert!(
            encode_instruction(
                RiscVOpcode::Beq,
                &RiscVInstOperands::branch(X10, X11, -4096)
            )
            .is_ok()
        );
        // One past each end: rejected as ImmediateOutOfRange { bits: 13 }.
        assert!(matches!(
            encode_instruction(RiscVOpcode::Beq, &RiscVInstOperands::branch(X10, X11, 4096)),
            Err(RiscVEncodeError::ImmediateOutOfRange { bits: 13, .. })
        ));
        assert!(matches!(
            encode_instruction(
                RiscVOpcode::Bne,
                &RiscVInstOperands::branch(X10, X11, -4098)
            ),
            Err(RiscVEncodeError::ImmediateOutOfRange { bits: 13, .. })
        ));
        // Odd (misaligned) offset: rejected (bit 0 must be 0).
        assert!(matches!(
            encode_instruction(RiscVOpcode::Blt, &RiscVInstOperands::branch(X10, X11, 3)),
            Err(RiscVEncodeError::ImmediateOutOfRange { bits: 13, .. })
        ));
    }
    ...
    /// FAIL-CLOSED: the maximum in-range (signed 21-bit, even) J-type offsets
    /// encode without error; out-of-range or odd offsets are REJECTED.
    #[test]
    fn j_type_range_check_fail_closed() {
        // Max positive (1048574) and max negative (-1048576) in-range, even: Ok.
        assert!(
            encode_instruction(RiscVOpcode::Jal, &RiscVInstOperands::jump(X0, 1_048_574)).is_ok()
        );
        assert!(
            encode_instruction(RiscVOpcode::Jal, &RiscVInstOperands::jump(X0, -1_048_576)).is_ok()
        );
        // One past each end: rejected as ImmediateOutOfRange { bits: 21 }.
        assert!(matches!(
            encode_instruction(RiscVOpcode::Jal, &RiscVInstOperands::jump(X0, 1_048_576)),
            Err(RiscVEncodeError::ImmediateOutOfRange { bits: 21, .. })
        ));
        assert!(matches!(
            encode_instruction(RiscVOpcode::Jal, &RiscVInstOperands::jump(X0, -1_048_578)),
            Err(RiscVEncodeError::ImmediateOutOfRange { bits: 21, .. })
        ));
        // Odd (misaligned) offset: rejected.
        assert!(matches!(
            encode_instruction(RiscVOpcode::Jal, &RiscVInstOperands::jump(X0, 5)),
            Err(RiscVEncodeError::ImmediateOutOfRange { bits: 21, .. })
        ));
    }
