// trust-cg-codegen/macho/symbol.rs - Mach-O symbol table and string table emission
//
...
impl SymbolTable {
    ...
    pub fn add_symbol(
        ...
        if is_global || is_private_extern {
            let idx = self.local_symbols.len() + self.external_symbols.len();
...
mod tests {
    ...
    fn test_get_symbol() {
        ...
        assert_eq!(symtab.symbol_category(0), Some(SymbolCategory::Local));
        assert_eq!(symtab.symbol_category(1), Some(SymbolCategory::External));
        assert_eq!(symtab.symbol_category(2), Some(SymbolCategory::Undefined));
