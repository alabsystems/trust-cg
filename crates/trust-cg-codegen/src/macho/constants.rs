// trust-cg-codegen/macho/constants.rs - Mach-O format constants
//
...
/// Embedded code signature linkedit data command.
...
/// Dyld exports trie linkedit data command.
pub const LC_DYLD_EXPORTS_TRIE: u32 = 0x8000_0033;

/// Dyld chained fixups linkedit data command.
...
// --- Thread-local section types (Darwin TLV) ---
// Reference: S_THREAD_LOCAL_* in mach-o/loader.h. These hold the per-thread
// initial-value template (`__thread_data`/`__thread_bss`) and the TLV
// descriptors (`__thread_vars`) that a `#[thread_local]` read addresses via
// `ARM64_RELOC_TLVP_LOAD_PAGE21`/`PAGEOFF12`.

/// Template of initial values for thread-local variables (`__thread_data`).
pub const S_THREAD_LOCAL_REGULAR: u32 = 0x11;

/// Template of initial zero values for thread-local variables (`__thread_bss`).
pub const S_THREAD_LOCAL_ZEROFILL: u32 = 0x12;

/// TLV descriptors (`__thread_vars`): the `{thunk, key, offset}` triples a
/// thread-local read addresses, one per thread-local variable symbol.
pub const S_THREAD_LOCAL_VARIABLES: u32 = 0x13;

/// Pointers to TLV descriptors (`__thread_ptr`). Reserved; the AArch64 emitter
/// references the descriptor symbol directly via TLVP relocations and does not
/// currently materialize a separate pointer section.
...
/// Size of uuid_command in bytes.
pub const UUID_COMMAND_SIZE: u32 = 24;

/// Size of linkedit_data_command in bytes.
