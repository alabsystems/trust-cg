// trust-cg-codegen/macho/x86_64_reloc.rs - x86-64 Mach-O relocation encoding
//
...
use super::reloc::{MachORelocationError, validate_macho_relocation_fields};
...
impl X86_64Relocation {
    ...
    pub fn section_relative(offset: u32, section_ordinal: u32, kind: X86_64RelocKind) -> Self {
        Self {
            offset,
...
pub fn encode_x86_64_relocation(reloc: &X86_64Relocation) -> Result<[u8; 8], MachORelocationError> {
    reloc.validate()?;
    Ok(encode_x86_64_relocation_validated(reloc))
}

pub(crate) fn encode_x86_64_relocation_validated(reloc: &X86_64Relocation) -> [u8; 8] {
...
impl core::fmt::Display for X86_64RelocDecodeError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        if let Some(address) = self.scattered_address {
            write!(
                f,
                "unsupported x86-64 scattered relocation record with address word {address:#x}"
            )
        } else {
...
pub fn decode_x86_64_relocation(
    bytes: &[u8; 8],
) -> Result<X86_64Relocation, X86_64RelocDecodeError> {
    ...
    if r_word0 & 0x8000_0000 != 0 {
        return Err(X86_64RelocDecodeError {
            type_val,
...
mod tests {
    ...
    fn encode_x86_64_relocation(reloc: &X86_64Relocation) -> [u8; 8] {
        super::encode_x86_64_relocation(reloc).unwrap()
    }
    ...
    #[test]
    fn test_encode_tlv_binary_layout() {
        // TLV: offset=0x08, symbol=8, pc_relative=true, length=2, extern=true, type=9
        let reloc = X86_64Relocation::tlv(0x08, 8);
        let bytes = encode_x86_64_relocation(&reloc);

        assert_eq!(bytes[0..4], [0x08, 0x00, 0x00, 0x00], "r_address");
        ...
        assert_eq!((r_word1 >> 25) & 3, 2, "length (2 = 4 bytes)");
        assert_eq!((r_word1 >> 27) & 1, 1, "is_extern");
        // X86_64_RELOC_TLV == 9 per <mach-o/x86_64/reloc.h>.
        assert_eq!((r_word1 >> 28) & 0xF, 9, "type (TLV)");
        assert_eq!(X86_64RelocKind::Tlv as u8, 9);
    }
    ...
    fn test_encode_overflow_symbol_index() {
        ...
        assert_eq!(
            super::encode_x86_64_relocation(&reloc),
    ...
    fn test_encode_and_decode_reject_scattered_address_bit() {
        let boundary = X86_64Relocation::branch(i32::MAX as u32, 0);
        assert_eq!(
            super::encode_x86_64_relocation(&boundary).unwrap()[0..4],
            (i32::MAX as u32).to_le_bytes(),
        );

        let scattered = X86_64Relocation::branch(i32::MAX as u32 + 1, 0);
        assert_eq!(
            super::encode_x86_64_relocation(&scattered),
        ...
        let error = decode_x86_64_relocation(&bytes).unwrap_err();
