// trust-cg-codegen/macho/section.rs - Mach-O section and segment definitions
//
...
impl SegmentCommand64 {
    ...
    /// Create an object segment with a command size already checked by the
    /// caller's single layout authority.
    pub fn new_object_with_cmdsize(
        cmdsize: u32,
        nsects: u32,
        vmsize: u64,
        fileoff: u64,
...
mod tests {
    ...
    fn test_segment_base_size() {
        let segment =
            SegmentCommand64::new_object_with_cmdsize(SEGMENT_COMMAND_64_SIZE, 0, 0, 0, 0);
        let mut buf = Vec::new();
