// trust-cg-codegen - DWARF CFI (Call Frame Information) for AArch64 and x86-64 macOS
//
...
//!
//! When Trust Codegen marks a frame as requiring Darwin DWARF fallback, including
//! frames with runtime-sized stack metadata or unusual layouts, the compact
//! unwind entry uses `UNWIND_ARM64_MODE_DWARF` and the unwinder looks up the
//! matching FDE in `__TEXT,__eh_frame`. Full runtime-sized allocation lowering
//! remains separate from this unwind metadata path.
//!
//! # Section layout
//!
...
//! - FDE pointer encoding: DW_EH_PE_pcrel (pointer-sized on LP64 Darwin)
//!
...
/// DW_EH_PE_sdata8: pointer is signed 8-byte value.
const DW_EH_PE_SDATA8: u8 = 0x0C;
/// DW_EH_PE_indirect: encoded value is an indirect pointer.
...
pub struct DwarfCie {
    ...
    /// DW_EH_PE_indirect | DW_EH_PE_pcrel | DW_EH_PE_sdata4 on Apple AArch64.
    ...
    /// DW_EH_PE_pcrel on Apple AArch64, which is pointer-sized on LP64 Darwin.
...
impl DwarfCie {
    ...
    pub fn aarch64_darwin() -> Self {
        ...
        Self {
            ...
            fde_pointer_encoding: DW_EH_PE_PCREL,
    ...
    pub fn aarch64_darwin_with_eh() -> Self {
        ...
        let personality_enc = DW_EH_PE_INDIRECT | DW_EH_PE_PCREL | DW_EH_PE_SDATA4;
        let lsda_enc = DW_EH_PE_PCREL;
    ...
    /// Create the standard AArch64 ELF (Linux) CIE (no personality/LSDA).
    ///
    /// Augmentation: "zR". The FDE pointer encoding is
    /// `DW_EH_PE_pcrel | DW_EH_PE_sdata4` (0x1b) — the GNU/Linux `.eh_frame`
    /// convention (4-byte self-relative pointers resolved by
    /// `R_AARCH64_PREL32` relocations in a relocatable object), unlike the
    /// Darwin scheme's pointer-sized `DW_EH_PE_pcrel` + subtractor pairs.
    pub fn aarch64_elf() -> Self {
        let mut initial_instructions = Vec::new();
        // DW_CFA_def_cfa SP, 0 — CFA starts at SP with zero offset
        initial_instructions.push(DW_CFA_DEF_CFA);
        encode_uleb128(SP_REGISTER, &mut initial_instructions);
    ...
    /// Create an AArch64 ELF (Linux) CIE with personality and LSDA support.
    ///
    /// Augmentation: "zPLR":
    ///
    /// * The personality pointer is `DW_EH_PE_indirect|pcrel|sdata4` (0x9b) —
    ///   the GCC/LLVM `DW.ref.<personality>` data-slot scheme. The 4-byte
    ///   pc-relative field points at an 8-byte writable slot
    ///   (`.data.DW.ref.<personality>`, `R_AARCH64_ABS64` to the personality)
    ///   rather than at the personality itself: the personality routine
    ///   normally lives in a SHARED library (`libstdc++.so` / Rust std), and
    ///   a direct `R_AARCH64_PREL32` against a preemptible symbol is
    ///   unrepresentable in a PIE link ("can not be used when making a
    ///   shared object" — the exact GNU ld failure this scheme avoids).
    /// * The LSDA pointer (FDE augmentation) is `DW_EH_PE_pcrel|sdata4`
    ///   (0x1b), targeting the function's entry in `.gcc_except_table`.
    /// * FDE pc-begin pointers are `DW_EH_PE_pcrel|sdata4` (0x1b).
    pub fn aarch64_elf_with_eh() -> Self {
        let mut initial_instructions = Vec::new();
        initial_instructions.push(DW_CFA_DEF_CFA);
        encode_uleb128(SP_REGISTER, &mut initial_instructions);
        encode_uleb128(0, &mut initial_instructions);
        ...
        Self {
            ...
            fde_pointer_encoding: DW_EH_PE_PCREL | DW_EH_PE_SDATA4,
            personality_encoding: Some(personality_enc),
            personality_pointer: Some(0), // placeholder, relocated (PREL32)
            lsda_encoding: Some(lsda_enc),
            initial_instructions,
        }
    }
    ...
    pub fn x86_64_darwin_with_eh() -> Self {
        ...
        // EH Lane 2 Step 4b / EH Lane 3: the personality pointer is GOT-INDIRECT
        // (DW_EH_PE_indirect|pcrel|sdata4 = 0x9b), matching clang/libunwind on
        // x86-64 macOS: the 4-byte pcrel slot points at a GOT entry that holds
        // the personality address, relocated via X86_64_RELOC_GOT. Encoding and
        // relocation kind MUST agree (a direct 0x1b would need a SIGNED reloc);
        // a mismatch resolves the personality to garbage -> std::terminate.
        //
        // The FDE PC-begin and LSDA pointers are DW_EH_PE_pcrel|absptr (0x10,
        // 8-byte, SELF-RELATIVE) — NOT sdata4-with-a-subtractor-pair. On x86-64
        // macOS, ld64's __eh_frame reader (unlike arm64's) does NOT process
        // SUBTRACTOR/UNSIGNED relocations for these pointers; it re-synthesizes
        // __unwind_info from the self-relative values it reads DIRECTLY out of
        // the section (exactly clang's form). A subtractor-pair scheme makes ld64
        // fail with "symbol index out of range". The x86 pipeline therefore BAKES
        // the self-relative 8-byte values and emits NO relocation for them (only
        // the personality keeps its GOT relocation). See
        // `add_x86_64_eh_frame_to_macho`. [EH Lane 3]
        let personality_enc = DW_EH_PE_INDIRECT | DW_EH_PE_PCREL | DW_EH_PE_SDATA4;
        let lsda_enc = DW_EH_PE_PCREL; // pcrel|absptr = 8-byte self-relative
        ...
        Self {
            ...
            data_alignment_factor: X86_64_DATA_ALIGN_FACTOR,
            return_address_register: X86_64_RA_REGISTER,
            fde_pointer_encoding: DW_EH_PE_PCREL, // pcrel|absptr = 8-byte self-relative
            personality_encoding: Some(personality_enc),
            personality_pointer: Some(0), // placeholder, relocated (with +4 baked addend)
            lsda_encoding: Some(lsda_enc),
            initial_instructions,
        }
    }
...
/// pointer in its augmentation data (a pointer-sized PC-relative value to the
/// function's LSDA in `__TEXT,__gcc_except_tab`).
#[derive(Debug, Clone)]
pub struct DwarfFde {
    ...
    /// LSDA pointer (pointer-sized, PC-relative, for "zPLR" CIE augmentation).
...
impl DwarfFde {
    ...
    /// Generate the prologue-describing CFI for `frame::emit_prologue`'s
    /// ACTUAL instruction sequence (the Apple-canonical
    /// `UNWIND_ARM64_MODE_FRAME` shape — extra callee-saved pairs at the
    /// bottom of the CSA, FP/LR at the TOP, `x29 = CFA - 16`):
    ///
    /// ```text
    ///   stp <pairN.reg2>, <pairN.reg1>, [sp, #-csa]!   ; CFA = SP + csa
    ///   stp <pair..>,     ...,          [sp, #16*k]    ; (num_extra-1 stores)
    ///   stp x29, x30, [sp, #csa-16]                    ; x29@cfa-16, x30@cfa-8
    ///   add x29, sp, #csa-16                           ; CFA = x29 + 16
    ///   sub sp, sp, #...                               ; (CFA already FP-based)
    /// ```
    ///
    /// pair[i] (i >= 1, register-number order, stored REVERSED so the
    /// lower-numbered register lands closer to FP): `reg2` at `CFA-16*(i+1)`,
    /// `reg1` at `CFA-(16*i+8)` — e.g. X19@cfa-24, X20@cfa-32.
    ///
    /// HISTORY: this generator used to describe the LEGACY bottom-FP prologue
    /// (`stp x29,x30,[sp,#-csa]!; mov x29,sp; ...` with `CFA = x29 + csa`).
    /// For any frame with extra callee-saved pairs (csa > 16) that CFA rule is
    /// off by `csa - 16` bytes against the real prologue, so every unwind
    /// walk through such a frame read a garbage return address — on ELF
    /// (where DWARF is the ONLY unwind info) `_Unwind_RaiseException` died
    /// with "failed to initiate panic" the moment a framed function with
    /// extra saves (e.g. `__rust_start_panic`) was on the throw path. On
    /// Mach-O the compact-unwind encoding (independently correct) masked it
    /// except for the rare DWARF-fallback frames.
    pub fn from_layout(
        layout: &FrameLayout,
        function_offset: u64,
        function_length: u32,
        ...
        let pairs = &layout.callee_saved_pairs;
        let csa = layout.callee_saved_area_size as u64;
        let factored = |bytes_below_cfa: u64| bytes_below_cfa / ((-DATA_ALIGN_FACTOR) as u64);

        if pairs.is_empty() {
            // No callee-saved area at all: the prologue is just the SP
            // adjustment (if any). Track it so mid-body PCs get the right
            // CFA (the CIE starts at CFA = SP + 0).
            let mut remaining = layout.sp_adjustment();
            let mut cumulative = 0u64;
            while remaining > 0 {
                let chunk = remaining.min(crate::frame::AARCH64_SP_ADJUST_CHUNK);
                cumulative += chunk as u64;
                encode_advance_loc(1, &mut instructions);
                instructions.push(DW_CFA_DEF_CFA_OFFSET);
                encode_uleb128(cumulative, &mut instructions);
                remaining -= chunk;
            }
            return Self {
                function_offset,
        ...
        let describe_pair = |i: usize, instructions: &mut Vec<u8>| {
            let pair = &pairs[i];
            // Stored reversed: reg2 at the LOWER address (CFA-16*(i+1)),
            // reg1 right above it (CFA-(16*i+8)).
            let reg2_dwarf = preg_to_dwarf(pair.reg2, pair.is_fpr);
            instructions.push(DW_CFA_OFFSET | (reg2_dwarf as u8 & 0x3F));
            encode_uleb128(factored(16 * (i as u64 + 1)), instructions);
            let reg1_dwarf = preg_to_dwarf(pair.reg1, pair.is_fpr);
            instructions.push(DW_CFA_OFFSET | (reg1_dwarf as u8 & 0x3F));
            encode_uleb128(factored(16 * i as u64 + 8), instructions);
        };
        let describe_fp_lr = |instructions: &mut Vec<u8>| {
            instructions.push(DW_CFA_OFFSET | (FP_REGISTER as u8 & 0x3F));
            encode_uleb128(factored(16), instructions);
            instructions.push(DW_CFA_OFFSET | (RA_REGISTER as u8 & 0x3F));
            encode_uleb128(factored(8), instructions);
        };

        // Instruction 1: the pre-index STP that allocates the whole CSA —
        // the bottom-most extra pair, or FP/LR itself when there are none.
        // (The red-zone minimal prologue is the num_extra == 0, csa == 16
        // shape: `stp x29,x30,[sp,#-16]!; mov x29,sp` — described by the
        // same rules.)
        encode_advance_loc(1, &mut instructions);
        instructions.push(DW_CFA_DEF_CFA_OFFSET);
        encode_uleb128(csa, &mut instructions);
        if num_extra == 0 {
            describe_fp_lr(&mut instructions);
        } else {
            describe_pair(num_pairs - 1, &mut instructions);
            // Remaining extra pairs, bottom-up (reverse index order).
            for i in (1..num_pairs - 1).rev() {
                encode_advance_loc(1, &mut instructions);
                describe_pair(i, &mut instructions);
            }
            // FP/LR at the top of the CSA.
            encode_advance_loc(1, &mut instructions);
            describe_fp_lr(&mut instructions);
        }

        if layout.uses_frame_pointer {
            // `add x29, sp, #csa-16` (or `mov x29, sp` when csa == 16):
            // x29 = CFA - 16, stable for the whole body regardless of later
            // SP adjustments.
            encode_advance_loc(1, &mut instructions);
            ...
            // No frame pointer: CFA stays SP-based, so each SUB SP chunk of
            // the local-area allocation must bump the CFA offset.
            let mut remaining = layout.sp_adjustment();
            let mut cumulative = csa;
            while remaining > 0 {
                let chunk = remaining.min(crate::frame::AARCH64_SP_ADJUST_CHUNK);
                cumulative += chunk as u64;
                encode_advance_loc(1, &mut instructions);
    ...
    /// Return a copy of this FDE with an LSDA relocation placeholder.
    pub fn with_lsda(mut self) -> Self {
        self.lsda_pointer = Some(0);
        self
    }
    ...
    /// - pointer-sized function start (PC-relative, relocated)
    /// - pointer-sized function length
    /// - ULEB128: augmentation data length
    /// - augmentation data (LSDA pointer if "zPLR" CIE)
    /// - CFI instructions
    /// - padding to 8-byte boundary
    pub fn to_bytes(&self, cie_offset: u32) -> Vec<u8> {
        self.to_bytes_with_sizes(cie_offset, 8, 8)
    }

    fn to_bytes_with_sizes(&self, cie_offset: u32, pc_size: usize, lsda_size: usize) -> Vec<u8> {
        let mut body = Vec::new();
        ...
        // PC begin and range use the CIE's FDE pointer encoding size.
        push_sized_value(&mut body, self.function_offset as i64, pc_size);
        push_sized_value(&mut body, self.function_length as i64, pc_size);

        // Augmentation data: LSDA pointer for "zPLR" CIE, empty for "zR".
        if let Some(lsda_ptr) = self.lsda_pointer {
            encode_uleb128(lsda_size as u64, &mut body);
            push_sized_value(&mut body, lsda_ptr, lsda_size);
        } else {
...
impl DwarfCfiSection {
    ...
    /// Create a new DWARF CFI section with AArch64 Darwin personality/LSDA CIE.
    pub fn new_aarch64_with_eh() -> Self {
        Self {
            cie: DwarfCie::aarch64_darwin_with_eh(),
            fdes: Vec::new(),
        }
    }

    /// Create a new DWARF CFI section with the standard AArch64 ELF CIE.
    pub fn new_aarch64_elf() -> Self {
        Self {
            cie: DwarfCie::aarch64_elf(),
            fdes: Vec::new(),
        }
    }

    /// Create a new DWARF CFI section with the AArch64 ELF personality/LSDA CIE.
    pub fn new_aarch64_elf_with_eh() -> Self {
        Self {
            cie: DwarfCie::aarch64_elf_with_eh(),
            fdes: Vec::new(),
        }
    }
    ...
    /// The section's CIE (emitter intent) — used by the ENC-EHDC decode-check
    /// gate to compare decoded eh_frame bytes against what the emitter built.
    pub(crate) fn cie(&self) -> &DwarfCie {
        &self.cie
    }

    /// The section's FDEs (emitter intent) — used by the ENC-EHDC decode-check
    /// gate to compare decoded eh_frame bytes against what the emitter built.
    pub(crate) fn fdes(&self) -> &[DwarfFde] {
        &self.fdes
    }
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        let pc_size = pointer_encoding_size(self.cie.fde_pointer_encoding);
        let lsda_size = self
            .cie
            .lsda_encoding
        ...
        for fde in &self.fdes {
            ...
            let fde_bytes = fde.to_bytes_with_sizes(cie_offset, pc_size, lsda_size);
    ...
    /// Serialize the section WITHOUT the trailing zero-length terminator.
    ///
    /// GNU-toolchain relocatable objects do not carry a `.eh_frame`
    /// terminator per object: each object contributes bare CIE+FDE records
    /// and the final 4-zero-byte terminator comes from `crtend.o` at link
    /// time. Emitting a mid-stream terminator would end the linker's (and
    /// any linear scanner's) walk at this object's boundary, hiding every
    /// following object's FDEs. Mach-O `__eh_frame` keeps the terminator
    /// ([`Self::to_bytes`]) — ld64 expects it there.
    pub fn to_bytes_no_terminator(&self) -> Vec<u8> {
        let mut data = self.to_bytes();
        if data.len() >= 4 {
            let tail = data.split_off(data.len() - 4);
            debug_assert_eq!(tail, [0u8; 4], "eh_frame terminator must be zero");
        }
        data
    }

    /// Byte offsets of each FDE PC-begin field in the serialized section.
    ///
    /// The FDE layout is `length, CIE pointer, PC begin, PC range, ...`, so the
    /// PC-begin relocation lands eight bytes after the start of each FDE.
    pub fn fde_pc_begin_relocation_offsets(&self) -> Vec<u32> {
        if self.fdes.is_empty() {
            return Vec::new();
        }
        ...
        let lsda_size = self
            ...
            .unwrap_or(pc_size);
        let cie_start = 0usize;
        let mut fde_start = cie_bytes.len() as u32;
        let mut offsets = Vec::with_capacity(self.fdes.len());

        for fde in &self.fdes {
            let fde_cie_ptr_offset = fde_start;
            let cie_offset = (fde_cie_ptr_offset + 4) - cie_start as u32;
            offsets.push(fde_start + 8);
            fde_start += fde
                .to_bytes_with_sizes(cie_offset, pc_size, lsda_size)
                .len() as u32;
        }
    ...
    /// Byte offset of the CIE personality pointer, if the CIE has one.
    pub fn cie_personality_relocation_offset(&self) -> Option<u32> {
        let personality_offset_in_aug_data = self.cie.personality_encoding.map(|_| 1u32)?;

        let mut body = Vec::new();
        body.extend_from_slice(&0u32.to_le_bytes());
        body.push(self.cie.version);
        body.extend_from_slice(&self.cie.augmentation);
        encode_uleb128(self.cie.code_alignment_factor, &mut body);
        encode_sleb128(self.cie.data_alignment_factor, &mut body);
        encode_uleb128(self.cie.return_address_register, &mut body);

        let mut aug_data = Vec::new();
        if let Some(personality_enc) = self.cie.personality_encoding {
            aug_data.push(personality_enc);
            aug_data.extend_from_slice(&self.cie.personality_pointer.unwrap_or(0).to_le_bytes());
        }
        if let Some(lsda_enc) = self.cie.lsda_encoding {
            aug_data.push(lsda_enc);
        }
        aug_data.push(self.cie.fde_pointer_encoding);

        encode_uleb128(aug_data.len() as u64, &mut body);
        Some(4 + body.len() as u32 + personality_offset_in_aug_data)
    }

    /// Byte offsets of each FDE LSDA pointer in the serialized section.
    pub fn fde_lsda_relocation_offsets(&self) -> Vec<u32> {
        if self.fdes.is_empty() {
            return Vec::new();
        }
        ...
        for fde in &self.fdes {
            let fde_cie_ptr_offset = fde_start;
            let cie_offset = (fde_cie_ptr_offset + 4) - cie_start as u32;
            if fde.lsda_pointer.is_some() {
                offsets.push(fde_start + 8 + (pc_size as u32 * 2) + 1);
            }
            fde_start += fde
                .to_bytes_with_sizes(cie_offset, pc_size, lsda_size)
                .len() as u32;
...
fn push_sized_value(out: &mut Vec<u8>, value: i64, size: usize) {
    match size {
        4 => out.extend_from_slice(&(value as i32).to_le_bytes()),
        8 => out.extend_from_slice(&value.to_le_bytes()),
        _ => panic!("unsupported DWARF pointer size {size}"),
    }
}
...
pub fn x86_64_fde_from_prologue(
    ...
    for (i, &reg) in callee_saved.iter().enumerate() {
        ...
        let push_size: u32 = if (8..=15).contains(&enc) { 2 } else { 1 };
...
mod tests {
    ...
    use trust_cg_ir::regs::{V8, V9, X19, X20, X21, X22, X29, X30};
    ...
    fn test_cie_creation() {
        ...
        assert_eq!(cie.fde_pointer_encoding, DW_EH_PE_PCREL);
    }
    ...
    fn test_cie_serialization_alignment() {
        ...
        assert_eq!(
            bytes.len() % 8,
            0,
            "CIE must be 8-byte aligned, got {}",
            bytes.len()
        );
    }
    ...
    fn test_fde_from_callee_save_layout() {
        ...
        assert!(
            fde.instructions.len() > 10,
            "Expected substantial CFI instructions"
        );
    }
    ...
    fn test_fde_serialization_alignment() {
        ...
        assert_eq!(
            bytes.len() % 8,
            0,
            "FDE must be 8-byte aligned, got {}",
            bytes.len()
        );
    }
    ...
    fn test_section_with_one_fde() {
        ...
        assert_eq!(
            last_4,
            &[0, 0, 0, 0],
            "Section must end with zero terminator"
        );
    }
    ...
    #[test]
    fn test_section_reports_fde_pc_begin_relocation_offsets() {
        let mut section = DwarfCfiSection::new();
        let layout1 = make_simple_layout();
        let layout2 = make_layout_with_callee_saves();
        section.add_fde(DwarfFde::from_layout(&layout1, 0, 64, 0));
        ...
        let bytes = section.to_bytes();
        let offsets = section.fde_pc_begin_relocation_offsets();
        ...
        let first_cie_len = u32::from_le_bytes(bytes[0..4].try_into().unwrap()) as usize + 4;
        assert_eq!(offsets[0] as usize, first_cie_len + 8);

        let first_fde_len =
            u32::from_le_bytes(bytes[first_cie_len..first_cie_len + 4].try_into().unwrap())
                as usize
                + 4;
        assert_eq!(offsets[1] as usize, first_cie_len + first_fde_len + 8);
    }

    #[test]
    fn test_section_with_eh_constructor_uses_aarch64_zplr() {
        let section = DwarfCfiSection::new_aarch64_with_eh();
        assert_eq!(section.cie.augmentation, b"zPLR\0");
        assert!(section.cie.has_personality());
        assert!(section.cie.lsda_encoding.is_some());
    }

    #[test]
    fn test_section_reports_cie_personality_relocation_offset() {
        let section = DwarfCfiSection::new_aarch64_with_eh();
        let cie = section.cie.to_bytes();

        assert_eq!(section.cie_personality_relocation_offset(), Some(19));
        assert_eq!(&cie[19..23], &0u32.to_le_bytes());
    }

    #[test]
    fn test_section_reports_fde_lsda_relocation_offsets() {
        let mut section = DwarfCfiSection::new_aarch64_with_eh();
        let layout = make_simple_layout();
        section.add_fde(DwarfFde::from_layout(&layout, 0, 64, 0).with_lsda());

        let bytes = section.to_bytes();
        let cie_len = u32::from_le_bytes(bytes[0..4].try_into().unwrap()) + 4;
        let offsets = section.fde_lsda_relocation_offsets();

        assert_eq!(offsets, vec![cie_len + 25]);
        assert_eq!(bytes[(cie_len + 24) as usize], 8);
        let lsda_offset = offsets[0] as usize;
        assert_eq!(&bytes[lsda_offset..lsda_offset + 8], &0i64.to_le_bytes());
    }
    ...
    fn test_fde_with_lsda_larger_than_without() {
        ...
        // FDE with LSDA has a pointer-sized value plus augmentation length.
        assert!(
    ...
    fn test_fde_without_lsda_aug_data_zero() {
        ...
        // After length(4) + CIE ptr(4) + PC begin(8) + PC range(8) = byte 24,
        // augmentation data length should be ULEB128(0) = 0x00
        // But we need to account for the length prefix: offset is 4+4+8+8 = 24 in body
        // The body starts at byte 4 (after the length field)
        // Body: cie_ptr(4) + pc_begin(8) + pc_range(8) = 20, then aug data len
        assert_eq!(bytes[4 + 20], 0x00, "Augmentation data length should be 0");
    }

    #[test]
    fn test_fde_with_lsda_aug_data_eight() {
        let layout = make_simple_layout();
        let mut fde = DwarfFde::from_layout(&layout, 0, 64, 0);
        fde.lsda_pointer = Some(0x42);
        let bytes = fde.to_bytes(24);

        // Augmentation data length should be 8 (pointer-sized LSDA pointer)
        assert_eq!(bytes[4 + 20], 0x08, "Augmentation data length should be 8");
        // LSDA pointer value
        let lsda_val = i64::from_le_bytes(bytes[4 + 21..4 + 29].try_into().unwrap());
    ...
    fn test_x86_64_cie_with_eh_larger_than_basic() {
        ...
        assert!(
            eh_cie.to_bytes().len() > basic_cie.to_bytes().len(),
            "zPLR CIE should be larger than zR CIE"
        );
    }
    ...
    fn test_x86_64_section_with_multiple_fdes() {
        ...
        section.add_fde(x86_64_fde_from_prologue(
            &[RBX, R12, R13, R14, R15],
            256,
            192,
            512,
    ...
    fn test_x86_64_cie_differs_from_aarch64() {
        ...
        assert_ne!(
            x86_cie.return_address_register,
            arm_cie.return_address_register
        );
