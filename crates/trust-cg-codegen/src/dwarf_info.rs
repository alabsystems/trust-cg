// trust-cg-codegen - DWARF debug information emission
//
...
//! DWARF debug information emission for Trust Codegen.
//!
...
//! - `__DWARF,__debug_loc` — optional DWARF4 location lists for ranged variables
//!
//! # DWARF version
//!
...
//!   "Trust Codegen proof-oriented compiler backend\0"
//!   "<function names>\0"
//!   ...
//!
//! __debug_line:
...
/// Errors emitted by fallible DWARF section construction.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum DwarfEmitError {
    /// A location list section or offset exceeded DWARF32's 4 GiB limit.
    DebugLocTooLarge,
    /// A ranged location-list value was asked to encode as a single exprloc.
    LocationListExprloc,
    /// A location-list DIE did not have a matching `.debug_loc` offset.
    MissingLocationListOffset { cursor: usize },
    /// A single DWARF location expression exceeded DWARF4's u16 length field.
    LocationExpressionTooLarge { len: usize },
    /// Adding a `__text` relocation for a DWARF address field failed (the
    /// underlying Mach-O writer rejected it).
    RelocationFailed(String),
}

impl core::fmt::Display for DwarfEmitError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            Self::DebugLocTooLarge => write!(f, ".debug_loc section exceeds DWARF32 limits"),
            Self::LocationListExprloc => write!(
                f,
                "location lists must be emitted through .debug_loc, not DW_FORM_exprloc"
            ),
            Self::MissingLocationListOffset { cursor } => {
                write!(
                    f,
                    "missing .debug_loc offset for location-list cursor {cursor}"
                )
            }
            Self::LocationExpressionTooLarge { len } => {
                write!(f, "DWARF location expression length {len} exceeds u16")
            }
            Self::RelocationFailed(msg) => {
                write!(f, "DWARF __text relocation emission failed: {msg}")
            }
        }
    }
}

impl std::error::Error for DwarfEmitError {}
...
/// DW_OP_constu: push an unsigned constant onto the DWARF stack.
...
/// DW_OP_stack_value: the top stack value is the variable value, not an address.
...
const PRODUCER: &str = "Trust Codegen proof-oriented compiler backend";
...
/// Abbreviation ID for DW_TAG_formal_parameter with a location list.
const ABBREV_FORMAL_PARAMETER_LOCATION_LIST: u32 = 13;

/// Abbreviation ID for DW_TAG_variable with a location list.
const ABBREV_VARIABLE_LOCATION_LIST: u32 = 14;
...
pub struct FunctionParam {
    ...
    /// Where the parameter is available at runtime.
...
/// One address range in a DWARF4 `.debug_loc` location list.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LocationListEntry {
    /// Start address of this location range (offset within __text section).
    pub low_pc: u64,
    /// Size of this location range in bytes.
    pub size: u32,
    /// Raw DWARF location expression active for this range.
    pub expression: Vec<u8>,
}

impl LocationListEntry {
    /// Fallible builder for a location-list entry from a single expression.
    pub fn try_from_location(
        low_pc: u64,
        size: u32,
        location: &VariableLocation,
    ) -> Result<Self, DwarfEmitError> {
        Ok(Self {
            low_pc,
            size,
            expression: try_encode_location_expr(location)?,
        })
    }

    /// Build a location-list entry from an existing single-location expression.
    pub fn from_location(low_pc: u64, size: u32, location: &VariableLocation) -> Self {
        Self::try_from_location(low_pc, size, location).unwrap_or_else(|_| Self {
            low_pc,
            size,
            expression: Vec::new(),
        })
    ...
    fn high_pc(&self) -> u64 {
        self.low_pc + u64::from(self.size)
    }
}
...
pub enum VariableLocation {
    ...
    RegOffset(u64, i64),
    /// Constant integer value (emits DW_OP_constu + DW_OP_stack_value).
    ConstantInt(u64),
    /// Ranged locations stored in `.debug_loc` and referenced by sec_offset.
    LocationList(Vec<LocationListEntry>),
}

impl VariableLocation {
    /// Fallible builder for a one-range location list from a single location
    /// expression.
    pub fn try_location_list(
        low_pc: u64,
        size: u32,
        location: VariableLocation,
    ) -> Result<Self, DwarfEmitError> {
        Ok(Self::LocationList(vec![
            LocationListEntry::try_from_location(low_pc, size, &location)?,
        ]))
    }

    /// Build a one-range location list from a single location expression.
    pub fn location_list(low_pc: u64, size: u32, location: VariableLocation) -> Self {
        Self::try_location_list(low_pc, size, location)
            .unwrap_or_else(|_| Self::LocationList(Vec::new()))
    }

    fn is_location_list(&self) -> bool {
        matches!(self, Self::LocationList(_))
    }
}
...
///
/// Represents a trust_ir aggregate type (e.g., Rust `struct`, C `struct`).
...
/// then emits the required DWARF sections. `.debug_loc` is emitted only when
/// at least one variable uses a location list.
///
/// # Usage
///
/// ```no_run
/// use trust_cg_codegen::dwarf_info::{DwarfDebugInfo, SourceLanguage, FunctionDebugInfo, BaseType};
///
...
impl DwarfDebugInfo {
    ...
    /// Add standard trust_ir base types (i8, i16, i32, i64, f32, f64, bool, u8, u16, u32, u64).
    ///
    ...
    pub fn emit_abbrev(&self) -> Vec<u8> {
        ...
        // Abbreviation 13: DW_TAG_formal_parameter with DW_FORM_sec_offset location list.
        encode_uleb128(ABBREV_FORMAL_PARAMETER_LOCATION_LIST as u64, &mut data);
        encode_uleb128(DW_TAG_FORMAL_PARAMETER as u64, &mut data);
        data.push(DW_CHILDREN_NO);
        emit_abbrev_attr(&mut data, DW_AT_NAME, DW_FORM_STRP);
        emit_abbrev_attr(&mut data, DW_AT_TYPE, DW_FORM_REF4);
        emit_abbrev_attr(&mut data, DW_AT_LOCATION, DW_FORM_SEC_OFFSET);
        data.push(0);
        data.push(0);

        // Abbreviation 14: DW_TAG_variable with DW_FORM_sec_offset location list.
        encode_uleb128(ABBREV_VARIABLE_LOCATION_LIST as u64, &mut data);
        encode_uleb128(DW_TAG_VARIABLE as u64, &mut data);
        data.push(DW_CHILDREN_NO);
        emit_abbrev_attr(&mut data, DW_AT_NAME, DW_FORM_STRP);
        emit_abbrev_attr(&mut data, DW_AT_TYPE, DW_FORM_REF4);
    ...
    pub fn emit_debug_info(&self) -> Vec<u8> {
        self.try_emit_debug_info().unwrap_or_default()
    }

    /// Fallible `.debug_info` section emission.
    pub fn try_emit_debug_info(&self) -> Result<Vec<u8>, DwarfEmitError> {
        Ok(self.try_emit_debug_info_with_addr_relocs()?.0)
    }

    /// Like [`try_emit_debug_info`](Self::try_emit_debug_info), but also returns
    /// the byte offsets (within the emitted `.debug_info` section) of every
    /// `DW_FORM_addr` field — the compile-unit and per-subprogram `DW_AT_low_pc`,
    /// plus each lexical-block `DW_AT_low_pc`. Each must carry an
    /// `ARM64_RELOC_UNSIGNED` relocation against `__text` so the linker,
    /// `dsymutil`, and the runtime `gimli` symbolizer can translate the o-local
    /// addresses stored here to final PCs. Without these relocations a linked
    /// image resolves function NAMES (from the STABS debug map) but no
    /// `file:line` (dsymutil emits an empty dSYM; the debug-map symbolizer
    /// cannot associate the DWARF addresses with `__text`).
    pub fn try_emit_debug_info_with_addr_relocs(
        &self,
    ) -> Result<(Vec<u8>, Vec<u32>), DwarfEmitError> {
        let (_, str_table) = self.build_string_table();
        let loclist_offsets = self.location_list_offsets()?;
        let mut loclist_cursor = 0usize;
        // Offsets (final-section-relative) of DW_FORM_addr fields needing a
        // __text relocation. `+4` throughout accounts for the unit_length field
        // prepended below (fields are recorded relative to `body`, which starts
        // AFTER that 4-byte length).
        let mut addr_relocs: Vec<u32> = Vec::new();
        ...
        // DW_AT_low_pc (addr) — relocated against __text.
        addr_relocs.push(4 + body.len() as u32);
        ...
        //
        // DW_FORM_ref4 uses the .debug_info offset of the target DIE. `body`
        // starts after the 4-byte unit_length field, so recorded offsets add
        // that initial-length field back.
        const DEBUG_INFO_INITIAL_LENGTH_SIZE: u32 = 4;

        // Emit subprogram DIEs
        for func in &self.functions {
            let has_children =
                !func.params.is_empty() || !func.variables.is_empty() || !func.scopes.is_empty();
            ...
            if has_children {
                ...
                for param in &func.params {
                    encode_uleb128(formal_parameter_abbrev(param) as u64, &mut body);
                    // DW_AT_name (strp)
                ...
                // Emit variable DIEs
                for var in &func.variables {
                    encode_uleb128(variable_abbrev(var) as u64, &mut body);
                    // DW_AT_name (strp)
        ...
        let base_type_offsets: Vec<u32> = {
            ...
            let mut offset = DEBUG_INFO_INITIAL_LENGTH_SIZE + body.len() as u32;
        ...
        for sty in &self.struct_types {
            ...
            for member in &sty.members {
                ...
                // DW_AT_name (strp)
                body.extend_from_slice(&str_table.offset_of(&member.name).to_le_bytes());
        ...
        for ety in &self.enum_types {
            ...
            let underlying_type_offset = if ety.underlying_type_index < base_type_offsets.len() {
            ...
            for enumerator in &ety.enumerators {
                ...
                // DW_AT_name (strp)
                body.extend_from_slice(&str_table.offset_of(&enumerator.name).to_le_bytes());
    ...
    fn fixup_type_refs(&self, body: &mut [u8], _str_table: &StringTable, type_offsets: &[u32]) {
            ...
            if has_children {
                ...
                for param in &func.params {
                    ...
                    if type_idx < type_offsets.len() {
                        ...
                        body[offset..offset + 4].copy_from_slice(&type_offset.to_le_bytes());
                    }
                    offset += 4;

                    offset = skip_location_attr(body, offset, &param.location);
                }
                ...
                for var in &func.variables {
                    ...
                    offset = skip_location_attr(body, offset, &var.location);
                }
    ...
    fn compute_cu_range(&self) -> (u64, u32) {
        ...
        let low = self.functions.iter().map(|f| f.low_pc).min().unwrap_or(0);
    ...
    /// Emit the DWARF4 `.debug_loc` section bytes.
    ///
    /// Each list is a sequence of `(start, end, expr_len, expr)` entries
    /// terminated by a zero start/end pair. `DW_AT_location` attributes using
    /// `DW_FORM_sec_offset` point at the byte offset where the corresponding
    /// list starts in this section.
    pub fn emit_debug_loc(&self) -> Vec<u8> {
        self.try_emit_debug_loc().unwrap_or_default()
    }

    /// Fallible DWARF4 `.debug_loc` section emission.
    pub fn try_emit_debug_loc(&self) -> Result<Vec<u8>, DwarfEmitError> {
        let mut data = Vec::new();
        let mut result: Result<(), DwarfEmitError> = Ok(());
        self.for_each_location(|location| {
            if result.is_ok()
                && let VariableLocation::LocationList(entries) = location
            {
                result = emit_location_list_entries(entries, &mut data);
            }
        });
        result?;
        Ok(data)
    ...
    fn has_location_lists(&self) -> bool {
        let mut has_lists = false;
        self.for_each_location(|location| {
            has_lists |= location.is_location_list();
        });
        has_lists
    }

    fn location_list_offsets(&self) -> Result<Vec<u32>, DwarfEmitError> {
        let mut offsets = Vec::new();
        let mut offset = 0u32;
        let mut result: Result<(), DwarfEmitError> = Ok(());
        self.for_each_location(|location| {
                ...
                && let VariableLocation::LocationList(entries) = location
            {
                ...
                result = (|| {
                    offset = offset
                        .checked_add(encoded_location_list_len(entries)?)
                        .ok_or(DwarfEmitError::DebugLocTooLarge)?;
                    Ok(())
                })();
            }
        });
    ...
    fn for_each_location(&self, mut visit: impl FnMut(&VariableLocation)) {
        for func in &self.functions {
            for param in &func.params {
                visit(&param.location);
            }
            for var in &func.variables {
                visit(&var.location);
            }
    ...
    pub fn emit_debug_line(&self) -> Vec<u8> {
        self.emit_debug_line_with_addr_relocs().0
    }

    /// Like [`emit_debug_line`](Self::emit_debug_line), but also returns the byte
    /// offsets (within the emitted `.debug_line` section) of every
    /// `DW_LNE_set_address` operand, each of which must carry an
    /// `ARM64_RELOC_UNSIGNED` relocation against `__text`. Without them the
    /// line-number program's addresses are not tied to the code section, so
    /// dsymutil/`gimli` cannot map a runtime PC back to a source line.
    pub fn emit_debug_line_with_addr_relocs(&self) -> (Vec<u8>, Vec<u32>) {
        // Offsets (final-section-relative) of DW_LNE_set_address operands.
        // `+4` accounts for the unit_length field prepended at the end.
        let mut addr_relocs: Vec<u32> = Vec::new();
        ...
        // Patch header_length
        let header_content_len = (header_body.len() - header_content_start) as u32;
        ...
        if has_line_entries {
            ...
            for func in &self.functions {
                if func.line_entries.is_empty() {
                    ...
                    let off = emit_set_address(&mut header_body, func.low_pc);
                    addr_relocs.push(4 + off as u32);
                    header_body.push(1); // DW_LNS_copy

                    let off = emit_set_address(&mut header_body, func.low_pc + func.size as u64);
                    addr_relocs.push(4 + off as u32);
                    continue;
                }
                ...
                for entry in &func.line_entries {
                    ...
                    if adj_line >= 0
                        ...
                        && addr_advance
                            < ((255 - OPCODE_BASE as u64 - adj_line as u64) / LINE_RANGE as u64 + 1)
                    {
                        let opcode = (adj_line
                            + LINE_RANGE as i64 * addr_advance as i64
                ...
                if cur_addr < end_pc {
                    let off = emit_set_address(&mut header_body, end_pc);
                    addr_relocs.push(4 + off as u32);
                }
            }
        } else {
...
pub fn encode_location_expr(loc: &VariableLocation) -> Vec<u8> {
    try_encode_location_expr(loc).unwrap_or_default()
}

/// Fallible DWARF location expression encoder.
pub fn try_encode_location_expr(loc: &VariableLocation) -> Result<Vec<u8>, DwarfEmitError> {
    ...
    match loc {
        ...
        VariableLocation::ConstantInt(value) => {
            expr.push(DW_OP_CONSTU);
            encode_uleb128(*value, &mut expr);
            expr.push(DW_OP_STACK_VALUE);
        }
        VariableLocation::LocationList(_) => {
            return Err(DwarfEmitError::LocationListExprloc);
        }
    }
    Ok(expr)
}

fn formal_parameter_abbrev(param: &FunctionParam) -> u32 {
    if param.location.is_location_list() {
        ABBREV_FORMAL_PARAMETER_LOCATION_LIST
    } else {
        ABBREV_FORMAL_PARAMETER
    }
}

fn variable_abbrev(var: &VariableDebugInfo) -> u32 {
    if var.location.is_location_list() {
        ABBREV_VARIABLE_LOCATION_LIST
    } else {
        ABBREV_VARIABLE
    }
}

fn emit_location_attr(
    location: &VariableLocation,
    loclist_offsets: &[u32],
    loclist_cursor: &mut usize,
    body: &mut Vec<u8>,
) -> Result<(), DwarfEmitError> {
    match location {
        VariableLocation::LocationList(_) => {
            let offset = loclist_offsets.get(*loclist_cursor).copied().ok_or(
                DwarfEmitError::MissingLocationListOffset {
                    cursor: *loclist_cursor,
                },
            )?;
            *loclist_cursor += 1;
        ...
        _ => {
            let loc_expr = try_encode_location_expr(location)?;
            encode_uleb128(loc_expr.len() as u64, body);
...
fn skip_location_attr(body: &[u8], offset: usize, location: &VariableLocation) -> usize {
...
fn encoded_location_list_len(entries: &[LocationListEntry]) -> Result<u32, DwarfEmitError> {
    let mut len = 16u32; // terminating start=0, end=0 address pair
    for entry in entries {
        len = len
            .checked_add(8 + 8 + 2)
            .and_then(|n| n.checked_add(entry.expression.len().try_into().ok()?))
            .ok_or(DwarfEmitError::DebugLocTooLarge)?;
    }
    Ok(len)
}

fn emit_location_list_entries(
    entries: &[LocationListEntry],
    out: &mut Vec<u8>,
) -> Result<(), DwarfEmitError> {
    for entry in entries {
        out.extend_from_slice(&entry.low_pc.to_le_bytes());
        out.extend_from_slice(&entry.high_pc().to_le_bytes());
        let expr_len: u16 = entry.expression.len().try_into().map_err(|_| {
            DwarfEmitError::LocationExpressionTooLarge {
                len: entry.expression.len(),
            }
        })?;
        out.extend_from_slice(&expr_len.to_le_bytes());
        out.extend_from_slice(&entry.expression);
    }
...
/// Emit a `DW_LNE_set_address` extended opcode and return the byte offset,
/// within `out`, of the emitted 8-byte address operand. Callers use that offset
/// (adjusted for the section's prepended `unit_length`) to place an
/// `ARM64_RELOC_UNSIGNED` relocation against `__text`, so the o-local address
/// written here is translated to the final PC at link time.
fn emit_set_address(out: &mut Vec<u8>, addr: u64) -> usize {
    out.push(0); // extended opcode marker
    encode_uleb128(1 + ADDRESS_SIZE as u64, out); // length
    out.push(0x02); // DW_LNE_set_address
    let addr_field_offset = out.len();
...
///
/// Emits the core sections in the `__DWARF` segment:
/// - `__debug_abbrev`
/// - `__debug_info`
/// - `__debug_str`
/// - `__debug_line`
/// - `__debug_loc` when location lists are present
///
...
pub fn add_debug_info_to_writer(
...
) -> Result<Option<(usize, usize, usize, usize)>, DwarfEmitError> {
    if debug_info.functions.is_empty() {
        return Ok(None);
    }

    let abbrev_data = debug_info.emit_abbrev();
    let (info_data, info_addr_relocs) = debug_info.try_emit_debug_info_with_addr_relocs()?;
    let str_data = debug_info.emit_debug_str();
    let (line_data, line_addr_relocs) = debug_info.emit_debug_line_with_addr_relocs();
    let loc_data = debug_info.try_emit_debug_loc()?;
    ...
    let info_idx =
        writer.add_custom_section(b"__debug_info", b"__DWARF", &info_data, 0, S_ATTR_DEBUG);

    let str_idx = writer.add_custom_section(b"__debug_str", b"__DWARF", &str_data, 0, S_ATTR_DEBUG);

    let line_idx =
        writer.add_custom_section(b"__debug_line", b"__DWARF", &line_data, 0, S_ATTR_DEBUG);

    if debug_info.has_location_lists() {
        writer.add_custom_section(b"__debug_loc", b"__DWARF", &loc_data, 0, S_ATTR_DEBUG);
    }

    // Relocate every DW_FORM_addr / DW_LNE_set_address operand against the code
    // section so the o-local addresses written into `.debug_info` / `.debug_line`
    // become final PCs at link time. Each is an `ARM64_RELOC_UNSIGNED`
    // (8-byte, non-extern) whose symbolnum is the `__text` section ordinal.
    // `__text` is always the first section (`add_text_section` before any
    // debug section), i.e. 1-based ordinal 1. Without these, `dsymutil` cannot
    // build a dSYM and a debug-map backtrace resolves names but not file:line.
    use crate::macho::reloc::{AArch64RelocKind, Relocation};
    const TEXT_SECTION_ORDINAL: u32 = 1;
    let reloc_err =
        |e: crate::macho::MachORelocationError| DwarfEmitError::RelocationFailed(format!("{e:?}"));
    for off in info_addr_relocs {
        writer
            .add_relocation(
                info_idx,
                Relocation::section_relative(off, TEXT_SECTION_ORDINAL, AArch64RelocKind::Unsigned),
            )
            .map_err(reloc_err)?;
    }
    for off in line_addr_relocs {
        writer
            .add_relocation(
                line_idx,
                Relocation::section_relative(off, TEXT_SECTION_ORDINAL, AArch64RelocKind::Unsigned),
    ...
    Ok(Some((abbrev_idx, info_idx, str_idx, line_idx)))
}
...
mod tests {
        ...
        dbg.add_function(FunctionDebugInfo {
            ...
            params: vec![
                ...
                FunctionParam {
                    name: "b".to_string(),
                    type_index: 0,
                    location: VariableLocation::Register(1),
                },
            ],
            variables: vec![],
            scopes: vec![],
    ...
    fn test_add_debug_info_to_writer_empty() {
        ...
        let result = add_debug_info_to_writer(&mut writer, &dbg)
            .expect("empty debug info should be handled");
        assert!(
            result.is_none(),
            "Should not add sections for empty debug info"
        );
    }
    ...
    fn test_add_debug_info_to_writer_with_function() {
        ...
        writer.add_symbol("_add", 1, 0, true).unwrap();

        let result =
            add_debug_info_to_writer(&mut writer, &dbg).expect("debug info should be added");
    ...
    fn test_c99_language_debug_info() {
        ...
        let lang = u16::from_le_bytes(info[lang_offset..lang_offset + 2].try_into().unwrap());
    ...
    fn test_full_round_trip_with_writer() {
        ...
        writer.add_symbol("_add", 1, 0, true).unwrap();
        writer.add_symbol("_sub", 1, 8, true).unwrap();
        ...
        let nsects =
            u32::from_le_bytes(bytes[nsects_offset..nsects_offset + 4].try_into().unwrap());
    ...
    #[test]
    fn test_encode_location_expr_constant_int_stack_value() {
        assert_eq!(
            encode_location_expr(&VariableLocation::ConstantInt(42)),
            vec![DW_OP_CONSTU, 0x2a, DW_OP_STACK_VALUE],
        );
        assert_eq!(
            encode_location_expr(&VariableLocation::ConstantInt(624)),
            vec![DW_OP_CONSTU, 0xf0, 0x04, DW_OP_STACK_VALUE],
        );
    }
    ...
    fn test_variable_die_emission() {
        let mut without_vars = DwarfDebugInfo::new("test.rs", "/tmp", SourceLanguage::Rust);
        ...
        let mut with_vars = DwarfDebugInfo::new("test.rs", "/tmp", SourceLanguage::Rust);
    ...
    fn test_pointer_type_die_emission() {
        let mut without_ptr = DwarfDebugInfo::new("test.rs", "/tmp", SourceLanguage::Rust);
        ...
        let mut with_ptr = DwarfDebugInfo::new("test.rs", "/tmp", SourceLanguage::Rust);
        ...
        let has_ptr_abbrev = info_with
            .windows(1)
            .any(|w| w[0] == ABBREV_POINTER_TYPE as u8);
        assert!(
            has_ptr_abbrev,
            "pointer type abbrev code should appear in debug_info"
        );
    }
    ...
    fn test_debug_line_with_source_entries() {
        let mut without_entries = DwarfDebugInfo::new("test.rs", "/tmp", SourceLanguage::Rust);
        ...
        let mut with_entries = DwarfDebugInfo::new("test.rs", "/tmp", SourceLanguage::Rust);
    ...
    fn test_abbrev_includes_variable_entry() {
        ...
        assert!(
            abbrev
                .windows(3)
                .any(|w| { w == [ABBREV_VARIABLE as u8, DW_TAG_VARIABLE as u8, DW_CHILDREN_NO] }),
    ...
    fn test_abbrev_includes_pointer_type_entry() {
        ...
        assert!(
            abbrev.windows(3).any(|w| {
                w == [
                    ABBREV_POINTER_TYPE as u8,
                    DW_TAG_POINTER_TYPE as u8,
                    DW_CHILDREN_NO,
                ]
            }),
    ...
    // --- trust_ir debug metadata wiring tests ---
    //
    ...
    #[test]
    fn test_variable_location_list_emits_debug_loc() {
        let mut dbg = DwarfDebugInfo::new("lib.rs", "/src", SourceLanguage::Rust);
        dbg.add_standard_types();
        dbg.add_function(FunctionDebugInfo {
            name: "process".to_string(),
            ...
            variables: vec![VariableDebugInfo {
                name: "acc".to_string(),
                type_index: 3,
                location: VariableLocation::location_list(0, 20, VariableLocation::FrameOffset(-8)),
            }],
            scopes: vec![],
            line_entries: vec![],
            decl_file: 1,
        ...
        let loc = dbg.emit_debug_loc();
        assert_eq!(&loc[0..8], &0u64.to_le_bytes());
        assert_eq!(&loc[8..16], &20u64.to_le_bytes());
        assert_eq!(u16::from_le_bytes(loc[16..18].try_into().unwrap()), 2);
        assert_eq!(&loc[18..20], &[DW_OP_FBREG, 0x78]); // SLEB128(-8)
        assert_eq!(&loc[20..28], &0u64.to_le_bytes());
        assert_eq!(&loc[28..36], &0u64.to_le_bytes());

        let abbrev = dbg.emit_abbrev();
        assert!(
            abbrev.windows(3).any(|w| {
                w == [
                    ABBREV_VARIABLE_LOCATION_LIST as u8,
                    DW_TAG_VARIABLE as u8,
                    DW_CHILDREN_NO,
                ]
            }),
            "abbreviation table should contain variable location-list entry"
        );

        let info = dbg.emit_debug_info();
        assert!(
            info.windows(4).any(|w| w == 0u32.to_le_bytes()),
            "variable DIE should reference .debug_loc offset 0"
        );
    }

    #[test]
    fn test_constant_variable_emits_stack_value_exprloc() {
        let mut dbg = DwarfDebugInfo::new("lib.rs", "/src", SourceLanguage::Rust);
        dbg.add_standard_types();
        dbg.add_function(FunctionDebugInfo {
            name: "process".to_string(),
            ...
            variables: vec![VariableDebugInfo {
                ...
                type_index: 2,
                location: VariableLocation::ConstantInt(42),
            }],
            scopes: vec![],
            line_entries: vec![],
            decl_file: 1,
        ...
        assert!(
            dbg.emit_debug_loc().is_empty(),
            "constant exprloc variables should not allocate .debug_loc entries"
        );
        let info = dbg.emit_debug_info();
        assert!(
            info.windows(4)
                .any(|w| w == [3, DW_OP_CONSTU, 0x2a, DW_OP_STACK_VALUE]),
            "variable DIE should carry DW_OP_constu 42; DW_OP_stack_value as DW_FORM_exprloc"
        );
    }

    #[test]
    fn test_formal_parameter_location_list_emits_debug_loc() {
        let mut dbg = DwarfDebugInfo::new("lib.rs", "/src", SourceLanguage::Rust);
        dbg.add_standard_types();
        dbg.add_function(FunctionDebugInfo {
            name: "process".to_string(),
            ...
            params: vec![FunctionParam {
                name: "x".to_string(),
                type_index: 3,
                location: VariableLocation::location_list(0, 12, VariableLocation::Register(0)),
            }],
            variables: vec![],
            scopes: vec![],
            line_entries: vec![],
        ...
        let loc = dbg.emit_debug_loc();
        assert_eq!(&loc[0..8], &0u64.to_le_bytes());
        assert_eq!(&loc[8..16], &12u64.to_le_bytes());
        assert_eq!(u16::from_le_bytes(loc[16..18].try_into().unwrap()), 1);
        assert_eq!(loc[18], DW_OP_REG0);
        assert_eq!(&loc[19..27], &0u64.to_le_bytes());
        assert_eq!(&loc[27..35], &0u64.to_le_bytes());

        let abbrev = dbg.emit_abbrev();
        assert!(
            abbrev.windows(3).any(|w| {
                w == [
                    ABBREV_FORMAL_PARAMETER_LOCATION_LIST as u8,
                    DW_TAG_FORMAL_PARAMETER as u8,
                    DW_CHILDREN_NO,
                ]
            }),
            "abbreviation table should contain formal-parameter location-list entry"
        );

        let info = dbg.emit_debug_info();
        assert!(
            info.windows(4).any(|w| w == 0u32.to_le_bytes()),
            "formal parameter DIE should reference .debug_loc offset 0"
        );
    }

    #[test]
    fn test_add_debug_info_to_writer_adds_debug_loc_for_location_lists() {
        let mut dbg = DwarfDebugInfo::new("lib.rs", "/src", SourceLanguage::Rust);
        dbg.add_standard_types();
        dbg.add_function(FunctionDebugInfo {
            name: "process".to_string(),
        ...
        let mut writer = MachOWriter::new();
        writer.add_text_section(&[0x1F, 0x20, 0x03, 0xD5]);
        writer.add_symbol("_process", 1, 0, true).unwrap();
        add_debug_info_to_writer(&mut writer, &dbg).expect("debug info should be added");
        ...
        let nsects =
            u32::from_le_bytes(bytes[nsects_offset..nsects_offset + 4].try_into().unwrap());
        assert_eq!(nsects, 6, "Expected __text + 5 DWARF sections");
    }
    ...
    fn test_full_wiring_round_trip_with_debug_meta() {
        ...
        let mut dbg = DwarfDebugInfo::new("example.rs", "/home/user/project", SourceLanguage::Rust);
        dbg.add_standard_types();
        dbg.add_function(FunctionDebugInfo {
            name: "sum".to_string(),
            linkage_name: None,
        ...
        writer.add_symbol("_sum", 1, 0, true).unwrap();
    ...
    fn test_lexical_block_increases_debug_info_size() {
        let mut without_scope = DwarfDebugInfo::new("test.rs", "/tmp", SourceLanguage::Rust);
        ...
        let mut with_scope = DwarfDebugInfo::new("test.rs", "/tmp", SourceLanguage::Rust);
    ...
    fn test_lexical_block_scoped_variable_in_string_table() {
        ...
        assert!(
            str_data
                .windows(b"scoped_var".len())
                .any(|w| w == b"scoped_var"),
    ...
    fn test_abbrev_includes_lexical_block_entry() {
        ...
        assert!(
            abbrev.windows(3).any(|w| {
                w == [
                    ABBREV_LEXICAL_BLOCK as u8,
                    DW_TAG_LEXICAL_BLOCK as u8,
                    DW_CHILDREN_YES,
                ]
            }),
    ...
    fn make_point_struct_dbg() -> DwarfDebugInfo {
        ...
        let mut dbg = DwarfDebugInfo::new("point.rs", "/home/user/project", SourceLanguage::Rust);
    ...
    fn test_struct_type_debug_info_size_grows_with_member_count() {
        let mut one_member = DwarfDebugInfo::new("a.rs", "/tmp", SourceLanguage::Rust);
        ...
        let mut three_members = DwarfDebugInfo::new("a.rs", "/tmp", SourceLanguage::Rust);
    ...
    fn make_color_enum_dbg() -> DwarfDebugInfo {
        ...
        let mut dbg = DwarfDebugInfo::new("color.rs", "/home/user/project", SourceLanguage::Rust);
    ...
    fn test_enum_type_debug_info_size_grows_with_enumerator_count() {
        let mut one_enumerator = DwarfDebugInfo::new("color.rs", "/tmp", SourceLanguage::Rust);
