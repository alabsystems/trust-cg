// trust-cg-codegen/constant_pool.rs - Float constant pool for x86-64
//
...
pub struct ConstantPool {
    ...
    /// Maps typed bit-pattern key -> entry index for deduplication.
    dedup: HashMap<ConstantPoolKey, usize>,
}
...
impl ConstantPool {
    ...
    pub fn add_f32(&mut self, val: f32) -> usize {
        let key = ConstantPoolKey::F32(val.to_bits());
    ...
    pub fn add_f64(&mut self, val: f64) -> usize {
        let key = ConstantPoolKey::F64(val.to_bits());
        if let Some(&idx) = self.dedup.get(&key) {
            return idx;
        }
        let idx = self.entries.len();
...
mod tests {
    ...
    #[test]
    fn no_cross_type_dedup_for_opposite_signed_zero() {
        // Regression: f32 +0.0 has raw key 0 and f64 -0.0 has sign-bit-only
        // bits. A marker-based untyped key aliased these and made MOVSD read
        // from a 4-byte f32 pool entry.
        let mut pool = ConstantPool::new();
        let idx_f32 = pool.add_f32(0.0_f32);
        let idx_f64 = pool.add_f64(-0.0_f64);
        assert_ne!(idx_f32, idx_f64);
        assert_eq!(pool.len(), 2);

        let total = pool.layout();
        assert_eq!(pool.entry_offset(idx_f32), 0);
        assert_eq!(pool.entry_offset(idx_f64), 8);
        assert_eq!(total, 16);
