// trust-cg-codegen/aarch64/encoding_mem.rs - AArch64 load/store and address encoding
//
...
///
/// The 2-bit `opc` field selects the element width. Its meaning depends on the
/// `V` (SIMD/FP) bit: for integer pairs (V=0) the legal values are `W32`=0b00
/// and `X64`=0b10; for FP/SIMD pairs (V=1) they are `S32`(0b00), `D64`(0b01),
/// and `Q128`(0b10). Only `D64` (used to save the low 64 bits of the callee-
/// saved V8–V15 registers) and the two integer widths are needed by the frame
/// lowering, so those are the variants modelled here.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum PairSize {
    /// 32-bit integer (W registers) — opc = 0b00
    W32 = 0b00,
    /// 64-bit FP (D registers, V=1) — opc = 0b01
    D64 = 0b01,
    /// 64-bit integer (X registers) — opc = 0b10
    X64 = 0b10,
}
...
mod tests {
    ...
    fn test_ldr_word_unsigned_offset() {
        ...
        let enc =
            encode_ldr_str_unsigned_offset(LoadStoreSize::Word, false, LoadStoreOp::Load, 16, 5, 3)
                .unwrap();
    ...
    fn test_unsigned_offset_reg_overflow() {
        let err =
            encode_ldr_str_unsigned_offset(LoadStoreSize::Word, false, LoadStoreOp::Load, 0, 32, 0);
        assert!(matches!(
            err,
    ...
    fn test_str_double_pre_index_neg16() {
        // STR X2, [SP, #-16]!
        let enc =
            encode_ldr_str_pre_index(LoadStoreSize::Double, false, LoadStoreOp::Store, -16, 31, 2)
                .unwrap();
    ...
    fn test_pre_index_imm9_min() {
        ...
        let enc =
            encode_ldr_str_pre_index(LoadStoreSize::Half, false, LoadStoreOp::Load, -256, 31, 0)
                .unwrap();
    ...
    fn test_pre_index_imm9_max() {
        // LDR H0, [X31, #255]!
        let enc =
            encode_ldr_str_pre_index(LoadStoreSize::Half, false, LoadStoreOp::Load, 255, 31, 0)
                .unwrap();
    ...
    fn test_pre_index_imm9_overflow() {
        let err =
            encode_ldr_str_pre_index(LoadStoreSize::Byte, false, LoadStoreOp::Load, 256, 0, 0);
        assert!(matches!(
            err,
    ...
    fn test_pre_index_imm9_underflow() {
        let err =
            encode_ldr_str_pre_index(LoadStoreSize::Byte, false, LoadStoreOp::Load, -257, 0, 0);
        assert!(matches!(
            err,
    ...
    fn test_ldr_double_post_index() {
        // LDR X30, [SP], #16
        let enc =
            encode_ldr_str_post_index(LoadStoreSize::Double, false, LoadStoreOp::Load, 16, 31, 30)
                .unwrap();
    ...
    fn test_ldr_byte_post_index_max() {
        // LDR B30, [X0], #255
        let enc =
            encode_ldr_str_post_index(LoadStoreSize::Byte, false, LoadStoreOp::Load, 255, 0, 30)
                .unwrap();
    ...
    fn test_register_offset_reg_overflow() {
        ...
        assert!(matches!(
            err,
            Err(EncodeError::RegisterOutOfRange { reg: 32, .. })
        ));
    }
    ...
    fn test_stp_x64_pre_index() {
        ...
        let enc = encode_ldp_stp_pre_index(PairSize::X64, false, PairOp::StorePair, -2, 30, 31, 29)
            .unwrap();
    ...
    fn test_ldp_x64_post_index() {
        ...
        let enc = encode_ldp_stp_post_index(PairSize::X64, false, PairOp::LoadPair, 2, 30, 31, 29)
            .unwrap();
    ...
    fn test_ldp_x64_signed_offset() {
        ...
        let enc =
            encode_ldp_stp_offset(PairSize::X64, false, PairOp::LoadPair, -8, 10, 31, 9).unwrap();
    ...
    fn test_stp_w32_pre_index() {
        ...
        let enc =
            encode_ldp_stp_pre_index(PairSize::W32, false, PairOp::StorePair, 63, 2, 3, 1).unwrap();
    ...
    fn test_ldp_imm7_boundary_max_pre() {
        ...
        let enc = encode_ldp_stp_pre_index(PairSize::X64, false, PairOp::LoadPair, 63, 30, 31, 29)
            .unwrap();
    ...
    fn test_pair_imm7_overflow() {
        let err = encode_ldp_stp_offset(PairSize::X64, false, PairOp::LoadPair, 64, 0, 0, 0);
    ...
    fn test_all_sizes_load() {
        ...
        for (size, expected_bits) in [
            ...
            let enc =
                encode_ldr_str_unsigned_offset(size, false, LoadStoreOp::Load, 0, 0, 0).unwrap();
            assert_eq!(
                enc >> 30,
                expected_bits,
                "size field mismatch for {:?}",
                size
            );
        }
    }
