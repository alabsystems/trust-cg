// trust-cg-codegen/benches/compare_clang.rs - Trust Codegen vs clang -O2 comparison
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// Benchmark comparison: measures Trust Codegen compile time and instruction count
// against clang -O2 for equivalent functions. Trust Codegen operates on pre-built
...
//
// Run with: cargo bench -p trust-cg-codegen --bench compare_clang
//
...
use trust_cg_codegen::pipeline::encode_function;
...
/// Warmup iterations for Trust Codegen benchmarks.
const WARMUP_ITERS: u32 = 10;

/// Minimum measurement iterations for Trust Codegen benchmarks.
...
/// A test case: pairs a name with an Trust Codegen MachFunction builder and a C source file.
struct TestCase {
    name: &'static str,
...
// ---------------------------------------------------------------------------
// Trust Codegen test function builders (pre-allocated physical registers)
// ---------------------------------------------------------------------------
...
fn build_factorial() -> MachFunction {
    ...
    append_inst(
        ...
        loop_bb,
        AArch64Opcode::BCond,
        vec![MachOperand::Imm(0xC), MachOperand::Imm(4)],
    ); // B.GT done (skip 4 instrs)
    append_inst(
        &mut func,
        loop_bb,
        AArch64Opcode::MulRR,
    ...
    append_inst(
        ...
        loop_bb,
        AArch64Opcode::B,
        vec![MachOperand::Imm(-4)],
    ); // B loop (back 4 instructions)
...
fn build_fibonacci() -> MachFunction {
    ...
    append_inst(
        ...
        loop_bb,
        AArch64Opcode::BCond,
        vec![MachOperand::Imm(0xA), MachOperand::Imm(6)],
    ); // B.GE done (0xA = GE, skip 6)
    append_inst(
        &mut func,
        loop_bb,
        AArch64Opcode::AddRR,
...
// ---------------------------------------------------------------------------
// Trust Codegen compilation: encode + Mach-O
// ---------------------------------------------------------------------------

/// Compile a MachFunction through Trust Codegen's backend: optimize + frame lower +
/// encode + Mach-O emission. Returns (object bytes, instruction count).
fn trust_cg_compile(func: &MachFunction) -> (Vec<u8>, usize) {
    ...
    // Frame lowering
    {
        ...
        frame::insert_prologue_epilogue(&mut f, &layout).unwrap();
    }
...
fn time_clang(clang: &Path, c_file: &Path, iters: u32) -> Option<Duration> {
    ...
    for _ in 0..2 {
        ...
        if status.is_err() || !status.unwrap().status.success() {
            eprintln!(
                "  WARNING: clang compilation failed for {}",
                c_file.display()
            );
            return None;
        }
...
struct CompareResult {
    name: &'static str,
    description: &'static str,
    trust_cg_time: Duration,
    clang_time: Option<Duration>,
    trust_cg_inst_count: usize,
...
impl CompareResult {
    fn time_ratio(&self) -> Option<f64> {
        self.clang_time.map(|ct| {
            let trust_cg_ns = self.trust_cg_time.as_nanos() as f64;
            let clang_ns = ct.as_nanos() as f64;
            if clang_ns == 0.0 {
                0.0
            } else {
                trust_cg_ns / clang_ns
            }
        })
    }
    ...
    fn inst_ratio(&self) -> Option<f64> {
        self.clang_inst_count.map(|ci| {
            if ci == 0 {
                ...
                self.trust_cg_inst_count as f64 / ci as f64
            }
        })
    }
}
...
fn find_inputs_dir() -> PathBuf {
    ...
    let candidates = [
        PathBuf::from("crates/trust-cg-codegen/benches/compare_inputs"),
...
/// Run the Trust Codegen benchmark for a single test case.
/// Returns (average duration, instruction count, iterations used).
fn bench_trust_cg(func: &MachFunction) -> (Duration, usize, u32) {
    // Warmup
    for _ in 0..WARMUP_ITERS {
        let _ = trust_cg_compile(func);
    }
    ...
    let min_iters = MIN_BENCH_DURATION
        .as_nanos()
        .checked_div(per_iter_ns)
        .map_or(BENCH_ITERS, |iters| iters as u32);
    let iters = min_iters.max(BENCH_ITERS);

    // Get instruction count from a single compile
    let (_, inst_count) = trust_cg_compile(func);
...
fn print_markdown_report(results: &[CompareResult], clang_version: &str, has_clang: bool) {
    println!();
    println!("# Trust Codegen vs clang -O2 Comparison");
    println!();
    ...
    println!("- **Trust Codegen pipeline:** Optimize(O0) + FrameLower + Encode + Mach-O");
    println!("- **Note:** Trust Codegen starts from pre-built MachIR; clang starts from C source.");
    println!("  This is intentional: Trust Codegen is designed for the use case where ISel has");
    ...
    println!();
    println!(
        "| Function   | Description              | Trust Codegen time | clang -O2 time | Ratio   | Speedup |"
    );
    println!(
        "|------------|--------------------------|------------|----------------|---------|---------|"
    );
    ...
    for r in results {
        let trust_cg_str = format_duration_human(r.trust_cg_time);
        let clang_str = r
            .clang_time
        ...
        println!(
            "| {:<10} | {:<24} | {:>10} | {:>14} | {:>7} | {:>7} |",
            r.name, r.description, trust_cg_str, clang_str, ratio_str, speedup_str
        );
    }
    ...
    println!();
    println!("| Function   | Trust Codegen insts | clang insts | Ratio  |");
    println!("|------------|-------------|-------------|--------|");
    ...
    for r in results {
        ...
        println!(
            "| {:<10} | {:>11} | {:>11} | {:>6} |",
            r.name, r.trust_cg_inst_count, clang_str, ratio_str
        );
    }
    ...
    if !time_ratios.is_empty() {
        ...
        let avg_speedup = if avg_ratio > 0.0 {
            1.0 / avg_ratio
        } else {
            f64::INFINITY
        };
        println!(
            "- **Average compile-time ratio:** {:.4}x (Trust Codegen is ~{:.0}x faster)",
            avg_ratio, avg_speedup
        );
        println!(
            "- Trust Codegen operates on pre-built MachIR, avoiding C parsing, LLVM IR generation,"
        );
    ...
    if !inst_ratios.is_empty() {
        ...
        println!(
            "- **Average instruction count ratio:** {:.2}x",
            avg_inst_ratio
        );
        if avg_inst_ratio > 1.5 {
            println!("- Trust Codegen currently produces more instructions than clang -O2.");
            println!(
                "  This is expected: Trust Codegen uses conservative codegen (no CSEL, no peephole"
            );
            println!("  opts at the MachIR level yet). Instruction count parity is a future goal.");
        } else {
            println!("- Trust Codegen instruction counts are competitive with clang -O2.");
        }
    }
...
fn main() {
    ...
    for case in &cases {
        ...
        // Build the Trust Codegen function
        let func = (case.build_fn)();

        // Trust Codegen benchmark
        let (trust_cg_time, trust_cg_inst_count, iters) = bench_trust_cg(&func);

        // clang benchmark
        let c_file = inputs_dir.join(case.c_filename);
        let clang_time = clang
            .as_ref()
            .and_then(|c| time_clang(c, &c_file, CLANG_ITERS));
        let clang_inst_count = clang
            .as_ref()
            .and_then(|c| clang_instruction_count(c, &c_file));

        eprintln!(
            " done (Trust Codegen: {} iters, {}/func)",
            iters,
            format_duration_human(trust_cg_time)
        );
...
/// Validate that all test functions compile successfully through Trust Codegen.
fn run_validation() {
    let cases = test_cases();
    let mut all_ok = true;
    ...
    for case in &cases {
        ...
        let func = (case.build_fn)();
        let (obj, count) = trust_cg_compile(&func);
        if obj.is_empty() {
