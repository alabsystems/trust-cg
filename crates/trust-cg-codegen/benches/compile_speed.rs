// trust-cg-codegen/benches/compile_speed.rs - Compilation speed benchmark suite
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// Comprehensive compilation speed benchmarks measuring all pipeline phases:
//   - ISel throughput (trust_ir -> LIR via translate_module)
//   - Optimization pipeline throughput per function
//   - Register allocation time
//   - Full pipeline end-to-end (trust_ir -> .o via Compiler API)
//   - Parallel vs serial compilation comparison
//
// Run with: cargo bench -p trust-cg-codegen --bench compile_speed
//
...
use trust_cg_codegen::pipeline::{OptLevel, apply_regalloc, ir_to_regalloc};
...
// ---------------------------------------------------------------------------
// Test module builders using trust_ir_build::ModuleBuilder
// ---------------------------------------------------------------------------

/// Build a small module: 1 function with ~5 operations.
/// f(a: i64, b: i64) -> i64 = (a + b) - (a * b) + 42
fn build_small_module() -> trust_ir::Module {
...
fn build_medium_module() -> trust_ir::Module {
...
fn build_large_module() -> trust_ir::Module {
...
fn build_xlarge_module() -> trust_ir::Module {
...
fn build_stress_module() -> trust_ir::Module {
...
///
/// The xxHash-derived constants and round shape are BSD-2-Clause; see
...
/// This is the hot path for ty's compiled state fingerprinting — a realistic
...
/// kernels above. See crates/trust-cg-codegen/tests/xxh3_main_loop_trust_ir.rs.
fn build_xxh3_main_loop_module() -> trust_ir::Module {
...
// ---------------------------------------------------------------------------
// Benchmark: ISel throughput (trust_ir -> LIR via translate_module)
// ---------------------------------------------------------------------------
...
fn bench_isel(module: &trust_ir::Module, name: &'static str) -> ISelResult {
    let func_count = module.functions.len();

    // Warmup
    for _ in 0..WARMUP_ITERS {
        let _ = trust_cg_lower::translate_module(module);
    }
...
fn bench_optimization(
    ...
    name: &'static str,
    opt_level: OptOptLevel,
) -> OptResult {
    // First translate to get LIR functions
    let lir_funcs = trust_cg_lower::translate_module(module).expect("translate_module failed");

    // Run ISel to get post-ISel IR functions (before optimization).
    let ir_funcs_fresh: Vec<_> = lir_funcs
        .iter()
...
fn bench_regalloc(module: &trust_ir::Module, name: &'static str) -> RegAllocResult {
    // Translate and run ISel + opt to get functions ready for regalloc
    let lir_funcs = trust_cg_lower::translate_module(module).expect("translate_module failed");

    let ir_funcs_for_ra: Vec<_> = lir_funcs
        .iter()
        .filter_map(|(lir_func, _)| {
            use trust_cg_lower::isel::InstructionSelector;
            let sig = trust_cg_lower::function::Signature {
    ...
    for _ in 0..WARMUP_ITERS {
        for f in &ir_funcs_for_ra {
            ...
            if let Ok(mut ra) = ra_func {
                let config = trust_cg_regalloc::AllocConfig::default_aarch64();
                if let Ok(result) = trust_cg_regalloc::allocate(&mut ra, &config) {
...
// ---------------------------------------------------------------------------
// Benchmark: Full pipeline end-to-end (trust_ir -> .o)
// ---------------------------------------------------------------------------
...
fn bench_e2e(module: &trust_ir::Module, name: &'static str, parallel: bool) -> E2EResult {
    let func_count = module.functions.len();
...
fn print_report(
    ...
    println!();
    println!("==============================================================================");
    println!("  Trust Codegen Compilation Speed Benchmark Report");
    println!("==============================================================================");
    println!();
    println!("  Architecture: AArch64 (Apple Silicon)");
    println!("  Optimization: O2");
    println!("  Pipeline: trust_ir -> ISel -> Opt -> RegAlloc -> Frame -> Encode -> Mach-O");
    println!();

    // ISel throughput
    println!("  --- ISel Throughput (trust_ir -> LIR) ---");
    println!();
    println!(
        "  {:<16} {:>5} {:>12} {:>12} {:>8}",
    ...
    // End-to-end
    println!("  --- Full Pipeline End-to-End (trust_ir -> .o) ---");
    println!();
    println!(
        "  {:<16} {:>5} {:>8} {:>12} {:>12} {:>8} {:>8}",
    ...
    for (s, p) in serial.iter().zip(parallel.iter()) {
        if s.module_name == p.module_name {
            ...
            let speedup = if parallel_ns > 0.0 {
                serial_ns / parallel_ns
            } else {
                0.0
            };
            println!(
                "  {:<16}  serial: {:>12}  parallel: {:>12}  speedup: {:.2}x",
                s.module_name,
