// trust-cg-codegen/benches/compile_bench.rs - Compilation throughput benchmarks
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// Benchmark framework for Trust Codegen compilation pipeline.
// Measures throughput (instructions/second) and per-phase timing.
//
// Run with: cargo bench -p trust-cg-codegen --bench compile_bench
//
...
use trust_cg_codegen::pipeline::{
    OptLevel, PipelineConfig, build_add_test_function, encode_function,
};
...
fn build_complex() -> MachFunction {
    ...
    append_inst(
        ...
        bb0,
        AArch64Opcode::BCond,
        vec![MachOperand::Imm(0xC), MachOperand::Imm(6)],
    ); // B.GT bb2 (skip 6 instrs)
    ...
    append_inst(&mut func, bb1, AArch64Opcode::B, vec![MachOperand::Imm(6)]); // B bb3 (skip bb2's 5 instrs + this = 6)
...
fn print_report(results: &[BenchResult]) {
    println!();
    println!("==============================================================================");
    println!("  Trust Codegen Compilation Benchmark Report");
    println!("==============================================================================");
    println!();
    println!("  Optimization level: O0 (no optimizations, measures raw pipeline overhead)");
    println!("  Architecture: AArch64 (Apple Silicon)");
    println!("  Note: ISel and RegAlloc are not benchmarked (require trust_ir input / VRegs)");
    ...
    for r in results {
        ...
        println!(
            "  {:<20} {:>12} {:>12} {:>12} {:>12}",
            r.complexity,
            phase_strs.first().map(|s| s.as_str()).unwrap_or("N/A"),
    ...
    if let Some(largest) = results.last() {
        let total_phase_ns: u128 = largest.phases.iter().map(|p| p.duration.as_nanos()).sum();
        if total_phase_ns > 0 {
            println!();
