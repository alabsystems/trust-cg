// trust-cg-codegen/tests/e2e_macho_link.rs - E2E Mach-O linking integration test
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// Proves the AArch64 backend works end-to-end:
//   trust_ir -> adapter -> ISel -> opt -> regalloc -> frame -> encode -> Mach-O -> otool -> ld -> run
//
...
fn make_test_dir(test_name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("trust_cg_e2e_link_{}", test_name));
...
/// Build trust_ir for: fn add_i32(a: i32, b: i32) -> i32 { a + b }
///
...
fn build_add_i32_trust_ir() -> (TrustIrFunction, TrustIrModule) {
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "add_i32", ft_id, BlockId::new(0));
...
/// Build trust_ir for: fn add_i64(a: i64, b: i64) -> i64 { a + b }
///
/// 64-bit variant of the add function.
fn build_add_i64_trust_ir() -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("test");
    let ft_id = module.add_func_type(FuncTy {
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "add_i64", ft_id, BlockId::new(0));
    func.blocks = vec![TrustIrBlock {
        id: BlockId::new(0),
...
fn build_sub_mul_trust_ir() -> (TrustIrFunction, TrustIrModule) {
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "sub_mul", ft_id, BlockId::new(0));
    func.blocks = vec![TrustIrBlock {
        id: BlockId::new(0),
        params: vec![
...
/// Build trust_ir for: fn abs_val(n: i64) -> i64 { if n < 0 { -n } else { n } }
///
...
fn build_abs_val_trust_ir() -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("test");
    let ft_id = module.add_func_type(FuncTy {
        params: vec![Ty::I64],
        returns: vec![Ty::I64],
...
fn test_e2e_link_add_i32_macho_structure() {
    ...
    assert_eq!(
        text_size % 4,
        0,
        "code size must be 4-byte aligned (AArch64)"
    );
    ...
    assert_eq!(
        cu_size, 32,
        "single function = one 32-byte compact unwind entry"
    );
    ...
    assert!(
        found_add,
        "code must contain an ADD instruction. Bytes: {:02X?}",
        code
    );
    assert!(
        found_ret,
        "code must contain a RET instruction. Bytes: {:02X?}",
        code
    );
}
...
fn test_e2e_link_add_i32_otool_validation() {
    ...
    if let Some(disasm) = run_otool_tv(&obj_path) {
        ...
        // Trivial leaf functions may use the documented zero-frame lowering,
        // so do not require STP/LDP here. Callee-save tests cover framed code.
    }
...
/// The definitive end-to-end test: compile trust_ir fn add_i32(i32, i32) -> i32,
...
fn test_e2e_link_add_i64_link_and_run() {
    ...
    let (trust_ir_func, module) = build_add_i64_trust_ir();
    let obj_bytes = compile_trust_ir_function(&trust_ir_func, &module, OptLevel::O0)
...
fn test_e2e_link_sub_mul_link_and_run() {
    ...
    let (trust_ir_func, module) = build_sub_mul_trust_ir();
    let obj_bytes = compile_trust_ir_function(&trust_ir_func, &module, OptLevel::O0)
...
fn test_e2e_link_abs_val_link_and_run() {
    ...
    let (trust_ir_func, module) = build_abs_val_trust_ir();
    let obj_bytes = compile_trust_ir_function(&trust_ir_func, &module, OptLevel::O0)
...
fn test_e2e_link_all_functions_produce_valid_macho() {
    let functions: Vec<(&str, TrustIrFunction, TrustIrModule)> = vec![
        {
            let (f, m) = build_add_i32_trust_ir();
            ("add_i32", f, m)
        },
        {
            let (f, m) = build_add_i64_trust_ir();
            ("add_i64", f, m)
        },
        {
            let (f, m) = build_sub_mul_trust_ir();
            ("sub_mul", f, m)
        },
        {
            let (f, m) = build_abs_val_trust_ir();
            ("abs_val", f, m)
        },
    ];
...
fn test_e2e_link_all_functions_otool_validates() {
    ...
    for (name, trust_ir_func, _module) in &functions {
        let obj_bytes = compile_trust_ir_function(trust_ir_func, _module, OptLevel::O0)
            .unwrap_or_else(|e| panic!("pipeline failed for {}: {}", name, e));
