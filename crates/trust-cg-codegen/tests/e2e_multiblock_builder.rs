// trust-cg-codegen/tests/e2e_multiblock_builder.rs
//
// Multi-block E2E tests using the trust_ir builder API.
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// These tests exercise multi-block control flow (if/else, loops, diamond CFG)
// through the FULL compilation pipeline using the trust_ir builder API:
//   ModuleBuilder -> Compiler::compile() -> Mach-O .o -> cc link -> run
//
// Unlike e2e_aarch64_link.rs which constructs trust_ir with raw struct literals,
...
fn make_test_dir(test_name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("trust_cg_e2e_mb_{}", test_name));
...
/// Compile a trust_ir module through the full Compiler::compile() pipeline.
fn compile_module(module: &TrustIrModule) -> Vec<u8> {
...
fn build_max_module() -> TrustIrModule {
...
fn build_sum_to_module() -> TrustIrModule {
...
fn build_clamp_module() -> TrustIrModule {
