// trust-cg-codegen/tests/e2e_correctness.rs - Correctness validation tests
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// These tests verify that Trust Codegen-compiled functions produce CORRECT computed
...
//   2. Full trust_ir pipeline through the Compiler API (ISel -> opt -> regalloc
...
use trust_cg_codegen::pipeline::{OptLevel, encode_function};
...
use trust_cg_ir::regs::{X0, X8, X9, X10, X11};
...
fn make_test_dir(test_name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("trust_cg_correctness_{}", test_name));
...
fn run_binary_with_output(binary: &Path) -> (i32, String) {
    ...
    let result = run_executable_with_timeout(binary, timeout).expect("run binary with timeout");
    let stdout = String::from_utf8_lossy(&result.output.stdout).into_owned();
    let stderr = String::from_utf8_lossy(&result.output.stderr);

    if result.timed_out {
        panic!(
            "Binary {} timed out after {:?} -- likely infinite loop; stdout={:?}, stderr={:?}",
            binary.display(),
            timeout,
            stdout,
            stderr
...
fn build_fibonacci_machir() -> MachFunction {
    ...
    let b_loop = MachInst::new(AArch64Opcode::B, vec![MachOperand::Imm(-6i64)]);
...
// ===========================================================================
// TYPE 2: trust_ir fibonacci through the Compiler API (full pipeline)
// ===========================================================================

/// Build a trust_ir module containing `fn _fibonacci_trust_ir(n: i64) -> i64`.
///
...
fn build_fibonacci_trust_ir_module() -> TrustIrModule {
    let mut module = TrustIrModule::new("correctness_test");
...
// ---------------------------------------------------------------------------
// Test: trust_ir fibonacci through Compiler API — full pipeline correctness
// ---------------------------------------------------------------------------
...
#[test]
fn test_correctness_fibonacci_trust_ir_compiler() {
    if !is_aarch64() || !has_cc() {
        eprintln!("Skipping e2e test: not AArch64 or cc not available");
        return;
    }

    let module = build_fibonacci_trust_ir_module();
    ...
    let driver_src = r#"
#include <stdio.h>
extern long _fibonacci_trust_ir(long n);
int main(void) {
    struct { long n; long expected; } tests[] = {
        {0, 0},
        {1, 1},
    ...
    for (int i = 0; i < n_tests; i++) {
        long result = _fibonacci_trust_ir(tests[i].n);
        printf("fibonacci_trust_ir(%ld) = %ld (expected %ld) %s\n",
               tests[i].n, result, tests[i].expected,
               result == tests[i].expected ? "OK" : "FAIL");
        if (result != tests[i].expected) return i + 1;
    }

    printf("All %d fibonacci_trust_ir tests passed.\n", n_tests);
    return 0;
}
"#;
    let driver_path = write_c_driver(&dir, "driver.c", driver_src);
    let binary = link_with_cc(&dir, &driver_path, &obj_path, "test_fibonacci_trust_ir");

    let (exit_code, stdout) = run_binary_with_output(&binary);
    eprintln!(
        "test_correctness_fibonacci_trust_ir_compiler stdout:\n{}",
        stdout
    );
    assert_eq!(
        exit_code, 0,
        "Fibonacci trust_ir Compiler correctness test failed at test {} (1-indexed). stdout:\n{}",
        exit_code, stdout
    );
...
// ===========================================================================
// Golden truth comparison: Trust Codegen vs clang
// ===========================================================================

/// Compiles the same fibonacci function with clang and Trust Codegen, then runs both
...
#include <stdio.h>
extern long fibonacci(long n);       /* Trust Codegen */
...
int main(void) {
    ...
    for (int i = 0; i < n_tests; i++) {
        ...
        long trust_cg_result = fibonacci(n);
        long clang_result = fibonacci_clang(n);
        printf("fib(%ld): trust-cg=%ld clang=%ld %s\n",
               n, trust_cg_result, clang_result,
               trust_cg_result == clang_result ? "MATCH" : "MISMATCH");
        if (trust_cg_result != clang_result) return i + 1;
    }
...
"#;
    ...
    // Link: compare.c + fib_trust-cg.o + fib_clang.o
    ...
    let (exit_code, stdout) = run_binary_with_output(&binary);
    eprintln!(
        "test_correctness_fibonacci_golden_truth stdout:\n{}",
        stdout
    );
    assert_eq!(
        exit_code, 0,
        "Golden truth comparison failed at test {} (1-indexed). Trust Codegen disagrees with clang. stdout:\n{}",
        exit_code, stdout
    );
...
fn build_sum_1_to_n_machir() -> MachFunction {
    ...
    let inst = MachInst::new(AArch64Opcode::B, vec![MachOperand::Imm(-4i64)]);
    let id = func.push_inst(inst);
    func.append_inst(bb_loop, id);
