// trust-cg-codegen/tests/panic_fuzz_macho_fixup.rs
...
//
// Run:
//   cargo test -p trust-cg-codegen --test panic_fuzz_macho_fixup
// Increase case count via env:
//   PROPTEST_CASES=100000 cargo test -p trust-cg-codegen --test panic_fuzz_macho_fixup
...
use trust_cg_codegen::macho::fixup::{
    Fixup, FixupList, FixupTarget, apply_branch26, apply_page21, apply_pageoff12,
};
use trust_cg_codegen::macho::linker::{MachOParser, ParsedSymbol, RelocationApplicator};
use trust_cg_codegen::macho::reloc::{AArch64RelocKind, Relocation};
...
proptest! {
    ...
    fn apply_branch26_in_range_never_panics(
        ...
        let byte_offset = word_offset * 4;
            ...
            // In-range inputs must now succeed (no panic, no Err).
            apply_branch26(&mut bytes, byte_offset).unwrap();
        });
    }
    ...
    fn apply_page21_in_range_never_panics(
        ...
        let mut bytes = insn.to_le_bytes();
            ...
            // In-range inputs must now succeed (no panic, no Err).
            apply_page21(&mut bytes, page_offset).unwrap();
        });
    }
    ...
    #[test]
    fn apply_pageoff12_aligned_shift_never_panics(
        insn in any::<u32>(),
        raw_page_offset in 0u32..4096,
        shift in 1u8..=4,
    ) {
        let alignment_mask = (1u32 << shift) - 1;
        let page_offset = raw_page_offset & !alignment_mask;
        let mut bytes = insn.to_le_bytes();
        catch("apply_pageoff12 (aligned shift)", move || {
            apply_pageoff12(&mut bytes, page_offset, shift);
        });
    }

    /// FINDING #10b: out-of-range inputs no longer panic — they return a typed
    /// `FixupError`. The property we assert is that ANY i64 byte offset is
    /// handled without panicking (returning either Ok or a controlled Err),
    /// and that wrapping in `catch_unwind` still observes no unwind.
    #[test]
    fn apply_branch26_out_of_range_is_caught(
        ...
        let mut bytes = insn.to_le_bytes();
        // Must not panic for arbitrary input; result may be Ok or Err.
        let res = panic::catch_unwind(panic::AssertUnwindSafe(move || {
            let _ = apply_branch26(&mut bytes, byte_offset);
        }));
        prop_assert!(res.is_ok(), "apply_branch26 must never panic");
    }
    ...
    fn macho_parse_with_valid_magic_never_panics(
        ...
        // Bias filetype toward MH_OBJECT (0x1) so the parser does NOT reject the
        // object at the early filetype check and we actually exercise the deeper
        // load-command / section walk (where the FINDING #4 OOB/overflow panics
        // lived); keep some random filetypes for the rejection path.
        filetype in prop_oneof![9 => Just(0x1u32), 1 => any::<u32>()],
        ncmds in 0u32..=64,
...
fn named_symbol_without_resolve_errors_cleanly() {
    ...
    assert!(
        result.is_err(),
        "expected error for unresolved named symbol"
    );
}
...
fn empty_section_branch26_returns_error() {
    ...
    let symbols: Vec<ParsedSymbol> = Vec::new();
    let symbol_addrs: HashMap<String, u64> = HashMap::new();
    let result = RelocationApplicator::apply(&mut data, 0, &relocs, &symbols, &symbol_addrs, &[]);
    assert!(
        result.is_err(),
        "empty section + BRANCH26 must error, not panic"
    );
}
