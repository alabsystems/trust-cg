// trust-cg-codegen/tests/e2e_abi_dual_target_linkrun.rs -- link+run ABI fixture (#466)
//
...
//
// This file:
//   * Builds a trust_ir fixture `sum8_i64(i0..i7: i64) -> i64 { i0+i1+...+i7 }`
...
//
// * Struct-by-value / sret. Listed in #466 as "if trust_ir uses them" -- current
//   trust_ir MVP does not use aggregate args; out of scope.
// * `ty`-specific instruction mix beyond int+fp arithmetic. That is a
...
mod common;
use common::rosetta::{
    codegen_link_timeout, codegen_probe_timeout, codegen_run_timeout, command_output_with_timeout,
...
fn probe_timeout() -> Duration {
    codegen_probe_timeout()
}

fn link_timeout() -> Duration {
...
fn has_cc_arch(arch: &str) -> bool {
    let dir =
        std::env::temp_dir().join(format!("trust_cg_cc_check_{}_{}", arch, std::process::id()));
    ...
    let compiled = match command_output_with_timeout(&mut compile_cmd, probe_timeout()) {
        ...
        Ok(result) => {
            ...
            eprintln!(
                "SKIP: cc -arch {} compile probe {} after {:?}; stdout={}, stderr={}",
                arch,
                if result.timed_out {
                    "timed out"
                } else {
        ...
        Err(e) => {
            eprintln!(
                "SKIP: cc -arch {} compile probe failed to start: {}",
                arch, e
            );
            false
        }
    ...
    let ran = if compiled {
        match run_executable_with_timeout(&out, probe_timeout()) {
            ...
            Ok(result) => {
                let (stdout, stderr) = output_text(&result.output);
                eprintln!(
                    "SKIP: cc -arch {} run probe {} after {:?}; stdout={}, stderr={}",
                    arch,
                    if result.timed_out {
                        "timed out"
                    } else {
            ...
            Err(e) => {
                eprintln!("SKIP: cc -arch {} run probe failed to start: {}", arch, e);
                false
            }
        }
    } else {
...
fn has_cc_aarch64() -> bool {
    static HAS_CC_AARCH64: OnceLock<bool> = OnceLock::new();
    *HAS_CC_AARCH64.get_or_init(|| has_cc_arch("arm64"))
}
fn has_cc_x86_64() -> bool {
    if cfg!(target_arch = "aarch64") && std::env::var_os("TRUST_CG_RUN_ROSETTA_LINKRUN").is_none() {
        eprintln!(
            "SKIP: Rosetta x86_64 link/run disabled on aarch64 host; set TRUST_CG_RUN_ROSETTA_LINKRUN=1 to opt in"
        );
        return false;
    }

    static HAS_CC_X86_64: OnceLock<bool> = OnceLock::new();
    *HAS_CC_X86_64.get_or_init(|| has_cc_arch("x86_64"))
}
...
fn make_test_dir(test_name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("trust_cg_e2e_abi_linkrun_{}", test_name));
    let _ = fs::remove_dir_all(&dir);
...
fn link(arch: &str, dir: &Path, driver_c: &Path, obj: &Path, output_name: &str) -> PathBuf {
    ...
    let result =
        command_output_with_timeout(&mut link_cmd, link_timeout()).expect("Failed to run cc");
...
/// Compile a trust_ir module through the public `Compiler` API for the given
...
fn link_and_run(
    ...
    let binary = link(
        ...
        &driver_path,
        &obj_path,
        &format!("bin_{}", test_name),
    );

    let run_result = run_executable_with_timeout(&binary, run_timeout())
        .unwrap_or_else(|e| panic!("running {} binary failed: {}", arch, e));
    if run_result.timed_out {
        let (stdout, stderr) = output_text(&run_result.output);
        panic!(
            "running {} binary timed out after {:?}: stdout={}, stderr={}",
            arch,
            run_timeout(),
            stdout.trim(),
            stderr.trim()
    ...
    // Keep dir if TRUST_CG_KEEP_LINKRUN is set, for post-mortem disassembly.
    if std::env::var_os("TRUST_CG_KEEP_LINKRUN").is_none() {
        let _ = fs::remove_dir_all(&dir);
    } else {
        eprintln!(
            "[{} {}] kept artifacts in {}",
...
fn build_sum8_i64_module() -> TrustIrModule {
    let mut module = TrustIrModule::new("sum8_i64_test");
    let ft = module.add_func_type(FuncTy {
        params: vec![Ty::I64; 8],
        returns: vec![Ty::I64],
        is_vararg: false,
    });
    let mut func = TrustIrFunction::new(FuncId::new(0), "_sum8_i64", ft, BlockId::new(0));

    // Params: v0..v7
    let params: Vec<(ValueId, Ty)> = (0..8).map(|i| (ValueId::new(i), Ty::I64)).collect();

    // Body: acc = v0; for i in 1..8 { acc = acc + vi; } return acc
    // The accumulator reuses a fresh ValueId each time (trust_ir is SSA).
    let mut body = Vec::new();
    let mut acc = ValueId::new(0); // v0
    for (next_id, i) in (8_u32..).zip(1..8u32) {
...
///   t3          = t2 + v8
///   return t3
fn build_sum6i_3d_module() -> TrustIrModule {
    let mut module = TrustIrModule::new("sum6i_3d_test");
    let ft = module.add_func_type(FuncTy {
        params: vec![
            Ty::I64,
            Ty::I64,
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "_sum6i_3d", ft, BlockId::new(0));
...
fn build_const_sentinel_module(name: &str, value: i64) -> TrustIrModule {
    let mut module = TrustIrModule::new("const_sentinel");
    let ft = module.add_func_type(FuncTy {
        params: vec![],
        returns: vec![Ty::I64],
        is_vararg: false,
