// trust-cg-codegen/tests/e2e_full_pipeline.rs - Full pipeline e2e tests from trust_ir
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// These tests exercise the COMPLETE compilation pipeline starting from trust_ir-like
...
// start from trust_ir and exercise every pipeline phase. This catches adapter and
...
use trust_ir::{BinOp, CastOp, ICmpOp, Inst, InstrNode};
use trust_ir::{
...
fn make_test_dir(test_name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("trust_cg_e2e_full_{}", test_name));
...
fn run_binary_with_output(binary: &Path) -> (i32, String) {
    ...
    let result = run_executable_with_timeout(binary, timeout)
        .unwrap_or_else(|e| panic!("Failed to run binary {}: {}", binary.display(), e));
    let stdout = String::from_utf8_lossy(&result.output.stdout).to_string();
    if result.timed_out {
        let stderr = String::from_utf8_lossy(&result.output.stderr);
        panic!(
            "Binary {} timed out after {:?}; stdout: {}; stderr: {}",
            binary.display(),
            timeout,
            stdout.trim(),
            stderr.trim()
...
/// Translate a trust_ir function through the adapter, then compile through the
...
fn compile_trust_ir_function(
    ...
    // Phase 0: Translate trust_ir -> LIR (adapter)
...
/// Build trust_ir for: fn is_prime(n: i64) -> i64
///
...
fn build_is_prime_trust_ir() -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("test");
    let ft_id = module.add_func_type(FuncTy {
        params: vec![Ty::I64],
        returns: vec![Ty::I64],
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "is_prime", ft_id, BlockId::new(0));
    func.blocks = vec![
        // bb0 (entry): check n <= 1
        TrustIrBlock {
            id: BlockId::new(0),
...
/// Build trust_ir for: fn sum_array(arr: *i64, len: i64) -> i64
///
...
///     i = i + 1
///     goto loop
fn build_sum_array_trust_ir() -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("test");
    let ft_id = module.add_func_type(FuncTy {
        params: vec![
            Ty::Ptr, // arr
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "sum_array", ft_id, BlockId::new(0));
    func.blocks = vec![
...
/// Build trust_ir for: fn simple_add(a: i64, b: i64) -> i64 { a + b }
///
/// Minimal full-pipeline test: just add two arguments and return.
fn build_simple_add_trust_ir() -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("test");
    let ft_id = module.add_func_type(FuncTy {
...
/// Build trust_ir for:
/// fn i128_arith_mix(a: i128, b: i128) -> i128 { (a + b) * (a - b) }
fn build_i128_arith_mix_trust_ir() -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("test");
    let ft_id = module.add_func_type(FuncTy {
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "i128_arith_mix", ft_id, BlockId::new(0));
    func.blocks = vec![TrustIrBlock {
        id: BlockId::new(0),
...
/// trust_ir -> adapter -> ISel -> opt -> regalloc -> frame -> encode -> Mach-O.
#[test]
fn test_full_pipeline_simple_add_encoding() {
    let (trust_ir_func, module) = build_simple_add_trust_ir();
...
/// Full pipeline: i128 add/sub/mul compiled, linked, and executed.
#[test]
fn test_full_pipeline_i128_arithmetic_run() {
    if !is_aarch64() || !has_cc() {
        eprintln!("Skipping: not AArch64 or cc not available");
        return;
    }

    let dir = make_test_dir("full_i128_arithmetic");

    let (trust_ir_func, module) = build_i128_arith_mix_trust_ir();
    let obj_bytes = compile_trust_ir_function(&trust_ir_func, &module, OptLevel::O0)
        .expect("full pipeline should compile i128_arith_mix");

    let obj_path = write_object_file(&dir, "i128_arith_mix.o", &obj_bytes);

    let driver_src = r#"
#include <stdint.h>
#include <stdio.h>

extern unsigned __int128 i128_arith_mix(unsigned __int128 a, unsigned __int128 b);

static unsigned __int128 make_u128(uint64_t hi, uint64_t lo) {
    return ((unsigned __int128)hi << 64) | (unsigned __int128)lo;
}

static uint64_t lo64(unsigned __int128 value) {
    return (uint64_t)value;
}

static uint64_t hi64(unsigned __int128 value) {
...
int main(void) {
    struct {
        ...
        {0x0000000000000002ULL, 0xfffffffffffffff0ULL, 0x0000000000000001ULL, 0x0000000000000020ULL},
        {0x123456789abcdef0ULL, 0xfedcba9876543210ULL, 0x0fedcba987654321ULL, 0x1111111111111111ULL},
        {0xffffffffffffffffULL, 0xffffffffffffff00ULL, 0x0000000000000000ULL, 0x0000000000000100ULL},
    };
    int count = (int)(sizeof(tests) / sizeof(tests[0]));
    for (int i = 0; i < count; i++) {
        unsigned __int128 a = make_u128(tests[i].ahi, tests[i].alo);
        unsigned __int128 b = make_u128(tests[i].bhi, tests[i].blo);
        unsigned __int128 got = i128_arith_mix(a, b);
        unsigned __int128 expected = (a + b) * (a - b);
        printf("case %d got=%016llx:%016llx expected=%016llx:%016llx\n",
               i,
               (unsigned long long)hi64(got),
               (unsigned long long)lo64(got),
               (unsigned long long)hi64(expected),
               (unsigned long long)lo64(expected));
        if (got != expected) return i + 1;
    }
    return 0;
}
"#;
    let driver_path = write_c_driver(&dir, "driver.c", driver_src);
    let binary = link_with_cc(&dir, &driver_path, &obj_path, "test_full_i128_arithmetic");

    let (exit_code, stdout) = run_binary_with_output(&binary);
    eprintln!("test_full_pipeline_i128_arithmetic stdout: {}", stdout);
    assert_eq!(
        exit_code, 0,
        "i128 arithmetic full pipeline test failed at case {} (1-indexed). stdout: {}",
        exit_code, stdout
    );
...
fn test_full_pipeline_is_prime_encoding() {
    let (trust_ir_func, module) = build_is_prime_trust_ir();
...
fn test_full_pipeline_sum_array_encoding() {
    let (trust_ir_func, module) = build_sum_array_trust_ir();
...
fn test_full_pipeline_opt_levels() {
    ...
    for opt_level in &[OptLevel::O0, OptLevel::O1, OptLevel::O2] {
        let obj_bytes = compile_trust_ir_function(&trust_ir_func, &module, *opt_level)
...
// ---------------------------------------------------------------------------
// Test: adapter integration — verify trust_ir->LIR->ISel chain preserves semantics
// ---------------------------------------------------------------------------

/// Verify the adapter correctly translates trust_ir fibonacci to LIR with expected
...
fn test_adapter_fibonacci_structure() {
    ...
    assert_eq!(
        ...
        vec![trust_cg_lower::types::Type::I64]
    );
    assert_eq!(
...
// ---------------------------------------------------------------------------
// trust_ir function builders: gcd and collatz_steps
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// trust_ir builders for harder programs
// ---------------------------------------------------------------------------

/// Build trust_ir for: fn power(base: i64, exp: i64) -> i64
///
...
fn build_power_trust_ir() -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("test");
    let ft_id = module.add_func_type(FuncTy {
        params: vec![Ty::I64, Ty::I64],
        returns: vec![Ty::I64],
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "power", ft_id, BlockId::new(0));
    func.blocks = vec![
...
/// Build trust_ir for: fn count_digits(n: i64) -> i64
///
...
fn build_count_digits_trust_ir() -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("test");
    let ft_id = module.add_func_type(FuncTy {
        params: vec![Ty::I64],
        returns: vec![Ty::I64],
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "count_digits", ft_id, BlockId::new(0));
    func.blocks = vec![
...
/// Build trust_ir for: fn nested_loop_sum(n: i64) -> i64
///
...
fn build_nested_loop_sum_trust_ir() -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("test");
    let ft_id = module.add_func_type(FuncTy {
        params: vec![Ty::I64],
        returns: vec![Ty::I64],
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "nested_loop_sum", ft_id, BlockId::new(0));
    func.blocks = vec![
        // bb0 (entry): param n, br -> bb1
        TrustIrBlock {
            id: BlockId::new(0),
...
fn test_full_pipeline_power_encoding() {
    let (trust_ir_func, module) = build_power_trust_ir();
...
/// Golden truth: Trust Codegen power vs clang power.
#[test]
fn test_full_pipeline_power_golden_truth() {
    ...
    let obj_bytes = compile_trust_ir_function(&trust_ir_func, &module, OptLevel::O0)
        .expect("full pipeline should compile power");
    let trust_cg_obj = write_object_file(&dir, "power_trust-cg.o", &obj_bytes);
...
int main(void) {
    ...
    for (int i = 0; i < n; i++) {
        long trust_cg = power(tests[i].base, tests[i].exp);
        long clang = power_clang(tests[i].base, tests[i].exp);
        printf("power(%ld,%ld): trust-cg=%ld clang=%ld %s\n",
               tests[i].base, tests[i].exp, trust_cg, clang,
               trust_cg == clang ? "MATCH" : "MISMATCH");
        if (trust_cg != clang) return i + 1;
    }
    return 0;
}
"#;
...
fn test_full_pipeline_count_digits_encoding() {
    let (trust_ir_func, module) = build_count_digits_trust_ir();
...
/// Golden truth: Trust Codegen count_digits vs clang count_digits.
#[test]
fn test_full_pipeline_count_digits_golden_truth() {
    ...
    let obj_bytes = compile_trust_ir_function(&trust_ir_func, &module, OptLevel::O0)
        .expect("full pipeline should compile count_digits");
    let trust_cg_obj = write_object_file(&dir, "count_digits_trust-cg.o", &obj_bytes);
...
int main(void) {
    ...
    for (int i = 0; i < n; i++) {
        long trust_cg = count_digits(inputs[i]);
        long clang = count_digits_clang(inputs[i]);
        printf("count_digits(%ld): trust-cg=%ld clang=%ld %s\n",
               inputs[i], trust_cg, clang,
               trust_cg == clang ? "MATCH" : "MISMATCH");
        if (trust_cg != clang) return i + 1;
    }
    return 0;
...
"#;
    ...
    let (exit_code, stdout) = run_binary_with_output(&binary);
    eprintln!(
        "test_full_pipeline_count_digits_golden_truth stdout: {}",
        stdout
    );
    assert_eq!(
        exit_code, 0,
...
fn test_full_pipeline_nested_loop_sum_encoding() {
    let (trust_ir_func, module) = build_nested_loop_sum_trust_ir();
...
/// Golden truth: Trust Codegen nested_loop_sum vs clang nested_loop_sum.
#[test]
fn test_full_pipeline_nested_loop_sum_golden_truth() {
    ...
    let obj_bytes = compile_trust_ir_function(&trust_ir_func, &module, OptLevel::O0)
        .expect("full pipeline should compile nested_loop_sum");
    let trust_cg_obj = write_object_file(&dir, "nested_loop_sum_trust-cg.o", &obj_bytes);
...
int main(void) {
    ...
    int n = sizeof(inputs) / sizeof(inputs[0]);
    for (int i = 0; i < n; i++) {
        long trust_cg = nested_loop_sum(inputs[i]);
        long clang = nested_loop_sum_clang(inputs[i]);
        printf("nested_loop_sum(%ld): trust-cg=%ld clang=%ld %s\n",
               inputs[i], trust_cg, clang,
               trust_cg == clang ? "MATCH" : "MISMATCH");
        if (trust_cg != clang) return i + 1;
    }
...
"#;
    ...
    let (exit_code, stdout) = run_binary_with_output(&binary);
    eprintln!(
        "test_full_pipeline_nested_loop_sum_golden_truth stdout: {}",
        stdout
    );
    assert_eq!(
        exit_code, 0,
