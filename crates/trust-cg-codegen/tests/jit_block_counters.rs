// trust-cg-codegen/tests/jit_block_counters.rs — Per-basic-block JIT counters.
//
...
fn block_counts_diamond_alternating_inputs() {
    ...
    assert_eq!(all, vec![(0, N), (1, odd_calls), (2, even_calls), (3, N)]);
...
#[cfg(target_arch = "aarch64")]
#[test]
fn block_counts_can_dump_binary_profdata_for_profile_use() {
    let jit = JitCompiler::new(JitConfig {
        profile_hooks: ProfileHookMode::BlockCounts,
        ..JitConfig::default()
    });
    ...
    const N: u64 = 16;
    for i in 0..N {
        let arg = if i % 4 == 0 { 0 } else { i };
        let expected = if arg == 0 { 11 } else { 77 };
        assert_eq!(diamond(arg), expected);
    }

    let profile_key = CacheKey::new(
        0x3960_0000_0000_0000_0000_0000_0000_b10c,
        3,
    ...
    let path = env::temp_dir().join(format!(
        "trust_cg_jit_block_counts_{}_{}.profdata",
        std::process::id(),
        N
    ));
    ...
    let captured = buf
        ...
        .expect("write binary block profdata");
    let bytes = fs::read(&path).expect("profdata file should be readable");
    assert_eq!(&bytes[..8], b"trcg-pgo");
    assert_ne!(bytes[0], b'{', "JIT dump must use binary v1 profdata");
    ...
    let function = loaded
        .function("diamond")
        .expect("captured function profile");
    assert_eq!(function.call_count, N);
    assert_eq!(function.block_hits(0), N);
    assert_eq!(function.block_hits(1), 12);
    assert_eq!(function.block_hits(2), 4);
    assert_eq!(function.block_hits(3), N);
    ...
    match enforce_fresh(&loaded, &stale_key) {
