// trust-cg-codegen integration test: Compact unwind emission
//
...
use trust_cg_codegen::frame::{UNWIND_ARM64_FRAME_X19_X20_PAIR, UNWIND_ARM64_MODE_FRAME};
use trust_cg_codegen::macho::MachOWriter;
use trust_cg_codegen::unwind::{
...
fn write_temp_o(bytes: &[u8], name: &str) -> std::path::PathBuf {
    ...
    let path = dir.join(format!("trust_cg_test_cu_{}.o", name));
...
fn run_otool(path: &std::path::Path) -> Option<String> {
        ...
        .output()
    {
        ...
        Err(err) if err.kind() == ErrorKind::NotFound => {
            eprintln!("SKIP: AArch64 Mach-O compact unwind inspection requires otool");
            return None;
        }
...
fn test_compact_unwind_section_present() {
    ...
    let section_index = add_compact_unwind_to_writer(&mut writer, &cu_section)
        .expect("compact unwind layout should be representable");
    assert!(
    ...
    let Some(otool_out) = run_otool(&path) else {
...
fn test_compact_unwind_multiple_functions() {
    ...
    writer.add_symbol("_func1", 1, 0, true).unwrap();
    writer
        .add_symbol("_func2", 1, func2_offset as u64, true)
        .unwrap();
...
fn test_compact_unwind_empty_section_not_added() {
    ...
    let section_index = add_compact_unwind_to_writer(&mut writer, &cu_section)
        .expect("empty compact unwind layout should be representable");
