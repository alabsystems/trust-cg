// trust-cg-codegen/tests/e2e_bridge.rs - trust-trust-cg-bridge integration tests
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// These tests validate that the LIR format produced by trust-trust-cg-bridge
// (in tRust) is valid input for Trust Codegen's compilation pipeline. Rather than
// depending on tRust/trust-types, we construct trust_cg_lower::Function instances
// directly, matching exactly what the bridge would produce.
//
// The trust-trust-cg-bridge (~/tRust/crates/trust-trust-cg-bridge/src/lib.rs)
// converts trust-types VerifiableFunction -> Trust Codegen LIR using these conventions:
...
//
// Part of #301 — trust-trust-cg-bridge integration
...
fn make_test_dir(test_name: &str) -> PathBuf {
    loop {
        ...
        match fs::create_dir(&dir) {
            ...
            Err(err) => panic!("Failed to create test directory {}: {err}", dir.display()),
        }
    }
}
...
// ---------------------------------------------------------------------------
// LIR function builders (matching trust-trust-cg-bridge output format)
// ---------------------------------------------------------------------------
...
fn build_bridge_midpoint() -> Function {
    ...
    Function {
        ...
        blocks,
        block_order: vec![Block(0), Block(1)],
        trust_ir_origins: HashMap::new(),
        entry_block: Block(0),
        stack_slots: vec![],
        value_types: HashMap::new(),
...
fn build_bridge_pick() -> Function {
    ...
    Function {
        ...
        blocks,
        block_order: vec![Block(0), Block(1), Block(2)],
        trust_ir_origins: HashMap::new(),
        entry_block: Block(0),
        stack_slots: vec![],
        value_types: HashMap::new(),
...
fn test_bridge_lir_structure_validation() {
    ...
    assert!(
        bb0.params.is_empty(),
        "bridge always uses empty block params"
    );
    ...
    assert!(matches!(
        pick.blocks[&Block(1)].instructions.last().unwrap().opcode,
        Opcode::Return
    ));
    assert!(matches!(
        pick.blocks[&Block(2)].instructions.last().unwrap().opcode,
        Opcode::Return
    ));
}
