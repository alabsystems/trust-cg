// trust-cg-codegen/tests/jit_profiling.rs - End-to-end JIT profiling hook tests
//
...
#[cfg(target_arch = "aarch64")]
use trust_cg_opt::pgo::{ProfDataError, enforce_fresh, read_from_path};
...
fn build_two_function_metrics_module() -> trust_ir::Module {
    use trust_ir::Ty;
...
fn build_single_function_metrics_module() -> trust_ir::Module {
    use trust_ir::Ty;
    use trust_ir_build::ModuleBuilder;
...
#[cfg(target_arch = "aarch64")]
#[test]
fn call_counts_can_dump_binary_profdata_across_reset_windows() {
    let jit = JitCompiler::new(JitConfig {
        profile_hooks: ProfileHookMode::CallCounts,
        ..JitConfig::default()
    });
    ...
    let profile_key = CacheKey::new(
        0x3960_0000_0000_0000_0000_0000_0000_c011,
        3,
    ...
    let path = env::temp_dir().join(format!(
        "trust_cg_jit_entry_counts_{}_first.profdata",
        std::process::id()
    ));

    let first = buf
        .write_entry_profdata_with_key(&profile_key, &path)
        .expect("write binary entry profdata");
    let bytes = fs::read(&path).expect("profdata file should be readable");
    assert_eq!(&bytes[..8], b"trcg-pgo");
    assert_ne!(bytes[0], b'{', "JIT entry dump must use binary v1 profdata");

    let loaded = read_from_path(&path).expect("binary entry profdata reloads");
    assert_eq!(loaded, first);
    enforce_fresh(&loaded, &profile_key).expect("matching full profile key is fresh");

    let alpha_profile = loaded.function("alpha").expect("alpha profile");
    assert_eq!(alpha_profile.call_count, 3);
    assert_eq!(alpha_profile.block_hits(0), 3);
    let beta_profile = loaded.function("beta").expect("beta profile");
    assert_eq!(beta_profile.call_count, 5);
    assert_eq!(beta_profile.block_hits(0), 5);
    ...
    let second_path = env::temp_dir().join(format!(
        "trust_cg_jit_entry_counts_{}_second.profdata",
        std::process::id()
    ));
    let second = buf
        .write_entry_profdata_with_key(&profile_key, &second_path)
        .expect("write reset-window entry profdata");
    let second_alpha = second.function("alpha").expect("alpha profile");
    let second_beta = second.function("beta").expect("beta profile");
    assert_eq!(second_alpha.call_count, 2);
    assert_eq!(second_alpha.block_hits(0), 2);
    assert_eq!(second_beta.call_count, 0);
    assert_eq!(second_beta.block_hits(0), 0);

    let stale_key = CacheKey::new(
        profile_key.module_hash(),
        2,
        profile_key.target_triple().to_string(),
        profile_key.cpu().to_string(),
        profile_key.features().to_vec(),
    );
    match enforce_fresh(&second, &stale_key) {
        Err(ProfDataError::StaleProfileKey { reason, .. }) => {
            assert_eq!(reason, "opt-level mismatch");
        }
        other => panic!("expected stale profile-key rejection, got {other:?}"),
    }
...
fn test_per_function_metrics_populated() {
    ...
    for metrics in &result.per_function_metrics {
        ...
        assert!(
            ...
            "code_size_bytes should be populated for {}",
            metrics.name
        );
        assert!(
            metrics
