// trust-cg-codegen/tests/e2e_macho_validation.rs - End-to-end Mach-O validation
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// These tests exercise the COMPLETE compilation pipeline starting from trust_ir,
...
fn make_test_dir(test_name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("trust_cg_e2e_macho_{}", test_name));
...
/// Build trust_ir for: fn return_const() -> i64 { return 42; }
///
/// Simplest possible trust_ir function: one block, const + return.
fn build_return_const_trust_ir() -> (TrustIrFunction, TrustIrModule) {
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "return_const", ft_id, BlockId::new(0));
...
/// Build trust_ir for: fn max_val(a: i64, b: i64) -> i64 { if a > b { a } else { b } }
///
/// Tests conditional branching with two exits:
///   bb0 (entry): cmp a > b, condbr -> bb1 (return a), bb2 (return b)
///   bb1: return a
///   bb2: return b
fn build_max_val_trust_ir() -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("test");
    let ft_id = module.add_func_type(FuncTy {
        params: vec![Ty::I64, Ty::I64],
        returns: vec![Ty::I64],
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "max_val", ft_id, BlockId::new(0));
    func.blocks = vec![
...
/// Build trust_ir for: fn simple_sub(a: i64, b: i64) -> i64 { a - b }
fn build_simple_sub_trust_ir() -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("test");
    let ft_id = module.add_func_type(FuncTy {
        params: vec![Ty::I64, Ty::I64],
        returns: vec![Ty::I64],
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "simple_sub", ft_id, BlockId::new(0));
    func.blocks = vec![TrustIrBlock {
        id: BlockId::new(0),
        params: vec![(ValueId::new(0), Ty::I64), (ValueId::new(1), Ty::I64)],
        body: vec![
...
/// Build trust_ir for a simple loop: fn count_down(n: i64) -> i64
///
...
///     i = i - 1
///     goto loop
fn build_count_down_trust_ir() -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("test");
    let ft_id = module.add_func_type(FuncTy {
        params: vec![Ty::I64],
        returns: vec![Ty::I64],
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "count_down", ft_id, BlockId::new(0));
    func.blocks = vec![
...
fn test_macho_return_const_binary_structure() {
    let (trust_ir_func, module) = build_return_const_trust_ir();
...
fn test_macho_simple_add_binary_structure() {
    ...
    assert!(
        text_size >= 8,
        "add function needs at least 2 instructions (8 bytes), got {}",
        text_size
    );
}
...
fn test_macho_simple_sub_disassembly() {
    ...
    let (trust_ir_func, module) = build_simple_sub_trust_ir();
    let obj_bytes = compile_trust_ir_function(&trust_ir_func, &module, OptLevel::O0)
...
fn test_macho_max_val_binary_structure() {
    let (trust_ir_func, module) = build_max_val_trust_ir();
...
fn test_macho_count_down_binary_structure() {
    let (trust_ir_func, module) = build_count_down_trust_ir();
...
fn test_macho_count_down_disassembly() {
    ...
    if let Some(disasm) = run_otool_tv(&obj_path) {
        ...
        let has_branch = disasm.contains("b ")
            || disasm.contains("b\t")
            || disasm.contains("b.")
            || disasm.contains("B.");
        assert!(
...
fn test_macho_opt_levels_otool_validates_all() {
    ...
    for (i, opt_level) in [OptLevel::O0, OptLevel::O1, OptLevel::O2]
        .iter()
        .enumerate()
    {
        let obj_bytes = compile_trust_ir_function(&trust_ir_func, &module, *opt_level)
...
fn test_macho_multiple_functions_all_valid() {
    let functions: Vec<(&str, TrustIrFunction, TrustIrModule)> = vec![
        {
            let (f, m) = build_return_const_trust_ir();
            ("return_const", f, m)
        },
        {
            let (f, m) = build_simple_add_trust_ir();
            ("simple_add", f, m)
        },
        {
            let (f, m) = build_simple_sub_trust_ir();
            ("simple_sub", f, m)
        },
        {
            let (f, m) = build_max_val_trust_ir();
            ("max_val", f, m)
        },
        {
            let (f, m) = build_count_down_trust_ir();
            ("count_down", f, m)
        },
    ];
...
fn test_macho_multiple_functions_otool_all_valid() {
    ...
    for (name, trust_ir_func, module) in &functions {
        ...
        assert!(lc_out.is_some(), "otool -l should succeed for {}", name);

        // otool -tv must succeed
        let disasm = run_otool_tv(&obj_path);
        assert!(disasm.is_some(), "otool -tv should succeed for {}", name);
...
fn test_macho_return_const_code_bytes() {
    ...
    let (text_offset, text_size) =
        find_section(&obj_bytes, b"__text").expect("__text section must exist");
...
fn test_macho_compact_unwind_entry_valid() {
    ...
    let (cu_offset, cu_size) =
        find_section(&obj_bytes, b"__compact_unwind").expect("__compact_unwind section must exist");
    ...
    // Trivial leaf functions use zero-frame lowering, which is encoded as
    // FRAMELESS (0x02xxxxxx) in compact unwind.
    let mode = encoding & 0x0F00_0000;
    assert_eq!(
        mode, 0x0200_0000,
        "Compact unwind encoding mode should be FRAMELESS (0x02), got 0x{:08X}",
        encoding
    );
    ...
    assert_eq!(
        personality, 0,
        "Personality should be 0 for non-exception code"
    );
...
fn test_macho_frame_lowering_in_disassembly() {
    ...
    if let Some(disasm) = run_otool_tv(&obj_path) {
        ...
        // Full pipeline leaf functions may use zero-frame lowering, so the
        // disassembly should prove useful code and return rather than forcing
        // a frame prologue/epilogue.
        assert!(
            disasm.contains("_simple_add"),
            "Full pipeline function should keep the symbol name.\nOutput:\n{}",
            disasm
        );
        assert!(
            disasm.contains("add") || disasm.contains("ADD"),
            "Full pipeline function should contain the add instruction.\nOutput:\n{}",
            disasm
        );
        assert!(
            disasm.contains("ret"),
            "Full pipeline function should return.\nOutput:\n{}",
            disasm
        );
    }
...
fn test_macho_object_sizes_reasonable() {
    let functions: Vec<(&str, TrustIrFunction, TrustIrModule, usize, usize)> = vec![
        // (name, trust_ir_func, module, min_bytes, max_bytes)
        {
            let (f, m) = build_return_const_trust_ir();
            ("return_const", f, m, 100, 10_000)
        },
        {
            let (f, m) = build_simple_add_trust_ir();
            ("simple_add", f, m, 100, 10_000)
        },
        {
            let (f, m) = build_max_val_trust_ir();
            ("max_val", f, m, 100, 10_000)
        },
        {
            let (f, m) = build_count_down_trust_ir();
            ("count_down", f, m, 100, 10_000)
        },
    ];

    for (name, trust_ir_func, module, min_size, max_size) in &functions {
        let obj_bytes = compile_trust_ir_function(trust_ir_func, module, OptLevel::O0)
            .unwrap_or_else(|e| panic!("pipeline failed for {}: {}", name, e));
