// guard_kernel_gate_x86_divzero_e2e.rs — Fail-closed DIV-ZERO authority tests on x86-64
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0

//! Public `DivNonZero` annotations and adapter-synthesized ids are non-authoritative. The adapter
//! reports no synthesized discharge, the selector leaves the carrier unbound, and the production
//! compiler retains its hardware divide-by-zero guard for both legacy environment values.
//!
//! The single-divisor fingerprint remains covered, while the tests explicitly distinguish stable
//! identity from proof authority.
...
/// Build a trust-ir module whose single function does `i64 sdiv(i64 a, i64 b) { return a / b; }` with
/// the division carrying the public `DivNonZero` annotation.
fn build_div_non_zero_module() -> (Module, TrustIrFunction) {
    let mut module = Module::new("guard_kernel_gate_x86_divzero_e2e");
    let ft = module.add_func_type(FuncTy {
        params: vec![Ty::I64, Ty::I64],
        returns: vec![Ty::I64],
        is_vararg: false,
    });
    let mut func = TrustIrFunction::new(FuncId::new(0), "proven_sdiv_x86", ft, BlockId::new(0));
    let sdiv = InstrNode::new(Inst::BinOp {
        op: BinOp::SDiv,
        ty: Ty::I64,
        lhs: ValueId::new(0),
        rhs: ValueId::new(1),
...
/// Count the live x86 div-zero-check carriers in the function.
fn live_x86_div_zero_carriers(func: &X86ISelFunction) -> usize {
    func.block_order
        .iter()
        .filter_map(|b| func.blocks.get(b))
        .flat_map(|b| b.insts.iter())
        .filter(|i: &&X86ISelInst| i.opcode == X86Opcode::TrapDivZeroExact)
        .count()
}
...
#[test]
fn x86_kernel_gate_rejects_div_non_zero_annotation_as_authority() {
    let (module, func) = build_div_non_zero_module();
    let (mut isel_func, synthesized) = lower_to_x86_isel(&func, &module);

    assert!(
        synthesized.is_empty(),
        "public DivNonZero annotation must not synthesize deletion authority"
    );

    // Real x86 lowering emits the carrier and deliberately leaves it unbound.
    assert_eq!(
        live_x86_div_zero_carriers(&isel_func),
        1,
        "real x86 lowering must emit exactly one div-zero-check carrier"
    );
    assert_eq!(
        isel_func.guard_obligations.len(),
        0,
        "annotation-only div-zero carrier must not receive an authority binding"
    );
    ...
    assert_eq!(
        live_x86_div_zero_carriers(&isel_func),
        1,
        "the x86 div-zero carrier must be retained without replay authority"
    );
...
#[test]
fn x86_annotation_only_div_zero_has_empty_authority_inputs() {
    let (module, func) = build_div_non_zero_module();
    let (mut isel_func, synthesized) = lower_to_x86_isel(&func, &module);

    assert_eq!(live_x86_div_zero_carriers(&isel_func), 1);
    ...
    assert_eq!(
        live_x86_div_zero_carriers(&isel_func),
        1,
        "empty authority inputs keep the div-zero guard fail-closed"
    );
    assert_eq!(pass.kernel_eliminations().len(), 0);
}

/// Count UD2 (0F 0B) occurrences in raw object bytes. A surviving x86 div-zero-check carrier expands to
/// a synthetic UD2 trap block, so UD2 presence is the observable for "guard kept".
...
/// Full-pipeline refutation: neither legacy environment value may authorize guard deletion. The
/// divide and runtime trap remain present in both objects.
#[test]
fn x86_div_zero_kernel_gate_env_values_keep_hardware_guard() {
    use common::disasm::{disassemble_x86_text, has_objdump};

    let (module, _func) = build_div_non_zero_module();
    ...
    let (off, on, off_ud2, on_ud2) = env_lock::with_env_edits(|env| {
        ...
        assert!(
            off_ud2 >= 1,
            "legacy value 0 must keep the UD2 div-zero trap"
        );
    ...
    assert!(
        on_ud2 >= 1,
        "legacy value 1 must keep the UD2 div-zero trap"
    );
    assert_eq!(
        on_ud2, off_ud2,
    ...
    // The divide survives in both real functions.
    ...
    // x86 stream and assert the divide and runtime trap are present under both legacy values.
    if has_objdump() {
        ...
        let has_idiv = |insns: &[common::disasm::DisasmInsn]| {
            insns.iter().any(|i| i.mnemonic.starts_with("idiv"))
        };
        assert!(
            has_idiv(&off_insns),
            "gate OFF: the divide (idiv) must be present — the access is never removed"
        );
        assert!(
            has_idiv(&on_insns),
            "legacy value 1: the divide (idiv) must still be present"
        );
        ...
        assert!(
            ud2s(&off_insns) >= 1,
            "gate OFF: the kept div-zero carrier expands to a real ud2 trap"
        );
        assert!(
            ud2s(&on_insns) >= 1,
            "legacy value 1 keeps the div-zero trap"
        );
    }
}

/// The fingerprint remains deterministic, but must remain unbound without exact replay.
#[test]
fn x86_div_zero_fingerprint_is_stable_but_unbound_without_replay() {
    let (module, func) = build_div_non_zero_module();
    ...
    let carrier = isel_func
        ...
        .flat_map(|b| b.insts.iter())
        .find(|i| i.opcode == X86Opcode::TrapDivZeroExact)
        .expect("div-zero carrier present");
    ...
    assert_eq!(
        refs.len(),
        1,
        "div-zero carrier fingerprints over a single [divisor] operand"
    );
    // The binding key folds in GuardKind::DivZero (defense-in-depth, Item B).
    let fp = trust_cg_ir::fingerprint_for_kind(GuardKind::DivZero, &refs);
    assert_eq!(
        fp,
        trust_cg_ir::fingerprint_for_kind(GuardKind::DivZero, &refs)
    );
    assert!(
