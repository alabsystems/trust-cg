// trust-cg-codegen integration test: AArch64 instruction encoding
//
...
use trust_cg_codegen::aarch64::encoding_fp::*;
use trust_cg_codegen::aarch64::encoding_mem::*;
...
fn write_temp_o(bytes: &[u8], name: &str) -> std::path::PathBuf {
    ...
    let path = dir.join(format!("trust_cg_aarch64_test_{}.o", name));
    let mut f = std::fs::File::create(&path).expect("failed to create temp file");
    f.write_all(bytes).expect("failed to write temp file");
    path
}

fn skip_disassembly_test(reason: impl std::fmt::Display) {
    eprintln!("SKIP: AArch64 Mach-O disassembly inspection unavailable: {reason}");
}
...
fn objdump_lacks_macho_support(output: &std::process::Output) -> bool {
...
fn run_otool_tv(path: &std::path::Path) -> Option<String> {
        ...
        .output()
    {
        ...
        Err(err) if err.kind() == ErrorKind::NotFound => {
            skip_disassembly_test("otool is not installed");
            return None;
        }
        Err(err) => panic!("failed to run otool -tv: {err}"),
    };
    assert!(
...
/// Run `objdump -d` on a .o file and return stdout.
fn run_objdump_d(path: &std::path::Path) -> Option<String> {
    ...
    for tool in ["llvm-objdump", "objdump"] {
            ...
            .output()
        {
            ...
            Err(err) => panic!("failed to run {tool} -d: {err}"),
        };
        ...
        if objdump_lacks_macho_support(&output) {
            incompatible.push(tool);
            continue;
        }

        panic!("{tool} -d failed: {}", command_text(&output));
    }
    ...
    skip_disassembly_test(format!(
        "no Mach-O-capable objdump found (missing: {missing}; incompatible: {incompatible})"
    ));
    None
}

// ===========================================================================
// 0. Encoding Verification Tests — RBIT
// ===========================================================================

#[test]
fn test_rbit_w0_w1_encoding() {
    assert_eq!(encode_bit_reverse(0, 1, 0), 0x5AC0_0020);
}

#[test]
fn test_rbit_x2_x3_encoding() {
    assert_eq!(encode_bit_reverse(1, 3, 2), 0xDAC0_0062);
}

#[test]
fn test_macho_roundtrip_rbit_disassembly() {
    let mut code = Vec::new();
    code.extend_from_slice(&encode_bit_reverse(0, 1, 0).to_le_bytes());
    code.extend_from_slice(&encode_bit_reverse(1, 3, 2).to_le_bytes());
    code.extend_from_slice(&0xD65F03C0u32.to_le_bytes());

    let mut writer = MachOWriter::new();
    writer.add_text_section(&code);
    writer.add_symbol("_rbit_test", 1, 0, true).unwrap();

    let bytes = writer.write().unwrap();
    let path = write_temp_o(&bytes, "rbit");
    let Some(disasm) = run_otool_tv(&path) else {
        std::fs::remove_file(&path).ok();
        return;
    };
    let disasm = disasm.to_lowercase();

    assert!(disasm.contains("rbit\tw0, w1"), "missing rbit w: {disasm}");
    assert!(disasm.contains("rbit\tx2, x3"), "missing rbit x: {disasm}");

    std::fs::remove_file(&path).ok();
}

#[test]
fn test_macho_objdump_neon_rbit_rev_disassembly() {
    let mut code = Vec::new();
    code.extend_from_slice(
        &encode_vec_byte_2reg(VectorArrangement::B8, VecByte2Op::Rbit, 1, 0)
            .unwrap()
            .to_le_bytes(),
    );
    code.extend_from_slice(
        &encode_vec_byte_2reg(VectorArrangement::B16, VecByte2Op::Rbit, 3, 2)
            .unwrap()
            .to_le_bytes(),
    );
    code.extend_from_slice(
        &encode_vec_byte_2reg(VectorArrangement::B8, VecByte2Op::Rev32, 5, 4)
            .unwrap()
            .to_le_bytes(),
    );
    code.extend_from_slice(
        &encode_vec_byte_2reg(VectorArrangement::B16, VecByte2Op::Rev64, 7, 6)
            .unwrap()
            .to_le_bytes(),
    );
    code.extend_from_slice(&0xD65F03C0u32.to_le_bytes());
    ...
    writer.add_text_section(&code);
    writer
        .add_symbol("_neon_rbit_rev_test", 1, 0, true)
        .unwrap();

    let bytes = writer.write().unwrap();
    let path = write_temp_o(&bytes, "neon_rbit_rev");
    let Some(disasm) = run_objdump_d(&path) else {
        std::fs::remove_file(&path).ok();
        return;
    };
    let disasm = disasm.to_lowercase();
    ...
    assert!(
        disasm.contains("rbit.8b\tv0, v1"),
        "missing rbit.8b: {disasm}"
    );
    assert!(
        disasm.contains("rbit.16b\tv2, v3"),
        "missing rbit.16b: {disasm}"
    );
    assert!(
        disasm.contains("rev32.8b\tv4, v5"),
        "missing rev32.8b: {disasm}"
    );
    assert!(
        disasm.contains("rev64.16b\tv6, v7"),
        "missing rev64.16b: {disasm}"
    );
...
fn test_ldr_x0_x1_zero_offset() {
    ...
    let enc =
        encode_ldr_str_unsigned_offset(LoadStoreSize::Double, false, LoadStoreOp::Load, 0, 1, 0)
            .unwrap();
...
fn test_ldr_w3_sp_4() {
    ...
    let enc =
        encode_ldr_str_unsigned_offset(LoadStoreSize::Word, false, LoadStoreOp::Load, 4, 31, 3)
            .unwrap();
...
fn test_str_x29_x28_zero() {
    ...
    let enc =
        encode_ldr_str_unsigned_offset(LoadStoreSize::Double, false, LoadStoreOp::Store, 0, 28, 29)
            .unwrap();
...
fn test_ldrb_w0_x1() {
    ...
    let enc =
        encode_ldr_str_unsigned_offset(LoadStoreSize::Byte, false, LoadStoreOp::Load, 0, 1, 0)
            .unwrap();
...
fn test_ldrh_w0_x1() {
    ...
    let enc =
        encode_ldr_str_unsigned_offset(LoadStoreSize::Half, false, LoadStoreOp::Load, 0, 1, 0)
            .unwrap();
...
fn test_strb_w0_x0() {
    ...
    let enc =
        encode_ldr_str_unsigned_offset(LoadStoreSize::Byte, false, LoadStoreOp::Store, 0, 0, 0)
            .unwrap();
...
fn test_str_x0_sp_zero() {
    ...
    let enc =
        encode_ldr_str_unsigned_offset(LoadStoreSize::Double, false, LoadStoreOp::Store, 0, 31, 0)
            .unwrap();
...
fn test_ldr_unsigned_v_bit() {
    ...
    let enc =
        encode_ldr_str_unsigned_offset(LoadStoreSize::Double, true, LoadStoreOp::Store, 0, 31, 31)
            .unwrap();
...
fn test_ldr_x30_sp_pre_pos16() {
    ...
    let enc = encode_ldr_str_pre_index(LoadStoreSize::Double, false, LoadStoreOp::Load, 16, 31, 30)
        .unwrap();
...
fn test_str_x0_x1_pre_neg1() {
    // STR X0, [X1, #-1]!
    // imm9=-1 => 0x1FF
    let enc = encode_ldr_str_pre_index(LoadStoreSize::Double, false, LoadStoreOp::Store, -1, 1, 0)
        .unwrap();
...
fn test_pre_index_max_imm9() {
    // STR X0, [X1, #255]!
    let enc = encode_ldr_str_pre_index(LoadStoreSize::Double, false, LoadStoreOp::Store, 255, 1, 0)
        .unwrap();
    // imm9=255 (0x0FF)
...
fn test_pre_index_min_imm9() {
    // STR X0, [X1, #-256]!
    let enc =
        encode_ldr_str_pre_index(LoadStoreSize::Double, false, LoadStoreOp::Store, -256, 1, 0)
            .unwrap();
    // imm9=-256 => 0x100
...
fn test_ldr_x0_x1_post_8() {
    // LDR X0, [X1], #8
    let enc = encode_ldr_str_post_index(LoadStoreSize::Double, false, LoadStoreOp::Load, 8, 1, 0)
        .unwrap();
...
fn test_str_x0_sp_post_neg16() {
    // STR X0, [SP], #-16
    let enc =
        encode_ldr_str_post_index(LoadStoreSize::Double, false, LoadStoreOp::Store, -16, 31, 0)
            .unwrap();
    // imm9=-16 => 0x1F0
...
fn test_ldr_w0_x1_post_max() {
    // LDR W0, [X1], #255
    let enc = encode_ldr_str_post_index(LoadStoreSize::Word, false, LoadStoreOp::Load, 255, 1, 0)
        .unwrap();
...
fn test_ldr_w0_x1_post_min() {
    // LDR W0, [X1], #-256
    let enc = encode_ldr_str_post_index(LoadStoreSize::Word, false, LoadStoreOp::Load, -256, 1, 0)
        .unwrap();
...
fn test_stp_x29_x30_sp_pre_neg16() {
    ...
    let enc =
        encode_ldp_stp_pre_index(PairSize::X64, false, PairOp::StorePair, -2, 30, 31, 29).unwrap();
...
fn test_ldp_x29_x30_sp_post_16() {
    ...
    let enc =
        encode_ldp_stp_post_index(PairSize::X64, false, PairOp::LoadPair, 2, 30, 31, 29).unwrap();
...
fn test_stp_w0_w1_sp_pre_neg8() {
    ...
    let enc =
        encode_ldp_stp_pre_index(PairSize::W32, false, PairOp::StorePair, -2, 1, 31, 0).unwrap();
    // 00 101 0 11 0 1111110 00001 11111 00000
    let expected =
        (0b101 << 27) | (0b11 << 23) | (((-2i8 as u8 & 0x7F) as u32) << 15) | (1 << 10) | (31 << 5);
    assert_eq!(enc, expected);
}
...
fn test_ldp_x0_x1_x2_zero_offset() {
    // LDP X0, X1, [X2, #0]
    let enc = encode_ldp_stp_offset(PairSize::X64, false, PairOp::LoadPair, 0, 1, 2, 0).unwrap();
    // opc=10, V=0, mode=10(signed), L=1, imm7=0, Rt2=1, Rn=2, Rt=0
    let expected =
        (0b10u32 << 30) | (0b101 << 27) | (0b10 << 23) | (1 << 22) | (1 << 10) | (2 << 5);
    assert_eq!(enc, expected);
}
...
fn test_stp_imm7_min() {
    ...
    let enc =
        encode_ldp_stp_offset(PairSize::W32, false, PairOp::StorePair, -64, 1, 31, 0).unwrap();
...
fn test_ldp_imm7_max() {
    ...
    let enc =
        encode_ldp_stp_pre_index(PairSize::X64, false, PairOp::LoadPair, 63, 30, 31, 29).unwrap();
...
fn test_fcmp_s0_s1() {
    ...
    // Rn=0, Rm=1
    let expected = (0b11110u32 << 24) | (1 << 21) | (1 << 16) | (0b1000 << 10);
    assert_eq!(enc, expected);
}
...
fn test_fcmp_d0_d1() {
    ...
    let expected = (0b11110u32 << 24) | (0b01 << 22) | (1 << 21) | (1 << 16) | (0b1000 << 10);
    assert_eq!(enc, expected);
}
...
fn test_fmov_d0_d1() {
    ...
    let expected = (0b11110u32 << 24) | (0b01 << 22) | (1 << 21) | (0b10000 << 10) | (1 << 5);
    assert_eq!(enc, expected);
}
...
fn test_imm12_boundary_ok() {
    ...
    assert!(
        encode_ldr_str_unsigned_offset(LoadStoreSize::Byte, false, LoadStoreOp::Load, 0, 0, 0)
            .is_ok()
    );
    assert!(
        encode_ldr_str_unsigned_offset(LoadStoreSize::Byte, false, LoadStoreOp::Load, 4095, 0, 0)
            .is_ok()
    );
}
...
fn test_imm12_boundary_err() {
    let err =
        encode_ldr_str_unsigned_offset(LoadStoreSize::Byte, false, LoadStoreOp::Load, 4096, 0, 0);
    assert!(matches!(
        err,
        Err(EncodeError::Imm12OutOfRange { value: 4096 })
    ));
}
...
fn test_imm9_boundary_ok() {
    assert!(
        encode_ldr_str_pre_index(LoadStoreSize::Double, false, LoadStoreOp::Store, -256, 0, 0)
            .is_ok()
    );
    assert!(
        encode_ldr_str_pre_index(LoadStoreSize::Double, false, LoadStoreOp::Store, 255, 0, 0)
            .is_ok()
    );
}
...
fn test_imm9_boundary_err_positive() {
    ...
    assert!(matches!(
        err,
        Err(EncodeError::Imm9OutOfRange { value: 256 })
    ));
}
...
fn test_imm9_boundary_err_negative() {
    let err =
        encode_ldr_str_pre_index(LoadStoreSize::Double, false, LoadStoreOp::Store, -257, 0, 0);
    assert!(matches!(
        err,
        Err(EncodeError::Imm9OutOfRange { value: -257 })
    ));
}
...
fn test_imm7_boundary_err() {
    ...
    assert!(matches!(
        err,
        Err(EncodeError::Imm7OutOfRange { value: -65 })
    ));
}
...
fn test_register_boundary_ok() {
    assert!(
        encode_ldr_str_unsigned_offset(LoadStoreSize::Double, false, LoadStoreOp::Load, 0, 0, 0)
            .is_ok()
    );
    assert!(
        encode_ldr_str_unsigned_offset(LoadStoreSize::Double, false, LoadStoreOp::Load, 0, 31, 31)
            .is_ok()
    );
}
...
fn test_register_boundary_err_rn() {
    let err =
        encode_ldr_str_unsigned_offset(LoadStoreSize::Double, false, LoadStoreOp::Load, 0, 32, 0);
    assert!(matches!(
        err,
        Err(EncodeError::RegisterOutOfRange { reg: 32, max: 31 })
    ));
}
...
fn test_register_boundary_err_rt() {
    let err =
        encode_ldr_str_unsigned_offset(LoadStoreSize::Double, false, LoadStoreOp::Load, 0, 0, 32);
    assert!(matches!(
        err,
        Err(EncodeError::RegisterOutOfRange { reg: 32, max: 31 })
    ));
...
fn test_sp_as_base_ldr_unsigned() {
    ...
    let enc =
        encode_ldr_str_unsigned_offset(LoadStoreSize::Double, false, LoadStoreOp::Load, 0, 31, 0)
            .unwrap();
...
fn test_sp_as_base_str_pre() {
    ...
    let enc =
        encode_ldr_str_pre_index(LoadStoreSize::Double, false, LoadStoreOp::Store, -16, 31, 0)
            .unwrap();
    assert_eq!((enc >> 5) & 0x1F, 31);
}
...
fn test_xzr_as_rt() {
    ...
    let enc =
        encode_ldr_str_unsigned_offset(LoadStoreSize::Double, false, LoadStoreOp::Store, 0, 1, 31)
            .unwrap();
...
fn test_sp_in_pair_rn() {
    // LDP X0, X1, [SP, #0]
    let enc = encode_ldp_stp_offset(PairSize::X64, false, PairOp::LoadPair, 0, 1, 31, 0).unwrap();
    assert_eq!((enc >> 5) & 0x1F, 31); // Rn = SP
}
...
fn test_extract_v_bit() {
    ...
    // V=1 (SIMD/FP)
    let enc_fp =
        encode_ldr_str_unsigned_offset(LoadStoreSize::Double, true, LoadStoreOp::Load, 0, 0, 0)
            .unwrap();
...
fn test_extract_opc_field() {
    ...
    // Store: opc=00
    let enc_store =
        encode_ldr_str_unsigned_offset(LoadStoreSize::Double, false, LoadStoreOp::Store, 0, 0, 0)
            .unwrap();
...
fn test_extract_imm12_field() {
    let enc =
        encode_ldr_str_unsigned_offset(LoadStoreSize::Double, false, LoadStoreOp::Load, 2048, 0, 0)
            .unwrap();
...
fn test_extract_rn_rt_fields() {
    let enc =
        encode_ldr_str_unsigned_offset(LoadStoreSize::Double, false, LoadStoreOp::Load, 0, 17, 23)
            .unwrap();
    let rn = (enc >> 5) & 0x1F;
    let rt = enc & 0x1F;
    assert_eq!(rn, 17);
...
fn test_extract_pair_mode_field() {
    for (mode, expected) in [
        ...
        let enc =
            encode_ldp_stp(PairSize::X64, false, PairOp::StorePair, mode, 0, 0, 0, 0).unwrap();
...
fn test_extract_imm9_field_negative() {
    ...
    let enc = encode_ldr_str_pre_index(LoadStoreSize::Double, false, LoadStoreOp::Store, -16, 0, 0)
        .unwrap();
...
fn test_extract_imm7_field_negative() {
    ...
    let enc =
        encode_ldp_stp_pre_index(PairSize::X64, false, PairOp::StorePair, -2, 0, 0, 0).unwrap();
...
fn test_macho_roundtrip_prologue_epilogue() {
    ...
    let mut writer = MachOWriter::new();
    writer.add_text_section(&code);
    writer.add_symbol("_test_func", 1, 0, true).unwrap();
    ...
    assert!(
        disasm.contains("stp"),
        "Missing stp in disassembly: {}",
        disasm
    );
    assert!(
        disasm.contains("ldp"),
        "Missing ldp in disassembly: {}",
        disasm
    );
    assert!(
        disasm.contains("ret"),
        "Missing ret in disassembly: {}",
        disasm
    );
...
fn test_macho_roundtrip_load_store() {
    ...
    // LDR X1, [SP, #0]
    let ldr_inst =
        encode_ldr_str_unsigned_offset(LoadStoreSize::Double, false, LoadStoreOp::Load, 0, 31, 1)
            .unwrap();
    ...
    let mut writer = MachOWriter::new();
    writer.add_text_section(&code);
    writer.add_symbol("_test_loads", 1, 0, true).unwrap();
    ...
    assert!(
        disasm.contains("str"),
        "Missing str in disassembly: {}",
        disasm
    );
    assert!(
        disasm.contains("ldr"),
        "Missing ldr in disassembly: {}",
        disasm
    );
    assert!(
        disasm.contains("adrp"),
        "Missing adrp in disassembly: {}",
        disasm
    );
    assert!(
        disasm.contains("ret"),
...
fn test_macho_roundtrip_fp_instructions() {
    ...
    let mut writer = MachOWriter::new();
    writer.add_text_section(&code);
    writer.add_symbol("_test_fp", 1, 0, true).unwrap();
...
fn test_macho_roundtrip_mixed_sequence() {
    ...
    // LDR X1, [X0, #0]
    let ldr =
        encode_ldr_str_unsigned_offset(LoadStoreSize::Double, false, LoadStoreOp::Load, 0, 0, 1)
            .unwrap();
    ...
    let mut writer = MachOWriter::new();
    writer.add_text_section(&code);
    writer.add_symbol("_mixed_func", 1, 0, true).unwrap();
...
fn test_macho_roundtrip_fp_conversions() {
    ...
    let mut writer = MachOWriter::new();
    writer.add_text_section(&code);
    writer.add_symbol("_test_conv", 1, 0, true).unwrap();
...
fn test_all_load_store_size_x_op_combinations() {
    ...
    let ops = [(LoadStoreOp::Store, 0b00u32), (LoadStoreOp::Load, 0b01)];
...
fn test_all_pair_size_x_op_x_mode_combinations() {
    let pair_sizes = [(PairSize::W32, 0b00u32), (PairSize::X64, 0b10)];
    let pair_ops = [(PairOp::StorePair, 0u32), (PairOp::LoadPair, 1)];
    let modes = [
        (PairMode::PostIndex, 0b01u32),
        (PairMode::SignedOffset, 0b10),
        (PairMode::PreIndex, 0b11),
