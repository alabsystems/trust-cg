// trust-cg-codegen/tests/jit_proof_certs.rs - End-to-end JIT proof certificate tests
//
...
// trust_ir→MachInst provenance is populated.
//
...
fn verify_on_attaches_certificate() {
    with_big_stack(|| {
        ...
        // property for ty is that the chain is populated.
        assert!(
        ...
        // Phase-1 provenance: one TrustIrPair per MachInst. The exact
        ...
        assert!(
            pairs.iter().any(|p| p.trust_ir_op == "Iadd_I32"),
            "expected at least one Iadd_I32 pair, got {:?}",
            pairs
                .iter()
                .map(|p| p.trust_ir_op.as_str())
                .collect::<Vec<_>>()
        );
...
/// certificate. Callers such as ty rely on this for cross-system
/// proof composition.
#[test]
fn export_proofs_emits_function_bundle() {
    with_big_stack(|| {
        ...
        let parsed: serde_json::Value =
            serde_json::from_str(&json).expect("export_proofs must emit valid JSON");
        assert!(parsed["functions"]["add"].is_object(), "json: {json}");

        assert!(
            parsed.get("lowering_certificates").is_none(),
            "partial JIT proof coverage must not emit lowering certificates: {json}"
        );
        assert!(
            parsed.get("trust_proof_certificates").is_none(),
            "partial JIT proof coverage must not emit trust-proof-cert certificates: {json}"
        );
    });
}
...
fn certificate_survives_execution() {
    with_big_stack(|| {
        ...
        assert!(!cert.trust_ir_pairs().is_empty());
    });
}
