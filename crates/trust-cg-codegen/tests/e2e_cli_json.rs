// trust-cg-codegen/tests/e2e_cli_json.rs - E2E test for the CLI trust_ir-to-binary JSON wire format
//
...
// ensuring the JSON wire format is a faithful representation of trust_ir modules.
//
// Part of #256 — E2E test for the CLI trust_ir-to-binary pipeline.
...
fn make_test_dir(test_name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("trust_cg_e2e_cli_json_{}", test_name));
...
fn e2e_json_wire_format_return_42() {
    ...
    // Step 1: Build a trust_ir module using the builder API.
    let mut mb = ModuleBuilder::new("cli_e2e_test");
    // Function names use leading underscore per Mach-O convention:
    // trust_ir name "_return_42" -> Mach-O symbol "__return_42" -> C sees "_return_42"
    ...
    let json_str = serde_json::to_string_pretty(&module_orig)
        .expect("serializing trust_ir module to JSON should succeed");

    eprintln!(
        "--- trust_ir JSON wire format ({} bytes) ---",
        json_str.len()
    );
    ...
    let module_rt = serde_json::from_str::<TrustIrModule>(&json_str)
        .expect("deserializing trust_ir module from JSON should succeed");
    assert_eq!(
    ...
    let result = compiler
        .compile(&module_rt)
        .expect("compiling JSON-deserialized trust_ir module should succeed");
    ...
    let module_from_file: TrustIrModule =
        serde_json::from_str(&json_from_file).expect("reading JSON from file should succeed");
...
// Trust Codegen emits Mach-O symbols with leading underscore: __return_42
...
"#;
    ...
    if !link_output.status.success() {
        ...
        panic!(
            "Linking failed.\nLinker stderr: {}\n\
             The Mach-O object produced from JSON-deserialized trust_ir is not linkable.",
            link_stderr
        );
    }
...
fn e2e_json_wire_format_add_args() {
    ...
    // JSON round-trip.
    let json_str =
        serde_json::to_string_pretty(&module_orig).expect("JSON serialization should succeed");
    let module_rt = serde_json::from_str::<TrustIrModule>(&json_str)
        .expect("JSON deserialization should succeed");
    assert_eq!(
        module_orig, module_rt,
        "JSON round-trip must preserve equality"
    );
...
fn e2e_json_file_roundtrip_compile() {
    // Build a simple trust_ir module.
    ...
    let json_path = test_dir.join("module.json");
    let json_str_file =
        serde_json::to_string_pretty(&module_orig).expect("serialize JSON should succeed");
    ...
    let module_from_file: TrustIrModule =
        serde_json::from_str(&json_from_file2).expect("read JSON from file should succeed");
    assert_eq!(
...
fn e2e_json_multi_function_link_run() {
    ...
    let module_rt: TrustIrModule = serde_json::from_str(&json_str).unwrap();
