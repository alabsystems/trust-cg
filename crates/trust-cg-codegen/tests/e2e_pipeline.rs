// trust-cg-codegen/tests/e2e_pipeline.rs - End-to-end compilation pipeline integration tests
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// Integration tests exercising the full compilation pipeline from trust_ir to Mach-O.
...
/// Compile a trust_ir function through the full pipeline (adapter -> ISel -> opt ->
...
fn build_empty_function() -> (TrustIrFunction, TrustIrModule) {
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "empty", ft_id, BlockId::new(0));
...
fn build_add_i32() -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("test");
    let ft_id = module.add_func_type(FuncTy {
...
fn build_branch_function() -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("test");
    let ft_id = module.add_func_type(FuncTy {
        params: vec![Ty::I64],
        returns: vec![Ty::I64],
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "abs_val", ft_id, BlockId::new(0));
    func.blocks = vec![
...
fn build_multi_arg_function() -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("test");
    let ft_id = module.add_func_type(FuncTy {
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "add4", ft_id, BlockId::new(0));
    func.blocks = vec![TrustIrBlock {
        id: BlockId::new(0),
        params: vec![
...
fn build_call_function() -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("test");
    let ft_id = module.add_func_type(FuncTy {
        params: vec![Ty::I64],
        returns: vec![Ty::I64],
    ...
    // Declare the callee `add_one` (FuncId 1) so the call site resolves. It is
    // a bodyless `Linkage::External` declaration (no blocks) — the trust_ir
    // shape for an external symbol resolved by the linker, which is exactly the
    // case this test exercises. A module that called an *unregistered* FuncId
    // would be malformed, and the adapter rightly fails it closed.
    let add_one = TrustIrFunction::new(FuncId::new(1), "add_one", ft_id, BlockId::new(0));
    module.add_function(add_one);

    let mut func = TrustIrFunction::new(FuncId::new(0), "call_test", ft_id, BlockId::new(0));
    func.blocks = vec![TrustIrBlock {
        id: BlockId::new(0),
        params: vec![(ValueId::new(0), Ty::I64)], // a
        body: vec![
            InstrNode::new(Inst::Call {
                callee: FuncId::new(1), // add_one
...
fn build_bitwise_function() -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("test");
    let ft_id = module.add_func_type(FuncTy {
        params: vec![Ty::I64, Ty::I64],
        returns: vec![Ty::I64],
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "bitwise", ft_id, BlockId::new(0));
    func.blocks = vec![TrustIrBlock {
        id: BlockId::new(0),
        params: vec![
            (ValueId::new(0), Ty::I64), // a
...
fn build_sub_function() -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("test");
    let ft_id = module.add_func_type(FuncTy {
        params: vec![Ty::I64, Ty::I64],
        returns: vec![Ty::I64],
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "sub_vals", ft_id, BlockId::new(0));
    func.blocks = vec![TrustIrBlock {
        id: BlockId::new(0),
        params: vec![(ValueId::new(0), Ty::I64), (ValueId::new(1), Ty::I64)],
        body: vec![
...
fn build_negation_function() -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("test");
    let ft_id = module.add_func_type(FuncTy {
        params: vec![Ty::I64],
        returns: vec![Ty::I64],
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "negate", ft_id, BlockId::new(0));
    func.blocks = vec![TrustIrBlock {
        id: BlockId::new(0),
        params: vec![(ValueId::new(0), Ty::I64)], // x
        body: vec![
...
fn build_shift_function() -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("test");
    let ft_id = module.add_func_type(FuncTy {
        params: vec![Ty::I64],
        returns: vec![Ty::I64],
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "shift_test", ft_id, BlockId::new(0));
    func.blocks = vec![TrustIrBlock {
        id: BlockId::new(0),
        params: vec![(ValueId::new(0), Ty::I64)], // x
        body: vec![
...
fn build_unsigned_div_function() -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("test");
    let ft_id = module.add_func_type(FuncTy {
        params: vec![Ty::I64, Ty::I64],
        returns: vec![Ty::I64],
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "udiv_test", ft_id, BlockId::new(0));
    func.blocks = vec![TrustIrBlock {
        id: BlockId::new(0),
        params: vec![
            (ValueId::new(0), Ty::I64), // a
...
fn build_mul_function() -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("test");
    let ft_id = module.add_func_type(FuncTy {
        params: vec![Ty::I64, Ty::I64],
        returns: vec![Ty::I64],
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "mul_vals", ft_id, BlockId::new(0));
    func.blocks = vec![TrustIrBlock {
        id: BlockId::new(0),
        params: vec![(ValueId::new(0), Ty::I64), (ValueId::new(1), Ty::I64)],
        body: vec![
...
fn test_empty_function_compiles() {
    let (trust_ir_func, module) = build_empty_function();

    let obj_bytes = compile_trust_ir(&trust_ir_func, &module, OptLevel::O0)
...
fn test_add_i32_function_compiles() {
    let (trust_ir_func, module) = build_add_i32();
...
fn test_branch_function_compiles() {
    let (trust_ir_func, module) = build_branch_function();
    ...
    let empty_obj = compile_trust_ir(&empty_func, &empty_module, OptLevel::O0)
...
fn test_multi_arg_function() {
    let (trust_ir_func, module) = build_multi_arg_function();
...
fn test_call_instruction() {
    let (trust_ir_func, module) = build_call_function();

    // Call instruction support may or may not be complete. The adapter should
    // at least translate the function successfully.
    let translate_result = trust_cg_lower::translate_function(&trust_ir_func, &module);
    assert!(
...
fn test_pipeline_o0_vs_o2() {
    ...
    let obj_o0 = compile_trust_ir(&trust_ir_func, &module, OptLevel::O0)
        .expect("sum_to_n at O0 should compile");
    let obj_o2 = compile_trust_ir(&trust_ir_func, &module, OptLevel::O2)
...
fn test_mach_o_output_structure() {
    ...
    let obj_bytes =
        compile_trust_ir(&trust_ir_func, &module, OptLevel::O0).expect("simple_add should compile");
    ...
    assert!(
        ncmds > 0,
        "must have at least one load command, got {}",
        ncmds
    );
...
fn test_bitwise_operations() {
    let (trust_ir_func, module) = build_bitwise_function();
...
fn test_sub_function_compiles() {
    let (trust_ir_func, module) = build_sub_function();
...
fn test_negation_function() {
    let (trust_ir_func, module) = build_negation_function();
...
fn test_multiple_opt_levels_all_valid() {
    ...
    for opt_level in &[OptLevel::O0, OptLevel::O1, OptLevel::O2, OptLevel::O3] {
        let obj_bytes = compile_trust_ir(&trust_ir_func, &module, *opt_level)
...
fn test_shift_operations() {
    let (trust_ir_func, module) = build_shift_function();
...
fn test_unsigned_division() {
    let (trust_ir_func, module) = build_unsigned_div_function();
...
fn test_mul_function_compiles() {
    let (trust_ir_func, module) = build_mul_function();
...
fn test_all_arithmetic_ops() {
    let functions: Vec<(&str, TrustIrFunction, TrustIrModule)> = vec![
        ...
        {
            let (f, m) = build_sub_function();
            ("sub_vals", f, m)
        },
        {
            let (f, m) = build_mul_function();
            ("mul_vals", f, m)
        },
        {
            let (f, m) = build_negation_function();
            ("negate", f, m)
        },
        {
            let (f, m) = build_bitwise_function();
            ("bitwise", f, m)
        },
        {
            let (f, m) = build_shift_function();
            ("shift_test", f, m)
        },
        {
            let (f, m) = build_unsigned_div_function();
            ("udiv_test", f, m)
        },
    ];
...
fn test_multi_block_at_multiple_opt_levels() {
    let functions: Vec<(&str, TrustIrFunction, TrustIrModule)> = vec![
        {
            let (f, m) = build_branch_function();
            ("abs_val", f, m)
        },
        {
            let (f, m) = build_loop_function();
            ("sum_to_n", f, m)
        },
    ];
...
fn test_consistent_cputype() {
    let functions: Vec<(&str, TrustIrFunction, TrustIrModule)> = vec![
        ...
        {
            let (f, m) = build_multi_arg_function();
            ("add4", f, m)
        },
    ];
...
fn test_empty_function_at_o2() {
    ...
    let obj_bytes = compile_trust_ir(&trust_ir_func, &module, OptLevel::O2)
