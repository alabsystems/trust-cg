// R10 — IS_DEF_EQ_BINDING: production binder-congruence OPENS INTO THE
// CONTEXT. The landed R9 slice (clean_ctx_whnf_slice.rs — context-aware
// whnf zeta + proof-irrelevance, verified native==JIT) carried one named
// residual: its def_eq compared Lam-vs-Lam / Pi-vs-Pi by RAW congruence —
// domains and bodies recursed with loose BVars, WITHOUT opening. Production
// is_def_eq_binding (tc/def_eq/binding.rs, the WHOLE file :12-64) instead:
//   * captures save_len = ctx_len() and the lhs binder discriminant, then
//     LOOPS over consecutive same-kind binders (a TELESCOPE — production
//     iterates, it does not recurse one binder at a time);
//   * per iteration: destructures lhs (binder-info IGNORED — the `_` at
//     :20) and rhs (bi2 KEPT, :26); runs the #3230 SYNTACTIC PRE-CHECK
//     `ty1 != ty2` (Expr::PartialEq — expr/mod.rs:363-375: ExprMeta word
//     compare (meta.rs:246-250) then the derived ExprKind eq, which DOES
//     compare BinderData and Let names) and only on pre-check failure the
//     full `is_def_eq_impl(ty1, ty2)` (:38); on domain failure:
//     ctx_truncate_to(save_len) + false (:39-40);
//   * the NO-LOOSE-BVARS FAST PATH (:43-47): if NEITHER body has loose
//     BVars (O(1) meta check — expr/subst.rs:628-631 over meta.rs:136-138
//     `loose_bvar_range() > 0`), the bodies are compared def_eq WITHOUT
//     opening, truncate, return;
//   * otherwise ONE fresh FVar is pushed — `ctx_push(Name::anon(),
//     ty2.clone(), bi2)` (:49 — the RHS domain and RHS binder-info, the
//     ANONYMOUS name: binder names/infos are irrelevant to def_eq) — and
//     BOTH bodies are opened with the SAME id (:50-51);
//   * if both opened bodies are again the ORIGINAL binder kind, the loop
//     continues (:52-58); else the opened bodies are compared
//     `is_def_eq_impl` and the context is batch-restored with
//     ctx_truncate_to(save_len) (:60-62 — LocalContext::truncate_to,
//     local_context.rs:236-242).
// CALL POSITIONS (production): quick_is_def_eq's (Lam,Lam)|(Pi,Pi) arm
// (tc/def_eq/mod.rs:499-502), consulted at is_def_eq_core ENTRY (:300-304)
// and again after Phase-1 whnf when a side changed (:350-356); AND Phase-6
// is_def_eq_structural (tc/def_eq/structural.rs:41-43) on the fully-reduced
// pair. The Some(result) SHORT-CIRCUITS is_def_eq_core — binding's verdict
// is FINAL for a binder-vs-binder pair in BOTH polarities; the proof-irrel
// consult (:360) NEVER sees a binder pair at top level (it gets its chance
// on the OPENED bodies inside the recursive is_def_eq_impl). Let is NOT
// handled by binding.rs — Let-vs-Let never reaches a congruence arm in
// production (Phase-1 whnf_core zeta-reduces let-heads; same in the pillar,
// whose whnf Let arm instantiates — the pillar Let congruence arm was and
// stays DEAD).
// THIS slice composes is_def_eq_binding into the R9 surface at the
// production position: def_eq's Lam/Pi arms now OPEN with shared fresh
// FVars on the SAME LocalContext (the R8 monotonic counter), so the R9
// context zeta and proof-irrelevance run UNDER binders — the new power.
//
// NEW SCENARIOS (they distinguish binding-aware from the R9-landed
// raw-congruence def_eq; the R9 configuration REJECTS both):
//   case 20 "def.birrel" — ACCEPT depends on PROOF IRRELEVANCE OVER THE
//     OPENED DOMAIN: value = λ(P:Sort 0). λ(C:Π(_:P).Sort 1). λ(hp:P).
//     ((λ (f : Π(h:P). Π(_:C hp). Sort 1). Sort 0)
//      (λ (h:P). λ (u:C h). Sort 0)).
//     The App-arm def_eq compares Π(h:P̂).Π(u:Ĉ #0).Sort 1 against
//     Π(h:P̂).Π(_:Ĉ ĥp̂).Sort 1: binding opens ĥ:P̂ (SHARED id), the
//     telescope CONTINUES, and the iter-2 domain compare Ĉ ĥ =?= Ĉ ĥp̂
//     descends to ĥ =?= ĥp̂ — equal ONLY by proof irrelevance, with ĥ's
//     type the CONTEXT LOOKUP of the FVar THAT BINDING PUSHED. The R9
//     config compares the raw bodies: #0 =?= ĥp̂ — the loose BVar cannot
//     be typed (quick → None catch-all; full → Err(UnboundVariable)) ⇒
//     proof irrel None ⇒ REJECT.
//   case 21 "def.bzeta" — ACCEPT depends on ZETA OF A LET PUSHED OUTSIDE
//     with the binder OPENED INSIDE (the ordering/telescope interaction):
//     value = λ(P:Sort 0). λ(C:Π(_:P).Sort 1). λ(hp:P).
//     let T : Sort 0 := P in
//     ((λ (f : Π(h:T). Π(_:C hp). Sort 1). Sort 0)
//      (λ (h:T). λ (u:C h). Sort 0)).
//     §7's Let arm pushes the VALUE-BEARING T̂ ↦ P̂ (OUTSIDE); binding then
//     opens ĥ:T̂ (INSIDE, iter-1) and the iter-2 proof-irrel consult types
//     ĥ as T̂ — whose Prop-ness is decided by whnf ZETA-reducing T̂ → P̂
//     through the context, and whose ty-def_eq T̂ =?= P̂ zeta-reduces the
//     same way. The R9 config again faces #0 =?= ĥp̂ untyped ⇒ REJECT.
//   case 22 "def.alpha" — THE ALPHA QUESTION: clean's Lam/Pi carry NO
//     binder names (only BinderData; names exist only in Let and in the
//     context's LocalDecl — binding pushes Name::anon()). Binder ATTRS
//     must not affect equality: declared type differs from the inferred
//     type in BinderData at BOTH Pi levels (info 1 / 3 vs the value's 0)
//     AND carries a beta-redex body; binding ignores the lhs bi entirely,
//     pushes bi2 (never read again), and ACCEPTS. Both configs accept
//     (raw congruence also ignores bi) — the alpha assertions live in the
//     probes: meta words of bi-differing Lam/Pi pairs are BIT-IDENTICAL
//     (production compute_meta ignores bi — kind.rs:583-584) and of
//     name-differing Let pairs BIT-IDENTICAL (mk_let_meta hashes no name —
//     kind.rs:585-586), while the production Expr::PartialEq pre-check
//     (which DOES see bi / Let names) rejects, and def_eq accepts.
//   THE BINDING-BLIND CONTROL: bnd_blind_root runs the SAME 23 cases
//   through clearly-marked *_blind copies of the R9-LANDED def_eq (raw
//   Lam/Pi congruence, NO binding, NO opening — but STILL context-aware:
//   whnf zeta + proof-irrel intact, = exactly the R9-landed configuration;
//   whnf itself is SHARED, its text is identical in both configurations).
//   Its verdict must AGREE on cases 0-19 (including the R9 divergence pair
//   18/19, which the R9 config accepts) and on 22, and DIVERGE (REJECT) on
//   EXACTLY {20, 21} — the falsifiability proof that OPENING INTO THE
//   CONTEXT is load-bearing.
//   probes (bnd_probe_root):
//     p0 binding TRUE via opening + irrel-under-binder on the case-20 pair
//        built directly; ctx len RESTORED (truncate_to), next_id advanced
//        by EXACTLY 1 (the telescope push — monotonic counter, truncate
//        does NOT reset it); the R9-config def_eq REJECTS the same pair.
//     p1 SHARED-vs-DISTINCT-FVAR CONTROL: on a DEPENDENT non-Prop pair
//        (body App(Ĉ2, #0), Ĉ2 : Π(_:Sort 0).Sort 1) production binding
//        (same id both sides) accepts; the distinct-id control variant
//        REJECTS (x̂a ≠ x̂b, Sort-0-typed — proof irrel cannot rescue) —
//        the SAME-FVar discipline is load-bearing and observable. On a
//        pair whose opened var is ERASED by reduction the control agrees.
//     p2 ALPHA: bi-differing Lam pair — meta words BIT-IDENTICAL,
//        expr_syntactic_eq FALSE (the production pre-check sees bi),
//        binding TRUE, full def_eq TRUE; name-differing Let pair — meta
//        words BIT-IDENTICAL, expr_syntactic_eq FALSE (Let names compared
//        with name_eq), def_eq TRUE (whnf zeta erases the name).
//     p3 POISONED-CONTEXT through BINDING: the case-21 inner pair with
//        T̂ ↦ P̂ (correct) is TRUE; a fresh context with T̂' ↦ Q̂' (WRONG
//        value, same shape) FLIPS to FALSE — identically native==JIT.
//     p4 REJECT-path CLEANUP: a pair failing at telescope depth 2 (Ĉ ĥ
//        vs D̂ ĥ) exits through the early truncate_to — ctx len RESTORED;
//        next_id advanced by exactly 2 (the ĥ push + the ONE infer-only
//        fallback push the proof-irrel consult makes on the shared ctx).
//     p5 the #3230 PRE-CHECK + FAST-PATH observables: both bodies closed
//        ⇒ NO opening (next_id UNCHANGED) and the body redex decided by
//        def_eq beta; a domain mismatch REJECTS without any push.
//     p6 pair-level divergence IN ONE MODULE: aware def_eq TRUE on the
//        case-21 pair, the R9-config def_eq FALSE on it — while the
//        R9-config def_eq is TRUE on the R9 zeta pair (Sort 0 vs let-FVar):
//        the control is context-aware, ONLY binding-blind.
//
// MODELED BOUNDARIES (the R9 list carried forward, plus R10 items):
...
//       resource-limit config, elided (as prior rungs).
//   B5. ExprKind is the 11-variant production core; SProp/Squash/Cubical*/
//       ZFC*/MData-map arms structurally absent; mode fixed Classical.
...
//       the R9 list + THIS round: `std::mem::discriminant(a.kind())` over
//       the {Lam, Pi} reachable domain (the :23/:29 unreachable!() arms) →
//       a `binder_is_lam` bool + matches! re-checks [B9-disc]; the :23/:29
//       unreachable!() arms → benign `return false` (dead on the
//       call-position domain — the prior-rung convention for dead panics);
//       production `Expr::PartialEq` (derived ExprKind eq) → the explicit
//       expr_syntactic_eq recursion with fieldwise BinderData compare and
//       index-loop Vec<Level> compare (element equality = the slice Level
//       PartialEq, the cfg(kani) body clean's own soundness harness checks
//       against the derived one; Name equality = name_eq = production
//       Name::eq); Arc field destructures hold Arc clones instead of &refs
//       (Arc::clone is inlined; values identical).
//   B10. delta-unfold ignores Const levels (env values universe-monomorphic).
//   B11. Lit-rule type names built per-call from literal parts (as prior).
//   [C-refcell] production reaches ctx through TypeChecker.ctx:
//       RefCell<LocalContext> short-lived borrows; transcribed as &/&mut
//       threading. binding's ctx_len/ctx_push/ctx_truncate_to are the
//       config.rs:41-74 one-line borrow wrappers — threaded directly.
//   [C-idx] index_by_id/used_ids HashMap/HashSet → backward scan / Vec
//       (R8, unchanged); truncate_to's index-map removal elided the same
//       way (the backward scan never sees popped entries).
//   [C-guard] freshness assert! conditions → guard_trips counter (R8).
//   [C-cache2] tc caches (whnf/whnf_core/def_eq/quick_infer/equiv_manager)
//       elided — pure memoization; routing verified in e2e_tc_stitching.rs.
//       NOTE the def-eq CACHE-KEY layer (DefEqCacheKey, the #1773
//       all_fvars_in_context negative-cache guard, equiv_manager add_equiv)
//       is part of this elision — cache-layer, not verdict logic.
//   [C-pillar] the composition point is the R6-inherited cert-pillar
//       whnf/def_eq (full-delta whnf up front), NOT tc's 8-phase
//       is_def_eq_core: binding is dispatched ONCE on the full-whnf'd pair
//       at the congruence position — production consults it at :300
//       (pre-whnf), :350-356 (post-Phase-1) and Phase-6 structural.rs:41;
//       whnf is the identity on Lam/Pi heads, so the pillar's single
//       dispatch covers all three sites. Production binding's domain/body
//       compares recurse into is_def_eq_impl (cache + 8 phases); the
//       pillar's recurse into the pillar def_eq (strictly-more-reduced
//       whnf, same def-eq relation). Bool.true reflection / branch-sharing
//       / lazy-delta / string-lit / unit-like phases remain elided.
//   [C-inferonly] the proof-irrel FULL fallback is production
//       infer_type_infer_only (R9, unchanged).
//   [C-proj-quick] quick Proj arm → None (R9, unchanged).
...
//   THE *_blind FAMILY IS NOT A TRANSCRIPTION: it is the R9-LANDED def_eq
//   configuration kept VERBATIM as the divergence CONTROL (raw binder
//   congruence, proof-irrel consult, shared context-aware whnf). The
//   is_def_eq_binding_distinct_control probe variant is likewise NOT a
//   transcription — it is the p1 falsification arm (distinct fresh ids).
//
// SOURCES (verbatim transcription targets in $HOME/clean/crates/clean-kernel/src):
//   tc/def_eq/binding.rs      — is_def_eq_binding, THE WHOLE FILE (:12-64).
//   tc/def_eq/mod.rs          — the call position: quick_is_def_eq
//                               (Lam,Lam)|(Pi,Pi) → Some(binding) (:499-502),
//                               consulted at :300-304 / :350-356; the
//                               proof-irrel position (:358-367, R9).
//   tc/def_eq/structural.rs   — the Phase-6 binder route (:41-43).
//   tc/local_context.rs       — truncate_to (:236-242); push/push_let/pop/
//                               get/len (R8).
//   tc/config.rs              — ctx_len/ctx_truncate_to wrappers (:62-74).
//   expr/mod.rs               — Expr::PartialEq (:363-375).
//   expr/meta.rs              — ExprMeta::PartialEq (:246-250),
//                               has_loose_bvars (:136-138).
//   expr/subst.rs             — Expr::has_loose_bvars (:628-631).
//   expr/kind.rs              — compute_meta Lam/Pi/Let arms ignore
//                               bi / Let-name (:583-586) — the alpha fact.
//   (R9 sources for everything unchanged — see the landed
//   clean_ctx_whnf_slice.rs header: tc/whnf.rs:455-461, tc/def_eq/
//   proof_irrel.rs, tc/infer.rs, tc/eta.rs:197, expr/subst.rs,
//   env/decl_add.rs.)
//
// Crate name is load-bearing (appears in the mangled extern-leaf symbols the
// JIT binds): it MUST stay `clean_binding_defeq_slice`.
//
...
//     ../../trust-cg/crates/trust-cg-codegen/tests/slices/clean_binding_defeq_slice.rs \
//     --crate-type=lib --mir-emit-closure <root> <out.tir>
//   roots: bnd_gate_root | bnd_blind_root | bnd_probe_root
...
impl<'env> Verifier<'env> {
    ...
    fn def_eq_inner(&self, a: &Expr, b: &Expr, ctx: &mut LocalContext) -> bool {
        ...
        // ── R10 NEW — THE PRODUCTION CALL POSITION (tc/def_eq/mod.rs:499-502:
        // quick_is_def_eq's (Lam,Lam)|(Pi,Pi) arm → Some(is_def_eq_binding),
        // consulted at is_def_eq_core ENTRY :300-304; Phase-6 structural.rs:41
        // routes the fully-reduced pair the same way): a binder-vs-binder
        // pair dispatches to is_def_eq_binding and the Some(result)
        // SHORT-CIRCUITS — the verdict is FINAL in BOTH polarities. The
        // proof-irrel consult below therefore never sees a binder pair
        // (production parity: quick_is_def_eq runs BEFORE the :360 proof
        // irrel; proof irrelevance gets its chance on the OPENED bodies
        // inside the recursive def_eq). [C-pillar]: dispatched ONCE on the
        // full-whnf'd pair — whnf is the identity on Lam/Pi heads, so this
        // single position covers production's :300 / :350-356 / Phase-6
        // sites. ──
        match (&a_whnf.kind, &b_whnf.kind) {
            (ExprKind::Lam(..), ExprKind::Lam(..))
            | (ExprKind::Pi(..), ExprKind::Pi(..)) => {
                return self.is_def_eq_binding(&a_whnf, &b_whnf, ctx);
            }
            _ => {}
        }
        // ── R9 — tc/def_eq/mod.rs:358-367 VERBATIM at the production
        ...
        let matched = match (&a_whnf.kind, &b_whnf.kind) {
            ...
            // R10: the raw Lam/Pi congruence arms are GONE — binder pairs
            // return through is_def_eq_binding above (production has no
            // Lam/Pi arm at this position either; its Phase-6 structural
            // routes them back to binding, structural.rs:41-43). The Let
            // arm below was and stays DEAD (whnf zeta-reduces let-heads).
    ...
    // ════════════════════════════════════════════════════════════════════
    // R10 NEW — THE ROUND'S TRANSCRIPTION TARGET: is_def_eq_binding
    // (tc/def_eq/binding.rs:12-64, the whole file). "Compare two binding
    // expressions by iteratively processing consecutive same-kind binders."
    // [C-refcell]: ctx_len/ctx_push/ctx_truncate_to (config.rs:41-74
    // one-line RefCell wrappers) → direct &mut LocalContext calls.
    // [B9-disc]: `std::mem::discriminant(a.kind())` captured at :14 and
    // compared at :52-53 — the reachable domain is EXACTLY {Lam, Pi} (the
    // :23/:29 unreachable!() arms guard it) — transcribed as a
    // `binder_is_lam` bool + matches! re-checks. The unreachable!() arms
    // themselves → benign `return false`, dead on the call-position domain
    // (the prior-rung convention for dead panics). Destructured Arc fields
    // are held as Arc clones instead of &refs [B9]. `is_def_eq_impl` is
    // the pillar def_eq_impl ([C-pillar]).
    // ════════════════════════════════════════════════════════════════════
    fn is_def_eq_binding(&self, a: &Expr, b: &Expr, ctx: &mut LocalContext) -> bool {
        // :13 — save the context depth for the batch restore.
        let save_len = ctx.len();
        // :14 — the lhs binder discriminant pins the TELESCOPE kind.
        ...
        loop {
            // :19-24 — lhs destructure; binder-info IGNORED (production `_`).
            let (ty1, body1): (Arc<Expr>, Arc<Expr>) = match &a.kind {
                ExprKind::Lam(_, ty, body) | ExprKind::Pi(_, ty, body) => {
                    (ty.clone(), body.clone())
                }
                // production: unreachable!("is_def_eq_binding: lhs is not Lam/Pi")
                _ => return false,
            };
            // :25-30 — rhs destructure; bi2 KEPT (feeds the ctx_push).
            let (bi2, ty2, body2): (BinderData, Arc<Expr>, Arc<Expr>) = match &b.kind {
                ...
                // production: unreachable!("is_def_eq_binding: rhs is not Lam/Pi")
                _ => return false,
            };

            // :32-41 — the #3230 SYNTACTIC PRE-CHECK: skip the full def_eq
            // when the domains are structurally identical (production
            // `ty1 != ty2` is Expr::PartialEq — expr_syntactic_eq here);
            // otherwise full def_eq; on failure truncate + false.
            if !expr_syntactic_eq(&ty1, &ty2) && !self.def_eq_impl(&ty1, &ty2, ctx) {
                ctx.truncate_to(save_len);
                return false;
            }

            // :43-47 — the NO-LOOSE-BVARS FAST PATH: neither body mentions
            // the binder ⇒ compare WITHOUT opening (no push at all).
            ...
            // :49-51 — push ONE fresh FVar (ANON name, RHS domain, RHS
            // binder-info) and open BOTH bodies with the SAME id.
            ...
            // :52-58 — the TELESCOPE: both opened bodies again the original
            // binder kind ⇒ iterate (same save_len — ONE batch restore).
            ...
            // :60-62 — compare the opened bodies, batch-restore, return.
    ...
    // ════════════════════════════════════════════════════════════════════
    // NOT A TRANSCRIPTION — the p1 SHARED-vs-DISTINCT-FVAR falsification
    // control: byte-for-byte is_def_eq_binding EXCEPT the two bodies are
    // opened with TWO DIFFERENT fresh FVars. On a body that USES the
    // opened variable (and is not rescued by proof irrelevance) this MUST
    // reject where production accepts — proving the same-id discipline at
    // binding.rs:49-51 is load-bearing and observable in machine code.
    // ════════════════════════════════════════════════════════════════════
    fn is_def_eq_binding_distinct_control(
        &self,
        a: &Expr,
        b: &Expr,
        ...
        loop {
            ...
            // THE ARMED DIFFERENCE: two DISTINCT fresh ids.
            let id_a = ctx.push(name_anon(), ty2.as_ref().clone(), bi2);
            let id_b = ctx.push(name_anon(), ty2.as_ref().clone(), bi2);
            let a_next = self.open_bvar(&body1, id_a);
            let b_next = self.open_bvar(&body2, id_b);
            let a_same = if binder_is_lam {
                matches!(&a_next.kind, ExprKind::Lam(_, _, _))
            } else {
                matches!(&a_next.kind, ExprKind::Pi(_, _, _))
...
// ════════════════════════════════════════════════════════════════════════════
// BINDING-BLIND CONTROL — NOT A PRODUCTION TRANSCRIPTION. This is the
// R9-LANDED def_eq configuration kept VERBATIM as the divergence CONTROL:
// raw Lam/Pi congruence (domains AND bodies recursed with loose BVars, NO
// opening, NO is_def_eq_binding), while everything R9 landed stays intact —
// context-aware whnf (SHARED: whnf_impl's text is identical in both
// configurations this round) and the proof-irrelevance consult (its final
// ty-def_eq routed to the blind def_eq, as the R9 configuration's was).
// Inference is the R8/R9 discipline routed to the blind def_eq.
// bnd_blind_root runs the same 23 decl cases through this chain: it must
// AGREE with bnd_gate_root on cases 0-19 (including the R9 pair 18/19 —
// this control ACCEPTS them: it is context-aware, only BINDING-blind) and
// on 22, and REJECT exactly {20, 21} — the falsifiability proof that
// OPENING INTO THE CONTEXT is load-bearing and observable.
// ════════════════════════════════════════════════════════════════════════════
...
impl<'env> Verifier<'env> {
    ...
    // The R9-landed def_eq text VERBATIM (context-aware whnf + proof-irrel
    // consult + RAW binder congruence — no binding, no opening).
    fn is_def_eq_blind(&self, a: &Expr, b: &Expr, ctx: &mut LocalContext) -> bool {
        self.def_eq_blind_inner(a, b, ctx)
    }
    fn def_eq_blind_impl(&self, a: &Expr, b: &Expr, ctx: &mut LocalContext) -> bool {
        self.def_eq_blind_inner(a, b, ctx)
    }
    fn def_eq_blind_inner(&self, a: &Expr, b: &Expr, ctx: &mut LocalContext) -> bool {
        ...
        let matched = match (&a_whnf.kind, &b_whnf.kind) {
            ...
            // THE R9-LANDED RAW CONGRUENCE ARMS — the control's whole point:
            // bodies compared with their loose BVars, WITHOUT opening.
            (ExprKind::Lam(_b1, ty1, b1), ExprKind::Lam(_b2, ty2, b2))
            | (ExprKind::Pi(_b1, ty1, b1), ExprKind::Pi(_b2, ty2, b2)) => self.def_eq_blind_impl(ty1, ty2, ctx) && self.def_eq_blind_impl(b1, b2, ctx),
    ...
    /// The R9 proof-irrel entry with its final ty-def_eq routed to the blind
    /// def_eq. The helper chain below is duplicated as _blind copies so the
    /// blind CLOSURE is fully self-contained (its Const arms route to
    /// const_type_blind — the B1 env model must not smuggle the aware
    /// def_eq/binding bodies into the control module; same discipline as
    /// the R9 blind fixture). type_is_quickly_not_in_prop is pure (no
    /// pillar calls) and stays shared; whnf is the shared context-aware
    /// pillar (it never reaches def_eq or infer).
    ...
    // The R8/R9 infer/gate chain with def_eq routed to the blind copy
    // (whnf is the SHARED context-aware pillar; inference discipline —
    // LocalContext, open→infer→close, FVar/BVar arms — byte-for-byte the
    // shared text).
    ...
    /// The R8/R9 gate text with §5/§7 routed to the blind chain (§2-§4/§6
...
pub extern "C" fn bnd_probe_root(
    ...
    if idx == 0 {
        ...
        let mut ctx = LocalContext::new();
        let c2 = ctx.push(nm1("C2"), Expr::pi(bdm(), Expr::sort0(), sort1.clone()), bdm());
...
fn main() {
    ...
    // Aware gate: 13 accepts / 10 rejects over the 23 cases.
    ...
    for case in 0u64..23 {
        ...
        let res = unsafe {
            bnd_gate_root(res_slot.as_mut_ptr(), ty_slot.as_mut_ptr(), case);
    ...
    ok = ok && accepts == 13 && rejects == 10;
    // The R9 verdict pattern must reproduce on 0..19.
    let r9_accepts = [0usize, 4, 6, 7, 11, 12, 16, 17, 18, 19];
    for case in 0..20usize {
        if aware_verdicts[case] != r9_accepts.contains(&case) {
            println!("case {case}: R9 verdict NOT reproduced");
            ok = false;
        }
    }
    for case in 20..23usize {
        if !aware_verdicts[case] {
            println!("case {case}: R10 case must ACCEPT");
            ok = false;
        }
    }

    // Blind gate (R9-landed config): 11 accepts / 12 rejects; divergence
    // EXACTLY {20, 21}.
    ...
    for case in 0u64..23 {
        ...
        let mut ty_slot = std::mem::MaybeUninit::<Expr>::uninit();
        let res = unsafe {
            bnd_blind_root(res_slot.as_mut_ptr(), ty_slot.as_mut_ptr(), case);
            let _ty = ty_slot.assume_init();
            res_slot.assume_init()
        };
    ...
    for idx in 0u64..7 {
        ...
        let (res, ids) = unsafe {
            bnd_probe_root(res_slot.as_mut_ptr(), ids_slot.as_mut_ptr(), idx);
        ...
        let good = match idx {
            0 => res.is_ok() && ids.flags == 15 && ids.guard_trips == 0,
            1 => res.is_ok() && ids.flags == 7 && ids.guard_trips == 0,
            2 => res.is_ok() && ids.flags == 127 && ids.guard_trips == 0,
            3 => res.is_ok() && ids.flags == 3 && ids.guard_trips == 0,
            4 => res.is_ok() && ids.flags == 7 && ids.id1 == ids.id0 + 2,
            5 => res.is_ok() && ids.flags == 7 && ids.id1 == ids.id0,
