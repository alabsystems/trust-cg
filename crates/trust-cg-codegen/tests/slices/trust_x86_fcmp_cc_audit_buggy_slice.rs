// !!! FAIL-LOUD CONTROL ARTIFACT — NOT the production table !!!
// This is `trust_x86_fcmp_cc_audit_slice.rs` with EXACTLY ONE deliberate
// mutation: `FloatCC::Equal` is lowered as bare `SingleCC(E)` instead of the
// production `AndNotParity(E)` — i.e. the parity (SETNP) guard is DROPPED. This
// is the x86 analog of the aarch64 owner-#10 bug (a bare CC with no NaN guard).
// On a NaN (UCOMISD sets ZF=1) bare `SETE` returns TRUE, but IEEE ordered-equal
// must be FALSE. The round-33 audit test JITs THIS module and asserts the
// Equal/NaN cell miscompiles (proving the harness + IEEE oracle would catch a
// real x86 fcmp table bug). Root renamed to `x86_fcmp_buggy_root`.
// Regen: derive from the audit slice by the one-line mutation above, then
//   trust_ir_mir --mir-emit-closure x86_fcmp_buggy_root.
//
...
//     [+OR SETP].)
//
// EMIT: stage1 `trust_ir_mir --mir-emit-closure x86_fcmp_buggy_root`;
...
pub fn x86_float_cmp_strategy(cc: FloatCC) -> X86FloatCmpStrategy {
    match cc {
        ...
        FloatCC::Equal => X86FloatCmpStrategy::SingleCC(X86CondCode::E), // MUTATED: dropped parity guard
...
#[no_mangle]
pub fn x86_fcmp_buggy_root(floatcc_tag: u32, scenario: u32, out: &mut FcmpAuditProps) {
