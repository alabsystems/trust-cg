...
impl<'env> Verifier<'env> {
    ...
    // ── §5 infer_sort — infer_type(e) then whnf -> require Sort (Pi-arm recurses imax). ──
    ...
    // ── §7 check_type — is_def_eq(infer_type(value), expected). ──
    ...
    // ── THE GATE: check_decl_readonly — §5 infer_sort + §6 theorem-is-Prop + §7 check_type. ──
    fn check_decl_readonly(&self, decl: &Declaration) -> Result<(), DeclError> {
        let (type_, opt_value, is_theorem): (&Expr, Option<&Expr>, bool) = match decl {
            Declaration::Definition(_n, type_, value) => (type_, Some(value), false),
            Declaration::Axiom(_n, type_) => (type_, None, false),
            Declaration::Theorem(_n, type_, value) => (type_, Some(value), true),
        };
        let sort = match self.infer_sort(type_) {
            Ok(s) => s,
            Err(e) => return Err(DeclError::TypeCheckFailedType(e)),
        };
        if is_theorem && !Level::is_zero(&sort) {
            return Err(DeclError::TheoremTypeNotProp);
        }
        if let Some(value) = opt_value {
            match self.check_type(value, type_) {
                Ok(()) => {}
                Err(e) => return Err(DeclError::TypeCheckFailedValue(e)),
            }
        }
        Ok(())
    }
...
pub enum Declaration {
    Definition(Name, Arc<Expr>, Arc<Expr>),
    Axiom(Name, Arc<Expr>),
    Theorem(Name, Arc<Expr>, Arc<Expr>),
}

#[derive(Clone, Debug)]
pub enum DeclError {
    TypeCheckFailedType(TypeError),
    TheoremTypeNotProp,
    TypeCheckFailedValue(TypeError),
}
...
// exported symbols; the lifetime-generic Verifier methods need a concrete
...
#[no_mangle]
pub extern "C" fn __root_check_decl(v: &Verifier, decl: &Declaration) -> u64 {
    match v.check_decl_readonly(decl) {
        Ok(()) => 1,
        Err(_) => 0,
    }
}
