// trust-cg-codegen/tests/e2e_riscv_elf.rs - E2E RISC-V ELF linking integration test
//
...
use trust_cg_codegen::riscv::{
    RiscVISelFunction, RiscVISelInst, RiscVISelOperand, RiscVPipeline, RiscVPipelineConfig,
    build_riscv_add_test_function, build_riscv_const_test_function, riscv_compile_to_bytes,
    riscv_compile_to_elf,
};
use trust_cg_ir::regs::{RegClass, VReg};
use trust_cg_ir::riscv_ops::RiscVOpcode;
use trust_cg_ir::riscv_regs::{A0, A1, A2, RA, ZERO};
...
fn make_test_dir(test_name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("trust_cg_e2e_riscv_{}", test_name));
...
fn verify_riscv_elf_header(bytes: &[u8]) {
    ...
    assert_eq!(
        e_machine, EM_RISCV,
        "must be RISC-V (EM_RISCV = 0xF3 = 243)"
    );
...
fn test_riscv_e2e_const42_elf_structure() {
    ...
    let elf_bytes =
        riscv_compile_to_elf(&func).expect("RISC-V pipeline should compile const42 to ELF");
    ...
    assert!(e_shnum >= 5, "need at least 5 sections, got {}", e_shnum);
...
fn test_riscv_e2e_add_elf_structure() {
    ...
    let elf_bytes = riscv_compile_to_elf(&func).expect("RISC-V pipeline should compile add to ELF");
...
fn test_riscv_e2e_raw_bytes_validation() {
    ...
    let bytes1 =
        riscv_compile_to_bytes(&func1).expect("RISC-V pipeline should compile const42 to bytes");
    ...
    assert!(
        ...
        "const42 must end with RET, got 0x{:08X}",
        last_inst1
    );
    ...
    let bytes2 =
        riscv_compile_to_bytes(&func2).expect("RISC-V pipeline should compile add to bytes");
    ...
    assert!(
        ...
        "add must end with RET, got 0x{:08X}",
        last_inst2
    );
}
...
fn test_riscv_e2e_elf_contains_raw_code() {
    ...
    let raw_bytes = riscv_compile_to_bytes(&func).expect("raw bytes compilation should succeed");

    let elf_bytes = riscv_compile_to_elf(&func).expect("ELF compilation should succeed");

    let text = find_elf_section(&elf_bytes, ".text").expect(".text section must exist");
...
fn test_riscv_e2e_pipeline_configs() {
    ...
    let bytes1 = pipeline1
        .compile_function(&func)
        .expect("no-elf no-frame should work");
    ...
    assert!(
        is_riscv_ret(last1),
        "no-frame: last inst must be RET, got 0x{:08X}",
        last1
    );
    ...
    let bytes2 = pipeline2
        .compile_function(&func)
        .expect("no-elf with-frame should work");
    ...
    let bytes3 = pipeline3
        .compile_function(&func)
        .expect("elf with-frame should work");
    ...
    let bytes4 = pipeline4
        .compile_function(&func)
        .expect("elf no-frame should work");
...
fn test_riscv_e2e_frame_lowering_patterns() {
    ...
    assert!(
        code.len() >= 16,
        "framed code must be at least 4 instructions"
    );

    let num_insts = code.len() / 4;
    let insts: Vec<u32> = (0..num_insts)
        .map(|i| decode_riscv_inst(&code, i * 4))
        .collect();
    ...
    assert_eq!(
        riscv_opcode(first),
        OP_IMM,
        "first inst must be ADDI (opcode=0010011)"
    );
    ...
    assert!(
        sp_adjust < 0,
        "prologue must subtract from SP, got adjustment {}",
        sp_adjust
    );
    assert_eq!(
        sp_adjust % 16,
        0,
        "stack adjustment {} must be 16-byte aligned",
        sp_adjust
    );
    ...
    assert!(
        has_ld_s0,
        "epilogue must restore S0/FP with LD s0, offset(sp)"
    );

    // Last instruction must be RET.
    assert!(
        is_riscv_ret(*insts.last().unwrap()),
        "code must end with RET, got 0x{:08X}",
        insts.last().unwrap()
    );
}
...
fn test_riscv_e2e_const42_encoding_correctness() {
    ...
    assert_eq!(
        code.len(),
        8,
        "const42 no-frame should be 8 bytes (2 instructions)"
    );
    ...
    assert_eq!(
        inst0, 0x02A00513,
        "inst 0 should be ADDI a0, zero, 42 (0x02A00513), got 0x{:08X}",
        inst0
    );
    ...
    assert_eq!(
        inst1, 0x00008067,
        "inst 1 should be JALR x0, ra, 0 (RET = 0x00008067), got 0x{:08X}",
        inst1
    );
}
...
fn test_riscv_e2e_add_encoding_correctness() {
    ...
    assert_eq!(
        code.len(),
        20,
        "add no-frame should be 20 bytes (5 instructions)"
    );
    ...
    assert_eq!(
        riscv_funct3(insts[0]),
        0b000,
        "inst 0 must have funct3=000 (ADDI)"
    );
    ...
    assert_eq!(
        riscv_funct3(insts[2]),
        0b000,
        "inst 2 must have funct3=000 (ADD)"
    );
    assert_eq!(
        riscv_funct7(insts[2]),
        0b0000000,
        "inst 2 must have funct7=0 (ADD, not SUB)"
    );
    ...
    assert!(
        is_riscv_ret(insts[4]),
        "inst 4 must be RET (0x00008067), got 0x{:08X}",
        insts[4]
    );
}
...
fn test_riscv_e2e_multi_block_branch() {
    ...
    let code = pipeline
        .compile_function(&func)
        .expect("multi-block compilation should succeed");
    ...
    assert_eq!(
        riscv_opcode(insts[0]),
        BRANCH,
        "first inst must be B-type branch"
    );
...
fn test_riscv_e2e_load_store_encoding() {
    ...
    let code = pipeline
        .compile_function(&func)
        .expect("load/store compilation should succeed");
    assert_eq!(
        code.len(),
        16,
        "load_store no-frame should be 16 bytes (4 instructions)"
    );
...
fn test_riscv_e2e_word_operations() {
    ...
    let code = pipeline
        .compile_function(&func)
        .expect("word op compilation should succeed");
    ...
    assert_eq!(
        riscv_opcode(inst0),
        op_32,
        "ADDW must have opcode=OP_32 (0b0111011)"
    );
...
fn test_riscv_e2e_m_extension_encoding() {
    ...
    let code = pipeline
        .compile_function(&func)
        .expect("mul compilation should succeed");
    ...
    assert_eq!(
        riscv_funct7(inst0),
        0b0000001,
        "MUL has funct7=0000001 (M extension)"
    );
...
fn test_riscv_e2e_lui_jal_encoding() {
    ...
    func.push_inst(
        ...
        RiscVISelInst::new(
            RiscVOpcode::Lui,
            vec![RiscVISelOperand::PReg(A0), RiscVISelOperand::Imm(0x12345)],
        ),
    );
    ...
    let code = pipeline
        .compile_function(&func)
        .expect("LUI+ADDI compilation should succeed");
    ...
    assert_eq!(
        lui_imm, 0x12345,
        "LUI immediate must be 0x12345, got 0x{:05X}",
        lui_imm
    );
...
fn test_riscv_e2e_regalloc_distinct_assignments() {
    ...
    let code = pipeline
        .compile_function(&func)
        .expect("sum3 compilation should succeed");
...
fn test_riscv_e2e_all_branch_types() {
    ...
    for (opcode, expected_funct3) in &branch_variants {
        ...
        let mut func = RiscVISelFunction::new(format!("branch_{:?}", opcode).to_lowercase(), sig);
        let b0 = Block(0);
        let b1 = Block(1);
        func.ensure_block(b0);
        func.ensure_block(b1);
        ...
        assert_eq!(
            code.len() % 4,
            0,
            "{:?}: code must be 4-byte aligned",
            opcode
        );

        let first_inst = decode_riscv_inst(&code, 0);
        assert_eq!(
            riscv_opcode(first_inst),
            BRANCH,
            "{:?}: first inst must be B-type (opcode=1100011)",
            opcode
        );
        assert_eq!(
            riscv_funct3(first_inst),
            *expected_funct3,
        ...
        assert_eq!(
            riscv_rs1(first_inst),
            10,
            "{:?}: rs1 must be a0 (x10)",
            opcode
        );
        assert_eq!(
            riscv_rs2(first_inst),
            11,
            "{:?}: rs2 must be a1 (x11)",
            opcode
        );
...
fn test_riscv_e2e_all_memory_widths() {
    ...
    for (load_op, load_f3, store_op, store_f3, width_name) in &widths {
        ...
        let mut func = RiscVISelFunction::new(format!("mem_{}", width_name), sig);
        let entry = Block(0);
        func.ensure_block(entry);
        ...
        assert_eq!(
            code.len(),
            16,
            "{}: expected 16 bytes (4 instructions)",
            width_name
        );
        ...
        assert_eq!(
            riscv_funct3(insts[0]),
            *load_f3,
            "{}: load funct3 mismatch",
            width_name
        );

        // Store
        assert_eq!(
            riscv_opcode(insts[1]),
            STORE,
            "{}: inst 1 must be STORE",
            width_name
        );
        assert_eq!(
            riscv_funct3(insts[1]),
            *store_f3,
            "{}: store funct3 mismatch",
            width_name
        );
    }
...
fn test_riscv_e2e_sub_encoding() {
    ...
    assert_eq!(
        riscv_funct7(inst0),
        0b0100000,
        "SUB has funct7=0100000 (bit 30 set)"
    );
...
fn test_riscv_e2e_auipc_encoding() {
    ...
    func.push_inst(
        ...
        RiscVISelInst::new(
            RiscVOpcode::Auipc,
            vec![RiscVISelOperand::PReg(A0), RiscVISelOperand::Imm(0xABCDE)],
        ),
    );
    ...
    let code = pipeline
        .compile_function(&func)
        .expect("auipc compilation should succeed");
...
fn test_riscv_e2e_div_rem_encoding() {
    ...
    for (opcode, expected_f3, name) in &div_ops {
        ...
        assert_eq!(
            riscv_funct7(inst0),
            0b0000001,
            "{}: must have M-extension funct7=0000001",
            name
        );
        assert_eq!(riscv_rd(inst0), 10, "{}: rd must be a0", name);
        assert_eq!(riscv_rs1(inst0), 10, "{}: rs1 must be a0", name);
...
fn test_riscv_e2e_jal_call_pattern() {
    ...
    func.push_inst(
        ...
        RiscVISelInst::new(
            RiscVOpcode::Jal,
            vec![RiscVISelOperand::PReg(RA), RiscVISelOperand::Block(b1)],
        ),
    );
    ...
    let code = pipeline
        .compile_function(&func)
        .expect("call pattern compilation should succeed");
...
fn test_riscv_e2e_elf_section_properties() {
    let func = build_riscv_const_test_function();
    let elf_bytes = riscv_compile_to_elf(&func).expect("RISC-V pipeline should compile to ELF");
    ...
    for i in 0..e_shnum {
        ...
        match name {
            ".text" => {
                ...
                assert!(sh_flags & SHF_ALLOC != 0, ".text must have SHF_ALLOC flag");
                assert!(
...
fn test_riscv_e2e_elf_symbol_details() {
    ...
    for i in 0..num_syms {
        ...
        if name == "add" {
            ...
            assert_eq!(
                sym_type, STT_FUNC,
                "'add' symbol must have type STT_FUNC (2), got {}",
                sym_type
            );
            assert_eq!(
                sym_bind, STB_GLOBAL,
                "'add' symbol must have binding STB_GLOBAL (1), got {}",
                sym_bind
            );
            assert!(
                st_size > 0,
                "'add' symbol must have non-zero size, got {}",
                st_size
            );
        }
    }
...
fn test_riscv_e2e_subw_encoding() {
    ...
    let code = pipeline
        .compile_function(&func)
        .expect("subw compilation should succeed");
...
fn test_riscv_e2e_negative_immediate() {
    ...
    let code = pipeline
        .compile_function(&func)
        .expect("neg_imm compilation should succeed");
...
fn test_riscv_e2e_backward_branch_offset() {
    ...
    assert_eq!(
        riscv_opcode(insts[2]),
        BRANCH,
        "inst 2 must be BNE (B-type)"
    );
