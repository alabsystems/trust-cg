// trust-cg-codegen/tests/pipeline_integration.rs - End-to-end pipeline tests
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0

//! Integration tests for the Trust Codegen compilation pipeline.
//!
...
use trust_cg_lower::function::{
    BasicBlock, Function as LirFunction, Signature as LirSignature, StackSlotInfo,
};
...
fn test_encode_add_function() {
    ...
    assert_eq!(
        code.len(),
        8,
        "Expected 2 instructions (8 bytes), got {}",
        code.len()
    );
    ...
    assert_eq!(
        add_word, 0x8B010000,
        "ADD X0, X0, X1 should be 0x8B010000, got 0x{:08X}",
        add_word
    );
    ...
    assert_eq!(
        ret_word, 0xD65F03C0,
        "RET should be 0xD65F03C0, got 0x{:08X}",
        ret_word
    );
}
...
fn test_pipeline_add_function_to_macho() {
    ...
    assert!(
        is_valid_macho(&obj_bytes),
        "Output should be a valid Mach-O file"
    );
    ...
    assert_eq!(
        ...
        1,
        "Should be MH_OBJECT (type 1)"
    );

    // Should be large enough to contain header + load commands + code.
    assert!(
        obj_bytes.len() > 100,
        "Object file should be non-trivial ({} bytes)",
        obj_bytes.len()
    );
}
...
fn test_pipeline_add_function_write_and_verify() {
    ...
    let obj_path = tmp_dir.join("trust_cg_test_add.o");
        ...
        Ok(output) => {
            ...
            assert!(
                stdout.contains("ARM64")
                    || stdout.contains("0x0100000c")
                    || stdout.contains("cputype"),
...
fn test_encode_simple_sub() {
    ...
    assert_eq!(
        sub_word, 0xCB010000,
        "SUB X0, X0, X1 should be 0xCB010000, got 0x{:08X}",
        sub_word
    );
}
...
fn test_encode_mov_imm() {
    ...
    assert_eq!(
        mov_word, expected,
        "MOVZ X0, #42 should be 0x{:08X}, got 0x{:08X}",
        expected, mov_word
    );
}
...
fn test_pipeline_add_imm() {
    ...
    assert_eq!(
        word, expected,
        "ADD X0, X0, #1 should be 0x{:08X}, got 0x{:08X}",
        expected, word
    );
}
...
fn test_compact_unwind_section_emitted() {
    ...
    assert!(
        cu.is_some(),
        "Mach-O output should contain __compact_unwind section"
    );
    ...
    assert_eq!(
        cu_size, 32,
        "One function should produce one 32-byte compact unwind entry"
    );
    ...
    assert!(
        func_len > 0,
        "Function length in compact unwind should be non-zero, got {}",
        func_len
    );

    // This leaf needs no frame record, so compact-unwind can use FRAMELESS mode.
    let encoding = u32::from_le_bytes(cu_data[12..16].try_into().unwrap());
    let mode = encoding & 0x0F00_0000;
    assert_eq!(
        mode, 0x0200_0000,
        "Encoding mode should be FRAMELESS (0x02), got 0x{:08X}",
        encoding
    );
...
#[test]
fn test_runtime_stack_addr_compiles_with_dwarf_fallback() {
    let entry = Block(0);
    let mut func = LirFunction::new(
        "runtime_stack_addr_codegen",
        LirSignature {
            params: vec![],
    ...
    let obj_bytes = pipeline
        ...
        .expect("runtime StackAddr should complete AArch64 codegen");

    assert!(is_valid_macho(&obj_bytes));
    assert!(
        find_section(&obj_bytes, b"__compact_unwind").is_some(),
        "dynamic frame should still emit compact-unwind DWARF-mode entry"
    );
    assert!(
        find_section(&obj_bytes, b"__eh_frame").is_some(),
        "dynamic frame should emit DWARF fallback"
    );
}

#[test]
fn test_overaligned_runtime_stack_addr_with_stack_arg_call_compiles() {
    let entry = Block(0);
    let mut func = LirFunction::new(
        "runtime_stack_addr_overaligned_call_codegen",
        LirSignature {
            params: vec![],
            returns: vec![LirType::I64],
        },
    ...
    for idx in 0..9 {
        instructions.push(Instruction {
            ...
            args: vec![],
            results: vec![Value(2 + idx)],
        });
    }
    instructions.push(Instruction {
        opcode: Opcode::Call {
            name: "stack_arg_callee".to_string(),
        },
        args: (0..9).map(|idx| Value(2 + idx)).collect(),
        results: vec![],
    });
    instructions.push(Instruction {
        opcode: Opcode::Return,
    ...
    let obj_bytes = pipeline
        .compile_function(&func)
        .expect("over-aligned runtime StackAddr with outgoing stack args should compile");

    assert!(is_valid_macho(&obj_bytes));
    assert!(
        find_section(&obj_bytes, b"__compact_unwind").is_some(),
        "dynamic frame should emit compact-unwind DWARF-mode entry"
    );
    assert!(
        find_section(&obj_bytes, b"__eh_frame").is_some(),
        "dynamic frame should emit DWARF fallback"
...
fn test_compact_unwind_nop_function() {
    ...
    assert!(
        cu.is_some(),
        "Even a minimal function should have compact unwind"
    );

    let (_cu_offset, cu_size) = cu.unwrap();
    assert_eq!(
        cu_size, 32,
        "Should have exactly one 32-byte compact unwind entry"
    );
}
...
fn test_compact_unwind_otool_verification() {
    ...
    let tmp_dir = std::env::temp_dir();
    let obj_path = tmp_dir.join("trust_cg_test_cu.o");
    std::fs::write(&obj_path, &obj_bytes).expect("should write .o file");
...
fn test_full_pipeline_produces_valid_object() {
    ...
    let mov0 = MachInst::new(
        AArch64Opcode::MovR,
        vec![
            MachOperand::VReg(trust_cg_ir::regs::VReg::new(v0, RegClass::Gpr64)),
            MachOperand::PReg(X0),
        ],
    );
    ...
    let mov1 = MachInst::new(
        AArch64Opcode::MovR,
        vec![
            MachOperand::VReg(trust_cg_ir::regs::VReg::new(v1, RegClass::Gpr64)),
            MachOperand::PReg(X1),
        ],
    );
    ...
    let add = MachInst::new(
        AArch64Opcode::AddRR,
        vec![
            MachOperand::VReg(trust_cg_ir::regs::VReg::new(v2, RegClass::Gpr64)),
            MachOperand::VReg(trust_cg_ir::regs::VReg::new(v0, RegClass::Gpr64)),
            MachOperand::VReg(trust_cg_ir::regs::VReg::new(v1, RegClass::Gpr64)),
        ],
    );
...
#[test]
fn test_prepare_ir_function_eliminates_vregs_before_encode() {
    use trust_cg_ir::regs::RegClass;

    let sig = Signature::new(vec![Type::I32, Type::I32], vec![Type::I32]);
    let mut func = MachFunction::new("add_prepare".to_string(), sig);
    let entry = func.entry;
    ...
    pipeline
        ...
        .expect("prepare_ir_function should eliminate VRegs for simple add fixture");

    assert!(
        func.insts.iter().all(|inst| {
            inst.opcode != AArch64Opcode::Copy && inst.operands.iter().all(|op| !op.is_vreg())
        }),
        "post-regalloc/pre-encode IR must not contain VRegs or Copy pseudos: {:?}",
        func.insts
    );
}
