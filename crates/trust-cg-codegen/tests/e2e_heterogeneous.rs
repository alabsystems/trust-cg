// trust-cg-codegen/tests/e2e_heterogeneous.rs - End-to-end heterogeneous compute tests
//
...
use trust_cg_codegen::coreml_emitter::CoreMLEmitError;
use trust_cg_codegen::metal_emitter::{MetalEmitError, emit_metal_kernels};
use trust_cg_codegen::pipeline::{
    DispatchVerifyMode, Pipeline, PipelineConfig, PipelineError, emit_coreml_program,
    generate_cpu_only_plan,
};
...
use trust_cg_lower::dispatch::{
    VerificationReport, generate_dispatch_plan, verify_dispatch_plan_properties,
};
...
fn make_graph_with_profitability(nodes: Vec<ComputeNode>, edges: Vec<DataEdge>) -> ComputeGraph {
...
fn assert_verification_passes(report: &VerificationReport) {
    assert!(
        ...
        report.missing_cpu_fallback,
        report.data_transfers_ok,
        report.data_transfer_errors,
        report.synchronization_ok,
        report.synchronization_errors,
        report.dependency_order_ok,
        report.dependency_errors,
        report.profitability_ok,
        report.profitability_mismatches,
    );
}
...
fn e2e_pure_cpu_graph_produces_no_gpu_ane_output() {
    ...
    assert_eq!(
        ...
        0,
        "CPU-only graph needs no transfers"
    );
...
// ===========================================================================
// Test 2: Manual GPU metadata cannot mint Metal semantics
// ===========================================================================
...
fn e2e_data_parallel_graph_produces_metal_kernel() {
    ...
    // Public kind/op/target labels are scheduling metadata, not semantics.
    // Only the TrustIR graph builder can attach the sealed exact recipe needed
    // for code generation, so this hand-built node must fail closed.
    let error = emit_metal_kernels(&plan, &graph)
        .expect_err("manual GPU metadata must not synthesize a Metal kernel");
    assert!(matches!(
        error,
        MetalEmitError::SemanticBinding { node_id, .. } if node_id == ComputeNodeId(1)
    ));
}

// ===========================================================================
// Test 3: Manual ANE metadata cannot mint CoreML tensor semantics
// ===========================================================================
...
fn e2e_matrix_heavy_graph_produces_coreml_program() {
    ...
    // `dominant_op = GEMM` carries neither tensor shapes nor operand identity.
    // Treating it as a matmul recipe would silently invent semantics.
    let error = emit_coreml_program(&plan, &graph)
        .expect_err("manual ANE metadata must not synthesize a CoreML program");
    assert!(matches!(
        error,
        PipelineError::CoreMLEmit(CoreMLEmitError::SemanticBinding { node_id, .. })
            if node_id == ComputeNodeId(0)
    ));

    // Metal emission should produce nothing (no GPU nodes).
    let metal_result = emit_metal_kernels(&plan, &graph);
    if let Ok(output) = metal_result {
        assert!(output.kernels.is_empty(), "No GPU nodes, no Metal kernels");
    }
}

// ===========================================================================
// Test 4: Mixed manual metadata fails closed at both accelerator boundaries
// ===========================================================================
...
fn e2e_mixed_cpu_gpu_ane_graph_produces_all_outputs() {
    ...
    assert!(
        plan.count_transfers() >= 2,
        "need CPU->GPU and GPU->ANE transfers"
    );
    ...
    assert_eq!(
        plan.assignment[&ComputeNodeId(2)],
        ComputeTarget::NeuralEngine
    );

    // The Metal host path cannot orchestrate ANE work, and neither accelerator
    // may infer executable semantics from these hand-built labels.
    let metal_error = emit_metal_kernels(&plan, &graph)
        .expect_err("mixed manual accelerator metadata must fail closed");
    assert!(matches!(
        metal_error,
        MetalEmitError::InvalidDispatchPlan { .. } | MetalEmitError::SemanticBinding { .. }
    ));

    let coreml_error = emit_coreml_program(&plan, &graph)
        .expect_err("manual ANE metadata must not synthesize a CoreML program");
    assert!(matches!(
        coreml_error,
        PipelineError::CoreMLEmit(CoreMLEmitError::SemanticBinding { node_id, .. })
            if node_id == ComputeNodeId(2)
    ));
...
fn e2e_dispatch_verification_passes_all_plan_types() {
    // --- (a) CPU-only graph ---
    {
        let nodes = vec![cpu_scalar_node(0, "ADD", 64), cpu_scalar_node(1, "MUL", 64)];
        let edge = DataEdge {
            from: ComputeNodeId(0),
            to: ComputeNodeId(1),
            transfer_bytes: 64,
...
fn e2e_profitability_filtering_keeps_small_workloads_on_cpu() {
    ...
    let graph = make_graph_with_profitability(vec![small_gpu_node.clone()], vec![]);
    ...
    assert_eq!(recs.len(), 1, "Should have 1 recommendation for 1 node");
    assert_eq!(
    ...
    if let Ok(output) = metal_result {
        assert!(
            output.kernels.is_empty(),
            "No GPU dispatch, no Metal kernels"
        );
    }
...
fn e2e_profitability_large_workload_goes_to_gpu() {
    ...
    let graph = make_graph_with_profitability(vec![large_gpu_node.clone()], vec![]);
    ...
    let plan = generate_dispatch_plan(&graph, &recs);
    assert_eq!(plan.assignment[&ComputeNodeId(0)], ComputeTarget::Gpu,);

    // Profitability may select a target, but it cannot supply semantics.
    let error = emit_metal_kernels(&plan, &graph)
        .expect_err("manual scheduling metadata must not mint a Metal recipe");
    assert!(matches!(
        error,
        MetalEmitError::SemanticBinding { node_id, .. } if node_id == ComputeNodeId(0)
    ));
}

/// Test that operation labels alone cannot forge a fused CoreML program.
#[test]
fn e2e_ane_multi_node_fusion_pipeline() {
    ...
    assert_eq!(
        plan.count_transfers(),
        0,
        "same-target nodes need no transfers"
    );

    let error = emit_coreml_program(&plan, &graph)
        .expect_err("manual op labels must not synthesize a fused CoreML graph");
    assert!(matches!(
        error,
        PipelineError::CoreMLEmit(CoreMLEmitError::SemanticBinding { node_id, .. })
            if node_id == ComputeNodeId(0)
...
fn e2e_pipeline_verify_dispatch_integration() {
    ...
    let graph = make_graph(vec![cpu.clone(), gpu.clone(), ane.clone()], edges);
    let recs = vec![
