// trust-cg-codegen/tests/e2e_stack_alloc.rs - E2E tests for stack allocation
// through the full trust_ir pipeline (Alloc/Store/Load)
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// These tests exercise stack allocation (Alloc/Store/Load) through the
// complete trust_ir-to-binary pipeline:
//   1. Build a trust_ir module using the builder API with Alloc/Store/Load instrs
...
//
// Part of #243 — Stack allocation E2E tests through full trust_ir pipeline.
...
fn make_test_dir(test_name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("trust_cg_e2e_stack_alloc_{}", test_name));
...
/// Compile a trust_ir module through Compiler::compile(), link with a C driver,
/// run, and return (exit_code, stdout).
fn compile_link_run(module: &TrustIrModule, test_name: &str, driver_src: &str) -> (i32, String) {
...
"#;
    ...
    assert_eq!(stdout.trim(), "42", "stack_simple() should print 42");
}
...
"#;
    ...
    assert_eq!(stdout.trim(), "42", "_stack_arg(42) should print 42");
}
...
fn e2e_stack_alloc_compiles_all_opt_levels() {
    ...
    for opt in &[OptLevel::O0, OptLevel::O1, OptLevel::O2, OptLevel::O3] {
        ...
        eprintln!(
            ...
            opt, result.metrics.code_size_bytes
        );
    }
}
