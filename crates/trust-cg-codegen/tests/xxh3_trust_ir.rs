// trust-cg-codegen/tests/xxh3_trust_ir.rs - xxh3 hash algorithm expressed as trust_ir
//
...
//
// Implements the xxh3 64-bit hash algorithm as trust_ir instructions compiled
// through the Trust Codegen pipeline. This demonstrates that:
// 1. Complex hash algorithms can be expressed naturally in trust_ir
// 2. Trust Codegen's optimizer handles the resulting code patterns well
// 3. The compiled output produces correct xxh3 hashes
//
// Operations used: 64-bit multiply, rotate (shift+or), XOR, load from pointer.
// All available as trust_ir BinOp instructions.
//
...
// ---------------------------------------------------------------------------
// trust_ir instruction builders (helpers for readability)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Build the xxh3_64_short trust_ir function
//
...
fn build_xxh3_64_short() -> (TrustIrFunction, TrustIrModule) {
    let mut module = TrustIrModule::new("xxh3");
    ...
    let mut func = TrustIrFunction::new(FuncId::new(0), "xxh3_64_short", ft_id, entry);
    ...
    // Actually I8 -> I64 cast. trust_ir CastOp::ZExt should handle I8 -> I64.
    let (b1_64, n) = {
        let r = vid.next();
        let node = InstrNode::new(Inst::Cast {
            op: CastOp::ZExt,
...
fn test_xxh3_reference_vectors() {
    ...
    assert_ne!(
        h1, h2,
        "different 1-byte inputs should produce different hashes"
    );
...
/// Test that the xxh3 trust_ir function compiles successfully at O0.
#[test]
fn test_xxh3_compiles_o0() {
    let (func, module) = build_xxh3_64_short();
    let obj_bytes =
        compile_trust_ir(&func, &module, OptLevel::O0).expect("xxh3 should compile at O0");
    assert_valid_macho(&obj_bytes, "xxh3 O0");
    assert!(obj_bytes.len() > 100, "object file should be substantial");
}

/// Test that the xxh3 trust_ir function compiles successfully at O2.
...
fn test_xxh3_compiles_o2() {
    let (func, module) = build_xxh3_64_short();
    let obj_bytes =
        compile_trust_ir(&func, &module, OptLevel::O2).expect("xxh3 should compile at O2");
...
fn test_xxh3_o2_optimizes() {
    let (func, module) = build_xxh3_64_short();
    let obj_o0 = compile_trust_ir(&func, &module, OptLevel::O0).unwrap();
    let obj_o2 = compile_trust_ir(&func, &module, OptLevel::O2).unwrap();
...
fn test_xxh3_e2e_empty() {
    ...
    let (func, module) = build_xxh3_64_short();
    let obj_bytes =
        compile_trust_ir(&func, &module, OptLevel::O0).expect("compilation should succeed");
...
fn test_xxh3_e2e_1byte_o2_bisect_382() {
    ...
    let (func, module) = build_xxh3_64_short();
    let obj_bytes =
        compile_trust_ir(&func, &module, OptLevel::O2).expect("O2 compilation should succeed");
...
fn test_xxh3_e2e_o2_compiles_and_runs() {
    ...
    let inputs: &[&[u8]] = &[b"", b"a", b"abc", b"test", b"abcdefgh", b"abcdefghijklmnop"];
    for input in inputs {
...
/// Verify the trust_ir function structure is well-formed.
#[test]
fn test_xxh3_trust_ir_structure() {
    let (func, _module) = build_xxh3_64_short();
    assert_eq!(
        func.blocks.len(),
        7,
        "should have 7 blocks (entry + 6 paths)"
    );
...
/// Count total trust_ir instructions to understand code complexity.
#[test]
fn test_xxh3_instruction_count() {
    ...
    assert!(
        total > 50,
        "xxh3 should have >50 trust_ir instructions, got {}",
        total
    );
    assert!(
        total < 500,
        "xxh3 should have <500 trust_ir instructions (sanity check), got {}",
        total
    );

    eprintln!(
        "xxh3_64_short: {} trust_ir instructions across {} blocks",
        total,
        func.blocks.len()
    );
}
...
/// input length for fingerprinting ty BFS state labels
...
/// Fingerprint determinism: compiling the same trust_ir module twice must
...
fn test_xxh3_compilation_is_deterministic() {
    ...
    // O0 path: two independent compiles of the same trust_ir must be identical.
    let obj_o0_a = compile_trust_ir(&func, &module, OptLevel::O0).expect("O0 compile A");
    let obj_o0_b = compile_trust_ir(&func, &module, OptLevel::O0).expect("O0 compile B");
    ...
    let obj_o2_a = compile_trust_ir(&func, &module, OptLevel::O2).expect("O2 compile A");
    let obj_o2_b = compile_trust_ir(&func, &module, OptLevel::O2).expect("O2 compile B");
