// trust-cg-codegen/tests/e2e_cli_tmbc.rs - E2E test for the binary trust_ir bitcode (.tmbc) wire format pipeline
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//
// Tests the full binary bitcode pipeline:
//   1. Build a trust_ir module programmatically using the builder API
//   2. Serialize to binary via encode_tmbc
//   3. Deserialize back via decode_tmbc
//   4. Compile via Compiler::compile
//   5. Write .o file
//   6. Link with system cc
//   7. Run and verify correct output
//
// Part of #277 - Binary trust_ir bitcode integration
...
use trust_cg_codegen::pipeline::{OptLevel, TMBC_MAGIC, TMBC_VERSION, decode_tmbc, encode_tmbc};
use trust_ir::BinOp;
...
fn make_test_dir(test_name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("trust_cg_e2e_cli_tmbc_{}", test_name));
...
fn build_return_42_module() -> TrustIrModule {
...
fn build_add_i64_module() -> TrustIrModule {
...
fn build_identity_module() -> TrustIrModule {
...
fn build_multi_function_module() -> TrustIrModule {
...
//
// Builds a trust_ir module with a single function `return_42() -> i64 { 42 }`,
...
fn e2e_tmbc_return_42() {
    ...
    let tmbc_bytes = encode_tmbc(&module_orig).expect("encode tMBC");
    assert!(!is_json_format(&tmbc_bytes));
    assert!(tmbc_bytes.starts_with(TMBC_MAGIC));

    eprintln!(
        "--- trust_ir binary bitcode ({} bytes) ---",
        tmbc_bytes.len()
    );

    // Step 2: Decode back from binary (validates round-trip fidelity).
    let module_rt = decode_tmbc(&tmbc_bytes).expect("deserializing trust_ir module should succeed");
    assert_eq!(
...
"#;
    ...
    if !link_output.status.success() {
        ...
        panic!(
            "Linking failed.\nLinker stderr: {}\n\
             The Mach-O object produced from tmbc-deserialized trust_ir is not linkable.",
            stderr
        );
    }
...
//
// Builds a trust_ir module with `add_i64(a, b) -> i64 { a + b }`, exercises
...
fn e2e_tmbc_file_roundtrip_compile() {
    ...
    let module_from_file = decode_tmbc(&bytes_from_file).expect("reading from file should succeed");
...
fn e2e_tmbc_binary_smaller_than_json() {
    ...
    let tmbc_bytes = encode_tmbc(&module).expect("encode tMBC");
    let json =
        serde_json::to_string_pretty(&module).expect("serializing module to JSON should succeed");
...
fn e2e_tmbc_multi_function_link_run() {
    ...
    let tmbc_bytes = encode_tmbc(&module_orig).expect("encode tMBC");
    let module_rt =
        decode_tmbc(&tmbc_bytes).expect("deserializing multi-function module should succeed");
    assert_eq!(
        module_orig, module_rt,
        "binary round-trip should preserve module equality"
    );
...
fn e2e_tmbc_invalid_magic_rejected() {
    let err = decode_tmbc(b"BAD!not_a_real_tmbc");
    assert!(
        err.is_err(),
        "invalid bytes should be rejected by tMBC decoder"
    );
}

#[test]
fn tmbc_round_trip_equality() {
    let module = build_multi_function_module();
    let bytes = encode_tmbc(&module).expect("encode tMBC");
    let decoded = decode_tmbc(&bytes).expect("decode tMBC");

    assert_eq!(
        module, decoded,
        "decode_tmbc(encode_tmbc(module)) should preserve module equality"
    );
}

#[test]
fn e2e_tmbc_corrupt_payload_rejected() {
    let mut bytes = Vec::new();
    bytes.extend_from_slice(TMBC_MAGIC);
    bytes.extend_from_slice(&TMBC_VERSION.to_le_bytes());
    bytes.extend_from_slice(b"not a TRUST_IR payload");

    let err = decode_tmbc(&bytes).expect_err("corrupt inner payload should fail");
    let msg = err.to_string();
    assert!(
        msg.contains("deserialize"),
        "corrupt payload error should mention deserialize, got: {msg}"
    );
}
