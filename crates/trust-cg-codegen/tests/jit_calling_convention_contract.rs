// trust-cg-codegen/tests/jit_calling_convention_contract.rs
//
// Author: Andrew Yates <andrewyates.name@gmail.com>
// Copyright 2026 Andrew Yates | License: Apache-2.0
//! Regression test that pins the #431 calling-convention contract. JIT-compiled
//! trust_ir functions must follow the host C ABI so that `std::mem::transmute` to
//! `extern "C" fn(...)` is sound; the contract is defined in `crates/trust-cg-codegen/src/jit.rs` module docs.
...
mod aarch64 {
    ...
    fn build_add() -> MachFunction {
        let mut f = MachFunction::new(
            "add".to_string(),
            Signature::new(vec![Type::I64, Type::I64], vec![Type::I64]),
        );
        let e = f.entry;
