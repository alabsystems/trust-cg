// trust-cg-codegen/tests/e2e_x86_64_macho.rs - End-to-end x86-64 Mach-O validation
//
...
//   6. trust_ir-to-x86-64-Mach-O full pipeline path
//
...
use trust_cg_codegen::macho::{MachOTarget, MachOWriter};
...
fn make_test_dir(test_name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("trust_cg_e2e_x86_64_macho_{}", test_name));
...
fn find_section(bytes: &[u8], target_sectname: &[u8]) -> Option<(u32, u64, u32, u32)> {
...
fn test_x86_64_macho_text_section_attributes() {
    ...
    // x86-64 text alignment: the SysV ABI floor is 2^4 = 16 bytes; since
    // 08dfb4d1 (static 32-byte loop-head alignment, the JCC-erratum
    // mitigation) code carrying loop-head padding raises the section to 2^5.
    // The contract is the FLOOR — never weaker than 16B.
    assert!(
        align >= 4,
        "x86-64 text section alignment must be at least 2^4 (SysV), got 2^{align}"
    );
...
fn test_x86_64_macho_build_version() {
    ...
    let (platform, minos) = find_build_version(&bytes).expect("LC_BUILD_VERSION not found");
...
fn test_x86_64_macho_add_function_roundtrip() {
    ...
    let (_, size, _, _) = find_section(&bytes, b"__text").expect("__text section not found");
    assert!(
        size >= 5,
        "add function with frame should have >= 5 bytes of code"
    );
}
...
fn test_x86_64_macho_writer_direct() {
    ...
    writer.add_symbol("_zero", 1, 0, true).unwrap();
...
fn test_x86_64_macho_multi_section() {
    ...
    writer.add_symbol("_answer", 2, 0, true).unwrap();
...
fn test_x86_64_macho_otool_l_validation() {
    ...
    assert!(
        output.contains("2^4 (16)") || output.contains("2^5 (32)"),
        "otool -l must show at least 16-byte text alignment (2^5 with the \
         08dfb4d1 loop-head padding)"
    );

    // Also validate header with otool -h to confirm CPU type.
    let header_output = run_otool_h(&obj_path).expect("otool -h should succeed on x86-64 Mach-O");
...
fn test_x86_64_macho_otool_h_validation() {
    ...
    // cpusubtype = 3
    assert!(output.contains("3"), "otool -h should show cpusubtype 3");
...
fn test_x86_64_macho_nm_validation() {
    ...
    let Some(output) = run_nm(&obj_path) else {
        eprintln!("SKIP: nm cannot parse generated x86-64 Mach-O object");
        cleanup(&dir);
        return;
    };
...
fn test_x86_64_macho_writer_direct_otool_validation() {
    ...
    writer.add_text_section(&code);
    writer.add_symbol("_answer", 1, 0, true).unwrap();
...
// ===========================================================================
// Tests: trust_ir-to-x86-64-Mach-O full pipeline
// ===========================================================================

#[test]
fn test_x86_64_macho_trust_ir_compile() {
    // Test the full trust_ir -> adapter -> ISel -> encode -> Mach-O pipeline for x86-64.
    ...
    // Build a simple trust_ir function: fn add(a: i64, b: i64) -> i64 { a + b }
    ...
    let mut trust_ir_func = TrustIrFunction::new(FuncId::new(0), "add", ft_id, BlockId::new(0));
    trust_ir_func.blocks = vec![TrustIrBlock {
        id: BlockId::new(0),
    ...
    // Translate trust_ir to LIR through the adapter.
    ...
    let bytes = pipeline
        ...
        .expect("trust_ir-to-x86-64 Mach-O compilation should succeed");
    ...
    assert!(
        align >= 4,
        "x86-64 text alignment must be at least 2^4 (SysV floor; 2^5 with \
         08dfb4d1 loop-head padding), got 2^{align}"
    );
    assert_ne!(
        flags & S_ATTR_PURE_INSTRUCTIONS,
        0,
