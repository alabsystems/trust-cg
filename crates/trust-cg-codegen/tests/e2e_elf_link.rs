// trust-cg-codegen/tests/e2e_elf_link.rs - E2E ELF linking integration test
//
...
use trust_cg_codegen::x86_64::{
    ...
    build_x86_const_test_function, x86_compile_to_bytes, x86_compile_to_elf,
};
...
fn make_test_dir(test_name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("trust_cg_e2e_elf_{}", test_name));
...
fn test_e2e_elf_const42_structure() {
    ...
    let elf_bytes =
        x86_compile_to_elf(&func).expect("x86-64 pipeline should compile const42 to ELF");
    ...
    assert!(
        has_ret,
        "x86-64 code must contain RET (0xC3). Code bytes: {:02X?}",
        code
    );

    // -- Verify the code materializes the immediate value 42 --
    // The backend materializes 42 in one of two equivalent ways:
    //   * MOV r64, imm64: REX.W + B8+rd opcode followed by an 8-byte immediate
    //     (the full 64-bit value 42 = 0x000000000000002A appears as an LE u64), or
    //   * MOV r32, imm32: B8+rd opcode followed by a 4-byte immediate, which
    //     zero-extends into the 64-bit register (the optimal encoding; the value
    //     42 = 0x0000002A appears as an LE u32, e.g. `mov ecx, 42` = B9 2A 00 00 00).
    // Both are semantically identical and return 42 (JIT-verified). Accept either.
    let has_42_imm64 = code
        .windows(8)
        .any(|w| u64::from_le_bytes([w[0], w[1], w[2], w[3], w[4], w[5], w[6], w[7]]) == 42);
    let has_42_imm32 = code
        .windows(4)
        .any(|w| u32::from_le_bytes([w[0], w[1], w[2], w[3]]) == 42);
    assert!(
        has_42_imm64 || has_42_imm32,
        "code must materialize immediate value 42 (as imm64 or imm32). Code bytes: {:02X?}",
        code
    );

    // The function must still terminate with a RET (0xC3); together with the
    // immediate-42 check above this keeps the assertion meaningful.
    assert_eq!(
        *code.last().unwrap(),
        0xC3,
        "const42 code must end with RET (0xC3). Code bytes: {:02X?}",
        code
    );
...
fn test_e2e_elf_add_structure() {
    ...
    let elf_bytes = x86_compile_to_elf(&func).expect("x86-64 pipeline should compile add to ELF");
    ...
    assert_eq!(
        *code.last().unwrap(),
        0xC3,
        "last byte of x86-64 code must be RET (0xC3). Code: {:02X?}",
        code
    );
    ...
    assert!(
        has_add,
        "code must contain an ADD instruction. Code: {:02X?}",
        code
    );
...
fn test_e2e_elf_raw_bytes_validation() {
    ...
    let bytes1 =
        x86_compile_to_bytes(&func1).expect("x86-64 pipeline should compile const42 to bytes");
    ...
    let bytes2 = x86_compile_to_bytes(&func2).expect("x86-64 pipeline should compile add to bytes");
...
fn test_e2e_elf_contains_raw_code() {
    ...
    let raw_bytes = x86_compile_to_bytes(&func).expect("raw bytes compilation should succeed");

    let elf_bytes = x86_compile_to_elf(&func).expect("ELF compilation should succeed");
...
fn test_e2e_elf_frame_lowering_patterns() {
    ...
    assert!(
        code.contains(&0x55),
        "framed code must contain PUSH RBP (0x55). Code: {:02X?}",
        code
    );
    ...
    assert!(
        code.contains(&0x5D),
        "framed code must contain POP RBP (0x5D). Code: {:02X?}",
        code
    );
    ...
    assert_eq!(
        code[code_len - 2],
        0x5D,
        "POP RBP (0x5D) must precede RET (0xC3)"
    );
}
...
fn test_e2e_elf_const42_link_and_run() {
    ...
    // Linking/running an ELF .o requires an ELF toolchain. On non-Linux hosts
    // (e.g. macOS) the system `cc` targets Mach-O and cannot link our ELF
    // object: `ld` rejects the unknown file format. This is host-infrastructure
    // gating, not a backend defect -- the ELF object itself is validated
    // in-process by the structure tests above. Skip the link/run leg off-Linux.
    if !cfg!(target_os = "linux") {
        eprintln!("Skipping: ELF link/run requires a Linux/ELF toolchain (host cc targets Mach-O)");
        return;
    }
...
fn test_e2e_elf_add_link_and_run() {
    ...
    // See test_e2e_elf_const42_link_and_run: linking an ELF .o needs a Linux/ELF
    // toolchain; the host cc on macOS targets Mach-O and cannot link it. The ELF
    // object structure is validated in-process elsewhere, so this is host gating,
    // not a backend bug.
    if !cfg!(target_os = "linux") {
        eprintln!("Skipping: ELF link/run requires a Linux/ELF toolchain (host cc targets Mach-O)");
        return;
