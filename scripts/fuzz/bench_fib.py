#!/usr/bin/env python3
# Interleaved best-of-15 iterative-fib (serial 3-carried-var recurrence)
...
wd = tempfile.mkdtemp(prefix="benchfib_")
tir = os.path.join(SCR, "fib.trust_ir")
...
open(cref, "w").write(r"""
int fib_kernel(int n){ unsigned a=0u,b=1u; for(int i=0;i<n;i++){ unsigned t=a+b; a=b; b=t; } return (int)b; }
""")
...
int main(int argc,char**argv){
  ...
  volatile int res=fib_kernel(N);
  double best=1e30;
  for(int r=0;r<REPS;r++){ double t0=now_ms(); res=fib_kernel(N); double dt=now_ms()-t0; if(dt<best)best=dt; }
  printf("%.3f %08x\n", best, (unsigned)res);
  return 0;
}
""")
...
print(f"fib N={N} tcg={best['tcg']:.2f}ms clang={best['clang']:.2f}ms ratio={ratio:.3f} bitid=OK => {verdict}")
